...
def mock_performance_report_ok(mock_performance_metrics_ok):
    ...
    return PerformanceReport(
        ...
        feedback_stats=FeedbackStats(
            avg_rating=0.75, total_count=100, positive_count=60, negative_count=20, neutral_count=20
        ),
...
def mock_mlflow_client():
    ...
    client.get_run = Mock(
        return_value=Mock(
            ...
            data=Mock(
                metrics={"precision": 0.85, "recall": 0.82, "f1_score": 0.83, "accuracy": 0.84}
            ),
        )
    )
    return client
...
class TestAutoRetrainOnCriticalDrift:
    ...
    def test_auto_retrain_triggered_on_critical_drift(
        ...
        with patch.object(orchestrator, "_generate_datasets", return_value="/data/test.parquet"):
            with patch.object(
                orchestrator, "_merge_with_feedback", return_value="/data/merged.parquet"
            ):
                with patch.object(orchestrator, "_trigger_training", return_value="mlflow-run-123"):
                    with patch.object(orchestrator, "_monitor_training", return_value=True):
                        with patch.object(
                            orchestrator,
                            "_compare_metrics",
                            return_value=({"f1_score": 0.85}, {"f1_score": 0.65}, True),
                        ):
    ...
    def test_no_retrain_when_performance_ok(
        ...
        # Create orchestrator
        orchestrator = AutoRetrainOrchestrator(
            mlflow_tracking_uri="http://localhost:5000", mongodb_uri="mongodb://localhost:27017"
        )
        ...
        # Execute
        result = orchestrator.check_performance_and_retrain(
            specialist_type="technical", force=False
        )
                ...
                with patch.object(orchestrator, "_trigger_training", return_value="forced-run-123"):
                    with patch.object(orchestrator, "_monitor_training", return_value=True):
                        with patch.object(
                            orchestrator,
                            "_compare_metrics",
                            return_value=({"f1_score": 0.82}, {"f1_score": 0.80}, True),
                        ):
                            with patch.object(orchestrator, "_send_notification"):
                                with patch.object(orchestrator, "_export_metrics"):
...
class TestDatasetGeneration:
    ...
    def test_generate_datasets_success(
        self, MockPerformanceMonitor, MockFeedbackCollector, MockRetrainingTrigger, mock_subprocess
    ):
        """
    ...
    def test_generate_datasets_success_english_format(
        ...
        assert (
            result == "/data/training/technical_specialist_dataset.parquet"
    ...
    def test_generate_datasets_success_mixed_case_saved_to(
        ...
        assert (
            result == "/algum/caminho/dataset.parquet"
    ...
    def test_generate_datasets_failure(
        ...
        mock_subprocess.return_value = Mock(
            returncode=1, stdout="", stderr="Error: Failed to connect to LLM provider"
        )
    ...
    def test_generate_datasets_timeout(
        ...
        mock_subprocess.side_effect = subprocess.TimeoutExpired(
            cmd=["python", "generate_training_datasets.py"], timeout=1800
        )
...
class TestFeedbackMerge:
    ...
    def test_merge_with_feedback_success(
        self, MockPerformanceMonitor, MockFeedbackCollector, MockRetrainingTrigger
    ):
        """
        ...
        mock_feedback.get_recent_feedback.return_value = [
            {
                "context": {"cognitive_plan": "Test plan 1", "opinion": "Good analysis"},
                "rating": 0.8,
                "feedback_text": "Excellent work",
            },
            {
                "context": {"cognitive_plan": "Test plan 2", "opinion": "Needs improvement"},
                "rating": 0.4,
            ...
            with patch("pandas.DataFrame.to_parquet") as mock_write:
                with patch("pathlib.Path.exists", return_value=True):
                    ...
                    result = orchestrator._merge_with_feedback(
                        specialist_type="technical", new_dataset_path="/data/test.parquet"
                    )
        ...
        mock_feedback.get_recent_feedback.assert_called_once_with(
            specialist_type="technical", limit=1000
        )
    ...
    def test_merge_without_feedback_data(
        ...
        with patch("pandas.read_parquet") as mock_read:
            with patch("pandas.DataFrame.to_parquet") as mock_write:
                with patch("pathlib.Path.exists", return_value=True):
                    mock_df = pd.DataFrame(
                        {"cognitive_plan": ["Plan A"], "opinion": ["Opinion A"], "rating": [0.7]}
                    )
                    mock_read.return_value = mock_df
...
class TestNotifications:
    ...
    def test_slack_notification_failure_logged(
        ...
        with patch.dict(os.environ, {"SLACK_WEBHOOK_URL": "https://hooks.slack.com/test"}):
            ...
            orchestrator._send_slack_notification(
                status="success", details={"specialist_type": "technical"}
            )
...
class TestDryRunMode:
    ...
    def test_dry_run_no_actual_retrain(
        ...
        result = orchestrator.check_performance_and_retrain(
            specialist_type="technical", force=False, dry_run=True
        )
...
class TestConvertFeedbackToDataset:
    ...
    def test_convert_feedback_structure(
        ...
        feedback_data = [
            ...
            {
                "context": {"cognitive_plan": "Plan for task B", "opinion": "Missing edge cases"},
                "rating": 0.45,
