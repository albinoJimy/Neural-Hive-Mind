...
class ModelPerformanceMonitor:
    ...
    def query_mlflow_metrics(
        self, specialist_type: str, last_n_runs: int = 100
        ...
        try:
            ...
            logger.error("mlflow_query_failed", specialist_type=specialist_type, error=str(e))
            return None

    def query_feedback_statistics(
        self, specialist_type: str, window_days: int = 30
        ...
        try:
            ...
            stats = self.feedback_collector.get_feedback_statistics(
                specialist_type=specialist_type, window_days=window_days
            )

            if not stats:
                logger.warning("no_feedback_found", specialist_type=specialist_type)
                return None
            ...
            logger.error("feedback_query_failed", specialist_type=specialist_type, error=str(e))
            return None

    def calculate_aggregate_score(
        self, mlflow_metrics: Optional[PerformanceMetrics], feedback_stats: Optional[FeedbackStats]
    ) -> float:
        """
        Calcula score agregado de performance.
    ...
    def detect_degradation(
        ...
        if aggregate_score < 0.65:
            reasons.append(f"Score agregado ({aggregate_score:.3f}) abaixo do threshold (0.65)")

        is_degraded = len(reasons) > 0

        if is_degraded:
            logger.warning("degradation_detected", reasons=reasons)
        else:
    ...
    def get_performance_report(
        self, specialist_type: str, last_n_runs: int = 100, window_days: int = 30
    ) -> PerformanceReport:
        """
        ...
        Returns:
            PerformanceReport
        """
        logger.info("generating_performance_report", specialist_type=specialist_type)
    ...
    def export_metrics_to_prometheus(
        self, report: PerformanceReport, pushgateway_url: Optional[str] = None
    ):
        """
        ...
        try:
            ...
            # Push to gateway
            if pushgateway_url:
                push_to_gateway(pushgateway_url, job="model_performance_monitor", registry=registry)
                logger.info("metrics_exported_to_prometheus", pushgateway_url=pushgateway_url)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Monitor de performance de modelos")
    parser.add_argument(
        "--specialist-type", required=True, help="Tipo do specialist (technical, business, etc.)"
    )
    parser.add_argument(
        "--last-n-runs",
        type=int,
    ...
    parser.add_argument(
        "--window-days", type=int, default=30, help="Janela de dias para feedback (default: 30)"
    )
    parser.add_argument(
        "--output-format",
        choices=["json", "text"],
    ...
    parser.add_argument(
        "--export-metrics", help="URL do Prometheus Pushgateway para exportar métricas"
    )
