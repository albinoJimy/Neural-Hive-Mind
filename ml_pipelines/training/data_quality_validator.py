...
class DataQualityValidator:
    ...
    def __init__(
        ...
        self.max_missing_pct = (
            ...
            else float(os.getenv("DATA_QUALITY_MAX_MISSING_PCT", "5.0"))
        )
        self.max_sparsity_pct = (
            max_sparsity_pct
            if max_sparsity_pct is not None
            else float(os.getenv("DATA_QUALITY_MAX_SPARSITY_PCT", "50.0"))
        )
        self.max_outlier_pct = (
            max_outlier_pct
            if max_outlier_pct is not None
            else float(os.getenv("DATA_QUALITY_MAX_OUTLIER_PCT", "10.0"))
        )
        self.min_class_pct = (
            min_class_pct
            if min_class_pct is not None
            else float(os.getenv("DATA_QUALITY_MIN_CLASS_PCT", "5.0"))
        )
        self.max_correlation = (
            max_correlation
            if max_correlation is not None
            else float(os.getenv("DATA_QUALITY_MAX_CORRELATION", "0.95"))
        )
    ...
    def validate(
        self, df: pd.DataFrame, feature_names: List[str], label_column: str = "label"
    ) -> Dict[str, Any]:
        """
        ...
        passed = (
            quality_score >= 0.6
            and not missing_results["threshold_exceeded"]
            and not sparsity_results["threshold_exceeded"]
            and not outlier_results["threshold_exceeded"]
            and not label_results.get("underrepresented_classes")
            and not correlation_results.get("redundant_features_detected")
            and schema_results["schema_valid"]
        )
    ...
    def _validate_missing_values(self, df: pd.DataFrame, feature_cols: List[str]) -> Dict[str, Any]:
    ...
    def _validate_sparsity(self, df: pd.DataFrame, feature_cols: List[str]) -> Dict[str, Any]:
    ...
    def _validate_outliers(self, df: pd.DataFrame, feature_cols: List[str]) -> Dict[str, Any]:
    ...
    def _validate_label_imbalance(self, df: pd.DataFrame, label_column: str) -> Dict[str, Any]:
        ...
        percentages = {
            label: round((count / total) * 100, 2) for label, count in distribution.items()
        }

        # Detectar classes subrepresentadas e dominantes
        underrepresented = [label for label, pct in percentages.items() if pct < self.min_class_pct]
    ...
    def _validate_schema(self, df: pd.DataFrame, expected_features: List[str]) -> Dict[str, Any]:
        ...
        extra_features = [col for col in actual_set - expected_set if col not in metadata_cols]
    ...
    def _collect_warnings(
        ...
        high_outlier_features = {
            k: v
            for k, v in outlier_results.get("features_with_outliers", {}).items()
        ...
        if label_results.get("dominant_classes"):
            warnings.append(f"Classes dominantes (> 80%): {label_results['dominant_classes']}")
        ...
        if schema_results.get("missing_features"):
            warnings.append(f"Features ausentes no dataset: {schema_results['missing_features']}")
    ...
    def _generate_recommendations(
        ...
        high_missing = {
            k: v
            for k, v in missing_results.get("features_with_missing", {}).items()
        ...
        if sparsity_results.get("sparse_features"):
            ...
            recommendations.append(f"Features sempre zero podem ser removidas: {features_list}")
    ...
    def generate_mlflow_report(self, validation_results: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def validate_and_raise(
        ...
        if (
            validation_results["quality_score"] < min_quality_score
            or not validation_results["passed"]
        ):
            error_msg = (
