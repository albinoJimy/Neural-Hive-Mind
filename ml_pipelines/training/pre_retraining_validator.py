...
class PreRetrainingValidator:
    ...
    def _connect(self):
        ...
        try:
            ...
            logger.error("Falha na conexão MongoDB para validação", error=str(e))
            raise
    ...
    def validate_prerequisites(
        ...
        label_distribution_result = self._check_label_distribution(
            specialist_type=specialist_type, days=days, min_feedback_rating=min_feedback_rating
        )
        ...
        temporal_integrity_result = self._validate_temporal_integrity(
            specialist_type=specialist_type, days=days
        )
    ...
    def _check_sample_count(
        self, specialist_type: str, days: int, min_samples: int, min_feedback_rating: float
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error("sample_count_check_failed", specialist_type=specialist_type, error=str(e))
            return {"passed": False, "current": 0, "required": min_samples, "error": str(e)}

    def _check_label_distribution(
        self, specialist_type: str, days: int, min_feedback_rating: float
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            pipeline = [
                ...
                {"$match": {"feedback_docs.human_rating": {"$gte": min_feedback_rating}}},
                {"$group": {"_id": "$feedback_docs.human_recommendation", "count": {"$sum": 1}}},
            ]
            ...
            underrepresented = [label for label, pct in percentages.items() if pct < 5.0]
            dominant = [label for label, pct in percentages.items() if pct > 80.0]
            ...
            logger.error(
                "label_distribution_check_failed", specialist_type=specialist_type, error=str(e)
            )
            return {"passed": False, "error": str(e), "distribution": {}, "percentages": {}}

    def _check_feature_quality(
        self, specialist_type: str, days: int, min_feedback_rating: float, sample_size: int = 100
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            pipeline = [
                ...
                {"$project": {"opinion_id": 1, "cognitive_plan": 1}},
            ]
            ...
            if expected_features:
                ...
                overall_missing_rate = (
                    total_missing_count / total_feature_checks if total_feature_checks > 0 else 1.0
                )
            else:
                ...
                for feature_name in all_feature_names:
                    ...
                    feature_missing_rates[feature_name] = round(
                        missing_for_feature / actual_sample_size, 4
                    )
            ...
            high_missing_features = {k: v for k, v in feature_missing_rates.items() if v >= 0.05}
            ...
            logger.error(
                "feature_quality_check_failed", specialist_type=specialist_type, error=str(e)
            )
            return {
                "passed": False,
                "sample_size": 0,
    ...
    def _validate_temporal_integrity(self, specialist_type: str, days: int) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not result:
                return {"passed": False, "error": "Nenhum dado encontrado para validação temporal"}
            ...
            logger.error(
                "temporal_integrity_check_failed", specialist_type=specialist_type, error=str(e)
            )
    ...
    def _compare_with_baseline(
        self, specialist_type: str, current_distribution: Dict[str, float]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {"comparison_available": False, "reason": "MLflow não disponível"}

        except Exception as e:
            logger.warning(
                "baseline_comparison_failed", specialist_type=specialist_type, error=str(e)
            )
    ...
    def _generate_recommendations(
        ...
        if recommendation == "wait_for_more_data":
            ...
            recommendations.append(f"Aguardar coleta de mais {deficit} amostras de feedback")
            recommendations.append("Verificar taxa de coleta de feedback no dashboard do Grafana")
        ...
        if recommendation == "investigate_distribution":
            if label_distribution_result.get("underrepresented_classes"):
                ...
                recommendations.append("Verificar se coleta de feedback está enviesada")

            if not feature_quality_result.get("passed"):
                recommendations.append("Revisar qualidade de cognitive_plan nas opiniões")
                recommendations.append("Verificar pipeline de extração de features")
