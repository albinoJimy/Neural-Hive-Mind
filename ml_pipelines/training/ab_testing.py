...
class ABTestMetrics:
    ...
    def record_prediction(
        self, model_type: str, confidence: float, correct: bool = None  # 'new' ou 'baseline'
    ):
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        new_confidence = (
            ...
            if self.new_model_predictions > 0
            else 0.0
        )
        baseline_confidence = (
            self.baseline_model_confidence_sum / self.baseline_model_predictions
            if self.baseline_model_predictions > 0
            else 0.0
        )
...
class ABTestManager:
    ...
    def __init__(
        self, mlflow_tracking_uri: str = None, mongodb_uri: str = None, state_file: str = None
    ):
        """
    ...
    def get_model_for_request(self, specialist_type: str, test_id: str = None) -> str:
        """
    ...
    def record_prediction(
        self, test_id: str, model_type: str, confidence: float, correct: bool = None
    ):
    ...
    def check_test_completion(self, test_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendation": self._generate_recommendation(config, metrics_summary, passed),
        }

    def _generate_recommendation(
        self, config: ABTestConfig, metrics_summary: Dict[str, Any], passed: bool
    ) -> str:
    ...
    def expand_traffic(self, test_id: str, new_percentage: float):
    ...
    def _get_active_test(self, specialist_type: str) -> Optional[str]:
        ...
        for test_id, config in self.tests.items():
            if (
                config.specialist_type == specialist_type
    ...
    def _save_state(self):
        ...
        state = {
            ...
            "metrics_summary": {
                test_id: metrics.get_summary() for test_id, metrics in self.metrics.items()
            },
        }
        ...
        try:
            ...
            for test_id, metrics_data in state.get("metrics_summary", {}).items():
                ...
                metrics.new_model_confidence_sum = (
                    metrics_data["new_model"]["confidence_mean"]
                    * metrics_data["new_model"]["predictions"]
                )
                metrics.baseline_model_confidence_sum = (
                    metrics_data["baseline_model"]["confidence_mean"]
                    * metrics_data["baseline_model"]["predictions"]
                )
            ...
            logger.info("Estado carregado", active_tests=len(self.tests))
...
class ABTestingIntegration:
    ...
    def get_model_with_ab_test(
        specialist_type: str, ab_manager: ABTestManager, mlflow_client
    ) -> tuple:
        """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Gerenciamento de testes A/B para especialistas")
    parser.add_argument(
        "--action",
        type=str,
    ...
    parser.add_argument(
        "--baseline-version", type=str, default="Production", help="Versão baseline"
    )
