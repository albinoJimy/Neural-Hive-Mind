...
async def log_audit_event(audit_logger, method_name: str, **kwargs) -> Optional[str]:
    ...
    try:
        ...
        logger.warning("audit_event_logging_failed", method=method_name, error=str(e))
        return None
...
SPECIALIST_REGISTRY_MAP: Dict[str, Dict[str, str]] = {
    "technical": {
        ...
        "stage": _resolve_setting_value(
            getattr(TechnicalSpecialistConfig, "mlflow_model_stage", "Production")
        ),
    },
    "evolution": {
        ...
        "stage": _resolve_setting_value(
            getattr(EvolutionSpecialistConfig, "mlflow_model_stage", "Production")
        ),
    },
}
...
def validate_registry_alignment(specialist_type: str) -> Dict[str, str]:
    ...
    if (
        entry["model_name"] != derived_model_name
        or entry["experiment_name"] != derived_experiment_name
    ):
        raise ValueError(
...
def parse_args():
    ...
    parser.add_argument("--specialist-type", type=str, required=True, help="Tipo do especialista")
    parser.add_argument(
        "--feedback-count",
        type=int,
        default=0,
    ...
    parser.add_argument("--window-days", type=int, default=30, help="Janela de feedbacks a incluir")
    parser.add_argument(
...
def load_base_dataset(specialist_type: str, dataset_path_template: str) -> Tuple[pd.DataFrame, str]:
    ...
    logger.info("Loading base dataset", specialist_type=specialist_type, path=dataset_path)
    ...
    if missing_features:
        logger.warning(
            "dataset_missing_features", missing=list(missing_features), path=dataset_path
        )
    ...
    if extra_features:
        logger.warning("dataset_extra_features", extra=list(extra_features), path=dataset_path)
    ...
    logger.info(
        "dataset_schema_validated", num_features=len(expected_features), num_samples=len(df)
    )

    logger.info(
        "Base dataset loaded", size=len(df), columns=list(df.columns), data_source=dataset_path
    )
...
def compare_distributions(
    real_dist: Dict[int, float], synthetic_baseline: Dict[int, float]
) -> Dict[str, Any]:
    """
    ...
    logger.info("distribution_comparison", **comparison)
...
def load_dataset_with_real_data_priority(
    ...
    if not _REAL_DATA_COLLECTOR_AVAILABLE:
        logger.warning(
            "real_data_collector_not_available", reason="RealDataCollector import failed"
        )
        ...
        try:
            ...
            distribution_report = collector.validate_label_distribution(
                df=df_real, specialist_type=specialist_type
            )
            ...
            if _DATA_QUALITY_VALIDATOR_AVAILABLE:
                ...
                quality_report = validator.validate(df=df_real, feature_names=feature_names)
                ...
                if mlflow.active_run():
                    ...
                    mlflow.log_metric(
                        "data_missing_values_max_pct",
                        quality_report["missing_values"]["max_missing_pct"],
                    )
                    mlflow.log_metric(
                        "data_sparsity_rate", quality_report["sparsity"]["sparsity_rate"]
                    )
                    mlflow.log_metric(
                        "data_max_outlier_pct", quality_report["outliers"]["max_outlier_pct"]
                    )
            ...
            synthetic_baseline = {1: 50.0, 0: 25.0, 2: 25.0}  # approve  # reject  # review_required

            comparison_report = compare_distributions(
                real_dist=distribution_report["percentages"], synthetic_baseline=synthetic_baseline
            )
            ...
            logger.info(
                ...
                **{
                    k: v
                    for k, v in real_data_metadata.items()
                    if k not in ["distribution_comparison", "quality_warnings"]
                },
            )
            ...
            logger.error(
                "unexpected_error_real_data_collection", error=str(e), error_type=type(e).__name__
            )
            ...
            if collector is not None:
                try:
                    ...
                    logger.warning("real_data_collector_close_failed", error=str(close_error))
    ...
    synthetic_baseline = {1: 50.0, 0: 25.0, 2: 25.0}  # approve  # reject  # review_required
    comparison_report = compare_distributions(
        real_dist=label_percentages, synthetic_baseline=synthetic_baseline
    )
...
def load_feedback_data(specialist_type: str, window_days: int, min_quality: float) -> pd.DataFrame:
    """
    ...
    try:
        ...
        logger.info(
            "Feedbacks loaded from MongoDB", specialist_type=specialist_type, count=len(feedbacks)
        )
        ...
        for _, feedback in feedback_df.iterrows():
            ...
            features["label"] = (
                1 if feedback["human_recommendation"] == opinion.get("recommendation") else 0
            )
        ...
        logger.info("Feedback features extracted", size=len(feedback_features_df))
...
def train_model(
    X_train: pd.DataFrame, y_train: pd.Series, model_type: str, hyperparameter_tuning: bool
) -> Any:
    """
...
def evaluate_model(model: Any, X_val: pd.DataFrame, y_val: pd.Series) -> Dict[str, float]:
    """
    ...
    if supports_proba:
        try:
            ...
            logger.warning(
                "calibration_metrics_failed", error=str(e), fallback="metrics without calibration"
            )
...
def get_baseline_model(
    specialist_type: str, registry_entry: Dict[str, str] = None
    ...
    try:
        ...
        logger.info("Baseline model loaded", model_name=model_name, metrics=baseline_metrics)

        return model, baseline_metrics

    except Exception as e:
        logger.warning("No baseline model found", specialist_type=specialist_type, error=str(e))
        return None, None
...
def should_promote_model(
    ...
    if baseline_metrics is None or len(baseline_metrics) == 0:
        logger.info(
            "No baseline metrics available - promoting new model based on absolute thresholds"
        )
        return True
...
def main():
    ...
    with mlflow.start_run():
        ...
        if audit_logger and _MODEL_AUDIT_LOGGER_AVAILABLE:
            ...
            logger.info(
                "audit_log_training_started", model_name=model_name, model_version=model_version
            )
        ...
        if args.skip_validation == "false":
            if _PRE_RETRAINING_VALIDATOR_AVAILABLE:
                ...
                try:
                    ...
                    mlflow.log_param(
                        "pre_validation_recommendation", validation_report["recommendation"]
                    )
                    ...
                    if validation_report["checks"]["label_distribution"].get("percentages"):
                        print(
                            f"   Distribuição de labels: {validation_report['checks']['label_distribution']['percentages']}"
                        )
                    print()
        ...
        if data_source == "real":
            mlflow.log_metric(
                "real_samples_count", data_metadata.get("real_samples_count", len(df_base))
            )
            mlflow.log_metric("quality_score", data_metadata.get("quality_score", 0.0))
            mlflow.log_metric(
                "is_balanced", 1.0 if data_metadata.get("is_balanced", False) else 0.0
            )
            ...
            if data_metadata.get("date_range_start") and data_metadata.get("date_range_end"):
                print(
                    f"   Date range: {data_metadata['date_range_start'][:10]} to {data_metadata['date_range_end'][:10]}"
                )

        elif data_source == "synthetic":
            mlflow.log_metric(
                "synthetic_samples_count",
                data_metadata.get("synthetic_samples_count", len(df_base)),
            )
            ...
            if "distribution_comparison" in data_metadata:
                ...
                for label_key, divergence in (
                    data_metadata["distribution_comparison"].get("divergences", {}).items()
                ):
        ...
        df_feedback = load_feedback_data(
            args.specialist_type, args.window_days, args.min_feedback_quality
        )
        ...
        if "created_at" not in df_enriched.columns:
            ...
            if not split_metadata.get("temporal_integrity_valid", True):
                logger.error("temporal_integrity_violation", split_metadata=split_metadata)
                raise ValueError(
        ...
        if split_metadata.get("split_type") == "temporal":
            print(
                f"   Temporal integrity: {'✓' if split_metadata.get('temporal_integrity_valid') else '✗'}"
            )
            ...
            if split_metadata.get("val_date_range", {}).get("min"):
                print(
                    f"   Val date range: {split_metadata['val_date_range']['min'][:10]} a {split_metadata['val_date_range']['max'][:10]}"
                )
            if split_metadata.get("test_date_range", {}).get("min"):
                print(
                    f"   Test date range: a partir de {split_metadata['test_date_range']['min'][:10]}"
                )
        print()
        ...
        try:
            model = train_model(
                X_train, y_train, args.model_type, args.hyperparameter_tuning == "true"
            )
        ...
        if metrics.get("brier_score") is not None:
            print(
                f"   Brier Score: {metrics['brier_score']:.4f} ({metrics.get('calibration_quality', 'unknown')})"
            )
        ...
        try:
            ...
            mlflow.set_tag(
                "output_signature_type", "predict_proba" if supports_proba else "predict"
            )
            ...
            if model_registered:
                print(
                    f"   ✅ Model registered as {model_name} v{registered_version} with explicit signature"
                )
                print(f"   📋 Feature schema: {len(feature_names)} features")
                print(
                    f"   📊 Output type: {'probabilistic (predict_proba)' if supports_proba else 'discrete (predict)'}"
                )
            else:
                logger.warning(
                ...
                print(
                    f"   ⚠️  Model logged but version not found in registry for run {current_run_id}"
                )
        ...
        if args.promote_if_better == "true":
            ...
            if not model_registered:
                ...
                baseline_model, baseline_metrics = get_baseline_model(
                    args.specialist_type, registry_entry
                )
                ...
                if should_promote_model(metrics, baseline_metrics):
                    ...
                    if version_to_promote is None:
                        ...
                        print(
                            f"   ❌ Error: No model version found for current run {current_run_id}"
                        )
                        mlflow.log_param("promoted", "false")
