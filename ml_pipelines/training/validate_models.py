...
class ModelValidator:
    ...
    def __init__(self, baseline_dir: str = None, new_model_dir: str = None, output_dir: str = None):
        """
    ...
    def compare_with_baseline(
        self, new_report: Dict[str, Any], baseline_report: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        if baseline_report:
            ...
            comparison["improvement"] = {
                ...
                "confidence_mean": new_report["confidence_mean"]
                - baseline_report["confidence_mean"],
            }
    ...
    def _label_to_name(self, label: int) -> str:
        ...
        mapping = {0: "reject", 1: "approve", 2: "review_required"}
        return mapping.get(label, f"class_{label}")

    def generate_confusion_matrix_plot(
        self, cm: np.ndarray, specialist_type: str, output_path: str = None
    ):
    ...
    def save_report(self, report: Dict[str, Any], comparison: Dict[str, Any] = None):
        """Salva relatório em arquivo JSON."""
        output_file = self.output_dir / f"{report['specialist_type']}_validation_report.json"

        full_report = {"validation_report": report, "baseline_comparison": comparison}
...
def validate_all_specialists(
    models_dir: str, test_data_dir: str, output_dir: str = "/tmp/model_validation"
    ...
    for specialist in specialists:
        ...
        try:
            ...
            feature_cols = [
                ...
                if c not in ["label", "opinion_id", "plan_id", "created_at", "specialist_type"]
            ]
            ...
            report = validator.validate_model(
                model=model, X_test=X_test, y_test=y_test, specialist_type=specialist
            )

            # Gerar plot de matriz de confusão
            validator.generate_confusion_matrix_plot(
                np.array(report["confusion_matrix"]), specialist
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Valida modelos ML de especialistas")
    parser.add_argument(
        "--models-dir", type=str, required=True, help="Diretório com os modelos treinados"
    )
    parser.add_argument(
        "--test-data-dir", type=str, required=True, help="Diretório com os dados de teste"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    reports = validate_all_specialists(
        models_dir=args.models_dir, test_data_dir=args.test_data_dir, output_dir=args.output_dir
    )
