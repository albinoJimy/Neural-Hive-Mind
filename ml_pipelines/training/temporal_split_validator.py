...
class TemporalSplitValidator:
    ...
    def validate_splits(
        ...
        if train_max > now:
            ...
            if hours_in_future > self.max_future_data_hours:
                errors.append(f"Train contém dados {hours_in_future:.1f}h no futuro")

        if val_max > now:
            hours_in_future = (val_max - now).total_seconds() / 3600
            if hours_in_future > self.max_future_data_hours:
                errors.append(f"Validation contém dados {hours_in_future:.1f}h no futuro")

        if test_max > now:
            hours_in_future = (test_max - now).total_seconds() / 3600
            if hours_in_future > self.max_future_data_hours:
                errors.append(f"Test contém dados {hours_in_future:.1f}h no futuro")

        # Validação 5: Ordenação interna
        if not all(
            train_timestamps[i] <= train_timestamps[i + 1] for i in range(len(train_timestamps) - 1)
        ):
    ...
    def _extract_timestamps(
        self, data: Union[pd.DataFrame, List[datetime]], timestamp_column: str
    ) -> List[datetime]:
