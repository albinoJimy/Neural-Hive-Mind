...
class OnlineLearningConfig(BaseSettings):
    ...
    mini_batch_size: int = Field(
        default=32, env="MINI_BATCH_SIZE", description="Tamanho do mini-batch para partial_fit"
    )
    learning_rate: float = Field(
        default=0.001, env="ONLINE_LEARNING_RATE", description="Taxa de aprendizado para SGD"
    )
    regularization_alpha: float = Field(
        default=0.0001, env="REGULARIZATION_ALPHA", description="Parâmetro de regularização (alpha)"
    )
    ...
    rollback_enabled: bool = Field(
        default=True, env="ROLLBACK_ENABLED", description="Habilitar rollback automático"
    )
    ...
    gradual_rollout_enabled: bool = Field(
        default=True, env="GRADUAL_ROLLOUT_ENABLED", description="Habilitar rollout gradual"
    )
    ...
    deployment_cooldown_minutes: int = Field(
        default=30, env="DEPLOYMENT_COOLDOWN_MINUTES", description="Cooldown entre deployments"
    )
    ...
    batch_model_weight: float = Field(
        default=0.7, env="BATCH_MODEL_WEIGHT", description="Peso do modelo batch no ensemble"
    )
    online_model_weight: float = Field(
        default=0.3, env="ONLINE_MODEL_WEIGHT", description="Peso do modelo online no ensemble"
    )
    ...
    mongodb_uri: str = Field(
        default="mongodb://localhost:27017", env="MONGODB_URI", description="URI do MongoDB"
    )
    mongodb_database: str = Field(
        default="neural_hive", env="MONGODB_DATABASE", description="Database do MongoDB"
    )
    ...
    max_checkpoint_size_mb: int = Field(
        default=100, env="MAX_CHECKPOINT_SIZE_MB", description="Tamanho máximo de checkpoint em MB"
    )
    ...
    slack_webhook_url: Optional[str] = Field(
        default=None, env="SLACK_WEBHOOK_URL", description="Webhook URL do Slack"
    )
    ...
    def validate_ensemble_strategy(cls, v):
        ...
        if v not in valid_strategies:
            raise ValueError(f"ensemble_strategy deve ser um de {valid_strategies}, recebido: {v}")
        return v
    ...
    def validate_shadow_accuracy_threshold(cls, v):
        ...
        if not 0.5 <= v <= 1.0:
            raise ValueError(f"shadow_accuracy_threshold deve estar entre 0.5 e 1.0, recebido: {v}")
        return v
    ...
    def validate_model_weights(cls, v):
        ...
        if not 0.0 <= v <= 1.0:
            raise ValueError(f"Peso de modelo deve estar entre 0.0 e 1.0, recebido: {v}")
        return v
    ...
    def validate_mini_batch_size(cls, v):
        ...
        if v > 1000:
            logger.warning(
                "mini_batch_size muito alto pode impactar performance", mini_batch_size=v
            )
        return v
