...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="online-learning", description="CLI para gerenciamento do Online Learning Pipeline"
    )

    subparsers = parser.add_subparsers(dest="command", help="Comandos disponíveis")

    # Comando: update
    update_parser = subparsers.add_parser("update", help="Executar ciclo de atualização online")
    ...
    update_parser.add_argument(
        "--force", action="store_true", help="Forçar atualização mesmo sem dados suficientes"
    )
    update_parser.add_argument(
        "--dry-run", action="store_true", help="Simular atualização sem aplicar mudanças"
    )

    # Comando: validate
    validate_parser = subparsers.add_parser("validate", help="Executar validação shadow de modelo")
    validate_parser.add_argument(
        "--specialist-type", type=str, required=True, help="Tipo de especialista para validar"
    )
    validate_parser.add_argument(
        "--samples", type=int, default=100, help="Número de amostras para validação"
    )

    # Comando: rollback
    rollback_parser = subparsers.add_parser(
        "rollback", help="Reverter para versão anterior do modelo"
    )
    rollback_parser.add_argument(
        "--specialist-type", type=str, required=True, help="Tipo de especialista para rollback"
    )
    rollback_parser.add_argument(
        "--version", type=str, help="Versão específica para rollback (opcional)"
    )
    rollback_parser.add_argument("--reason", type=str, required=True, help="Motivo do rollback")

    # Comando: status
    status_parser = subparsers.add_parser("status", help="Mostrar status do pipeline")
    status_parser.add_argument(
        "--specialist-type", type=str, help="Tipo específico de especialista (opcional)"
    )
    status_parser.add_argument("--json", action="store_true", help="Saída em formato JSON")

    # Comando: versions
    versions_parser = subparsers.add_parser("versions", help="Listar versões de modelos")
    versions_parser.add_argument(
        "--specialist-type", type=str, required=True, help="Tipo de especialista"
    )
    versions_parser.add_argument(
        "--limit", type=int, default=10, help="Número máximo de versões a mostrar"
    )

    # Comando: metrics
    metrics_parser = subparsers.add_parser("metrics", help="Mostrar métricas de online learning")
    ...
    # Comando: deploy
    deploy_parser = subparsers.add_parser("deploy", help="Deploy gradual de modelo online")
    deploy_parser.add_argument(
        "--specialist-type", type=str, required=True, help="Tipo de especialista"
    )
    deploy_parser.add_argument(
        "--traffic-percentage", type=float, required=True, help="Percentual de tráfego (0.0-1.0)"
    )
    deploy_parser.add_argument(
        "--auto-rollback",
        action="store_true",
...
async def cmd_update(args) -> int:
    ...
    for specialist_type in specialist_types:
        try:
            ...
            result = await orchestrator.run_deployment_cycle(force=args.force, validation_data=None)
            ...
            logger.error("update_exception", specialist_type=specialist_type, error=str(e))
...
async def cmd_rollback(args) -> int:
    ...
    manager = RollbackManager(config=config, specialist_type=args.specialist_type)
...
def _load_batch_model(specialist_type: str, config: "OnlineLearningConfig") -> Any:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "batch_model_loaded", specialist_type=specialist_type, model_name=model_name
            )
            return model
        except Exception as e:
            logger.warning("mlflow_model_not_found", specialist_type=specialist_type, error=str(e))
    except ImportError:
        logger.warning("mlflow_not_available")
    except Exception as e:
        logger.warning("batch_model_load_failed", specialist_type=specialist_type, error=str(e))
...
def _get_feedback_collector(specialist_type: str, config: "OnlineLearningConfig") -> Optional[Any]:
    ...
    try:
        ...
        logger.warning("feedback_collector_not_available", specialist_type=specialist_type)
        return None
    except Exception as e:
        logger.warning(
            "feedback_collector_init_failed", specialist_type=specialist_type, error=str(e)
        )
        return None
...
def _get_feature_extractor(specialist_type: str) -> Optional[callable]:
    ...
    try:
        ...
        logger.warning("feature_extractor_not_available", specialist_type=specialist_type)
        return None
    except Exception as e:
        logger.warning(
            "feature_extractor_init_failed", specialist_type=specialist_type, error=str(e)
        )
        return None


def _try_load_online_checkpoint(
    online_learner: Any, specialist_type: str, config: "OnlineLearningConfig"
) -> bool:
    """
    ...
    try:
        ...
        logger.info(
            "online_checkpoint_loaded", specialist_type=specialist_type, checkpoint=checkpoints[0]
        )
        return True
    except Exception as e:
        logger.warning(
