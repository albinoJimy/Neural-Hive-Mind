...
deny contains msg if {
	...
	msg := sprintf("Service Redis %s deve ter annotation 'redis.security/auth: required'", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("Service Redis %s em produção deve ter annotation 'redis.security/tls: required'", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("Secret %s com credenciais Redis deve ser tipo 'Opaque'", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("Pod %s conectando ao Redis deve ter sidecar Istio injetado", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("ConfigMap %s contém configuração Redis com senha insegura", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("RedisCluster %s deve ter autenticação configurada", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("RedisCluster %s em produção deve ter TLS habilitado", [input.metadata.name])
}
...
warn contains msg if {
	...
	msg := sprintf("RedisCluster %s deveria ter resource limits definidos", [input.metadata.name])
}
...
warn contains msg if {
	...
	msg := sprintf("RedisCluster %s em produção deveria ter backup habilitado", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("NetworkPolicy %s não deve permitir acesso irrestrito ao Redis", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("Service Redis %s deve especificar namespaces autorizados", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("Secret Redis %s classificado como confidencial deve ter encryption extra", [input.metadata.name])
}

# Helper: TLS satisfeito para RedisCluster (TLS habilitado OU não é produção)
# Rego não tem operador 'or' infix; usamos múltiplas definições (disjunção lógica)
cluster_tls_satisfied if input.spec.TLS.enabled == true

cluster_tls_satisfied if input.metadata.labels["neural-hive.io/environment"] != "prod"
...
allow if {
	...
	cluster_tls_satisfied

	# Tem resource limits
	input.spec.kubernetesConfig.resources.limits
}

# Helper: TLS satisfeito para Service (annotation TLS required OU não é produção)
# Rego não tem operador 'or' infix; usamos múltiplas definições (disjunção lógica)
service_tls_satisfied if object.get(input.metadata.annotations, "redis.security/tls", "") == "required"

service_tls_satisfied if input.metadata.labels["neural-hive.io/environment"] != "prod"
...
allow if {
	...
	service_tls_satisfied
}

# Helper para calcular score de segurança Redis
# Em Rego não se pode mutar uma variável local acumulada; usamos uma lista de
# checks booleanos e contamos os verdadeiros, preservando exatamente cada condição.
redis_security_score := score if {
	...
	checks := [
		...
		input.spec.kubernetesConfig.redisSecret != null,
		# Check 2: TLS habilitado (condição final original: TLS habilitado == true)
		input.spec.TLS.enabled == true,
		# Check 3: Resource limits
		input.spec.kubernetesConfig.resources.limits != null,
		# Check 4: Backup habilitado
		object.get(input.metadata.annotations, "neural-hive.io/backup", "") == "enabled",
		# Check 5: Persistence habilitada
		input.spec.storage != null,
		# Check 6: Métricas habilitadas para monitoring
		input.spec.redisExporter.enabled == true,
	]

	passed_checks := count([c | c := checks[_]; c == true])
