...
deny contains msg if {
	...
	msg := sprintf("Service %s deve ter annotation 'auth.neural-hive/required: true'", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("VirtualService %s precisa de RequestAuthentication para paths protegidos", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("AuthorizationPolicy %s deve validar claims JWT obrigatórios", [input.metadata.name])
}
...
warn contains msg if {
	...
	msg := sprintf("RequestAuthentication %s deveria ter cache JWKS configurado", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("ConfigMap %s não deve conter secrets OAuth2 em texto plano", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("ServiceAccount %s deve ter RBAC configurado", [input.metadata.name])
}
...
warn contains msg if {
	...
	msg := sprintf("Secret OAuth2 %s deveria ter rotação automática", [input.metadata.name])
}
...
deny contains msg if {
	...
	msg := sprintf("EnvoyFilter %s deve configurar validação JWT", [input.metadata.name])
}
...
oauth2_security_score := score if {
	...
	# Containers e variáveis usados nos checks
	main_container := input.spec.template.spec.containers[0]
	...
	# Lista de checks booleanos (preserva exatamente a semântica original)
	checks := [
		...
		object.get(input.metadata.annotations, "neural-hive.io/https-only", "") == "true",
		# Check 4: Security context apropriado
		security_context.runAsNonRoot == true,
		# Check 5: Resource limits
		main_container.resources.limits != null,
	]

	# Conta quantos checks passaram (são verdadeiros)
