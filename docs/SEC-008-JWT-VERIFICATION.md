...
- `spiffe_trust_bundle_updates_total`: Atualizações de trust bundle (importada de `spiffe_manager.py` para evitar duplicação)
- `spiffe_trust_bundle_keys_validated_total`: Chaves validadas por status
- `spiffe_trust_bundle_validation_duration_seconds`: Duração da validação
