...
- [ ] 12. Pipeline de dados reais (sair do sintético) — DIAGNÓSTICO root-cause (2026-06-21)
  ...
  - **CONCLUSÃO HONESTA:** o f1-vs-sintético é **inalcançável** sem resolver a tensão privacidade-vs-ML: os opinions reais existem em volume (≥1600/specialist) e têm feedback suficiente (486>400), mas o **mascaramento GDPR remove o texto necessário para features** → 0 amostras válidas. Fingir um f1 seria verde-falso. **Fix necessário (decisão de data-governance):** desmascarar/desencriptar o texto para treino (com consentimento), OU persistir features não-PII no momento da opinion, OU treinar sobre uma representação não-textual. NÃO é um fix bounded.
