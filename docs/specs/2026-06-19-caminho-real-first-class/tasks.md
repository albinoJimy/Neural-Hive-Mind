...
- [ ] 12. Pipeline de dados reais (sair do sintético) — DIAGNÓSTICO root-cause (2026-06-21)
  ...
  - **PENDENTE (Task 12 mantém-se aberta):** (a) acumular ≥100 (duração) / ≥1000 (specialists) amostras reais com o pipeline a correr + re-treinar; (b) **FIX FUNDACIONAL #2** — ETL telemetria→ClickHouse (tabelas time-series a 0 linhas) para o Prophet/load; (c) ativar `RealDataCollector`. O DoD exige acumulação ao longo do tempo — não instantâneo.
