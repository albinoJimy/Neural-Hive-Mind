...
- [ ] 12. Pipeline de dados reais (sair do sintético) — DIAGNÓSTICO root-cause (2026-06-21)
  ...
  - **NOTAS:** (a) BUG corrigido: `RealDataCollector` default `FEEDBACK_COLLECTION` `feedback`(vazio)→`specialist_feedback`. (b) Registo MLflow falhou por mismatch de API (`logged-models` client 3.14 vs server antigo) — treino+f1 concluíram; registo precisa de cliente compatível. (c) Para o f1 real MELHORAR: balanceamento de classes, mais labels minoritários (active learning — já existe), tuning. (d) infra: port-forward a pods Mongo bloqueado por istio mTLS → correr in-cluster.
