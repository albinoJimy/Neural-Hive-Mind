...
- [ ] 12. Pipeline de dados reais (sair do sintético) — DIAGNÓSTICO root-cause (2026-06-21)
  ...
  - **PENDENTE (Task 12 mantém-se aberta):** (a) executar a pipeline de re-treino dos specialists com os 8291 opinions reais (RUNNABLE agora); (b) fix auth do ETL `sync-mongodb-clickhouse` + produção de telemetria→Mongo→ClickHouse para o Prophet/load; (c) ativar `RealDataCollector` em contínuo.
