...
- [ ] 2. Garantir que J3_BUILD inicia o FluxoGWorkflow (direto e pós-aprovação)
  ...
  - [ ] 2.1 Testes: resume pós-aprovação seleciona workflow por journey (J3→FluxoG); J1/UNKNOWN sem execução; J2/J4→Orchestration
  - [ ] 2.2 Corrigir o ponto de resume pós-aprovação para honrar `_select_workflow_class_by_journey`
  - [ ] 2.3 Gate cluster: plano J3 aprovado inicia FluxoGWorkflow (não Orchestration)

### Fase 2 — Contrato `ExecutionTicket` canónico

- [ ] 3. Unificar o contrato de ticket entre produtor, worker e code-forge
  - **DoR:** Fase 1 fechada.
...
- [ ] 4. G1 gera requisitos; G6 (code-forge) gera código FastAPI real
  ...
  - [ ] 4.1 Testes: G6 produz artefacto não-vazio; geração vazia/erro → FAILED (anti-verde-falso)
  - [ ] 4.2 Garantir G6 chama code-forge real (sem `stub://`) e persiste code_artifact
  - [ ] 4.3 Gate cluster: plano J3 produz code_artifact com código FastAPI real
...
- [ ] 5. Build real do código gerado em imagem de container
  ...
  - [ ] 5.3 Gate cluster: imagem do microserviço gerado publicada e puxável
