...
# Guia de Migracao dos Scripts

## 1. Introducao
- Periodo de transicao: scripts antigos mantidos com avisos de deprecacao.
- Migracao gradual recomendada; use CLIs unificados como destino final.
- Suporte: consulte README principal ou abra issue.
...
## 3. Migracao de Variaveis e Flags
...
## 4. Migracao de Makefile Targets
| Target Antigo | Novo Target | Comando Equivalente |
|---------------|-------------|---------------------|
| build-local | build-local | ./scripts/build.sh --target local |
| deploy-gateway | deploy-gateway | ./scripts/deploy.sh --services gateway |
| test-e2e | test-e2e | ./tests/run-tests.sh --type e2e |
| validate-all | validate | ./scripts/validate.sh --target all |
| security-init | security-init | ./scripts/security.sh vault init && ./scripts/security.sh spire deploy |
| ml-train | ml-train | ./ml_pipelines/ml.sh train --all |
| observability-dashboards | observability-dashboards | ./scripts/observability.sh dashboards access |

## 5. Migracao de Workflows CI/CD
Antes:
```yaml
- name: Build Images
  run: ./scripts/build-local-parallel.sh
- name: Push to ECR
...
## 6. Scripts Movidos da Raiz (referencia)
...
- ver [MIGRATION_MAP.md](./MIGRATION_MAP.md) para lista completa de movidos

## 7. Scripts Removidos
- scripts/deploy/quick-deploy-phase2-stubs.sh.DEPRECATED

## 8. Troubleshooting de Migracao
- "Script nao encontrado" → Verificar novo caminho ou CLIs.
- "Opcao invalida" → Consultar `--help` do CLI.
- "Dependencia faltando" → Conferir prerequisitos em README e CLI reference.

## 9. Referencias
- [MIGRATION_MAP.md](./MIGRATION_MAP.md)
- CLIs: build.sh, deploy.sh, run-tests.sh, validate.sh, security.sh, ml.sh
