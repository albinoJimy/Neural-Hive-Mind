...
def extract_basic_nlp_features(text):
    ...
    features = {
        # Domínios
        "domain_security": 1.0 if re.search(r'\b(security|ssl|tls|authentication|authorization|password|login)\b', text, re.I) else 0.0,
        "domain_performance": 1.0 if re.search(r'\b(performance|optimize|index|cache|speed|latency|query)\b', text, re.I) else 0.0,
        "domain_database": 1.0 if re.search(r'\b(database|db|sql|mongo|query|table|schema|migration)\b', text, re.I) else 0.0,
        "domain_devops": 1.0 if re.search(r'\b(deploy|container|docker|kubernetes|ci/cd|pipeline|build)\b', text, re.I) else 0.0,
        "domain_testing": 1.0 if re.search(r'\b(test|testing|unit|integration|e2e|coverage)\b', text, re.I) else 0.0,

        # Ações
        "action_create": 1.0 if re.search(r'\b(create|add|insert|new|make)\b', text, re.I) else 0.0,
        "action_update": 1.0 if re.search(r'\b(update|modify|change|edit|alter)\b', text, re.I) else 0.0,
        "action_delete": 1.0 if re.search(r'\b(delete|drop|remove|destroy|clean)\b', text, re.I) else 0.0,
        "action_read": 1.0 if re.search(r'\b(get|fetch|select|read|query|find)\b', text, re.I) else 0.0,
        "action_deploy": 1.0 if re.search(r'\b(deploy|release|publish|ship)\b', text, re.I) else 0.0,
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--test":
        ...
        for feedback in feedbacks:
            try:
                ...
                enriched += 1
                print(f"{enriched}. {feedback['feedback_id'][:8]}... | {nlp_features['primary_domain']}/{nlp_features['primary_action']} | risk={nlp_features['simple_risk_score']:.2f}")
