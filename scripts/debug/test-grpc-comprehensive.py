...
class PayloadGenerator:
            ...
            "factors": [
                {
                    "name": "data_volume",
                    "score": 0.75,
                    "description": "Large data volume requires careful processing",
                },
                {
                    "name": "complexity",
                    "score": 0.85,
                    "description": "High complexity with nested dependencies",
                },
                {
                    "name": "latency_sensitivity",
                    "score": 0.60,
                    "description": "Moderate latency requirements",
                },
            ],
        }
    ...
    def generate_minimal_payload(self, specialist_type: str) -> Dict[str, Any]:
        ...
        plan_id = self._next_id()
        return {"plan_id": plan_id, "intent_id": f"intent-minimal-{plan_id}"}
...
class ComprehensiveSpecialistTester:
    ...
    def __init__(
        self, namespace: str = "neural-hive", output_dir: str = "/tmp/grpc-comprehensive-tests"
    ):
        ...
        logger.info(
            "comprehensive_tester_initialized", namespace=namespace, output_dir=str(output_dir)
        )
    ...
    def initialize(self) -> bool:
        ...
        for specialist_type, service_name in self.SPECIALISTS.items():
            try:
                ...
                logger.info(
                    "connecting_to_specialist", specialist=specialist_type, endpoint=endpoint
                )
                ...
                logger.error(
                    "specialist_connection_failed", specialist=specialist_type, error=str(e)
                )
                return False
    ...
    def test_evaluate_plan_with_scenario(
        self, specialist_type: str, scenario_name: str, payload: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # Save full payload
            payload_file = (
                self.output_dir / "payloads" / f"{specialist_type}_{scenario_name}_payload.json"
            )
            ...
            try:
                ...
                logger.info(
                    "validation_5_passed",
                    check="timestamp_fields_accessible",
                    seconds=seconds,
                    nanos=nanos,
                )
            ...
            if response.HasField("opinion"):
                result["recommendation"] = (
                    response.opinion.recommendation[:100]
                    if response.opinion.recommendation
                    else "N/A"
                )
                result["confidence_score"] = response.opinion.confidence_score
                result["reasoning_summary"] = (
                    response.opinion.reasoning_summary[:100]
                    if response.opinion.reasoning_summary
                    else "N/A"
                )
            else:
            ...
            result["traceback"] = traceback.format_exc()
            logger.error(
                "scenario_test_failed_validation",
                specialist=specialist_type,
                scenario=scenario_name,
                error=str(e),
            )
            ...
            result["traceback"] = traceback.format_exc()
            logger.error(
                "scenario_test_failed_unexpected",
                specialist=specialist_type,
                scenario=scenario_name,
                error=str(e),
            )
    ...
    def _save_stack_trace(
        ...
        stack_trace_file = (
            self.output_dir / "stacktraces" / f"{specialist_type}_{scenario_name}_stacktrace.txt"
        )
    ...
    def test_specialist_all_scenarios(self, specialist_type: str) -> List[Dict[str, Any]]:
        ...
        scenarios = [
            ...
            (
                "special_chars",
                self.payload_generator.generate_special_characters_payload(specialist_type),
            ),
    ...
    def test_all_specialists_all_scenarios(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for specialist_type in self.SPECIALISTS.keys():
            ...
            logger.info(
                "completed_specialist_tests", specialist=specialist_type, total_tests=len(results)
            )
...
class TestResultsDocumenter:
    ...
    def generate_json_report(self, results: List[Dict[str, Any]], namespace: str) -> str:
        ...
        output_file = (
            self.output_dir / f"test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
"""
        ...
        output_file = (
            self.output_dir / f"TEST_RESULTS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
        with open(output_file, "w") as f:
            f.write(md_content)
    ...
    def _group_results_by_specialist(
...
def main():
    ...
    try:
        # Initialize tester
        tester = ComprehensiveSpecialistTester(namespace=args.namespace, output_dir=args.output_dir)
        ...
        if args.focus_business:
            ...
            result = tester.test_evaluate_plan_with_scenario(
                args.specialist, args.scenario, payload
            )
            results = [result]
