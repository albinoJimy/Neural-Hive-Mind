...
class IsolatedSpecialistTester:
        ...
        for specialist_type, endpoint in self.endpoints.items():
            try:
                ...
                try:
                    ...
                    logger.info("Channel ready", specialist_type=specialist_type, endpoint=endpoint)
                ...
                logger.info(
                    "Channel initialized", specialist_type=specialist_type, endpoint=endpoint
                )
    ...
    async def test_evaluate_plan(self, specialist_type: str) -> dict:
        ...
        try:
            # Invoke with timeout
            response = await asyncio.wait_for(stub.EvaluatePlan(request), timeout=30.0)
            ...
            logger.debug("Validation 1/5: Response is not None", specialist_type=specialist_type)

            # Validate response type
            if not isinstance(response, specialist_pb2.EvaluatePlanResponse):
                raise TypeError(f"Expected EvaluatePlanResponse, got {type(response).__name__}")
            ...
            logger.debug(
                "Validation 3/5: evaluated_at field exists", specialist_type=specialist_type
            )
            ...
            try:
                ...
                # Convert to datetime
                evaluated_datetime = datetime.fromtimestamp(seconds + nanos / 1e9, tz=timezone.utc)
            ...
            logger.error("Timeout waiting for response", specialist_type=specialist_type)
            return {
                "specialist_type": specialist_type,
                "success": False,
    ...
    async def test_all_specialists(self) -> dict:
        ...
        for specialist_type in self.endpoints.keys():
            logger.info("Testing specialist", specialist_type=specialist_type)

            result = await self.test_evaluate_plan(specialist_type)
            self.test_results[specialist_type] = result

            if result["success"]:
                logger.info("Test PASSED", specialist_type=specialist_type)
            else:
                logger.error(
                    "Test FAILED", specialist_type=specialist_type, error=result.get("error")
                )
    ...
    async def close(self):
        ...
        for specialist_type, channel in self.channels.items():
            try:
                await channel.close()
                logger.debug("Channel closed", specialist_type=specialist_type)
            except Exception as e:
                logger.warning(
                    "Error closing channel", specialist_type=specialist_type, error=str(e)
                )
...
async def main() -> int:
    ...
    try:
        ...
        logger.error(
            "Fatal error in test execution", error=str(e), traceback=traceback.format_exc()
        )
        return 1
    finally:
        ...
        try:
            ...
            logger.warning("Error during cleanup", error=str(close_error))
...
if __name__ == "__main__":
    try:
        ...
        logger.error("Erro fatal no teste", error=str(e), traceback=traceback.format_exc())
        sys.exit(1)
