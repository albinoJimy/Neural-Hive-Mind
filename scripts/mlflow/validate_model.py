...
def validate_model(
    ...
    try:
        ...
        logger.info("Validating model", model_name=model_name, version=model_version, run_id=run_id)
        ...
        for metric_name, (actual, threshold) in validations.items():
            if actual < threshold:
                failed_validations.append(
                    {"metric": metric_name, "actual": actual, "threshold": threshold}
                )
                logger.warning(
        ...
        if failed_validations:
            logger.error("Model validation failed", failed_validations=failed_validations)
            return False
        ...
        logger.error(
            "Model validation error", model_name=model_name, version=model_version, error=str(e)
        )
        return False
