...
class E2EMetricsCollector:
    ...
    def get_summary_statistics(self) -> Dict[str, Any]:
        ...
        timestamp_validation_rate = (
            (valid_timestamps / len(self.timestamp_validations) * 100)
            if self.timestamp_validations
            else 0
        )
        ...
        for specialist, latencies in self.latencies.items():
            specialist_results = [
                r for r in self.iterations_results if r["scenario"].startswith(specialist)
            ]
...
def validate_timestamp_in_response(response_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    validation_result = {
        ...
        "timestamps_valid": 0,
        "errors": [],
        "details": {"main_timestamp": None, "evaluated_at_timestamps": [], "processing_times": []},
    }
    ...
    if "metadata" in response_data:
        ...
        if isinstance(metadata, dict):
            if "evaluated_at" in metadata:
                if validate_single_timestamp(
                    str(metadata["evaluated_at"]), "metadata.evaluated_at"
                ):
                    validation_result["details"]["evaluated_at_timestamps"].append(
                        str(metadata["evaluated_at"])
                    )
            ...
            if isinstance(opinions, list):
                for idx, opinion in enumerate(opinions):
                    if isinstance(opinion, dict) and "evaluated_at" in opinion:
                        if validate_single_timestamp(
                            str(opinion["evaluated_at"]), f"{opinion_key}[{idx}].evaluated_at"
                        ):
                            validation_result["details"]["evaluated_at_timestamps"].append(
                                str(opinion["evaluated_at"])
                            )
        ...
        if isinstance(decision, dict) and "opinions" in decision:
            ...
            if isinstance(opinions, list):
                for idx, opinion in enumerate(opinions):
                    if isinstance(opinion, dict) and "evaluated_at" in opinion:
                        if validate_single_timestamp(
                            str(opinion["evaluated_at"]), f"decision.opinions[{idx}].evaluated_at"
                        ):
                            validation_result["details"]["evaluated_at_timestamps"].append(
                                str(opinion["evaluated_at"])
                            )
...
def verify_specialist_routing(
    response_data: Dict[str, Any], expected_specialist: str = None
) -> bool:
...
def send_intent_with_metrics(
    intent_text: str, specialist_type: str, iteration_num: int
    ...
    try:
        ...
        if response.status_code != 200:
            return (
                {"passed": False, "error": f"HTTP {response.status_code}", "response": None},
                latency_ms,
                {"valid": False, "errors": [f"HTTP {response.status_code}"]},
            )
        ...
        print(
            f"  NLU: {GREEN+'✓' if nlu_ok else RED+'✗'}{RESET}, "
        ...
        latency_ms = (time.time() - start_time) * 1000
        return (
            {"passed": False, "error": "Timeout", "response": None},
            latency_ms,
            {"valid": False, "errors": ["Request timeout"]},
        )

    except Exception as e:
        latency_ms = (time.time() - start_time) * 1000
        return (
            {"passed": False, "error": str(e), "response": None},
            latency_ms,
...
def run_test_scenario_with_metrics(
    scenario: Dict[str, str], iteration_num: int, metrics_collector: E2EMetricsCollector
) -> Dict[str, Any]:
    """
    ...
    result, latency_ms, timestamp_validation = send_intent_with_metrics(
        scenario["intent"], scenario["specialist"], iteration_num
    )

    # Record in metrics collector
    metrics_collector.record_iteration(
        iteration_num, scenario["specialist"], result, latency_ms, timestamp_validation
    )

    # Record errors if any
    if not result["passed"]:
        metrics_collector.record_error(
            iteration_num, scenario["specialist"], result.get("error", "Unknown"), str(result)
        )
...
def run_multiple_iterations(
    scenarios: List[Dict[str, str]], num_iterations: int
) -> E2EMetricsCollector:
    """
    ...
    try:
        for iteration in range(1, num_iterations + 1):
            print(f"\n{BOLD}{GREEN}╔{'═'*58}╗{RESET}")
            print(
                f"{BOLD}{GREEN}║  ITERATION {iteration}/{num_iterations}{' '*(45 - len(str(iteration)) - len(str(num_iterations)))}║{RESET}"
            )
            ...
            for scenario in scenarios:
                try:
                    ...
                    metrics_collector.record_error(
                        iteration, scenario["specialist"], type(e).__name__, str(e)
                    )
        ...
        print(
            f"Generating report for {len(metrics_collector.iterations_results)} completed tests..."
        )
...
def generate_summary_report(metrics_collector: E2EMetricsCollector):
    ...
    print(
        f"{color}✓ Tests Passed:{RESET} {stats['passed_tests']}/{stats['total_tests']} ({stats['success_rate']:.1f}%)"
    )
    print(
    ...
    print(
        f"{ts_color}✓ Timestamps Valid:{RESET} {stats['valid_timestamps']}/{stats['valid_timestamps'] + stats['invalid_timestamps']} ({stats['timestamp_validation_rate']:.1f}%)"
    )
    print(
        f"{RED}✗ Timestamps Invalid:{RESET} {stats['invalid_timestamps']}/{stats['valid_timestamps'] + stats['invalid_timestamps']} ({100-stats['timestamp_validation_rate']:.1f}%)"
    )
    ...
    for specialist in ["business", "technical", "behavior", "evolution", "architecture"]:
        if specialist in stats["specialist_stats"]:
            ...
            rate_color = (
                GREEN
                if sp_stats["success_rate"] >= 95
                else (YELLOW if sp_stats["success_rate"] >= 90 else RED)
            )
            print(
    ...
    if (
        ...
        and stats["timestamp_validation_rate"] == 100
        and stats["total_errors"] == 0
    ):
        ...
        print(
            f"{YELLOW}⚠ System is mostly stable - {stats['success_rate']:.1f}% success rate{RESET}"
        )
        ...
        print(
            f"{RED}✗ System has stability issues - {stats['success_rate']:.1f}% success rate{RESET}"
        )
...
def main():
    ...
    parser.add_argument(
        "--iterations", type=int, default=10, help="Number of iterations to run (default: 10)"
    )
    parser.add_argument(
        "--gateway-url",
        default="http://10.97.189.184:8000",
    ...
    parser.add_argument(
        "--output-dir",
        default="/tmp/e2e-validation",
        help="Output directory for results (default: /tmp/e2e-validation)",
    )
    parser.add_argument(
        "--skip-health-check", action="store_true", help="Skip initial health check"
    )
