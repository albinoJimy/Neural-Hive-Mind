...
def find_gateway_pod(namespace: str) -> Optional[str]:
    ...
    stdout, _, rc = run_kubectl(
        "get pods -l app=gateway-intencoes -o jsonpath='{.items[0].metadata.name}'", namespace
    )
    if rc == 0 and stdout:
...
def check_pod_status(namespace: str, pod_name: str) -> Dict[str, Any]:
    ...
    stdout, _, rc = run_kubectl(f"get pod {pod_name} -o json", namespace)
    if rc != 0:
...
def check_environment_variables(namespace: str, pod_name: str) -> Dict[str, Any]:
    ...
    stdout, _, rc = run_kubectl(f"exec {pod_name} -- env | grep -E 'OTEL|REDIS|KAFKA'", namespace)
...
def check_health_endpoint(namespace: str, pod_name: str) -> Dict[str, Any]:
    ...
    stdout, stderr, rc = run_kubectl(
        f"exec {pod_name} -- wget -qO- http://localhost:8000/health 2>&1", namespace
    )

    if rc != 0:
        return {"status": "ERROR", "error": stderr or stdout, "http_code": None}
...
"""
    ...
    # Executar teste
    stdout, stderr, rc = run_kubectl(f"exec {pod_name} -- python3 /tmp/test_redis.py", namespace)

    return {"output": stdout, "errors": stderr, "return_code": rc}
...
def check_otel_connectivity(namespace: str, pod_name: str, otel_endpoint: str) -> Dict[str, Any]:
    ...
    stdout, stderr, rc = run_kubectl(
        f"exec {pod_name} -- wget -qO- {http_endpoint}/health 2>&1 || echo 'HTTP_ERROR'", namespace
    )
...
def main():
    parser = argparse.ArgumentParser(description="Diagnóstico completo do Gateway Neural Hive-Mind")
    parser.add_argument(
        "--namespace",
    ...
    parser.add_argument(
        "--pod", help="Nome específico do pod (se não informado, encontra automaticamente)"
    )
    ...
    if "error" in pod_status:
        ...
        if pod_status["restart_count"] > 5:
            print_warning(
                f"Pod tem {pod_status['restart_count']} restarts - possível problema de inicialização"
            )
    ...
    if total_issues == 0 and health.get("status") == "healthy":
        ...
        if any("OTEL_ENDPOINT usa HTTPS" in i.get("message", "") for i in env_check["issues"]):
            ...
            print(
                "  3. Re-deploy: helm upgrade gateway-intencoes helm-charts/gateway-intencoes -f values-staging.yaml"
            )
