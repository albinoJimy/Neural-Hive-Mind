...
def _monkey_patch_sklearn_trees():
    ...
    try:
        ...
        logger.error(
            "Failed to apply sklearn compatibility patch", error=str(e), error_type=type(e).__name__
        )
...
def patch_model_after_loading(model):
    ...
    try:
        ...
        logger.warning(
            "Failed to patch model after loading", error=str(e), error_type=type(e).__name__
        )
