...
class TestHealthHandler:
    ...
    def test_ready_endpoint_not_serving(self, mock_specialist, mock_config):
        ...
        mock_specialist.health_check.return_value = {
            'status': 'NOT_SERVING',
            'details': {}
        }
        HealthHandler.specialist = mock_specialist
...
class TestCreateHttpServer:
    ...
    def test_create_http_server(self, mock_logger, mock_http_server_class,
                                mock_specialist, mock_config):
    ...
    @patch('src.http_server.HTTPServer')
    @patch('src.http_server.structlog.get_logger')
    def test_http_server_bind_address(self, mock_logger, mock_http_server_class,
                                      mock_specialist, mock_config):
...
class TestFastAPIApp:
    ...
    def test_create_fastapi_app(self, mock_logger, mock_feedback_router,
                                 mock_specialist_fastapi, config_fastapi):
    ...
    @patch('src.http_server_fastapi.FEEDBACK_AVAILABLE', False)
    @patch('src.http_server_fastapi.structlog.get_logger')
    def test_health_endpoint_returns_200(self, mock_logger, mock_specialist_fastapi, config_fastapi):
    ...
    @patch('src.http_server_fastapi.structlog.get_logger')
    def test_metrics_endpoint(self, mock_logger, mock_generate_latest,
                               mock_specialist_fastapi, config_fastapi):
        """Testa endpoint /metrics."""
        from src.http_server_fastapi import create_fastapi_app
        from fastapi.testclient import TestClient
