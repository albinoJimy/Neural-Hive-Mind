...
class TestMetadataIntegration:
    ...
    def test_metadata_contains_all_scores(self, mock_logger, real_specialist, complete_ux_plan):
        ...
        # Verificar ranges
        for score_key in ['usability_score', 'accessibility_score',
                          'response_time_score', 'interaction_cost_score']:
...
class TestConfidenceRiskRelationship:
    ...
    @patch('specialist.logger')
    def test_high_confidence_low_risk_correlation(self, mock_logger, real_specialist, complete_ux_plan):
    ...
    @patch('specialist.logger')
    def test_confidence_plus_risk_approximates_one(self, mock_logger, real_specialist, complete_ux_plan):
...
class TestMitigationGeneration:
    ...
    def test_mitigations_for_low_usability(self, mock_logger, real_specialist):
        ...
        plan = {
            ...
            'original_domain': 'ui-development',
            'tasks': [
                {'task_id': f't{i}', 'estimated_duration_ms': 1000}
                for i in range(20)
            ]
        }
        ...
        usability_mitigations = [
            ...
            if m['mitigation_type'] == 'improve_usability'
        ]
...
class TestDifferentDomains:
    ...
    def test_accessibility_evaluation_domain(self, mock_logger, real_specialist):
        ...
        plan = {
            'plan_id': 'a11y-test',
            'original_domain': 'accessibility-evaluation',
            'tasks': []
        }
        result = real_specialist._evaluate_plan_internal(plan, {})
    ...
    def test_non_ui_domain(self, mock_logger, real_specialist):
        ...
        plan = {
            'plan_id': 'backend-test',
            'original_domain': 'backend-processing',
            'tasks': []
        }
        result = real_specialist._evaluate_plan_internal(plan, {})
