...
class TestApprovalFlowEndToEnd:
    ...
    def mock_mongodb_client(self):
        ...
        async def get_pending(limit, offset, filters):
            return [a for a in storage.values() if a.status == ApprovalStatus.PENDING][:limit]
    ...
    async def test_full_approval_flow(self, approval_service, mock_mongodb_client, mock_producer):
        ...
        decision = await approval_service.approve_plan(
            plan_id="flow-plan-001", user_id="admin@test.com", comments="Aprovado apos analise"
        )
    ...
    async def test_multiple_plans_flow(self, approval_service, mock_mongodb_client, mock_producer):
        ...
        # Aprovar primeiro
        await approval_service.approve_plan(plan_id="multi-plan-0", user_id="admin@test.com")

        # Rejeitar segundo
        await approval_service.reject_plan(
            plan_id="multi-plan-1", user_id="admin@test.com", reason="Rejeitado"
        )
...
class TestApprovalFlowConcurrency:
    ...
    async def test_concurrent_approval_same_plan(
        ...
        result1 = await approval_service.approve_plan(plan_id="plan-001", user_id="admin1@test.com")
        ...
        with pytest.raises(ValueError) as exc_info:
            await approval_service.approve_plan(plan_id="plan-001", user_id="admin2@test.com")
...
class TestApprovalFlowEdgeCases:
    ...
    async def test_rejection_with_long_reason(
        ...
        result = await approval_service.reject_plan(
            plan_id="plan-001", user_id="admin@test.com", reason=long_reason
        )
