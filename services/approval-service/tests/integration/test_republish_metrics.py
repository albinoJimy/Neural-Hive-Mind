...
class TestRepublishMetrics:
    ...
    def mock_mongodb_with_approval(self, approved_approval_request):
        ...
        client = MagicMock()
        client.get_approval_by_plan_id = AsyncMock(return_value=approved_approval_request)
        return client
    ...
    def approval_service(
        self, mock_settings, mock_mongodb_with_approval, mock_producer, mock_metrics
    ):
    ...
    @pytest.mark.asyncio()
    async def test_republish_success_increments_metrics(self, approval_service, mock_metrics):
        ...
        await approval_service.republish_approved_plan(
            plan_id="plan-001", user_id="admin@test.com", force=False
        )

        # Verificar que metrica de sucesso foi incrementada
        success_metric = approval_republish_total.labels(outcome="success", force="false")
        assert success_metric._value.get() >= 1

    @pytest.mark.asyncio()
    async def test_republish_forced_increments_force_label(
        self, mock_settings, mock_producer, mock_metrics, sample_cognitive_plan
    ):
        ...
        await service.republish_approved_plan(
            plan_id="plan-002", user_id="admin@test.com", force=True
        )

        # Verificar que metrica de sucesso com force=true foi incrementada
        forced_metric = approval_republish_total.labels(outcome="success", force="true")
        assert forced_metric._value.get() >= 1

    @pytest.mark.asyncio()
    async def test_republish_not_found_increments_failure_metric(
        self, mock_settings, mock_producer, mock_metrics
    ):
        ...
        with pytest.raises(ValueError) as exc_info:
            await service.republish_approved_plan(plan_id="plan-999", user_id="admin@test.com")

        assert "nao encontrado" in str(exc_info.value)

        # Verificar que metrica de falha foi incrementada
        not_found_metric = approval_republish_failures_total.labels(failure_reason="not_found")
    ...
    async def test_republish_not_approved_increments_failure_metric(
        ...
        with pytest.raises(ValueError) as exc_info:
            await service.republish_approved_plan(
                plan_id="plan-003", user_id="admin@test.com", force=False
            )
    ...
    async def test_republish_no_cognitive_plan_increments_failure_metric(
        ...
        # Tentar republicar
        with pytest.raises(ValueError) as exc_info:
            await service.republish_approved_plan(plan_id="plan-004", user_id="admin@test.com")
    ...
    async def test_republish_kafka_error_increments_failure_metrics(
        self, mock_settings, mock_mongodb_with_approval, mock_metrics
    ):
        ...
        with pytest.raises(Exception) as exc_info:
            await service.republish_approved_plan(plan_id="plan-001", user_id="admin@test.com")

        assert "Kafka" in str(exc_info.value)

        # Verificar que metricas de falha foram incrementadas
        kafka_error_metric = approval_republish_failures_total.labels(failure_reason="kafka_error")
        assert kafka_error_metric._value.get() >= 1

        failure_metric = approval_republish_total.labels(outcome="failure", force="false")
        assert failure_metric._value.get() >= 1

    @pytest.mark.asyncio()
    async def test_republish_duration_is_observed(self, approval_service):
        """Teste que duracao da republicacao e registrada"""
        # Executar republicacao
        await approval_service.republish_approved_plan(plan_id="plan-001", user_id="admin@test.com")
    ...
    @pytest.mark.asyncio()
    async def test_multiple_republishes_accumulate_metrics(self, approval_service, mock_metrics):
        """Teste que multiplas republicacoes acumulam nas metricas"""
        # Executar 3 republicacoes
        for _ in range(3):
            await approval_service.republish_approved_plan(
                plan_id="plan-001", user_id="admin@test.com"
            )
