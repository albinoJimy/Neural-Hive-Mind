...
class TestListPendingApprovals:
    ...
    async def test_list_pending_service_error(self, admin_user):
        ...
        mock_service = MagicMock()
        mock_service.get_pending_approvals = AsyncMock(side_effect=Exception("Database error"))
...
class TestGetApproval:
    ...
    async def test_get_approval_success(self, sample_approval_request, admin_user):
        ...
        mock_service = MagicMock()
        mock_service.get_approval_by_plan_id = AsyncMock(return_value=sample_approval_request)

        result = await get_approval(plan_id="plan-001", user=admin_user, service=mock_service)
    ...
    async def test_get_approval_not_found(self, admin_user):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await get_approval(plan_id="plan-999", user=admin_user, service=mock_service)
...
class TestApprovePlan:
    ...
    async def test_approve_success(self, sample_approval_decision, admin_user):
        ...
        result = await approve_plan(
            plan_id="plan-001", body=body, user=admin_user, service=mock_service
        )
    ...
    async def test_approve_without_comments(self, sample_approval_decision, admin_user):
        ...
        result = await approve_plan(
            plan_id="plan-001", body=None, user=admin_user, service=mock_service
        )
    ...
    async def test_approve_not_found(self, admin_user):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await approve_plan(plan_id="plan-999", body=None, user=admin_user, service=mock_service)
    ...
    async def test_approve_already_decided(self, admin_user):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await approve_plan(plan_id="plan-001", body=None, user=admin_user, service=mock_service)
...
class TestRejectPlan:
    ...
    async def test_reject_not_found(self, admin_user):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await reject_plan(plan_id="plan-999", body=body, user=admin_user, service=mock_service)
    ...
    async def test_reject_empty_reason(self, admin_user):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await reject_plan(plan_id="plan-001", body=body, user=admin_user, service=mock_service)
...
class TestGetApprovalStats:
    ...
    async def test_get_stats_success(self, admin_user):
        ...
        result = await get_approval_stats(user=admin_user, service=mock_service)
    ...
    async def test_get_stats_service_error(self, admin_user):
        ...
        mock_service = MagicMock()
        mock_service.get_approval_stats = AsyncMock(side_effect=Exception("Aggregation error"))

        with pytest.raises(HTTPException) as exc_info:
            await get_approval_stats(user=admin_user, service=mock_service)
...
class TestRepublishApprovedPlan:
    ...
    async def test_republish_not_found(self, admin_user):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await republish_approved_plan(
                plan_id="plan-999", body=None, user=admin_user, service=mock_service
            )
    ...
    async def test_republish_missing_cognitive_plan(self, admin_user):
        ...
        mock_service = MagicMock()
        mock_service.republish_approved_plan = AsyncMock(
            side_effect=ValueError("Plano nao possui cognitive_plan para republicar: plan-001")
        )
