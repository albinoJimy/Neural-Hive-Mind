...
async def test_get_dashboard_stats(mock_approval_service, mock_user):
    ...
    # Configurar mocks
    mock_approval_service.db.plan_approvals.count_documents = AsyncMock(side_effect=[
        100,  # total_approvals
        5,    # pending_approvals
        10,   # approved_today
        2,    # rejected_today
    ...
    with patch('src.api.routers.dashboard.get_current_admin_user', return_value=mock_user):
        with patch('src.api.routers.dashboard.get_approval_service', return_value=mock_approval_service):
            result = await dashboard.get_dashboard_stats(days=7, user=mock_user, service=mock_approval_service)
...
async def test_get_approval_trends(mock_approval_service, mock_user):
    ...
    with patch('src.api.routers.dashboard.get_current_admin_user', return_value=mock_user):
        with patch('src.api.routers.dashboard.get_approval_service', return_value=mock_approval_service):
            result = await dashboard.get_approval_trends(days=3, user=mock_user, service=mock_approval_service)
...
async def test_get_stats_by_risk_band(mock_approval_service, mock_user):
    ...
    with patch('src.api.routers.dashboard.get_current_admin_user', return_value=mock_user):
        with patch('src.api.routers.dashboard.get_approval_service', return_value=mock_approval_service):
            result = await dashboard.get_stats_by_risk_band(user=mock_user, service=mock_approval_service)
...
async def test_get_ml_performance_stats(mock_approval_service, mock_user):
    ...
    mock_approval_service.db.specialist_feedback.count_documents = AsyncMock(side_effect=[
    ...
    with patch('src.api.routers.dashboard.get_current_admin_user', return_value=mock_user):
        with patch('src.api.routers.dashboard.get_approval_service', return_value=mock_approval_service):
            result = await dashboard.get_ml_performance_stats(user=mock_user, service=mock_approval_service)
...
async def test_get_recent_activity(mock_approval_service, mock_user):
    ...
    mock_approval_service.db.plan_approvals.find.return_value.sort = MagicMock(return_value=mock_cursor)
    mock_approval_service.db.plan_approvals.find.return_value.sort.return_value.limit = MagicMock(return_value=mock_cursor)

    with patch('src.api.routers.dashboard.get_current_admin_user', return_value=mock_user):
        with patch('src.api.routers.dashboard.get_approval_service', return_value=mock_approval_service):
            result = await dashboard.get_recent_activity(limit=10, user=mock_user, service=mock_approval_service)
...
async def test_get_recent_activity_error(mock_approval_service, mock_user):
    ...
    mock_approval_service.db.plan_approvals.find = MagicMock(
        side_effect=Exception("DB error")
    )
