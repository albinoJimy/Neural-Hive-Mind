...
class TestActiveLearningE2E:
    ...
    def e2e_components(self):
        ...
        async def mock_aggregate(pipeline):
            ...
            return [{"_id": "approve", "count": 450}, {"_id": "reject", "count": 34}]
        ...
        priority_queue.get_pending_cases = MagicMock(
            return_value=[
                {
                    "queue_id": "queue-1",
                    "plan_id": "plan-1",
        ...
        priority_queue.claim_case = MagicMock(
            return_value={
                "queue_id": "queue-1",
                "status": "in_review",
        ...
        priority_queue.release_case = MagicMock(
            return_value={"queue_id": "queue-1", "status": "pending"}
        )
        priority_queue.mark_feedback_submitted = MagicMock(
            return_value={"queue_id": "queue-1", "status": "completed", "feedback_id": "feedback-1"}
        )
    ...
    async def test_e2e_full_active_learning_flow(self, approval_service, e2e_components):
        ...
        with patch("src.services.approval_service.HAS_ACTIVE_LEARNING", True):
            ...
            claim_result = priority_queue.claim_case(
                queue_id="queue-1", assigned_to="user@example.com"
            )
            ...
            complete_result = priority_queue.mark_feedback_submitted(
                queue_id="queue-1", feedback_id="feedback-1"
            )
    ...
    async def test_e2e_claim_and_release_flow(self, e2e_components):
        ...
        # Claim caso
        claim_result = priority_queue.claim_case(queue_id="queue-1", assigned_to="user@example.com")
        assert claim_result["status"] == "in_review"
...
class TestActiveLearningAPIE2E:
    ...
    def fastapi_app(self):
        ...
        priority_queue.release_case = MagicMock(
            return_value={"queue_id": "q1", "status": "pending"}
        )
    ...
    def test_e2e_claim_endpoint(self, fastapi_app):
        ...
        response = client.post(
            "/api/v1/active-learning/q1/claim", json={"assigned_to": "user@example.com"}
        )
