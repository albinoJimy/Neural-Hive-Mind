...
class CognitiveLedgerClient:
    ...
    async def get_opinions_by_plan_id(self, plan_id: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("Opinioes recuperadas do ledger", plan_id=plan_id, count=len(opinions))

            return opinions

        except Exception as e:
            logger.error("Erro ao buscar opinioes do ledger", plan_id=plan_id, error=str(e))
            raise
