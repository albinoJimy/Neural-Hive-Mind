...
class FeedbackConsumer:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                'mongodb_client_initialized_for_feedback_consumer',
                uri=self.settings.mongodb_uri
            )
        except Exception as e:
            logger.warning(
                'mongodb_connection_failed_for_feedback_consumer',
                error=str(e)
            )
    ...
    async def start_consuming(
        ...
        """
        self.running = True
        logger.info('Iniciando consumo de feedbacks', topic=self.settings.kafka_specialist_feedback_topic)
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    logger.error(
                        'Erro ao processar mensagem de feedback',
                        error=str(e),
                        offset=msg.offset()
                    )
    ...
    async def _process_batch(
        ...
        try:
            ...
            logger.info(
                'lote_feedbacks_processado',
                batch_size=len(batch)
            )
        except Exception as e:
            logger.error(
                'erro_ao_processar_lote_feedbacks',
                batch_size=len(batch),
                error=str(e)
            )
            raise
    ...
    async def _deserialize_message(self, msg: Message) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                'falha_ao_deserializar_feedback',
                error=str(e),
                offset=msg.offset()
            )
            return None
    ...
    async def _enrich_from_mongodb(self, feedback_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'metadata' in doc and isinstance(doc['metadata'], dict):
                enrichment['specialist_recommendation'] = doc['metadata'].get('specialist_recommendation')
