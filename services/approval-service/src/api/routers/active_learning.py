...
class FeedbackRequest(BaseModel):
    ...
    submitted_by: str = Field(
        ...,
        description="Identificador do usuário (email, user_id)"
    )

    @validator('human_recommendation')
...
async def get_balance_analyzer(
    ...
    if not analyzer:
        raise HTTPException(
            ...
            detail='Balance analyzer not available'
        )
    return analyzer
...
async def get_feedback_queue(
    ...
    if not queue:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail='Feedback queue not available'
        )
    return queue
...
async def get_metrics(
    http_request: Request,
    analyzer=Depends(get_balance_analyzer)
):
    """
...
async def submit_feedback(
    ...
    try:
        ...
        result = queue.mark_feedback_submitted(
            queue_id=queue_id,
            feedback_id=feedback_id or f'manual-{queue_id}'
        )

        if not result:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f'Case {queue_id} not found'
            )
