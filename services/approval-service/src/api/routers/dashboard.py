...
async def get_dashboard_stats(
    ...
    try:
        ...
        rejected_today = await service.db.plan_approvals.count_documents(
            {"status": "rejected", "decision_timestamp": {"$gte": today_start}}
        )
        ...
        auto_approved = await service.db.plan_approvals.count_documents(
            {"created_at": {"$gte": week_start}, "auto_decided": True, "status": "approved"}
        )
    ...
    for i in range(days):
        ...
        try:
            ...
            approved = await service.db.plan_approvals.count_documents(
                {"created_at": {"$gte": day_start, "$lt": day_end}, "status": "approved"}
            )

            rejected = await service.db.plan_approvals.count_documents(
                {"created_at": {"$gte": day_start, "$lt": day_end}, "status": "rejected"}
            )
...
async def get_stats_by_risk_band(
    ...
    for band in risk_bands:
        try:
            total = await service.db.plan_approvals.count_documents({"risk_band": band})

            approved = await service.db.plan_approvals.count_documents(
                {"risk_band": band, "status": "approved"}
            )

            rejected = await service.db.plan_approvals.count_documents(
                {"risk_band": band, "status": "rejected"}
            )

            pending = await service.db.plan_approvals.count_documents(
                {"risk_band": band, "status": "pending"}
            )
...
async def get_ml_performance_stats(
    ...
    try:
        ...
        total_auto = await service.db.plan_approvals.count_documents({"auto_decided": True})
        auto_approve = await service.db.plan_approvals.count_documents(
            {"auto_decided": True, "status": "approved"}
        )
        auto_reject = await service.db.plan_approvals.count_documents(
            {"auto_decided": True, "status": "rejected"}
        )
