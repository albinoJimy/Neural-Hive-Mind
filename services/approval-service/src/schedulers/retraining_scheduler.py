...
class RetrainingScheduler:
    ...
    async def start(self):
        ...
        logger.info(
            "retraining_scheduler_iniciado", retrain_interval_hours=self._retrain_interval_hours
        )
    ...
    async def _scheduler_loop(self):
        ...
        while self._status != SchedulerStatus.STOPPED:
            try:
                ...
                logger.error("erro_no_scheduler_loop", error=str(e))
    ...
    async def _run_scheduled_retraining(self):
        ...
        try:
            ...
            if validation_result["status"] == ValidationStatus.PASSED:
                logger.info("shadow_validation_passed", result=validation_result)
                # TODO: Implementar deploy do novo modelo
            else:
                logger.warning("shadow_validation_failed", result=validation_result)

        except Exception as e:
            logger.error("erro_no_retreino_agendado", error=str(e))
    ...
    async def _check_and_trigger_drift_retraining(self):
        ...
        try:
            ...
            if drift_result.get("drift_detected"):
                ...
                logger.debug("drift_nao_detectado", drift_score=drift_result.get("drift_score", 0))
    ...
    async def _trigger_retraining(
        ...
        """
        logger.info("retraining_triggered", trigger=trigger, reason=reason, metadata=metadata)
    ...
    async def _run_shadow_validation(self, sample_size: Optional[int] = None) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "shadow_validation_iniciada", validation_id=validation_id, sample_size=sample_size
            )
            ...
            result["error"] = str(e)
            logger.error("shadow_validation_falhou", validation_id=validation_id, error=str(e))
    ...
    async def trigger_manual_retraining(self, reason: str, requested_by: str) -> dict[str, Any]:
        ...
        """
        logger.info("retraining_manual_solicitado", reason=reason, requested_by=requested_by)

        await self._trigger_retraining(
            trigger=RetrainingTrigger.MANUAL, reason=reason, metadata={"requested_by": requested_by}
        )
