...
from .clients.redis_client import close_redis_client, get_redis_client
from .config import get_settings
from .engine import DependencyCoordinator, ExecutionEngine
    ...
    try:
        ...
        if config.spiffe_enabled:
            try:
                ...
                logger.warning(
                    "SPIFFE habilitado mas biblioteca neural-hive-security não disponível"
                )
            except Exception as e:
                logger.exception("Erro ao inicializar SPIFFE integration", error=str(e))
                raise
        else:
        ...
        if config.vault_enabled:
            try:
                ...
                logger.warning(
                    "Vault habilitado mas biblioteca neural-hive-security não disponível"
                )
            except Exception as e:
                logger.exception("Erro ao inicializar Vault integration", error=str(e))
        ...
        if config.code_forge_enabled:
            try:
                code_forge_client = CodeForgeClient(
                    base_url=config.code_forge_url, timeout=config.code_forge_timeout_seconds
                )
        ...
        if getattr(config, "argocd_enabled", False) and config.argocd_url:
            try:
                ...
                argocd_client = ArgoCDClient(
                    base_url=config.argocd_url, token=config.argocd_token, timeout=600
                )
        ...
        if vault_client and kafka_creds and kafka_creds.get("ttl", 0) > 0:
            ...
            async def _kafka_credential_update_callback(new_creds):
                ...
                try:
                    ...
                    logger.exception("Falha ao recriar KafkaResultProducer", error=str(e))
                    raise
        ...
        jenkins_client = None
        if (
            getattr(config, "jenkins_enabled", False)
            and config.jenkins_url
            and config.jenkins_token
        ):
            try:
        ...
        if getattr(config, "neo4j_enabled", False):
            try:
                ...
                logger.warning(
                    "Neo4j Client init falhou, continuando sem Neo4j", error=str(neo4j_error)
                )
                neo4j_client = None
        ...
        if getattr(config, "k8s_jobs_enabled", False):
            try:
                ...
                logger.warning(
                    "k8s_jobs_client_init_failed", error="kubernetes-asyncio não instalado"
                )
            except Exception as e:
        ...
        if getattr(config, "local_runtime_enabled", True):
            try:
                ...
                logger.info(
                    "local_client_initialized", working_dir=config.local_runtime_working_dir
                )
            except Exception as e:
        ...
        if getattr(config, "dlq_alert_enabled", True):
            try:
                ...
                logger.warning(
                    "DLQ Alert Manager init falhou, continuando sem alertas", error=str(alert_error)
                )
                dlq_alert_manager = None
        ...
        if getattr(config, "kafka_dlq_enabled", True):
            try:
                ...
                logger.warning("dlq_consumer_init_failed_continuing_without", error=str(dlq_error))
        ...
        while agent_id is None and registration_attempts < max_registration_attempts:
            try:
                ...
                logger.info(
                    "worker_agent_registered", agent_id=agent_id, attempt=registration_attempts
                )
        ...
        await integration_registry.register_agent(
            AgentInfo(
...
async def shutdown():
    ...
    try:
        ...
        if app_state.get("spiffe_manager"):
        ...
        if app_state.get("mongodb_client"):
            try:
        ...
        if app_state.get("dlq_alert_manager"):
            try:
        ...
        try:
            ...
            if execution_engine:
                ...
                if total_executions > 0:
                    success_rate = max(
                        0.0, min(1.0, (total_executions - failed_executions) / total_executions)
                    )
            else:
            ...
            if success:
                ...
                if consecutive_failures >= max_consecutive_failures:
                    ...
                    try:
                        ...
                        logger.exception(
                            "re_registration_after_heartbeat_failure_failed",
            ...
            consecutive_failures += 1
            logger.exception(
                "heartbeat_loop_error", error=str(e), consecutive_failures=consecutive_failures
            )
...
async def main():
    ...
    uvicorn_config = uvicorn.Config(
        app, host="0.0.0.0", port=config.http_port, log_level=config.log_level.lower()
    )
