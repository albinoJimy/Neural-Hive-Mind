...
class K8sSecurityContext(BaseModel):
    ...
    run_as_user: int | None = Field(default=1000, description="UID para execução")
    run_as_group: int | None = Field(default=1000, description="GID para execução")
    read_only_root_filesystem: bool = Field(default=True, description="Filesystem read-only")
    allow_privilege_escalation: bool = Field(
        default=False, description="Permitir escalação de privilégios"
    )
...
class K8sJobRequest(BaseModel):
    ...
    name: str | None = Field(default=None, description="Nome do Job (gerado se não fornecido)")
    ...
    service_account: str | None = Field(default=None, description="ServiceAccount a usar")
    security_context: K8sSecurityContext = Field(default_factory=K8sSecurityContext)
    labels: dict[str, str] | None = Field(default=None, description="Labels do Job")
    annotations: dict[str, str] | None = Field(default=None, description="Annotations do Job")
    ttl_seconds_after_finished: int = Field(default=300, description="TTL para cleanup automático")
    active_deadline_seconds: int | None = Field(default=None, description="Deadline do Job")


class K8sJobStatus(StrEnum):
...
class KubernetesJobsClient:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.exception("k8s_jobs_init_failed", error=str(e))
    ...
    async def _create_job(self, manifest: dict[str, Any], namespace: str) -> str:
        ...
        with tracer.start_as_current_span("k8s.create_job") as span:
            ...
            try:
                ...
                await self._batch_api.create_namespaced_job(namespace=namespace, body=body)
                self.logger.info("k8s_job_created", job_name=job_name, namespace=namespace)
                return job_name

            except Exception as e:
                self.logger.exception("k8s_job_create_failed", job_name=job_name, error=str(e))
    ...
    async def _get_pod_for_job(self, job_name: str, namespace: str) -> str | None:
    ...
    async def _get_pod_logs(self, pod_name: str, namespace: str) -> str:
        """Obtém logs do Pod."""
        try:
            logs = await self._core_api.read_namespaced_pod_log(name=pod_name, namespace=namespace)
            return logs or ""
        except Exception as e:
            self.logger.warning("k8s_pod_logs_failed", pod_name=pod_name, error=str(e))
            return f"[Erro ao obter logs: {e}]"

    async def _get_pod_exit_code(self, pod_name: str, namespace: str) -> int | None:
    ...
    async def execute_job(self, request: K8sJobRequest, metrics=None) -> K8sJobResult:
        """
            ...
            try:
                ...
                if metrics:
                    ...
                    if hasattr(metrics, "k8s_job_duration_seconds"):
                        metrics.k8s_job_duration_seconds.labels(stage="total").observe(
                            duration_ms / 1000
                        )
                ...
                self.logger.exception("k8s_job_execution_failed", job_name=job_name, error=str(e))
                if metrics and hasattr(metrics, "k8s_jobs_executed_total"):
                ...
                if job_name and self.cleanup_jobs:
                    try:
                        ...
                        self.logger.warning(
                            "k8s_job_cleanup_failed", job_name=job_name, error=str(e)
                        )
