...
class ResourceLimits(BaseModel):
    ...
    cpu_shares: int | None = Field(default=None, description="CPU shares (peso relativo)")
    memory_swap: str | None = Field(
        default=None, description="Limite de swap (ex: 1g, -1 para ilimitado)"
    )
...
class DockerExecutionRequest(BaseModel):
    ...
    args: list[str] | None = Field(default=None, description="Argumentos adicionais")
    ...
    volumes: list[VolumeMount] | None = Field(default=None, description="Volumes a montar")
    user: str | None = Field(default=None, description="Usuário para execução (ex: 1000:1000)")
    entrypoint: list[str] | None = Field(default=None, description="Entrypoint customizado")
...
class DockerRuntimeClient:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.exception("aiodocker_not_installed")
            raise DockerRuntimeError("aiodocker não está instalado")
        except Exception as e:
            self.logger.exception("docker_runtime_init_failed", error=str(e))
    ...
    async def _pull_image(self, image: str) -> bool:
        ...
        with tracer.start_as_current_span("docker.pull_image") as span:
            ...
            try:
                ...
                self.logger.exception("docker_pull_failed", image=image, error=str(e))
                raise DockerRuntimeError(f"Falha ao pull de imagem {image}: {e}")

    async def execute_command(
        self, request: DockerExecutionRequest, metrics=None
            ...
            try:
                ...
                try:
                    wait_result = await asyncio.wait_for(
                        container.wait(), timeout=request.timeout_seconds
                    )
                ...
                if metrics:
                    ...
                    if hasattr(metrics, "docker_execution_duration_seconds"):
                        metrics.docker_execution_duration_seconds.labels(stage="total").observe(
                            duration_ms / 1000
                        )
                ...
                self.logger.exception("docker_execution_failed", image=request.image, error=str(e))
                if metrics and hasattr(metrics, "docker_executions_total"):
                ...
                if container and self.cleanup_containers:
                    try:
                        ...
                        self.logger.debug(
                            "docker_container_cleaned", container_id=container.id[:12]
                        )
                    except Exception as e:
                        self.logger.warning(
