...
class KafkaResultProducer:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.exception("kafka_producer_init_failed", error=str(e))
            raise
        ...
        try:
            ...
            def _produce():
                self.producer.produce(
                    topic=topic, key=key, value=value, on_delivery=_delivery_callback
                )
            ...
            self.logger.exception(
                "publish_result_failed", ticket_id=ticket_id, status=status, error=str(e)
            )
            if self.metrics:
