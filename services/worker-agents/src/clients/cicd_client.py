...
class CICDRunStatus:
    ...
    url: str | None = None
    raw_status: str | None = None  # Status original do provider
...
class CICDClient:
    ...
    def has_provider(self, provider: str) -> bool:
        ...
        if provider == CICDProvider.GITLAB_CI:
    ...
    async def trigger(self, provider: str, **kwargs) -> str:
        """
        ...
        if provider == CICDProvider.GITHUB_ACTIONS:
            ...
            try:
                return await self.github_actions_client.trigger_workflow(
            ...
            try:
                return await self.gitlab_ci_client.trigger_pipeline(
    ...
    async def poll_status(self, provider: str, run_id: str, **kwargs) -> CICDRunStatus:
        """
    ...
    async def wait_for_completion(
        ...
        **kwargs,
        ...
        self.logger.info(
            "cicd_waiting_for_completion", provider=provider, run_id=run_id, timeout=timeout
        )
        ...
        if provider == CICDProvider.GITHUB_ACTIONS:
            ...
            try:
                status = await self.github_actions_client.wait_for_run(
                    run_id=run_id, poll_interval=poll_interval, timeout=timeout, repo=repo
                )
    ...
    async def fetch_test_report(self, provider: str, run_id: str, **kwargs) -> TestReport:
        """
        ...
        if provider == CICDProvider.GITLAB_CI:
            ...
            return TestReport(
                total=total, passed=passed, failed=failed, skipped=skipped, errors=error
            )
    ...
    async def fetch_coverage_report(
        self, provider: str, run_id: str, **kwargs
    ) -> CoverageReport | None:
        """
    ...
    def _normalize_github_status(self, status: str, conclusion: str | None) -> str:
