...
class DLQAlertManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.info(
                "dlq_alert_manager_initialized", alert_topic=self.config.dlq_alert_kafka_topic
            )

        except Exception as e:
            self.logger.exception("dlq_alert_manager_init_failed", error=str(e))
            raise
    ...
    async def send_alert(self, alert_payload: dict) -> bool:
        ...
        try:
            ...
            def delivery_callback(err, msg):
                if err:
                    self.logger.error(
                        "dlq_alert_delivery_failed", ticket_id=ticket_id, error=str(err)
                    )
                else:
            ...
            if delivery_result["success"]:
                ...
                if self.metrics:
                    self.metrics.dlq_alerts_sent_total.labels(
                        severity=alert_event["severity"], task_type=task_type
                    ).inc()
            ...
            self.logger.exception(
                "dlq_alert_failed", ticket_id=alert_payload.get("ticket_id"), error=str(e)
            )
            return False
