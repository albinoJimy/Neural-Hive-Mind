...
def _ensure_metrics():
    ...
    existing_collectors = {
        getattr(c, "_name", ""): c for c in REGISTRY._names_to_collectors.values()
    }
...
class ServiceRegistryClient:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if spiffe_x509_enabled:
                ...
                is_internal_cluster = ".svc.cluster.local" in target or target.startswith(
                    "localhost"
                )
            ...
            self.logger.exception("service_registry_client_init_failed", error=str(e))
            raise
    ...
    async def register(self) -> str:
        ...
        try:
            ...
            self.logger.exception("registration_failed", error=str(e))
            raise
    ...
    async def heartbeat(self, telemetry: dict[str, Any]) -> bool:
        ...
        try:
            ...
            request = service_registry_pb2.HeartbeatRequest(
                agent_id=self.agent_id, telemetry=agent_telemetry
            )
            ...
            if e.code() == grpc.StatusCode.NOT_FOUND and "não encontrado" in e.details():
                ...
                try:
                    await self.register()
                    self.logger.info("agent_re_registered", new_agent_id=self.agent_id)
                    return True
                except Exception as reg_error:
                    self.logger.exception("re_registration_failed", error=str(reg_error))
                    return False
            else:
    ...
    async def deregister(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                "worker_agent_deregistered", agent_id=self.agent_id, success=response.success
            )

            return response.success

        except Exception as e:
            self.logger.exception("deregister_failed", error=str(e))
            return False
    ...
    async def discover_agents(
        ...
        try:
            ...
            self.logger.exception("discover_agents_failed", capabilities=capabilities, error=str(e))
            return []
