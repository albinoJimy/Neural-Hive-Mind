...
class JenkinsClient:
    ...
    async def trigger_job(self, job_name: str, parameters: dict[str, str] | None = None) -> int:
        """
        ...
        try:
            ...
            self.logger.info("jenkins_job_triggered", job_name=job_name, queue_id=queue_id)
    ...
    async def get_queue_item(self, queue_id: int) -> JenkinsQueueItem:
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                return JenkinsQueueItem(queue_id=queue_id, blocked=False, buildable=False, why=None)
            raise JenkinsAPIError(
                f"Failed to get queue item: {e.response.text}", status_code=e.response.status_code
            )
    ...
    async def get_build_status(self, job_name: str, build_number: int) -> JenkinsBuildStatus:
        """
        ...
        try:
            ...
            raise JenkinsAPIError(
                f"Failed to get build status: {e.response.text}", status_code=e.response.status_code
            )

    async def wait_for_build_number(
        self, job_name: str, queue_id: int, poll_interval: int = 5, timeout: int = 300
    ) -> int:
        """
        ...
        self.logger.info("jenkins_waiting_for_build_number", job_name=job_name, queue_id=queue_id)
    ...
    async def get_test_report(self, job_name: str, build_number: int) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                return {"passCount": 0, "failCount": 0, "skipCount": 0, "totalCount": 0}
            raise JenkinsAPIError(
                f"Failed to get test report: {e.response.text}", status_code=e.response.status_code
            )

    async def get_junit_results(self, job_name: str, build_number: int) -> dict[str, Any]:
        """
    ...
    async def get_coverage_report(self, job_name: str, build_number: int) -> float | None:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(line_coverage, dict):
                    percentage = line_coverage.get(
                        "percentageFloat", line_coverage.get("percentage")
                    )
    ...
    async def stop_build(self, job_name: str, build_number: int) -> bool:
        ...
        try:
            ...
            response.raise_for_status()
            self.logger.info("jenkins_build_stopped", job_name=job_name, build_number=build_number)
            return True
            ...
            raise JenkinsAPIError(
                f"Failed to stop build: {e.response.text}", status_code=e.response.status_code
            )
    ...
    async def get_last_build(self, job_name: str) -> JenkinsBuildStatus | None:
        """
        ...
        try:
            ...
            if e.response.status_code == 404:
                return None
            raise JenkinsAPIError(
                f"Failed to get last build: {e.response.text}", status_code=e.response.status_code
            )
    ...
    async def get_job_info(self, job_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise JenkinsAPIError(
                f"Failed to get job info: {e.response.text}", status_code=e.response.status_code
            )
