...
class LambdaPayload(BaseModel):
    ...
    args: list | None = Field(default=None, description="Argumentos do comando")
    ...
    timeout_seconds: int | None = Field(default=None, description="Timeout da execução")
    stdin: str | None = Field(default=None, description="Input para stdin")
...
class LambdaRuntimeClient:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "lambda_runtime_client_initialized", region=self.region, function=self.function_name
            )

        except ImportError:
            self.logger.exception("aioboto3_not_installed")
            raise LambdaRuntimeError("aioboto3 não está instalado")
        except Exception as e:
            self.logger.exception("lambda_runtime_init_failed", error=str(e))
    ...
    def _parse_log_result(self, log_result: str | None) -> str | None:
    ...
    def _parse_response_payload(self, payload: bytes) -> LambdaExecutionOutput:
        ...
        try:
            ...
            return LambdaExecutionOutput(exit_code=0, stdout=str(data), stderr="")

        except json.JSONDecodeError:
            # Payload não é JSON
            return LambdaExecutionOutput(
                exit_code=0, stdout=payload.decode("utf-8", errors="replace"), stderr=""
            )
        except Exception as e:
            return LambdaExecutionOutput(
                exit_code=-1, stdout="", stderr="", error=f"Erro ao parsear resposta: {e}"
            )

    def _extract_metrics_from_logs(self, logs: str | None) -> dict[str, Any]:
    ...
    async def invoke_lambda(
        self, request: LambdaInvocationRequest, metrics=None
            ...
            try:
                ...
                async with self._session.client("lambda", **client_kwargs) as lambda_client:
                    try:
                        response = await asyncio.wait_for(
                            lambda_client.invoke(**invoke_params), timeout=self.timeout
                        )
                    except TimeoutError:
                ...
                if payload_stream:
                    ...
                    execution_output = LambdaExecutionOutput(
                        exit_code=-1, error="Nenhum payload retornado"
                    )
                ...
                if metrics:
                    ...
                    if billing_metrics.get("billed_duration_ms") and hasattr(
                        metrics, "lambda_billed_duration_seconds"
                    ):
                ...
                self.logger.exception(
                    "lambda_invocation_failed", function_name=function_name, error=str(e)
                )
                if metrics and hasattr(metrics, "lambda_invocations_total"):
    ...
    async def get_function_info(self, function_name: str | None = None) -> dict[str, Any]:
