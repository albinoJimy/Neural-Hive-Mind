...
class KafkaTicketConsumer:
    ...
    async def initialize(self, redis_client=None):
        ...
        self.logger.info("backpressure_initialized", max_concurrent_tickets=max_concurrent)
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    "schema_registry_enabled", url=self.config.kafka_schema_registry_url
                )
            except Exception as exc:
                self.logger.warning("schema_registry_unavailable_fallback_json", error=str(exc))
            ...
            self.logger.exception("kafka_consumer_init_failed", error=str(e))
            raise
        ...
        try:
            while self.running:
                ...
                try:
                    ...
                    asyncio.create_task(self._process_ticket_with_cleanup(ticket, message))

                except Exception as e:
                    ticket_id_for_log = (
                        ticket.get("ticket_id") if isinstance(ticket, dict) else None
                    )

                    # Incrementar contador de retries
                    retry_count = (
                        await self._increment_retry_count(ticket_id_for_log)
            ...
            self.logger.exception("kafka_consumer_loop_failed", error=str(e))
            raise
        finally:
            self.running = False
    ...
    def _configure_security(self) -> dict:
        ...
        security_config = {"security.protocol": self.config.kafka_security_protocol}
    ...
    async def _get_retry_count(self, ticket_id: str) -> int:
        ...
        try:
            ...
            self.logger.exception("get_retry_count_failed", ticket_id=ticket_id, error=str(e))
    ...
    async def _increment_retry_count(self, ticket_id: str) -> int:
        ...
        try:
            ...
            self.logger.debug("retry_count_incremented", ticket_id=ticket_id, retry_count=new_count)

            return new_count
        except Exception as e:
            self.logger.exception("increment_retry_count_failed", ticket_id=ticket_id, error=str(e))
            return 1  # Fail-open
    ...
    async def _clear_retry_count(self, ticket_id: str):
        ...
        try:
            ...
            self.logger.exception("clear_retry_count_failed", ticket_id=ticket_id, error=str(e))
    ...
    async def _publish_to_dlq(self, ticket: dict, error: Exception, retry_count: int) -> bool:
        ...
        try:
            ...
            self.logger.exception(
                "dlq_serialization_failed", ticket_id=ticket_id, error=str(serialize_error)
            )
            return False
                ...
                if delivery_result["success"]:
                    ...
                    if self.metrics:
                        self.metrics.dlq_messages_total.labels(
                            reason="max_retries_exceeded", task_type=task_type
                        ).inc()
                        self.metrics.dlq_publish_duration_seconds.observe(duration)
                        self.metrics.ticket_retry_count.labels(task_type=task_type).observe(
                            retry_count
                        )
