...
class KafkaDLQConsumer:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.exception("kafka_dlq_consumer_init_failed", error=str(e))
            raise
        ...
        try:
            while self.running:
                ...
                try:
                    ...
                    self.logger.error("dlq_message_processing_failed", error=str(e), exc_info=True)
                    # Nao propagar - auto-commit garantira que nao reprocessamos

        except Exception as e:
            self.logger.exception("kafka_dlq_consumer_loop_failed", error=str(e))
            raise
        finally:
            self.running = False
    ...
    async def _persist_dlq_message(self, dlq_message: dict):
        ...
        try:
            ...
            self.logger.info("dlq_message_persisted", ticket_id=dlq_message.get("ticket_id"))
    ...
    async def _alert_sre(self, dlq_message: dict):
        ...
        try:
            ...
            self.logger.exception(
                "sre_alert_failed", ticket_id=dlq_message.get("ticket_id"), error=str(e)
            )
