...
class WorkerAgentMetrics:
    ...
    def __init__(self, config):
        ...
        self.startup_total = Counter(
            "worker_agent_startup_total", "Total de inicializações do Worker Agent"
        )

        self.registered_total = Counter(
            "worker_agent_registered_total", "Total de registros no Service Registry"
        )

        self.heartbeat_total = Counter(
            "worker_agent_heartbeat_total", "Total de heartbeats enviados", ["status"]
        )

        self.deregistered_total = Counter(
            "worker_agent_deregistered_total", "Total de deregistros do Service Registry"
        )
        ...
        self.ticket_tokens_obtained_total = Counter(
            "worker_agent_ticket_tokens_obtained_total", "Total de tokens JWT obtidos"
        )
        ...
        self.results_published_total = Counter(
            "worker_agent_results_published_total", "Total de resultados publicados", ["status"]
        )

        self.kafka_producer_errors_total = Counter(
            "worker_agent_kafka_producer_errors_total", "Total de erros no producer Kafka"
        )
        ...
        self.deploy_rollbacks_total = Counter(
            "worker_agent_deploy_rollbacks_total", "Total de rollbacks executados", ["reason"]
        )
        ...
        self.tests_passed_total = Counter(
            "worker_agent_tests_passed_total", "Total de testes aprovados", ["suite"]
        )
        self.tests_failed_total = Counter(
            "worker_agent_tests_failed_total", "Total de testes reprovados", ["suite"]
        )
        ...
        self.opa_api_calls_total = Counter(
            "worker_agent_opa_api_calls_total", "Total de chamadas à API OPA", ["method", "status"]
        )
        ...
        self.docker_executions_total = Counter(
            "worker_agent_docker_executions_total", "Total de execuções Docker", ["status"]
        )
        ...
        self.k8s_jobs_executed_total = Counter(
            "worker_agent_k8s_jobs_executed_total", "Total de Jobs K8s executados", ["status"]
        )
        ...
        self.lambda_invocations_total = Counter(
            "worker_agent_lambda_invocations_total", "Total de invocações Lambda", ["status"]
        )
        ...
        self.local_executions_total = Counter(
            "worker_agent_local_executions_total", "Total de execuções locais", ["status"]
        )
        ...
        self.parallel_queue_size = Gauge(
            "worker_agent_parallel_queue_size", "Tamanho da fila de execução paralela", ["priority"]
        )
        ...
        self.parallel_processors_running = Gauge(
            "worker_agent_parallel_processors_running", "Número de processor workers em execução"
        )
