...
class WorkerAgentSettings(BaseSettings):
    ...
    # Campo 'namespace_' com underscore para evitar conflito com env var NAMESPACE
    # Valor será obtido via validator que usa POD_NAMESPACE
    ...
    def validate_namespace_from_env(self) -> "WorkerAgentSettings":
        ...
        Prioriza POD_NAMESPACE sobre a env var NAMESPACE para evitar conflitos.
        """
    ...
    capabilities: list[str] = Field(
        default_factory=lambda: [
            "python",
            "terraform",
            "kubernetes",  # Original capabilities
            "read",
            "write",
            "compute",  # STE DAG capabilities
            "analyze",
            "transform",
            "test",
            "code",  # Additional capabilities (code added for STE compatibility)
            "security",
            "scan",
    ...
    kafka_dlq_topic: str = Field(
        default="execution.tickets.dlq", description="Topico Kafka para Dead Letter Queue"
    )
    kafka_dlq_enabled: bool = Field(default=True, description="Habilitar consumo do DLQ")
    kafka_max_retries_before_dlq: int = Field(
        default=3, description="Numero maximo de retries antes de enviar para DLQ"
    )
    ...
    argocd_notifications_webhook: str | None = None

    # GitOps / Flux CD
    flux_enabled: bool = False
    flux_namespace: str = "flux-system"
    flux_kubeconfig_path: str | None = None
    ...
    vault_fail_open: bool = Field(default=True, description="Fail-open em erros do Vault")
    ...
    spiffe_enable_x509: bool = Field(default=True, description="Habilitar X.509-SVID para mTLS")
    ...
    docker_base_url: str = Field(
        default="unix:///var/run/docker.sock", description="URL do Docker daemon"
    )
    docker_timeout_seconds: int = Field(
        default=600, description="Timeout padrão para execuções Docker"
    )
    docker_verify_ssl: bool = Field(default=True, description="Verificar SSL para conexões TCP")
    docker_default_cpu_limit: float = Field(
        default=1.0, description="Limite de CPU padrão em cores"
    )
    ...
    docker_cleanup_containers: bool = Field(
        default=True, description="Remover containers após execução"
    )

    # Kubernetes Jobs Runtime
    k8s_jobs_enabled: bool = Field(default=False, description="Habilitar K8s Jobs Runtime")
    k8s_jobs_namespace: str = Field(
        default="neural-hive-execution", description="Namespace para Jobs"
    )
    k8s_jobs_kubeconfig_path: str | None = Field(
        default=None, description="Caminho para kubeconfig"
    )
    ...
    k8s_jobs_default_memory_request: str = Field(
        default="128Mi", description="Memory request padrão"
    )
    k8s_jobs_default_memory_limit: str = Field(default="512Mi", description="Memory limit padrão")
    k8s_jobs_service_account: str = Field(
        default="worker-agent-executor", description="ServiceAccount padrão"
    )
    ...
    k8s_jobs_poll_interval_seconds: int = Field(
        default=5, description="Intervalo de polling do Job"
    )
    ...
    lambda_access_key: str | None = Field(default=None, description="AWS Access Key")
    lambda_secret_key: str | None = Field(default=None, description="AWS Secret Key")
    lambda_function_name: str = Field(
        default="neural-hive-executor", description="Nome da função Lambda"
    )
    lambda_timeout_seconds: int = Field(default=900, description="Timeout máximo (max 900s)")

    # Local Runtime
    local_runtime_enabled: bool = Field(
        default=True, description="Habilitar Local Runtime (fallback)"
    )
    local_runtime_timeout_seconds: int = Field(default=300, description="Timeout padrão")
    local_runtime_enable_sandbox: bool = Field(
        default=True, description="Habilitar validação de comandos"
    )
    ...
    local_runtime_working_dir: str = Field(
        default="/tmp/neural-hive-execution", description="Diretório de trabalho"
    )

    # Runtime Selection Strategy
    default_runtime: str = Field(
        default="local", description="Runtime padrão (k8s, docker, lambda, local)"
    )
    ...
    redis_url: str | None = Field(
        default=None, description="URL completa do Redis (se fornecida, ignora host/port)"
    )
    redis_host: str = Field(
        default="neural-hive-cache.redis-cluster.svc.cluster.local", description="Host do Redis"
    )
    ...
    redis_enabled: bool = Field(
        default=True, description="Habilitar deduplicação via Redis (fail-open se indisponível)"
    )
    ...
    mongodb_database: str = Field(
        default="neural_hive_workers", description="Nome do database MongoDB"
    )
    mongodb_dlq_collection: str = Field(
        default="execution_tickets_dlq", description="Coleção para DLQ"
    )
    mongodb_enabled: bool = Field(
        default=True, description="Habilitar MongoDB para persistência DLQ"
    )
    mongodb_max_pool_size: int = Field(default=10, description="Tamanho máximo do pool de conexões")
    mongodb_server_selection_timeout_ms: int = Field(
        default=5000, description="Timeout seleção de servidor"
    )

    # DLQ Alerting
    dlq_alert_kafka_topic: str = Field(
        default="sre.alerts", description="Tópico Kafka para alertas de DLQ"
    )
    dlq_alert_enabled: bool = Field(
        default=True, description="Habilitar alertas para mensagens DLQ"
    )

    # DLQ Retry
    dlq_publish_max_retries: int = Field(
        default=3, description="Número máximo de retries para publicar no DLQ"
    )
    dlq_publish_retry_backoff_base_seconds: float = Field(
        default=1.0, description="Base do backoff exponencial (segundos)"
    )
    dlq_publish_retry_backoff_max_seconds: float = Field(
        default=30.0, description="Máximo do backoff exponencial (segundos)"
    )

    # Neo4j (para queries de grafo - opcional)
    neo4j_enabled: bool = Field(default=False, description="Habilitar Neo4j para queries")
    neo4j_uri: str = Field(
        default="bolt://neo4j.neural-hive-data.svc.cluster.local:7687",
        description="URI de conexão Neo4j",
    )
    neo4j_user: str = Field(default="neo4j", description="Usuário Neo4j")
    neo4j_password: str | None = Field(default=None, description="Senha Neo4j")
    neo4j_database: str = Field(default="neo4j", description="Nome do banco Neo4j")
    neo4j_max_pool_size: int = Field(
        default=50, description="Tamanho máximo do pool de conexões Neo4j"
    )
    neo4j_connection_timeout: int = Field(
        default=30, description="Timeout de conexão Neo4j (segundos)"
    )
    ...
    def validate_https_in_production(self) -> "WorkerAgentSettings":
        ...
        http_endpoints = []
        if self.kafka_schema_registry_url.startswith("http://") and not is_internal_cluster_url(
            self.kafka_schema_registry_url
        ):
            http_endpoints.append(("kafka_schema_registry_url", self.kafka_schema_registry_url))
        if self.otel_exporter_endpoint.startswith("http://") and not is_internal_cluster_url(
            self.otel_exporter_endpoint
        ):
            http_endpoints.append(("otel_exporter_endpoint", self.otel_exporter_endpoint))
        if (
            self.vault_enabled
            and self.vault_address.startswith("http://")
            and not is_internal_cluster_url(self.vault_address)
        ):
    ...
    def validate_gitops_config(self) -> "WorkerAgentSettings":
        ...
        if self.argocd_enabled:
            if not self.argocd_url:
                raise ValueError("argocd_url obrigatório quando argocd_enabled=True")
            if not self.argocd_token:
                raise ValueError("argocd_token obrigatório quando argocd_enabled=True")
            # Validar formato de URL
            if not (
                self.argocd_url.startswith("http://") or self.argocd_url.startswith("https://")
            ):
                raise ValueError(
        ...
        # Validar Flux
        if self.flux_enabled and not self.flux_kubeconfig_path:
            raise ValueError("flux_kubeconfig_path obrigatório quando flux_enabled=True")
