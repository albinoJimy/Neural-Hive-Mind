...
class ExecuteExecutor(BaseTaskExecutor):
    ...
    def _select_runtime(self, requested_runtime: str | None) -> str:
        """
        ...
        fallback_chain = getattr(
            self.config, "runtime_fallback_chain", ["k8s", "docker", "local", "simulation"]
        )
    ...
    async def _execute_k8s(
        ...
        request = K8sJobRequest(
            ...
            timeout_seconds=parameters.get(
                "timeout_seconds", getattr(self.config, "k8s_jobs_timeout_seconds", 600)
            ),
            resource_limits=K8sResourceRequirements(
                cpu_request=parameters.get(
                    "cpu_request", getattr(self.config, "k8s_jobs_default_cpu_request", "100m")
                ),
                cpu_limit=parameters.get(
                    "cpu_limit", getattr(self.config, "k8s_jobs_default_cpu_limit", "1000m")
                ),
                memory_request=parameters.get(
                    "memory_request",
                    getattr(self.config, "k8s_jobs_default_memory_request", "128Mi"),
                ),
                memory_limit=parameters.get(
                    "memory_limit", getattr(self.config, "k8s_jobs_default_memory_limit", "512Mi")
                ),
            ),
            service_account=parameters.get(
                "service_account", getattr(self.config, "k8s_jobs_service_account", None)
            ),
        )
    ...
    async def _execute_docker(
        ...
        request = DockerExecutionRequest(
            ...
            timeout_seconds=parameters.get(
                "timeout_seconds", getattr(self.config, "docker_timeout_seconds", 600)
            ),
            resource_limits=ResourceLimits(
                cpu_limit=parameters.get(
                    "cpu_limit", getattr(self.config, "docker_default_cpu_limit", 1.0)
                ),
                memory_limit=parameters.get(
                    "memory_limit", getattr(self.config, "docker_default_memory_limit", "512m")
                ),
            ),
            network_mode=DockerNetworkMode(
                parameters.get(
                    "network_mode", getattr(self.config, "docker_network_mode", "bridge")
                )
            ),
        )
    ...
    async def _execute_lambda(
        ...
        request = LambdaInvocationRequest(
            function_name=parameters.get(
                "function_name",
                getattr(self.config, "lambda_function_name", "neural-hive-executor"),
            ),
    ...
    async def _execute_local(
        ...
        request = LocalExecutionRequest(
            ...
            working_dir=parameters.get("working_dir"),
            timeout_seconds=parameters.get(
                "timeout_seconds", getattr(self.config, "local_runtime_timeout_seconds", 300)
            ),
    ...
    async def _execute_code_forge(
        ...
        try:
            ...
            self.log_execution(
                ticket_id, "code_forge_execution_failed", level="warning", error=str(e)
            )
            return None
    ...
    async def execute(self, ticket: dict[str, Any]) -> dict[str, Any]:
        ...
        with tracer.start_as_current_span("task_execution") as span:
            ...
            self.log_execution(ticket_id, "execution_started", parameters=parameters)
            ...
            if code_forge_result:
                ...
                if self.metrics and hasattr(self.metrics, "execute_duration_seconds"):
                    self.metrics.execute_duration_seconds.labels(runtime="code_forge").observe(
                        code_forge_elapsed
                    )
                ...
                span.set_attribute(
                    ...
                    "success" if code_forge_result["success"] else "failed",
                )
            ...
            if requested_runtime and requested_runtime != selected_runtime:
                if self.metrics and hasattr(self.metrics, "execute_runtime_fallbacks_total"):
                    self.metrics.execute_runtime_fallbacks_total.labels(
                        from_runtime=requested_runtime, to_runtime=selected_runtime
                    ).inc()
            ...
            try:
                ...
                if self.metrics and hasattr(self.metrics, "execute_duration_seconds"):
                    self.metrics.execute_duration_seconds.labels(runtime=selected_runtime).observe(
                        elapsed_seconds
                    )
                ...
                current_index = (
                    fallback_chain.index(selected_runtime)
                    if selected_runtime in fallback_chain
                    else -1
                )
                ...
                for next_runtime in fallback_chain[current_index + 1 :]:
                    if next_runtime in self._get_available_runtimes():
                        ...
                        if self.metrics and hasattr(
                            self.metrics, "execute_runtime_fallbacks_total"
                        ):
                            self.metrics.execute_runtime_fallbacks_total.labels(
                                from_runtime=selected_runtime, to_runtime=next_runtime
                            ).inc()

                        try:
                            fallback_start = time.monotonic()
                            if next_runtime == "docker":
                                fallback_result = await self._execute_docker(
                                    ticket_id, parameters, span
                                )
                            elif next_runtime == "local":
                                fallback_result = await self._execute_local(
                                    ticket_id, parameters, span
                                )
                            elif next_runtime == "simulation":
                                fallback_result = await self._execute_simulation(
                                    ticket_id, parameters, span
                                )
                            else:
                                continue
                            ...
                            if self.metrics and hasattr(self.metrics, "execute_duration_seconds"):
                                self.metrics.execute_duration_seconds.labels(
                ...
                if self.metrics and hasattr(self.metrics, "execute_duration_seconds"):
                    self.metrics.execute_duration_seconds.labels(runtime="simulation").observe(
                        final_elapsed
                    )
