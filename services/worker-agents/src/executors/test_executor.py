...
class TestExecutor(BaseTaskExecutor):
    ...
    async def _execute_internal(self, ticket: dict[str, Any], span=None) -> dict[str, Any]:
        ...
        self.log_execution(ticket_id, "test_started", parameters=parameters)
        ...
        if provider == "github_actions":
            if self.github_actions_client:
                return await self._try_cicd_with_local_fallback(
    ...
    async def _try_local_or_simulation(
        ...
        if parameters.get("test_command"):
            self.log_execution(ticket_id, "test_fallback_to_local", reason=reason)
            return await self._execute_local(
                ticket_id=ticket_id,
                parameters=parameters,
                test_suite=test_suite,
        ...
        return await self._execute_simulation(
            ticket_id=ticket_id, test_suite=test_suite, reason=reason, span=span
        )
        ...
        for attempt in range(max_retries):
            try:
                ...
                run_id = await self.github_actions_client.trigger_workflow(
                    repo=repo, workflow_id=workflow_id, ref=ref, inputs=inputs
                )

                if self.metrics and hasattr(self.metrics, "github_actions_api_calls_total"):
                    self.metrics.github_actions_api_calls_total.labels(
                        method="trigger", status="success"
                    ).inc()

                # Wait for completion
                status = await self.github_actions_client.wait_for_run(
                    run_id=run_id, poll_interval=poll_interval, timeout=timeout_seconds, repo=repo
                )
                ...
                if self.metrics and hasattr(self.metrics, "test_tasks_executed_total"):
                    self.metrics.test_tasks_executed_total.labels(
                        status="timeout", suite=test_suite
                    ).inc()
                return {
                    "success": False,
                    "output": {
                ...
                if attempt < max_retries - 1:
                    ...
                    self.log_execution(
                        ticket_id, "test_retry_scheduled", attempt=attempt + 1, wait_time=wait_time
                    )
                ...
                if self.metrics and hasattr(self.metrics, "test_tasks_executed_total"):
                    self.metrics.test_tasks_executed_total.labels(
                        status="failed", suite=test_suite
                    ).inc()
        ...
        for attempt in range(max_retries):
            try:
                ...
                if self.metrics and hasattr(self.metrics, "gitlab_ci_api_calls_total"):
                    self.metrics.gitlab_ci_api_calls_total.labels(
                        method="trigger", status="success"
                    ).inc()
                ...
                if tests_passed == 0 and tests_failed == 0:
                    self.log_execution(
                        ticket_id, "test_gitlab_ci_parsing_artifacts", pipeline_id=pipeline_id
                    )
        ...
        for attempt in range(max_retries):
            try:
                ...
                queue_id = await self.jenkins_client.trigger_job(
                    job_name=job_name, parameters=job_parameters
                )

                if self.metrics and hasattr(self.metrics, "jenkins_api_calls_total"):
                    self.metrics.jenkins_api_calls_total.labels(
                        method="trigger", status="success"
                    ).inc()

                # Wait for build number
                build_number = await self.jenkins_client.wait_for_build_number(
                    job_name=job_name, queue_id=queue_id, poll_interval=5, timeout=300
                )
    ...
    async def _execute_local(
        ...
        self.log_execution(
            ticket_id, "test_local_started", command=test_command, working_dir=working_dir
        )
        ...
        try:
            ...
            if tests_passed == 0 and tests_failed == 0:
                report_path = Path(working_dir or ".") / parameters.get(
                ...
                if report_path.exists():
                    try:
                        ...
                        tests_passed = (
                            report_data.get("tests_passed") or report_data.get("passed") or 0
                        )
                        tests_failed = (
                            report_data.get("tests_failed") or report_data.get("failed") or 0
                        )
            ...
            self.log_execution(
                ticket_id, "test_timeout", level="error", timeout_seconds=timeout_seconds
            )
            if self.metrics and hasattr(self.metrics, "test_tasks_executed_total"):
                self.metrics.test_tasks_executed_total.labels(
                    status="timeout", suite=test_suite
            ...
            self.log_execution(ticket_id, "test_execution_error", level="error", error=str(exc))
            if self.metrics and hasattr(self.metrics, "test_tasks_executed_total"):
                self.metrics.test_tasks_executed_total.labels(
                    status="failed", suite=test_suite
                ).inc()
    ...
    async def _execute_simulation(
        ...
        Returns:
            Resultado simulado
        """
        self.log_execution(ticket_id, "test_fallback_simulation", reason=reason)
    ...
    def _parse_junit_xml_file(
        ...
        try:
            ...
            if self.metrics and hasattr(self.metrics, "test_report_parsing_total"):
                self.metrics.test_report_parsing_total.labels(
                    format="junit", status="success"
                ).inc()
    ...
    def _parse_coverage_file(
        self, coverage_path: str, working_dir: str | None = None
    ) -> float | None:
        """
        ...
        """
        try:
            from utils.test_report_parser import (
                CoberturaXMLParser,
                LCOVParser,
            ...
            if format_type == "cobertura":
                ...
                if self.metrics and hasattr(self.metrics, "coverage_report_parsing_total"):
                    self.metrics.coverage_report_parsing_total.labels(
                        format="cobertura", status="success"
                    ).inc()
