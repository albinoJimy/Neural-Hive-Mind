...
class ValidateExecutor(BaseTaskExecutor):
    ...
    def __init__(
        ...
        self.opa_url: str | None = getattr(config, "opa_url", None)
        ...
        try:
            ...
            self.sonarqube_client = (
                ...
                if getattr(config, "sonarqube_enabled", False)
                else None
            )
        except Exception:
    ...
    async def _execute_internal(self, ticket: dict[str, Any], span=None) -> dict[str, Any]:
        ...
        self.log_execution(ticket_id, "validation_started", parameters=parameters)
            ...
            try:
                ...
                if self.metrics and hasattr(self.metrics, "validate_tasks_executed_total"):
                    self.metrics.validate_tasks_executed_total.labels(
                        status="success" if success else "failed", tool="trivy"
                    ).inc()
        ...
        if validation_type == "sonarqube" and self.sonarqube_client:
            analysis = await self.sonarqube_client.trigger_analysis(
                parameters.get("project_key"), working_dir
            )
            if self.metrics and hasattr(self.metrics, "validate_tasks_executed_total"):
                self.metrics.validate_tasks_executed_total.labels(
                    status="success" if analysis.passed else "failed", tool="sonarqube"
                ).inc()
                task_metric_recorded = True
            if self.metrics and hasattr(self.metrics, "validate_tools_executed_total"):
                self.metrics.validate_tools_executed_total.labels(
                    tool="sonarqube", status="success" if analysis.passed else "failed"
                ).inc()
            return {
        ...
        if validation_type == "snyk" and self.snyk_client:
            report = await self.snyk_client.test_dependencies(
                parameters.get("manifest_path", working_dir)
            )
            if self.metrics and hasattr(self.metrics, "validate_tasks_executed_total"):
                self.metrics.validate_tasks_executed_total.labels(
                    status="success" if report.passed else "failed", tool="snyk"
                ).inc()
                task_metric_recorded = True
            if self.metrics and hasattr(self.metrics, "validate_tools_executed_total"):
                self.metrics.validate_tools_executed_total.labels(
                    tool="snyk", status="success" if report.passed else "failed"
                ).inc()
            return {
        ...
        if validation_type == "iac" and self.checkov_client:
            ...
            if self.metrics and hasattr(self.metrics, "validate_tasks_executed_total"):
                self.metrics.validate_tasks_executed_total.labels(
                    status="success" if report.passed else "failed", tool="checkov"
                ).inc()
                task_metric_recorded = True
            if self.metrics and hasattr(self.metrics, "validate_tools_executed_total"):
                self.metrics.validate_tools_executed_total.labels(
                    tool="checkov", status="success" if report.passed else "failed"
                ).inc()
            return {
                "success": report.passed,
                "output": {
    ...
    async def _execute_opa_with_client(
        ...
        self.logger.info("opa_using_dedicated_client", ticket_id=ticket_id, policy_path=policy_path)
        ...
        try:
            ...
            request = PolicyEvaluationRequest(
                policy_path=policy_path, input_data=input_data, decision=parameters.get("decision")
            )
            ...
            if self.metrics:
                if hasattr(self.metrics, "validate_tasks_executed_total"):
                    self.metrics.validate_tasks_executed_total.labels(
                        status="success" if response.allow else "failed", tool="opa"
                    ).inc()
                if hasattr(self.metrics, "validate_tools_executed_total"):
                    self.metrics.validate_tools_executed_total.labels(
                        tool="opa", status="success" if response.allow else "failed"
                    ).inc()
                ...
                if hasattr(self.metrics, "opa_api_calls_total"):
                    self.metrics.opa_api_calls_total.labels(
                        method="evaluate", status="success"
                    ).inc()
            ...
            if self.metrics:
                if hasattr(self.metrics, "validate_tasks_executed_total"):
                    self.metrics.validate_tasks_executed_total.labels(
                        status="timeout", tool="opa"
                    ).inc()
                if hasattr(self.metrics, "opa_api_calls_total"):
                    self.metrics.opa_api_calls_total.labels(
                        method="evaluate", status="timeout"
                    ).inc()
            ...
            self.logger.exception(
                "opa_validation_error", ticket_id=ticket_id, policy_path=policy_path, error=str(exc)
            )

            if self.metrics and hasattr(self.metrics, "validate_tasks_executed_total"):
                self.metrics.validate_tasks_executed_total.labels(
                    status="validation_error", tool="opa"
                ).inc()
            ...
            return {
                ...
                "output": {
                    ...
                    "violations": [
                        {"rule_id": "opa_validation_error", "message": str(exc), "severity": "HIGH"}
                    ],
                    "validation_type": "policy",
                    "rules_checked": 0,
                },
                ...
                "logs": ["Validation started", f"OPA validation error: {exc}"],
            }

        except Exception as exc:
            self.logger.exception(
                "opa_unexpected_error", ticket_id=ticket_id, policy_path=policy_path, error=str(exc)
            )
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                response = await client.post(
                    f"{self.opa_url}/v1/data/{policy_path}", json={"input": input_data}
                )
                ...
                if self.metrics and hasattr(self.metrics, "validate_tasks_executed_total"):
                    self.metrics.validate_tasks_executed_total.labels(
                        status="success" if allow else "failed", tool="opa"
                    ).inc()
                if self.metrics and hasattr(self.metrics, "validate_tools_executed_total"):
                    self.metrics.validate_tools_executed_total.labels(
                        tool="opa", status="success" if allow else "failed"
                    ).inc()
                return result
        except Exception as exc:
            self.log_execution(ticket_id, "validation_opa_error", level="error", error=str(exc))
            # fallback para simulação
            if self.metrics and hasattr(self.metrics, "validate_tasks_executed_total"):
    ...
    async def _execute_opa_fallback(
        self, ticket_id: str, policy_path: str, reason: str
    ) -> dict[str, Any]:
        """
        ...
        for severity, count in severity_counts.items():
            if count > 0:
                ...
                if hasattr(self.metrics, "validate_violations_total"):
                    self.metrics.validate_violations_total.labels(
                        severity=severity.value.lower(), tool="opa"
                    ).inc(count)
