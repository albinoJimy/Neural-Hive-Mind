...
class QueryExecutor(BaseTaskExecutor):
    ...
    def validate_ticket(self, ticket: dict[str, Any]) -> None:
        ...
        if query_type == "mongodb":
            self.validate_required_parameters(ticket_id, parameters, required=["collection"])
    ...
    async def execute(self, ticket: dict[str, Any]) -> dict[str, Any]:
        ...
        with span_context as span:
            ...
            self.log_execution(
                ticket_id, "query_execution_started", query_type=query_type, parameters=parameters
            )
            ...
            try:
                ...
                if self.metrics and hasattr(self.metrics, "query_executed_total"):
                    ...
                    self.metrics.query_executed_total.labels(
                        status=status, query_type=query_type
                    ).inc()
                ...
                return {
                    ...
                    "logs": [f"Query execution failed: {e!s}"],
                }
    ...
    async def _execute_mongodb_query(
        ...
        try:
            ...
            return {
                "success": True,
                "output": {"documents": serialized_docs, "count": len(serialized_docs)},
                "metadata": {
                    "executor": "QueryExecutor",
                    "query_type": "mongodb",
                    "collection": collection_name,
            ...
            self.log_execution(ticket_id, "mongodb_query_failed", level="error", error=str(e))
            return {
                "success": False,
                "output": None,
                "metadata": {"executor": "QueryExecutor", "query_type": "mongodb", "error": str(e)},
                "logs": [f"MongoDB query failed: {e!s}"],
            }
    ...
    async def _execute_neo4j_query(
        ...
        try:
            ...
            results = await self.neo4j_client.execute_query(
                query=cypher_query, parameters=query_params, timeout=timeout
            )
            ...
            self.log_execution(
                ticket_id, "neo4j_query_completed", result_count=len(serialized_results)
            )

            return {
                "success": True,
                "output": {"results": serialized_results, "count": len(serialized_results)},
                "metadata": {
                    "executor": "QueryExecutor",
                    "query_type": "neo4j",
            ...
            self.log_execution(ticket_id, "neo4j_query_failed", level="error", error=str(e))
            return {
                "success": False,
                "output": None,
                "metadata": {"executor": "QueryExecutor", "query_type": "neo4j", "error": str(e)},
                "logs": [f"Neo4j query failed: {e!s}"],
            }

    async def _execute_kafka_query(
        self, ticket_id: str, parameters: dict[str, Any], span
    ) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                self.log_execution(
                    ticket_id, "kafka_query_completed", topic=topic, message_count=len(messages)
                )
            ...
            self.log_execution(ticket_id, "kafka_query_failed", level="error", error=str(e))
            return {
                "success": False,
                "output": None,
                "metadata": {"executor": "QueryExecutor", "query_type": "kafka", "error": str(e)},
                "logs": [f"Kafka query failed: {e!s}"],
            }
    ...
    async def _execute_redis_query(
        ...
        try:
            ...
            if operation == "get":
                ...
                self.log_execution(
                    ticket_id, "redis_get_completed", key=key, exists=value is not None
                )
                ...
                return {
                    ...
                    "logs": [f"Redis GET on {key}", "Query completed successfully"],
                }
            ...
            if operation == "scan":
                ...
                self.log_execution(
                    ticket_id, "redis_scan_completed", pattern=pattern, key_count=len(keys)
                )
            ...
            if operation == "keys":
                ...
                self.log_execution(
                    ticket_id, "redis_keys_completed", pattern=pattern, key_count=len(keys)
                )
            ...
            self.log_execution(
                ticket_id, "redis_query_failed", level="error", operation=operation, error=str(e)
            )
            return {
                "success": False,
                "output": None,
                ...
                "logs": [f"Redis {operation} failed: {e!s}"],
            }
    ...
    def _error_result(self, message: str, query_type: str) -> dict[str, Any]:
        ...
        return {
            "success": False,
            "output": None,
            "metadata": {"executor": "QueryExecutor", "query_type": query_type, "error": message},
            "logs": [message],
        }
