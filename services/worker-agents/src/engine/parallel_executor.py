...
class ParallelExecutor:
    ...
    async def submit_ticket(
        ...
        if self.metrics:
            if hasattr(self.metrics, 'parallel_tickets_submitted_total'):
    ...
    async def execute_parallel_independent(
        ...
        self.logger.info(
            'parallel_independent_execution_start',
        ...
        if self.metrics:
            if hasattr(self.metrics, 'parallel_batch_duration_seconds'):
    ...
    async def execute_with_dependencies(
        ...
        self.logger.info(
            'parallel_execution_with_deps_start',
            num_tickets=len(tickets)
        )
        ...
        while len(completed) + len(failed) < len(tickets):
            ...
            if not ready_tickets:
                ...
                remaining = [
                    tid for tid in tickets_map.keys()
                ...
                ready_tickets = [
                    tickets_map[tid] for tid in tickets_map.keys()
                    if tid not in completed and tid not in failed
                ]
                if not ready_tickets:
                    break
...
async def execute_parallel_tickets(
    ...
    return await executor.execute_parallel_independent(
        tickets,
