...
class DependencyCoordinator:
    ...
    async def wait_for_dependencies(self, ticket: dict) -> bool:
        ...
        self.logger.info("waiting_for_dependencies", ticket_id=ticket_id, dependencies=dependencies)

        start_time = datetime.now()

        for _attempt in range(self.config.dependency_check_max_attempts):
            all_completed = True
            failed_deps = []
            ...
            if failed_deps:
                self.logger.error(
                    "dependencies_failed", ticket_id=ticket_id, failed_dependencies=failed_deps
                )
                if self.metrics:
    ...
    async def _check_dependency_status(self, dependency_ticket_id: str) -> str:
        ...
        # Atualizar cache
        self._status_cache[dependency_ticket_id] = {"status": status, "timestamp": datetime.now()}
