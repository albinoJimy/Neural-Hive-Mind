...
class TaskCancellationRequest(BaseModel):
    ...
    reason: str = Field(
        ..., description="Reason for cancellation (e.g., 'preemption', 'timeout', 'user_request')"
    )
    preempted_by: str | None = Field(None, description="Ticket ID of the preempting task")
    grace_period_seconds: int = Field(
        default=30, ge=0, le=300, description="Grace period for checkpointing"
    )
...
class SPIFFEJWTValidator:
        ...
        try:
            ...
            if JWT_AVAILABLE and spiffe_manager:
                ...
                if kid and kid in trust_bundle_keys:
                    ...
                    first_key = next(iter(trust_bundle_keys.values()))
            ...
            self.logger.exception("jwt_validation_error", error=str(e))
            return None
    ...
    def _jwk_to_pem(self, jwk: dict) -> str | None:
        ...
        try:
            ...
            self.logger.exception("jwk_to_pem_failed", error=str(e))
            return None
...
def create_http_server(config, app_state):
    ...
    def _is_internal_orchestrator_call(request: Request) -> bool:
        ...
        return internal_caller == f"spiffe://{trust_domain}/orchestrator-dynamic"
    ...
    async def verify_spiffe_token(
        ...
        if preemption_enabled and _is_internal_orchestrator_call(request):
            logger.info(
                "internal_orchestrator_call_allowed", path=request.url.path, method=request.method
            )
    ...
    async def cancel_task(
        ...
        spiffe_id: str | None = Depends(verify_spiffe_token),
    ):
        """
        ...
        try:
            ...
            logger.exception("task_cancellation_failed", ticket_id=ticket_id, error=str(e))
