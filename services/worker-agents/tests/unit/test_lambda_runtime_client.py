...
class TestLambdaRuntimeClientInitialization:
    ...
    async def test_initialize_success(self):
        ...
        client = LambdaRuntimeClient(
            region="us-east-1", access_key="test-key", secret_key="test-secret"
        )
...
class TestLambdaRuntimeClientInvocation:
    ...
    async def test_invoke_lambda_success(self):
        ...
        mock_payload_stream.read = AsyncMock(
            return_value=json.dumps({"exit_code": 0, "stdout": "Hello World", "stderr": ""}).encode(
                "utf-8"
            )
        )
        ...
        request = LambdaInvocationRequest(
            ...
            payload=LambdaPayload(command="echo", args=["Hello", "World"]),
    ...
    async def test_invoke_lambda_with_function_error(self):
        ...
        mock_payload_stream.read = AsyncMock(
            return_value=json.dumps(
                {"errorMessage": "Command failed", "errorType": "RuntimeError"}
            ).encode("utf-8")
        )
        ...
        request = LambdaInvocationRequest(
            function_name="test-function", payload=LambdaPayload(command="bad-command")
        )
    ...
    async def test_invoke_lambda_timeout(self):
        ...
        request = LambdaInvocationRequest(
            function_name="test-function", payload=LambdaPayload(command="slow-command")
        )
...
class TestLambdaRuntimeClientParsing:
    ...
    def test_parse_response_payload_dict(self):
        ...
        payload = json.dumps(
            {"exit_code": 0, "stdout": "Output text", "stderr": "Error text"}
        ).encode("utf-8")
...
class TestLambdaRuntimeClientFunctionInfo:
    ...
    async def test_get_function_info_success(self):
        ...
        mock_lambda_client.__aenter__ = AsyncMock(return_value=mock_lambda_client)
        mock_lambda_client.__aexit__ = AsyncMock(return_value=None)
        mock_lambda_client.get_function = AsyncMock(
            return_value={
                "Configuration": {
...
class TestLambdaRuntimeClientModels:
    ...
    def test_lambda_invocation_result_model(self):
        ...
        result = LambdaInvocationResult(
            ...
            status_code=200,
            function_error=None,
            response=LambdaExecutionOutput(exit_code=0, stdout="Success", stderr=""),
            duration_ms=150,
