...
def create_mock_code_forge_client(
    ...
    async def wait_for_completion(*args, **kwargs):
        if status == "timeout":
            return create_pipeline_status(
                pipeline_id=pipeline_id, status="timeout", stage="pending"
            )
    ...
    client.get_pipeline_status = AsyncMock(
        return_value=create_pipeline_status(pipeline_id=pipeline_id, status=status)
    )
...
def create_mock_github_actions_client(
    ...
    client.get_workflow_run = AsyncMock(
        return_value=WorkflowRunStatus(
            run_id=run_id,
            status="completed",
            conclusion="success" if success else "failure",
            passed=tests_passed,
...
def create_mock_sonarqube_client(
    ...
    client = AsyncMock()
    client.trigger_analysis = AsyncMock(
        return_value=SonarQubeAnalysis(
            task_id="task-123",
            passed=passed,
            issues=issues or [],
...
def create_mock_snyk_client(
    ...
    client = AsyncMock()
    client.test_dependencies = AsyncMock(
        return_value=SnykReport(
            passed=passed,
...
def create_mock_checkov_client(
    ...
    client = AsyncMock()
    client.scan_iac = AsyncMock(
        return_value=CheckovReport(
            passed=passed,
