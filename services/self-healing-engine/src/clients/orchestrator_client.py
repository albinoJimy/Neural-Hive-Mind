...
try:
    from src.proto import (
        orchestrator_strategic_pb2 as pb2,
        orchestrator_strategic_pb2_grpc as pb2_grpc,
    )
except ImportError:
...
class OrchestratorClient:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info("orchestrator_client.initialized", target=target, use_mtls=use_mtls)

        except Exception as e:
            logger.error("orchestrator_client.initialization_failed", target=target, error=str(e))
        ...
        with tracer.start_as_current_span("self_healing.orchestrator.pause_workflow") as span:
            ...
            try:
                request = pb2.PauseWorkflowRequest(
                    workflow_id=workflow_id, reason=reason, adjustment_id=adj_id
                )
                if duration_seconds:
                ...
                response = await self.stub.PauseWorkflow(
                    request, timeout=self.timeout_seconds, metadata=metadata or None
                )

                duration = (datetime.now() - start_time).total_seconds()
                ORCHESTRATOR_CALL_DURATION.labels(operation=operation).observe(duration)
                ORCHESTRATOR_CALLS_TOTAL.labels(
                    operation=operation, status="success" if response.success else "failed"
                ).inc()
                ...
                duration = (datetime.now() - start_time).total_seconds()
                ORCHESTRATOR_CALL_DURATION.labels(operation=operation).observe(duration)
                ORCHESTRATOR_CALLS_TOTAL.labels(operation=operation, status="error").inc()
    ...
    async def get_workflow_status(
        ...
        with tracer.start_as_current_span("self_healing.orchestrator.get_workflow_status") as span:
            ...
            try:
                ...
                duration = (datetime.now() - start_time).total_seconds()
                ORCHESTRATOR_CALL_DURATION.labels(operation=operation).observe(duration)
                ORCHESTRATOR_CALLS_TOTAL.labels(operation=operation, status="success").inc()
                ...
                if hasattr(response, "sla_remaining_seconds") and response.HasField(
                ...
                if response.allocated_resources:
                    ...
                    if response.allocated_resources.HasField("gpu_count"):
                        result["allocated_resources"][
                            "gpu_count"
                        ] = response.allocated_resources.gpu_count
