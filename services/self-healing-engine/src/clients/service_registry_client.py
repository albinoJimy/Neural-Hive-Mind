...
class ServiceRegistryClient:
    ...
    async def notify_agent(self, agent_id: str, notification: Dict) -> bool:
        ...
        try:
            ...
            await self.stub.NotifyAgent(
                request, timeout=self.timeout_seconds, metadata=grpc_metadata
            )
            ...
            logger.warning(
                "service_registry_client.notify_agent_failed", agent_id=agent_id, error=str(e)
            )
            return False
    ...
    async def get_agent_info(self, agent_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            response = await self.stub.GetAgent(
                request, timeout=self.timeout_seconds, metadata=grpc_metadata
            )
            ...
            logger.warning(
                "service_registry_client.get_agent_failed", agent_id=agent_id, error=str(e)
            )
            return None
