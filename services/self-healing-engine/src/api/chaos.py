...
async def execute_experiment(
    ...
    try:
        report = await chaos_engine.execute_experiment(
            experiment_id, executed_by=request.executed_by
        )
        return report
...
async def list_scenarios(
    ...
    for name in scenarios:
        ...
        if info:
            scenario_info.append({"name": name, **info})

    return {"total": len(scenario_info), "scenarios": scenario_info}
...
async def get_scenario(
    ...
    if not info:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Cenário não encontrado: {scenario_name}"
        )

    return {"name": scenario_name, **info}
...
async def execute_scenario(
    ...
    try:
        report = await chaos_engine.execute_scenario(
            request.scenario_name, config, executed_by=request.executed_by
        )
        return report
    except ValueError as e:
...
async def list_reports(
    ...
    try:
        reports = await chaos_engine.mongodb_client.find(
            "chaos_reports", {}, limit=limit, skip=offset, sort=[("generated_at", -1)]
        )

        return {"total": len(reports), "offset": offset, "limit": limit, "reports": reports}
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
...
async def get_report(
    ...
    if not chaos_engine.mongodb_client:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="MongoDB não configurado"
        )

    try:
        report = await chaos_engine.mongodb_client.find_one(
            "chaos_reports", {"experiment_id": experiment_id}
        )
...
async def chaos_health(request: Request):
    ...
    if not chaos_engine:
        return {"status": "disabled", "message": "Chaos Engine não está habilitado"}
