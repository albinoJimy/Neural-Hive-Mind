...
def _get_manager(request: Request) -> RemediationManager:
    ...
    if not manager:
        raise HTTPException(
            ...
            detail="Remediation manager unavailable",
        )
    return manager
...
def _get_executor(request: Request) -> PlaybookExecutor:
    ...
    if not executor:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Playbook executor unavailable"
        )
    return executor


@router.post(
    "/remediation/execute", response_model=RemediationResponse, status_code=status.HTTP_202_ACCEPTED
)
...
async def remediation_status(
    remediation_id: str, manager: RemediationManager = Depends(_get_manager)
):
...
async def list_playbooks(executor: PlaybookExecutor = Depends(_get_executor)):
