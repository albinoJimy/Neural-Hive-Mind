...
class FaultParameters(BaseModel):
    ...
    packet_loss_percent: Optional[float] = Field(
        default=None, ge=0, le=100, description="% de perda de pacotes"
    )
    bandwidth_limit_kbps: Optional[int] = Field(
        default=None, ge=0, description="Limite de bandwidth em Kbps"
    )
    correlation_percent: Optional[float] = Field(
        default=None, ge=0, le=100, description="Correlação de perda"
    )

    # Resource parameters
    cpu_cores: Optional[int] = Field(default=None, ge=1, description="Núcleos de CPU para stress")
    cpu_load_percent: Optional[int] = Field(
        default=None, ge=0, le=100, description="% de carga de CPU"
    )
    memory_bytes: Optional[int] = Field(
        default=None, ge=0, description="Bytes de memória para stress"
    )
    disk_fill_bytes: Optional[int] = Field(
        default=None, ge=0, description="Bytes para preencher disco"
    )
    disk_path: Optional[str] = Field(default=None, description="Path para disk fill")

    # Application parameters
    http_status_code: Optional[int] = Field(
        default=None, ge=100, le=599, description="Código HTTP para injetar"
    )
...
class ValidationCriteria(BaseModel):
    ...
    max_recovery_time_seconds: int = Field(
        default=300, ge=0, description="Tempo máximo de recuperação"
    )
    min_availability_percent: float = Field(
        default=99.0, ge=0, le=100, description="Disponibilidade mínima"
    )
    max_error_rate_percent: float = Field(
        default=1.0, ge=0, le=100, description="Taxa máxima de erros"
    )
    max_latency_p95_ms: Optional[int] = Field(
        default=None, ge=0, description="Latência P95 máxima em ms"
    )
    ...
    required_playbook: Optional[str] = Field(
        default=None, description="Playbook que deve ser executado"
    )
...
class FaultInjection(BaseModel):
    ...
    parameters: FaultParameters = Field(
        default_factory=FaultParameters, description="Parâmetros da falha"
    )
...
class ChaosExperiment(BaseModel):
    ...
    validation_criteria: ValidationCriteria = Field(
        default_factory=ValidationCriteria, description="Critérios de validação"
    )
    rollback_strategy: RollbackStrategy = Field(
        default=RollbackStrategy.AUTOMATIC, description="Estratégia de rollback"
    )
    timeout_seconds: int = Field(
        default=600, ge=60, le=3600, description="Timeout total em segundos"
    )
...
class ValidationResult(BaseModel):
    ...
    recovery_time_seconds: Optional[float] = Field(
        default=None, description="Tempo de recuperação em segundos"
    )
    availability_percent: Optional[float] = Field(
        default=None, description="Disponibilidade durante teste"
    )
    error_rate_percent: Optional[float] = Field(
        default=None, description="Taxa de erros durante teste"
    )
...
class ChaosExperimentRequest(BaseModel):
    ...
    validation_criteria: Optional[ValidationCriteria] = Field(
        default=None, description="Critérios de validação"
    )
    rollback_strategy: RollbackStrategy = Field(
        default=RollbackStrategy.AUTOMATIC, description="Estratégia rollback"
    )
