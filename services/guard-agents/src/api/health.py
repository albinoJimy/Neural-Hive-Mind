...
async def readiness(request: Request) -> HealthResponse:
    ...
    checks = {
        ...
        and request.app.state.mongodb.is_healthy(),
        "redis": getattr(request.app.state, "redis", None) and request.app.state.redis.is_healthy(),
        # Dependências de mensageria
        "kafka_security": getattr(request.app.state, "security_consumer", None)
        and request.app.state.security_consumer.is_healthy(),
        "kafka_orchestration": getattr(request.app.state, "orchestration_consumer", None)
        and request.app.state.orchestration_consumer.is_healthy(),
        # Dependências de infraestrutura
        "kubernetes": getattr(request.app.state, "k8s", None)
        and request.app.state.k8s.is_healthy(),
        "service_registry": getattr(request.app.state, "service_registry", None)
        and request.app.state.service_registry.is_healthy(),
    ...
    if hasattr(request.app.state, "message_handler"):
        ...
        checks["orchestrator"] = (
            hasattr(handler, "orchestrator") and handler.orchestrator is not None
        )
        checks["threat_detector"] = (
            hasattr(handler, "threat_detector") and handler.threat_detector is not None
        )
        checks["incident_classifier"] = (
            hasattr(handler, "incident_classifier") and handler.incident_classifier is not None
        )
        checks["policy_enforcer"] = (
            hasattr(handler, "policy_enforcer") and handler.policy_enforcer is not None
        )
        checks["remediation_coordinator"] = (
            hasattr(handler, "remediation_coordinator")
            and handler.remediation_coordinator is not None
        )
