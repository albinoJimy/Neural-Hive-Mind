"""
Wrapper LLM Client para requirements-engineering usando neural_hive_llm.
...
class LLMClient:
    ...
    Wrapper que mantém a API existente do requirements-engineering
    ...
    async def generate(
        ...
        Args:
            ...
            response_format: Formato de resposta (json_object)
    ...
    async def close(self):
        """Fecha o cliente LLM."""
        if self._client:
            # neural_hive_llm doesn't have explicit close, just clear
            self._client = None
            self._started = False
