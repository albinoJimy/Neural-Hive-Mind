...
class StrategicDecisionEngine:
    ...
    async def process_consolidated_decision(
        ...
        try:
            ...
            divergence_score = decision_data.get("consensus_metrics", {}).get(
            ...
            needs_action = (
                aggregated_risk > 0.7
                or divergence_score > 0.05
                or requires_human_review
                or len(guardrails_triggered) > 0
            )
    ...
    async def _get_critical_incidents(self) -> list[str]:
        ...
        try:
            ...
            incidents = await self.mongodb_client.db.incidents.find(
                {"severity": {"$in": ["CRITICAL", "HIGH"]}, "resolved": False}
            ).to_list(length=20)
    ...
    async def _perform_analysis(
        ...
        try:
            ...
            if context.active_plans:
                neo4j_results = await self.neo4j_client.query_strategic_context(
            ...
            for plan_id in context.active_plans:
                ...
                pheromone_signals[plan_id] = signals.get("SUCCESS", 0.0) - signals.get(
                    "FAILURE", 0.0
                )
    ...
    async def _determine_action(
        ...
        if event_type == "critical_incident":
            ...
            divergence = (
                trigger.get("decision_data", {})
                .get("consensus_metrics", {})
                .get("divergence_score", 0.0)
            )
            if divergence > 0.05:
                return (
    ...
    async def _calculate_confidence(
        self, context: DecisionContext, analysis: DecisionAnalysis
    ) -> float:
        """
    ...
    async def _get_historical_success_rate(self) -> float:
        ...
        try:
            ...
            pipeline = [
                {"$match": {"created_at": {"$gte": cutoff_timestamp}}},
                {
                    "$group": {
                        "_id": None,
                        "total": {"$sum": 1},
    ...
    async def _validate_guardrails(
        ...
        try:
            ...
            opa_result = await self.opa_client.evaluate_policy(
                policy_path="neuralhive/queen/ethical_guardrails", input_data=opa_input
            )
            ...
            QueenAgentMetrics.opa_evaluations_total.labels(
                policy="ethical_guardrails", result="allowed"
            ).inc()
