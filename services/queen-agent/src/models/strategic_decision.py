...
class StrategicDecision(BaseModel):
    ...
    decision_id: str = Field(
        default_factory=lambda: str(uuid4()), description="ID único da decisão"
    )
    decision_type: DecisionType = Field(..., description="Tipo de decisão")
    correlation_id: str = Field(
        default_factory=lambda: str(uuid4()), description="ID de correlação"
    )
    ...
    context: DecisionContext = Field(
        default_factory=DecisionContext, description="Contexto estratégico"
    )
    analysis: DecisionAnalysis = Field(
        default_factory=DecisionAnalysis, description="Análise realizada"
    )
    ...
    def to_avro_dict(self) -> dict[str, Any]:
        ...
        data["analysis"] = {
            "neo4j_query_results": {
                k: json.dumps(v) for k, v in self.analysis.neo4j_query_results.items()
            },
    ...
    def from_avro_dict(cls, data: dict[str, Any]) -> "StrategicDecision":
        ...
        if "decision" in data and "parameters" in data["decision"]:
            data["decision"]["parameters"] = {
                k: json.loads(v) for k, v in data["decision"]["parameters"].items()
            }
