...
class QoSAdjustment(BaseModel):
    ...
    adjustment_id: str = Field(
        default_factory=lambda: str(uuid4()), description="ID único do ajuste"
    )
    ...
    def to_grpc_request(
        ...
        if self.adjustment_type in (
            AdjustmentType.INCREASE_PRIORITY,
            AdjustmentType.DECREASE_PRIORITY,
        ):
        ...
        if self.adjustment_type == AdjustmentType.TRIGGER_REPLANNING:
            ...
            trigger_type = trigger_map.get(
                trigger_str, orchestrator_strategic_pb2.TRIGGER_TYPE_STRATEGIC
            )
