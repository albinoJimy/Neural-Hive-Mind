...
REDIS_AVAILABLE = (
    os.getenv("REDIS_TEST_HOST") or os.getenv("INTEGRATION_TESTS", "false").lower() == "true"
)
...
def pheromone_client(mock_redis_client_wrapper, mock_settings):
    ...
    return PheromoneClient(redis_client=mock_redis_client_wrapper, settings=mock_settings)
...
async def test_get_success_trails_real_redis(pheromone_client, redis_client):
    ...
    for i in range(len(result) - 1):
        assert (
            result[i]["strength"] >= result[i + 1]["strength"]
