    ...
    async def test_action_when_high_risk(self, engine):
        ...
        engine._determine_action = AsyncMock(
            return_value=(
                ...
                DecisionAction(
                    action="adjust_priorities", target_entities=[], parameters={}, rationale="test"
                ),
            )
        )
        engine._calculate_confidence = AsyncMock(return_value=0.8)
        engine._assess_risk = AsyncMock(
            return_value=RiskAssessment(risk_score=0.3, risk_factors=[], mitigations=[])
        )
    ...
    async def test_action_when_divergence_high(self, engine):
        ...
        engine._determine_action = AsyncMock(
            return_value=(
                ...
                DecisionAction(
                    action="resolve_conflict", target_entities=[], parameters={}, rationale="test"
                ),
            )
        )
    ...
    async def test_sla_violation_triggers_action(self, engine):
        ...
        engine._determine_action = AsyncMock(
            return_value=(
                ...
                DecisionAction(
                    action="trigger_replanning", target_entities=[], parameters={}, rationale="test"
                ),
            )
        )
        engine._calculate_confidence = AsyncMock(return_value=0.7)
    ...
    async def test_critical_incident_triggers_action(self, engine):
        ...
        engine._determine_action = AsyncMock(
            return_value=(
                ...
                DecisionAction(
                    action="pause_execution", target_entities=[], parameters={}, rationale="test"
                ),
            )
        )
        engine._calculate_confidence = AsyncMock(return_value=0.7)
...
class TestDetermineAction:
    ...
    async def test_divergence_triggers_conflict_resolution(self, engine):
        ...
        trigger = {
            ...
            "decision_data": {"consensus_metrics": {"divergence_score": 0.1}},
        }
        context = DecisionContext(active_plans=["plan-123"])
        analysis = DecisionAnalysis()
...
class TestAssessRisk:
    ...
    async def test_high_risk_triggers_mitigations(self, engine):
        ...
        assert (
            "require_human_approval" in risk.mitigations
            or "increase_monitoring" in risk.mitigations
        )
...
class TestValidateGuardrails:
    ...
    async def test_opa_allows_decision(self, engine):
        ...
        engine.opa_client.evaluate_policy = AsyncMock(
            return_value={
                "allow": True,
...
class TestExecuteDecisionAction:
    ...
    async def test_reallocate_resources_via_grpc(self, engine):
        ...
        engine.orchestrator_client.rebalance_resources = AsyncMock(return_value={"success": True})
