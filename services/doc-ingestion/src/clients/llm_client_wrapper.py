"""
Wrapper LLM Client para doc-ingestion usando neural_hive_llm.
...
class LLMClient:
    ...
    Wrapper que mantém a API existente do doc-ingestion
    enquanto delega para a biblioteca neural_hive_llm.
    """

    def __init__(self, api_key: str | None = None, model: str = "gpt-4", provider: str = "openai"):
        """Inicializa o cliente LLM.

        Args:
            api_key: Chave de API (opcional, usa settings se None)
            model: Nome do modelo LLM
            provider: Provider LLM ("openai" ou "anthropic")
        """
        ...
        # Para compatibilidade, determinar a API key correta baseado no provider
        if provider == "openai":
    ...
    async def _ensure_client(self):
        ...
        if not self._client and self.api_key:
            try:
                llm_provider = (
                    LLMProvider.OPENAI if self.provider == "openai" else LLMProvider.ANTHROPIC
                )
    ...
    # Compatibilidade com AsyncOpenAI - chat.completions.create
    class ChatCompletions:
        """Namespace para compatibilidade com openai.AsyncOpenAI.chat.completions."""
        ...
        async def create(
            ...
            """Cria completion (compatibilidade OpenAI)."""
    ...
    def chat(self) -> "Chat":  # noqa: F821 - Chat é classe local definida no corpo deste property
        ...
        class Chat:
            """Namespace para compatibilidade com openai.AsyncOpenAI.chat."""

            def __init__(self, parent: "LLMClient"):
                self._parent = parent

            @property
            def completions(self) -> LLMClient.ChatCompletions:
                """Propriedade para compatibilidade com openai.AsyncOpenAI.chat.completions."""
                return LLMClient.ChatCompletions(self._parent)
