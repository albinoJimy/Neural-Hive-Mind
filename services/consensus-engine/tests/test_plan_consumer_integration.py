...
def mock_specialists_client():
    ...
    client.evaluate_plan_parallel = AsyncMock(
        return_value=[
            {
                "opinion_id": "op-1",
    ...
    client.evaluate_plan = AsyncMock(
        return_value={
            "opinion_id": "op-seq-1",
            "specialist_type": "business",
            "specialist_version": "1.0.0",
...
async def test_plan_consumer_sequential_fallback(
    ...
    # Create consumer
    consumer = PlanConsumer(
        config, mock_specialists_client, mock_mongodb_client, mock_pheromone_client
    )
    consumer.orchestrator = mock_orchestrator
    ...
    called_types = [call[0][0] for call in mock_specialists_client.evaluate_plan.call_args_list]
...
async def test_plan_consumer_handles_specialist_errors_gracefully(
    ...
    mock_specialists_client.evaluate_plan_parallel = AsyncMock(
        return_value=[
            {
                "opinion_id": "op-1",
                "specialist_type": "business",
