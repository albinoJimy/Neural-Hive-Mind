...
class TestCircuitBreaker:
    ...
    async def test_circuit_breaker_opens_on_systemic_process_errors(self, plan_consumer):
        ...
        with (
            ...
            systemic_calls = [
                c for c in mock_inc_error.call_args_list if c[1].get("is_systemic") is True
            ]
            assert len(systemic_calls) >= 1

    async def test_circuit_breaker_stays_closed_on_business_errors(
        self, plan_consumer, mock_config_resilience
    ):
