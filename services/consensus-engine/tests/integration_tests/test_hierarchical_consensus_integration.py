...
def mock_pheromone_client():
    ...
    client.get_aggregated_pheromone = AsyncMock(
        return_value={'net_strength': 0.8}
    )
...
class TestHierarchicalConsensusIntegration:
    ...
    async def test_hierarchical_weights_applied_in_consensus(
        ...
        business_vote = next(
            v for v in decision.specialist_votes
            if v.specialist_type == 'business'
        )
        technical_vote = next(
            v for v in decision.specialist_votes
            if v.specialist_type == 'technical'
        )
    ...
    @pytest.mark.asyncio
    async def test_expert_architecture_has_highest_weight(
        self, mock_config, mock_pheromone_client
    ):
        ...
        architecture_vote = next(
            v for v in decision.specialist_votes
            if v.specialist_type == 'architecture'
        )
        business_vote = next(
            v for v in decision.specialist_votes
            if v.specialist_type == 'business'
    ...
    @pytest.mark.asyncio
    async def test_seniority_distribution_in_metrics(
        self, mock_config, mock_pheromone_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_hierarchical_disabled_uses_base_weights(
        self, mock_pheromone_client
    ):
...
class TestHierarchicalConsensusScenarios:
    ...
    @pytest.mark.asyncio
    async def test_unanimous_experts_approve_quickly(
        self, mock_config, mock_pheromone_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_mixed_seniority_divergent_opinions(
        self, mock_config, mock_pheromone_client
    ):
