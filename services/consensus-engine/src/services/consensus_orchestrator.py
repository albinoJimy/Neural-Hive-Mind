...
class ConsensusOrchestrator:
    ...
    async def process_consensus(
        ...
        weights = await self._calculate_dynamic_weights(cognitive_plan, specialist_opinions)
        ...
        consensus_method = self._determine_consensus_method(
            aggregated_confidence, divergence, is_unanimous
        )
        ...
        pheromone_strength = await self._get_average_pheromone_strength(
            specialist_opinions, cognitive_plan.get("original_domain", "BUSINESS")
        )
        ...
        if not correlation_id or (isinstance(correlation_id, str) and not correlation_id.strip()):
            ...
            # WARNING: correlation_id gerado neste ponto - upstream deve ser verificada
            # Não é mais ERROR pois temos geração de fallback em todos os serviços upstream
            logger.warning(
                "F1-corrigido: correlation_id ausente no cognitive_plan - UUID gerado para manter rastreabilidade",
                plan_id=cognitive_plan["plan_id"],
        ...
        decision = ConsolidatedDecision(
            ...
            reasoning_summary=self._generate_reasoning_summary(
                final_decision, aggregated_confidence, aggregated_risk, is_unanimous, violations
            ),
            ...
            compliance_checks={
                ...
                "base_confidence_threshold_passed": aggregated_confidence
                >= self.config.min_confidence_score,
                "base_divergence_threshold_passed": divergence
                <= self.config.max_divergence_threshold,
                # Campos adaptativos (refletem thresholds efetivamente aplicados pelo ComplianceFallback)
                "adaptive_confidence_threshold_passed": aggregated_confidence
                >= adaptive_thresholds["min_confidence"],
                "adaptive_divergence_threshold_passed": divergence
                <= adaptive_thresholds["max_divergence"],
    ...
    def _determine_consensus_method(
        self, aggregated_confidence: float, divergence: float, is_unanimous: bool
    ) -> ConsensusMethod:
    ...
    def _build_specialist_votes(
        ...
        for opinion in specialist_opinions:
            ...
            if self.config.enable_hierarchical_consensus:
                ...
                if "seniority_level" in opinion:
                    from src.models.seniority import SENIORITY_MULTIPLIERS, parse_seniority_level
    ...
    def _calculate_seniority_distribution(
        ...
        """
        distribution = {"trainee": 0, "junior": 0, "mid_level": 0, "senior": 0, "expert": 0}
        ...
        for opinion in specialist_opinions:
            ...
            if not seniority_str:
                seniority_str = self.config.specialist_seniority.get(
                    opinion["specialist_type"], "mid_level"
                )
