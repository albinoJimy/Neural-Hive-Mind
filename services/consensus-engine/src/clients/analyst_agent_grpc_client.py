...
class AnalystAgentGrpcClient:
    ...
    async def initialize(self):
        ...
        if spiffe_x509_enabled:
            ...
            logger.info(
                "analyst_agent_mtls_channel_configured", endpoint=endpoint, environment=environment
            )
        else:
        ...
        self.channel = instrument_grpc_channel(self.channel, service_name="analyst-agent")
        ...
        try:
            ...
            logger.info("analyst_agent_grpc_client_initialized", endpoint=endpoint)
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                response = await self.stub.QueryInsights(
                    request, metadata=grpc_metadata, timeout=15.0
                )
                ...
                return {"insights": insights, "total_count": response.total_count}
    ...
    async def execute_analysis(
        ...
        request = analyst_agent_pb2.ExecuteAnalysisRequest(
            analysis_type=analysis_type, parameters=parameters or {}
        )
    ...
    async def get_insight(self, insight_id: str) -> Optional[dict[str, Any]]:
        ...
        request = analyst_agent_pb2.GetInsightRequest(insight_id=insight_id)

        for attempt in range(MAX_RETRIES):
            try:
                grpc_metadata = await self._get_grpc_metadata()
                response = await self.stub.GetInsight(request, metadata=grpc_metadata, timeout=10.0)
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            grpc_metadata = await self._get_grpc_metadata()
            response = await self.stub.HealthCheck(request, metadata=grpc_metadata, timeout=5.0)
    ...
    async def close(self):
        ...
        if self.spiffe_manager:
            try:
                ...
                logger.warning("analyst_agent_spiffe_manager_close_error", error=str(e))
