...
class Settings(BaseSettings):
    ...
    mongodb_features_collection: str = Field(
        default="feature_store", description="Collection para features"
    )
    mongodb_lineage_collection: str = Field(
        default="feature_lineage", description="Collection para lineage de features"
    )
    ...
    # Redis configuration
    redis_url: str = Field(
        default="redis://redis.redis.svc.cluster.local:6379/0", description="URL do Redis"
    )
    redis_cache_ttl_seconds: int = Field(
        default=3600, description="TTL padrão para cache em segundos (1h)"
    )
    ...
    enable_async_computation: bool = Field(
        default=True, description="Habilitar computação assíncrona de features"
    )
    computation_timeout_seconds: int = Field(
        default=30, description="Timeout para computação de features"
    )
    max_parallel_computations: int = Field(
        default=10, description="Máximo de computações paralelas"
    )

    # Feature validation
    validate_feature_schema: bool = Field(default=True, description="Validar schema de features")
    allow_null_features: bool = Field(default=True, description="Permitir features nulas")
    ...
    rate_limit_requests_per_minute: int = Field(
        default=200, description="Limite de requests por minuto"
    )
