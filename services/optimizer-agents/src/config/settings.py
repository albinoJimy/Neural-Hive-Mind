...
class Settings(BaseSettings):
    ...
    # gRPC Clients
    consensus_engine_endpoint: str = Field(
        default="consensus-engine.consensus-engine.svc.cluster.local:50051"
    )
    orchestrator_endpoint: str = Field(
        default="orchestrator-dynamic.orchestrator-dynamic.svc.cluster.local:50051"
    )
    analyst_agents_endpoint: str = Field(
        default="analyst-agents.analyst-agents.svc.cluster.local:50051"
    )
    queen_agent_endpoint: str = Field(default="queen-agent.queen-agent.svc.cluster.local:50051")
    service_registry_endpoint: str = Field(
        default="service-registry.service-registry.svc.cluster.local:50051"
    )
    ...
    mongodb_insights_collection: str = Field(
        default="insights", description="MongoDB collection for insights"
    )
    mongodb_ab_test_results_collection: str = Field(
        default="ab_test_results", description="MongoDB collection for A/B test results"
    )
    ...
    min_improvement_threshold: float = Field(
        default=0.05, description="Minimum improvement to apply optimization"
    )
    max_weight_adjustment: float = Field(
        default=0.2, description="Max weight adjustment per iteration"
    )
    max_slo_adjustment_percentage: float = Field(
        default=0.1, description="Max SLO adjustment percentage"
    )
    ...
    degradation_threshold: float = Field(
        default=0.05, description="Degradation threshold for rollback"
    )
    ...
    clickhouse_host: str = Field(
        default="clickhouse.clickhouse.svc.cluster.local", description="ClickHouse host"
    )
    ...
    clickhouse_password: str | None = Field(
        default=None, description="ClickHouse password (OBRIGATÓRIO em produção)"
    )
    ...
    ml_load_forecast_horizons: list[int] = Field(
        default=[60, 360, 1440], description="Forecast horizons in minutes (1h, 6h, 24h)"
    )
    ml_prophet_seasonality_mode: str = Field(
        default="additive", description="Prophet seasonality mode (additive/multiplicative)"
    )
    ml_prophet_changepoint_prior_scale: float = Field(
        default=0.05, description="Prophet changepoint prior scale"
    )

    ml_scheduling_epsilon: float = Field(
        default=0.1, description="Epsilon for scheduling RL exploration"
    )
    ml_scheduling_learning_rate: float = Field(
        default=0.01, description="Learning rate for scheduling Q-learning"
    )
    ml_scheduling_discount_factor: float = Field(
        default=0.95, description="Discount factor for scheduling RL"
    )

    ml_training_interval_hours: int = Field(
        default=24, description="Interval for periodic model retraining (hours)"
    )
    ml_training_window_days: int = Field(
        default=540, description="Training data window (18 months)"
    )
    ...
    ml_forecast_cache_ttl_seconds: int = Field(
        default=300, description="Forecast cache TTL (5 minutes)"
    )

    enable_load_prediction: bool = Field(
        default=True, description="Enable load prediction features"
    )
    enable_scheduling_optimization: bool = Field(
        default=True, description="Enable scheduling optimization"
    )
    ...
    min_weight_value: float = Field(
        default=0.05, description="Valor mínimo permitido para peso de especialista"
    )
    max_weight_value: float = Field(
        default=0.50, description="Valor máximo permitido para peso de especialista"
    )

    # Configurações para Orchestrator Optimization (extensões gRPC)
    min_error_budget_percentage: float = Field(
        default=0.20, description="Mínimo error budget para aplicar SLO"
    )
    slo_cache_ttl_seconds: int = Field(default=300, description="TTL do cache de SLOs no Redis")
    gradual_rollout_enabled: bool = Field(
        default=True, description="Habilitar rollout gradual de SLOs"
    )
    min_slo_latency_ms: int = Field(default=100, description="Latência mínima permitida para SLO")
    min_slo_availability: float = Field(
        default=0.95, description="Disponibilidade mínima permitida para SLO"
    )
    max_slo_error_rate: float = Field(
        default=0.10, description="Error rate máximo permitido para SLO"
    )

    # Configuração RL para extensões
    rl_learning_rate: float = Field(
        default=0.1, description="Taxa de aprendizado do Q-learning para extensões"
    )
    rl_discount_factor: float = Field(
        default=0.9, description="Fator de desconto do Q-learning para extensões"
    )
    rl_exploration_rate: float = Field(
        default=0.1, description="Taxa de exploração (epsilon) para extensões"
    )
    ...
    spiffe_socket_path: str = Field(
        default="unix:///run/spire/sockets/agent.sock", description="SPIRE Workload API socket"
    )
    ...
    ab_test_default_alpha: float = Field(
        default=0.05, description="Nivel de significancia padrao para testes A/B"
    )
    ab_test_default_power: float = Field(default=0.80, description="Power estatistico padrao")
    ab_test_min_sample_size: int = Field(
        default=100, description="Tamanho minimo de amostra por grupo"
    )
    ab_test_max_sample_size: int = Field(
        default=1000000, description="Tamanho maximo de amostra por grupo"
    )
    ab_test_early_stopping_enabled: bool = Field(
        default=True, description="Habilitar parada antecipada por default"
    )
    ab_test_sequential_testing_enabled: bool = Field(
        default=True, description="Habilitar sequential testing (SPRT)"
    )
    ab_test_bayesian_analysis_enabled: bool = Field(
        default=True, description="Habilitar analise Bayesiana por default"
    )
    ab_test_guardrail_check_interval_seconds: int = Field(
        default=30, description="Intervalo de verificacao de guardrails"
    )
    ab_test_default_traffic_split: float = Field(
        default=0.5, description="Split de trafego padrao (50/50)"
    )
    ab_test_max_duration_days: int = Field(
        default=30, description="Duracao maxima de experimentos em dias"
    )
    ab_test_metrics_retention_days: int = Field(
