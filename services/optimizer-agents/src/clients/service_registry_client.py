...
class ServiceRegistryClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "service_registry_grpc_connection_check_failed",
                    endpoint=target,
    ...
    async def register(self, capabilities: list[str], metadata: dict | None = None) -> str | None:
        """
    ...
    async def heartbeat(self, health_status: str = "HEALTHY", metrics: dict | None = None) -> bool:
        """
            ...
            if e.code() == grpc.StatusCode.NOT_FOUND and "não encontrado" in e.details():
                ...
                try:
                    new_agent_id = await self.register(
                        capabilities=getattr(self.settings, "capabilities", ["optimization"]),
                        metadata=getattr(self.settings, "metadata", {}),
                    )
                    if new_agent_id:
                ...
                logger.error(
                    "heartbeat_failed", agent_id=self.agent_id, error=str(e), code=e.code()
                )
                return False
        except Exception as e:
    ...
    async def discover_agents(
        self, capabilities: list[str], filters: dict | None = None
    ) -> list[dict] | None:
        """
        ...
        try:
            ...
            request = service_registry_pb2.DiscoverRequest(
                capabilities=capabilities, filters=filters or {}, max_results=100
            )
    ...
    async def update_health_status(self, health_status: str, metrics: dict | None = None) -> bool:
        """
