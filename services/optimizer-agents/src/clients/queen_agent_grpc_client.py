...
class QueenAgentGrpcClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "queen_agent_grpc_connection_check_failed",
                    endpoint=target,
    ...
    async def request_approval(
        ...
        try:
            ...
            response = await self.stub.RequestExceptionApproval(
                request, timeout=self.settings.grpc_timeout, metadata=metadata
            )

            response_dict = MessageToDict(response, preserving_proto_field_name=True)
            decision = {
                "approved": response_dict.get("status", "").upper()
                in {"APPROVED", "APPROVED_WITH_CONDITIONS"},
                "approval_type": response_dict.get("status", ""),
                "decision_id": response_dict.get("exceptionId")
                or response_dict.get("exception_id"),
    ...
    async def notify_optimization_result(self, optimization_id: str, result: dict) -> bool:
        ...
        try:
            ...
            logger.error(
                "notify_optimization_result_failed",
                optimization_id=optimization_id,
                error=str(e),
                code=e.code(),
            )
            ...
            logger.error(
                "notify_optimization_result_failed", optimization_id=optimization_id, error=str(e)
            )
            return False

    async def get_strategic_priorities(self) -> dict | None:
        """
