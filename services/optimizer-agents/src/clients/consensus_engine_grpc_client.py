...
class ConsensusEngineGrpcClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            if PROTO_AVAILABLE:
                self.stub = consensus_engine_extensions_pb2_grpc.ConsensusOptimizationStub(
                    self.channel
                )
            ...
            try:
                ...
                logger.warning(
                    "consensus_engine_grpc_connection_check_failed",
                    endpoint=target,
    ...
    async def get_current_weights(self) -> dict[str, float] | None:
        """
        ...
        try:
            if PROTO_AVAILABLE and self.stub:
                ...
                response = await self.stub.GetCurrentWeights(
    ...
    async def update_weights(
        ...
        try:
            if PROTO_AVAILABLE and self.stub:
                ...
                response = await self.stub.UpdateWeights(
                    request, timeout=self.settings.grpc_timeout
                )
                logger.info(
                    "weights_updated",
    ...
    async def get_consensus_metrics(self, time_range: str = "1h") -> dict | None:
        """
        ...
        try:
            if PROTO_AVAILABLE and self.stub:
                request = consensus_engine_extensions_pb2.GetConsensusMetricsRequest(
                    time_range=time_range
                )
                response = await self.stub.GetConsensusMetrics(
                    request, timeout=self.settings.grpc_timeout
                )
                metrics = {
                ...
                # Stub temporário quando proto não compilado
                logger.warning(
                    "get_consensus_metrics_stub_called",
                    time_range=time_range,
                    reason="proto_not_available",
                )
                metrics = {
    ...
    async def validate_weight_adjustment(self, proposed_weights: dict[str, float]) -> bool:
        ...
        try:
            if PROTO_AVAILABLE and self.stub:
                ...
                response = await self.stub.ValidateWeightAdjustment(
                    request, timeout=self.settings.grpc_timeout
                )
                logger.info(
                    "weight_adjustment_validated",
                ...
                logger.warning(
                    "validate_weight_adjustment_stub_called", reason="proto_not_available"
                )
    ...
    async def rollback_weights(self, optimization_id: str) -> bool:
        ...
        try:
            if PROTO_AVAILABLE and self.stub:
                ...
                response = await self.stub.RollbackWeights(
                    request, timeout=self.settings.grpc_timeout
                )
                logger.info(
                    "weights_rolled_back",
