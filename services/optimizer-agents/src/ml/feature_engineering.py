...
class FeatureEngineering:
    ...
    async def extract_load_features(self, clickhouse_data: list[dict]) -> pd.DataFrame:
        """
    ...
    async def extract_scheduling_state_features(self, current_metrics: dict) -> np.ndarray:
        """
    ...
    async def compute_historical_aggregates(self, clickhouse_client, window_days: int = 30) -> dict:
        """
    ...
    def normalize_timeseries(
        self, data: pd.DataFrame, method: str = "minmax", columns: list[str] | None = None
    ) -> pd.DataFrame:
        """
    ...
    def compute_qos_score(self, metrics: dict) -> float:
        ...
        duration_penalty = min(
            1.0, metrics.get("avg_duration_ms", 0) / 10000.0
        )  # 10s como referência
    ...
    def decompose_trend_seasonality(self, timeseries: pd.Series, period: int = 24) -> dict:
        """
        ...
        try:
            decomposition = seasonal_decompose(
                timeseries, model="additive", period=period, extrapolate_trend="freq"
            )
    ...
    async def _get_cached_aggregates(self, cache_key: str) -> dict | None:
