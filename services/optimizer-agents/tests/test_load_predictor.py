...
def mock_clickhouse():
    ...
    client = AsyncMock()
    client.query_historical_loads = AsyncMock(
...
class TestLoadPredictorInitialization:
    ...
    async def test_initialize_without_models(
        self, mock_config, mock_clickhouse, mock_redis, mock_model_registry, mock_metrics
    ):
        """Testa inicialização sem modelos pré-treinados."""
        predictor = LoadPredictor(
            mock_clickhouse, mock_redis, mock_model_registry, mock_metrics, mock_config
        )
    ...
    async def test_initialize_with_mlflow_models(
        self, mock_config, mock_clickhouse, mock_redis, mock_model_registry, mock_metrics
    ):
        ...
        mock_model_registry.load_load_model = AsyncMock(
            return_value={"model": mock_prophet_model, "metadata": {"mape": 12.5}}
        )
...
class TestLoadPrediction:
    ...
    async def test_predict_load_with_prophet(
        self,
        mock_config,
        mock_clickhouse,
        mock_redis,
    ...
    async def test_predict_load_insufficient_data_fallback_arima(
        self, mock_config, mock_clickhouse, mock_redis, mock_model_registry, mock_metrics
    ):
        ...
        with patch("pmdarima.auto_arima") as mock_auto_arima:
            ...
            mock_arima_model.predict = Mock(
                return_value=(np.array([100] * 60), np.array([[90, 110]] * 60))
            )
    ...
    async def test_predict_load_missing_data_handling(
        self, mock_config, mock_clickhouse, mock_redis, mock_model_registry, mock_metrics
    ):
...
class TestBottleneckPrediction:
    ...
    async def test_predict_bottlenecks_worker_saturation(
        self, mock_config, mock_clickhouse, mock_redis, mock_model_registry, mock_metrics
    ):
    ...
    async def test_predict_bottlenecks_demand_spike(
        self, mock_config, mock_clickhouse, mock_redis, mock_model_registry, mock_metrics
    ):
...
class TestTraining:
    ...
    async def test_calculate_forecast_accuracy(
        self, mock_config, mock_clickhouse, mock_redis, mock_model_registry, mock_metrics
    ):
...
class TestHelpers:
    ...
    def test_prepare_timeseries_data(
        self,
        mock_config,
        mock_clickhouse,
        mock_redis,
    ...
    def test_backfill_missing_data(
        ...
        df_with_gaps = pd.DataFrame(
            {
                "ds": pd.date_range(
                    start="2024-01-01", periods=100, freq="10T"
                ),  # A cada 10 min (com gaps)
    ...
    def test_validate_data_quality_invalid(
        self, mock_config, mock_clickhouse, mock_redis, mock_model_registry, mock_metrics
    ):
    ...
    def test_estimate_resource_demand(
        self, mock_config, mock_clickhouse, mock_redis, mock_model_registry, mock_metrics
    ):
    ...
    async def test_get_current_worker_capacity(
        self, mock_config, mock_clickhouse, mock_redis, mock_model_registry, mock_metrics
    ):
