...
async def generate_readme(
    request: ReadmeRequest, repository: DocumentsRepository = Depends(get_repository)
):
...
async def generate_diagram(
    ...
    try:
        ...
        document = await generator.generate(description=description, diagram_type=diagram_type)
...
async def generate_project_docs(
    ...
    try:
        generator = get_code_doc_generator()
        document = await generator.generate_for_project(files=files, project_name=project_name)
...
async def list_documents(
    ...
    try:
        documents, total = await repository.list(doc_type=doc_type, limit=limit, skip=skip)

        return {"total": total, "items": documents, "filters": {"doc_type": doc_type}}
...
async def get_document(document_id: str, repository: DocumentsRepository = Depends(get_repository)):
...
async def delete_document(
    document_id: str, repository: DocumentsRepository = Depends(get_repository)
):
    """Deleta um documento."""
    try:
...
async def search_documents(
    ...
    try:
        ...
        return {"query": query, "total": len(documents), "results": documents}
