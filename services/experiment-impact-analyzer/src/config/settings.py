...
class Settings(BaseSettings):
    ...
    mongodb_impacts_collection: str = Field(
        default="experiment_impacts",
        description="MongoDB collection for impact analyses"
    )
    mongodb_experiments_collection: str = Field(
        default="experiments",
        description="MongoDB collection for experiments"
    )
    mongodb_hypotheses_collection: str = Field(
        default="hypotheses",
        description="MongoDB collection for hypotheses"
    )
    mongodb_max_pool_size: int = Field(
        default=100,
        description="Maximum MongoDB connections in pool"
    )
    mongodb_min_pool_size: int = Field(
        default=10,
        description="Minimum MongoDB connections in pool"
    )
    ...
    kafka_experiment_events_topic: str = Field(
        default="experiment.events",
        description="Kafka topic for experiment events"
    )
    kafka_consumer_group_id: str = Field(
        default="experiment-impact-analyzer",
    ...
    short_term_window_days: int = Field(
        default=7,
        description="Days to consider for short-term impact"
    )
    long_term_window_days: int = Field(
        default=90,
        description="Days to consider for long-term impact"
    )
    min_sample_size_for_impact: int = Field(
        default=100,
        description="Minimum sample size for impact calculation"
    )
    statistical_significance_threshold: float = Field(
        default=0.95,
        description="Statistical significance threshold (0-1)"
    )
    ...
    enable_long_term_analysis: bool = Field(
        default=True,
        description="Enable long-term impact analysis"
    )
    enable_correlation_analysis: bool = Field(
        default=True,
        description="Enable experiment correlation analysis"
    )
