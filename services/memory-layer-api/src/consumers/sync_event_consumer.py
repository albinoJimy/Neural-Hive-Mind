...
def _initialize_metrics():
    ...
    SYNC_CONSUMER_LAG = _get_or_create_metric(
        Gauge, "memory_sync_consumer_lag", "Lag do consumer de sincronização", ["partition"]
    )

    DLQ_EVENTS_SENT = _get_or_create_metric(
        Counter, "memory_sync_dlq_events_total", "Total de eventos enviados para DLQ", ["reason"]
    )
...
def get_avro_schema():
    ...
    if _avro_schema is None:
        if AVRO_SCHEMA_PATH.exists():
            ...
            logger.warning("Schema Avro não encontrado para consumer", path=str(AVRO_SCHEMA_PATH))
    return _avro_schema
...
class SyncEventConsumer:
    ...
    def _deserialize_message(self, msg) -> Dict[str, Any]:
        ...
        if self._use_avro and self._avro_schema:
            try:
                ...
                logger.debug("Falha na deserialização Avro, tentando JSON", error=str(avro_error))
        ...
        try:
            ...
            raise ValueError(
                f"Falha ao deserializar mensagem: Avro e JSON falharam. JSON error: {json_error}"
            )
    ...
    async def _process_message(self, msg):
        ...
        try:
            ...
            await self._send_to_dlq(
                {"raw": msg.value.decode("utf-8", errors="replace")}, "decode_error"
            )
    ...
    async def _process_event(self, event: Dict[str, Any]):
        ...
        if exists:
            logger.debug("Registro já existe no ClickHouse", entity_id=entity_id, table=table_name)
            return
    ...
    async def _check_exists(self, table: str, entity_id: str, created_at: datetime) -> bool:
        ...
        try:
            ...
            result = self.clickhouse.client.query(
                query, parameters={"entity_id": entity_id, "created_at": created_at}
            )
    ...
    async def _send_to_dlq(self, event: Dict, reason: str):
        ...
        try:
            ...
            logger.info("Evento enviado para DLQ", event_id=event.get("event_id"), reason=reason)
        except Exception as e:
