...
class Settings(BaseSettings):
    ...
    mongodb_context_collection: str = Field(
        default="operational_context", description="Collection for operational context"
    )
    mongodb_lineage_collection: str = Field(
        default="data_lineage", description="Collection for data lineage"
    )
    mongodb_quality_collection: str = Field(
        default="data_quality_metrics", description="Collection for quality metrics"
    )
    ...
    kafka_sync_topic: str = Field(
        default="memory.sync.events", description="Topic para eventos de sincronização de memória"
    )
    ...
    kafka_consumer_group: str = Field(
        default="memory-sync-consumer", description="Consumer group para sincronização"
    )
    enable_realtime_sync: bool = Field(
        default=True, description="Habilitar sincronização em tempo real via Kafka"
    )
    ...
    hot_data_threshold_seconds: int = Field(
        default=300, description="Threshold for hot data (Redis) in seconds"
    )
    warm_data_threshold_days: int = Field(
        default=30, description="Threshold for warm data (MongoDB) in days"
    )
    cold_data_threshold_months: int = Field(
        default=18, description="Threshold for cold data (ClickHouse) in months"
    )

    # Data Quality
    quality_check_enabled: bool = Field(default=True, description="Enable quality checks")
    completeness_threshold: float = Field(
        default=0.95, description="Minimum completeness score (0-1)"
    )
    accuracy_threshold: float = Field(default=0.95, description="Minimum accuracy score (0-1)")
    freshness_threshold_hours: int = Field(default=24, description="Maximum freshness age in hours")
    ...
    @field_validator("completeness_threshold", "accuracy_threshold")
    @classmethod
