...
async def lifespan(app: FastAPI):
    ...
    mongodb_client = MongoDBClient(uri=settings.mongodb_uri, database=settings.mongodb_database)
    ...
    if settings.enable_realtime_sync:
        try:
            ...
            logger.warning(
                "Kafka producer initialization failed, continuing without it", error=str(e)
            )
    ...
    if settings.enable_realtime_sync and clickhouse_client:
        try:
            ...
            logger.warning(
                "Kafka consumer initialization failed, continuing without it", error=str(e)
            )
...
async def get_lineage(entity_id: str, depth: int = 3):
    ...
    try:
        lineage = await lineage_tracker.get_lineage_tree(entity_id=entity_id, depth=depth)
        return lineage
    except Exception as e:
...
async def get_quality_stats(data_type: str = None):
    ...
    try:
        stats = await quality_monitor.get_quality_trends(data_type=data_type or "context", days=7)
...
async def list_data_assets(limit: int = 100, offset: int = 0):
    ...
    try:
        ...
        assets = await mongodb_client.find(
            collection="data_assets", filter={}, limit=limit, skip=offset
        )
