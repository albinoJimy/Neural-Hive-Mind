...
def mock_clickhouse_with_data():
    ...
    clickhouse.client.query = MagicMock(return_value=MagicMock(result_rows=[(95.0, 2.0, 30)]))
    return clickhouse
            ...
            historical_data.append(
                {
                    ...
                    "metrics": {"completeness": {"completeness_score": 95.0 + (i % 5)}},
                }
            )
        ...
        recent_data = [
            {
                ...
                "metrics": {"completeness": {"completeness_score": 70.0}},
            }
        ]
            ...
            all_data.append(
                {
                    ...
                    "metrics": {"completeness": {"completeness_score": 95.0 + (i % 3)}},
                }
            )
            ...
            historical_data.append(
                {
                    "collection": "context",
                    "timestamp": now - timedelta(days=i + 1),
                    "metrics": {"completeness": {"completeness_score": 95.0}},
                }
            )
...
class TestValidateData:
    ...
    async def test_validate_data_valid_document(self, settings, mock_mongodb):
        ...
        schema = {"required": ["entity_id"], "types": {"count": int}, "ranges": {"score": (0, 100)}}
        data = {"entity_id": "test-123", "count": 42, "score": 85}
    ...
    async def test_detect_anomalies_real_clickhouse_integration(self, settings):
        ...
        try:
            ...
            anomalies = await monitor.detect_anomalies(
                "context", "completeness_score", window_hours=24, baseline_days=7
            )
