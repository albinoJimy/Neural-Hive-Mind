...
from ..models.alert_rule import Alert, AlertChannel, AlertSeverity
...
class PostgreSQLClient:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "postgresql_connected", host=self.settings.host, database=self.settings.database
            )
    ...
    def _handle_connection_error(self, error: Exception) -> None:
        ...
        if isinstance(
            error, (asyncpg.PostgresConnectionError, asyncpg.InterfaceError, ConnectionError)
        ):
            sla_metrics.record_postgresql_error()
    ...
    async def get_slo(self, slo_id: str) -> Optional[SLODefinition]:
        """Busca SLO por ID."""
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow("SELECT * FROM slo_definitions WHERE slo_id = $1", slo_id)
            if row:
    ...
    async def update_slo(self, slo_id: str, updates: dict[str, Any]) -> bool:
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                if success:
                    self.logger.info(
                        "slo_updated_in_db", slo_id=slo_id, fields=list(updates.keys())
                    )
                else:
    ...
    async def save_budget(self, budget: ErrorBudget) -> str:
        ...
        async with self.pool.acquire() as conn:
            ...
            """,
                ...
                budget.slo_id,
                budget.service_name,
                budget.calculated_at,
                budget.window_start,
                budget.window_end,
                budget.sli_value,
                budget.slo_target,
                budget.error_budget_total,
                budget.error_budget_consumed,
                budget.error_budget_remaining,
                budget.status.value,
                [br.model_dump() for br in budget.burn_rates],
                budget.violations_count,
                budget.last_violation_at,
                budget.metadata,
            )
            return budget.budget_id

    async def update_violations_count(self, slo_id: str, violations_count: int) -> bool:
        """
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                if success:
                    self.logger.debug(
                        "violations_count_updated", slo_id=slo_id, violations_count=violations_count
                    )
                return success

        except Exception as e:
            self.logger.error("violations_count_update_failed", slo_id=slo_id, error=str(e))
            raise
    ...
    async def get_budget_trends(self, slo_id: str, days: int = 30) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("budget_trends_query_failed", slo_id=slo_id, days=days, error=str(e))
            raise
    ...
    async def create_policy(self, policy: FreezePolicy) -> str:
        ...
        async with self.pool.acquire() as conn:
            ...
            """,
                ...
                policy.trigger_threshold_percent,
                policy.auto_unfreeze,
                policy.unfreeze_threshold_percent,
                policy.enabled,
                policy.created_at,
                policy.metadata,
            )
    ...
    async def get_policy(self, policy_id: str) -> Optional[FreezePolicy]:
        ...
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT * FROM freeze_policies WHERE policy_id = $1", policy_id
            )
            if row:
    ...
    async def update_policy(self, policy_id: str, updates: dict[str, Any]) -> bool:
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                if success:
                    self.logger.info(
                        "policy_updated_in_db", policy_id=policy_id, fields=list(updates.keys())
                    )
                else:
    ...
    async def create_freeze_event(self, event: FreezeEvent) -> str:
        ...
        async with self.pool.acquire() as conn:
            ...
            """,
                ...
                event.budget_remaining_percent,
                event.burn_rate,
                event.active,
                event.metadata,
            )
    ...
    async def save_alert(self, alert: Alert) -> str:
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                """
                INSERT INTO alerts
                (alert_id, rule_id, rule_name, severity, title, message, details,
                 slo_id, service_name, triggered_at, acknowledged, acknowledged_by,
                 acknowledged_at, resolved, resolved_at, dispatched_channels, dispatch_errors)
            ...
            """,
                ...
                alert.resolved,
                alert.resolved_at,
                json.dumps([c.value for c in alert.dispatched_channels]),
                json.dumps(alert.dispatch_errors),
            )
            self.logger.info("alert_saved", alert_id=alert.alert_id)
    ...
    async def get_alert(self, alert_id: str) -> Optional[Alert]:
        ...
        async with self.pool.acquire() as conn:
            ...
            if row:
                ...
                data["dispatched_channels"] = (
                    [AlertChannel(c) for c in json.loads(data["dispatched_channels"])]
                    if data["dispatched_channels"]
                    else []
                )
                data["dispatch_errors"] = (
                    json.loads(data["dispatch_errors"]) if data["dispatch_errors"] else {}
                )
    ...
    async def list_alerts(
        ...
        query += f" ORDER BY triggered_at DESC LIMIT ${len(params) + 1} OFFSET ${len(params) + 2}"
    ...
    async def get_alert_statistics(self) -> dict[str, Any]:
        ...
        async with self.pool.acquire() as conn:
            ...
            for row in recent_rows:
                data = dict(row)
                data["details"] = {}
                data["dispatched_channels"] = [AlertChannel(c) for c in []]
                data["dispatch_errors"] = {}
                data["severity"] = AlertSeverity(data["severity"])
                recent_alerts.append(Alert(**data))
    ...
    async def cleanup_old_alerts(self, days: int = 30) -> int:
        ...
        async with self.pool.acquire() as conn:
            ...
            self.logger.info("old_alerts_cleaned", deleted=deleted, days=days)
            return deleted
