...
from ..models.error_budget import BudgetStatus, BurnRate, BurnRateLevel, ErrorBudget
...
class BudgetCalculator:
    ...
    async def calculate_budget(self, slo: SLODefinition) -> ErrorBudget:
        ...
        try:
            ...
            violations_count = await self.prometheus_client.count_slo_violations(
                slo_name=slo.name, window_hours=24
            )
    ...
    async def get_budget(self, slo_id: str, use_cache: bool = True) -> Optional[ErrorBudget]:
    ...
    async def _calculate_burn_rates(self, service_name: str) -> List[BurnRate]:
        ...
        for window_hours in windows:
            rate = await self.prometheus_client.calculate_burn_rate(
                service_name, window_hours, baseline_days=self.settings.error_budget_window_days
            )
