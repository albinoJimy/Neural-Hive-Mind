...
OUT_DIR="$SERVICE_DIR/src/proto_gen"
...
python3 -m grpc_tools.protoc \
    ...
    --pyi_out="$OUT_DIR" \
    "$PROTO_DIR/ticket_service.proto"
...
if [[ "$(uname)" == "Darwin" ]]; then
    sed -i '' 's/^import ticket_service_pb2/from . import ticket_service_pb2/' \
        "$OUT_DIR/ticket_service_pb2_grpc.py"
else
    sed -i 's/^import ticket_service_pb2/from . import ticket_service_pb2/' \
        "$OUT_DIR/ticket_service_pb2_grpc.py"
fi
...
if [[ "$(uname)" == "Darwin" ]]; then
    sed -i '' 's/^import ticket_service_pb2/from . import ticket_service_pb2/' \
        "$LIB_OUT_DIR/ticket_service_pb2_grpc.py"
else
    sed -i 's/^import ticket_service_pb2/from . import ticket_service_pb2/' \
        "$LIB_OUT_DIR/ticket_service_pb2_grpc.py"
fi
...
ls -la "$OUT_DIR"/ticket_service_pb2*.py 2>/dev/null || echo "  (server-side stubs)"
ls -la "$LIB_OUT_DIR"/ticket_service_pb2*.py 2>/dev/null || echo "  (library stubs)"
