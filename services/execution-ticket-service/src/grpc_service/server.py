...
async def _bind_port_with_retry(
    server, port: int, max_attempts: int, initial_delay: float, max_delay: float
) -> None:
    """
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if not _check_port_available(port):
                logger.warning(
                    "grpc_port_in_use", port=port, attempt=attempt, max_attempts=max_attempts
                )
            ...
            if bound_port <= 0:
                raise RuntimeError(
                    f"Falha ao fazer bind da porta gRPC {port}: add_insecure_port retornou {bound_port}"
                )
...
async def stop_grpc_server(server, health_servicer=None):
    ...
    if HEALTH_CHECK_AVAILABLE and health_servicer is not None:
        ...
        health_servicer.set(
            "ticket_service.TicketService", health_pb2.HealthCheckResponse.NOT_SERVING
        )
