...
class TicketServiceSettings(BaseSettings):
    ...
    mongodb_collection_audit: str = Field(
        default="ticket_audit_log", description="Collection de audit log"
    )
    ...
    schemas_base_path: str = Field(
        default="/app/schemas", description="Diretório base dos schemas Avro"
    )
    ...
    jwt_token_expiration_seconds: int = Field(
        default=3600, description="Expiração do token (1 hora)"
    )
    ...
    grpc_bind_retry_attempts: int = Field(
        default=5, description="Número de tentativas para bind da porta gRPC"
    )
    grpc_bind_retry_initial_delay: float = Field(
        default=1.0, description="Delay inicial entre retries de bind gRPC (exponential backoff)"
    )
    grpc_bind_retry_max_delay: float = Field(
        default=30.0, description="Delay máximo entre retries de bind gRPC"
    )
    ...
    enable_status_updates: bool = Field(
        default=True, description="Habilitar atualizações de status"
    )

    # Connection retry configuration
    max_connection_retries: int = Field(
        default=5, description="Número máximo de tentativas de conexão"
    )
    initial_retry_delay_seconds: float = Field(
        default=1.0, description="Delay inicial entre retries (exponential backoff)"
    )
    ...
    enable_idempotency: bool = Field(
        default=True, description="Habilitar verificação de idempotência"
    )
    ...
    @classmethod
    def validate_webhook_timeout(cls, v: int) -> int:
