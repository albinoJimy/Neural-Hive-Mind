...
class MockSpecialistMetrics:
    ...
    def __init__(self):
        ...
        self.evaluation_duration_seconds.labels = MagicMock(return_value=MagicMock(
            observe=MagicMock()
        ))

        self.confidence_score = MagicMock()
        self.confidence_score.labels = MagicMock(return_value=MagicMock(
            set=MagicMock()
        ))

        self.risk_score = MagicMock()
        self.risk_score.labels = MagicMock(return_value=MagicMock(
            set=MagicMock()
        ))
        ...
        self.step_duration_seconds.labels = MagicMock(return_value=MagicMock(
            observe=MagicMock()
        ))
...
class TestEmitLatencyMetric:
    ...
    def test_emit_total_evaluation_latency(self, mock_metrics, sample_evaluation_result):
        ...
        mock_metrics.evaluation_duration_seconds.labels(
            specialist_type='technical'
    ...
    def test_latency_bottleneck_detection(self):
        ...
        bottlenecks = [
            (step, time_ms)
            for step, time_ms in step_timings.items()
...
class TestMetricsAggregation:
    ...
    def test_aggregate_evaluation_metrics(self, mock_metrics):
        ...
        approval_rate = sum(1 for e in evaluations if e['recommendation'] == 'approve') / len(evaluations)
...
class TestMetricsHistogram:
    ...
    def test_confidence_distribution(self):
        ...
        bucket_07_to_09 = sum(1 for s in confidence_scores if 0.7 <= s < 0.9)  # 0.7, 0.75, 0.8, 0.85
