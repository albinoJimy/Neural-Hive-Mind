...
class TestConsensusDecisionConsumer:
    ...
    def mock_explainability_service(self):
        ...
        service.generate_explanation = AsyncMock(return_value={
    ...
    async def test_consumer_connects_to_kafka(
        ...
        with patch('src.consumers.consensus_decision_consumer.AIOKafkaConsumer', return_value=mock_kafka_consumer):
            consumer = ConsensusDecisionConsumer(
                bootstrap_servers='localhost:9092',
                group_id='explainability-group',
                explainability_service=mock_explainability_service,
    ...
    async def test_handle_decision_with_existing_explanation(
        ...
        mock_explainability_service.get_explainability_by_decision_id.assert_called_once_with('decision-123')
...
class TestExplanationProducer:
    ...
    async def test_producer_connects_to_kafka(self, mock_kafka_producer):
        ...
        with patch('src.producers.explanation_producer.AIOKafkaProducer', return_value=mock_kafka_producer):
            producer = ExplanationProducer(
                bootstrap_servers='localhost:9092',
                topic='consensus.explanations'
            )
...
class TestKafkaIntegrationE2E:
    ...
    def mock_services(self):
        ...
        return {
            'explainability_service': MagicMock(),
            'producer': MagicMock()
        }
    ...
    async def test_decision_to_explanation_flow(self, mock_services):
        ...
        # Setup mocks
        mock_services['explainability_service'].get_explainability_by_decision_id = AsyncMock(return_value=None)
        mock_services['explainability_service'].generate_explanation = AsyncMock(return_value={
            'explainability_token': 'token-123',
            'decision_id': 'decision-456'
        })
