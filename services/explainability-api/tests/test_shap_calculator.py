...
class TestKernelSHAP:
    ...
    def test_high_risk_contributes_negatively(self):
        ...
        result = calculator.calculate_shap(
            decision_data=high_risk_data,
            features=['confidence', 'risk']
        )

        attribution = result['feature_attribution']
        # Risco alto (>0.5) deve ter contribuição negativa
        assert attribution['risk'] < 0, f"High risk should contribute negatively, got {attribution['risk']}"
        ...
        return [
            {
                'decision_id': 'dec-1',
                'specialist_votes': [
                    {'confidence_score': 0.85, 'risk_score': 0.15}
                ],
                'final_decision': 'approve'
            },
            {
                'decision_id': 'dec-2',
                'specialist_votes': [
                    {'confidence_score': 0.60, 'risk_score': 0.40}
                ],
                'final_decision': 'reject'
            }
        ]
