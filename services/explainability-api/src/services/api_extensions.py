...
class ExplainabilityAPIExtensions:
    ...
    def __init__(self, mongodb_client, shap_calculator=None,
                 quality_scorer=None, reasoning_extractor=None):
        """
    ...
    async def generate_explanation(
        ...
        if request.get('include_quality_score', True) and self.quality_scorer:
            ...
            quality_input = request.get('explanation', {
    ...
    def _format_as_text(self, explanation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        narrative_lines = []
        narrative_lines.append(f"=== Explicação de Decisão ===\n")
    ...
    def _format_as_html(self, explanation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        html_parts.append('.section h3 { color: #2c3e50; border-bottom: 2px solid #3498db; padding-bottom: 5px; }')
        ...
        if 'shap_values' in explanation:
            ...
            for feature, value in explanation['shap_values'].items():
                ...
                html_parts.append(f'<span class="{css_class}"><strong>{feature}:</strong> {value:+.2f}</span>')
                html_parts.append('</div>')
            html_parts.append('</div>')
        ...
        if 'explanation_quality' in explanation:
            ...
            html_parts.append(f'<p><strong>Score Geral: {quality.get("overall", 0):.2f}</strong></p>')
            html_parts.append('</div>')
...
def setup_extensions(app, mongodb_client, shap_calculator=None,
                     quality_scorer=None, reasoning_extractor=None):
    """
