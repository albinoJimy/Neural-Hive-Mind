...
def main():
    """Função principal de inicialização do Business Specialist."""

    logger.info("Starting Business Specialist")

    # Carregar configuração
    try:
        config = BusinessSpecialistConfig()
        logger.info(
    ...
    # Criar especialista
    try:
        specialist = BusinessSpecialist(config)
        logger.info("Business Specialist initialized successfully")
    except Exception as e:
    ...
    def shutdown_handler(signum, frame):
        ...
        logger.info("Business Specialist shut down successfully")
        sys.exit(0)
    ...
    try:
        ...
        if agent_id:
            ...
            logger.info("Registered with service-registry", agent_id=agent_id)
    except Exception as e:
        logger.warning("Service-registry registration failed - continuing without", error=str(e))
