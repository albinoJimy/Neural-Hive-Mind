...
class BusinessSpecialist(BaseSpecialist):
    ...
    def _evaluate_plan_internal(
        ...
        with self._profile_step("risk_calculation", plan_id):
            ...
            risk_score = self._calculate_business_risk(
                cognitive_plan, workflow_score, kpi_score, cost_score
            )
        ...
        with self._profile_step("reasoning_generation", plan_id):
            ...
            reasoning_summary = self._generate_reasoning(
                workflow_score, kpi_score, cost_score, recommendation
            )
    ...
    def _analyze_kpis(self, cognitive_plan: Dict, context: Dict) -> float:
        ...
        priority_map = {"low": 0.3, "normal": 0.6, "high": 0.8, "critical": 1.0}

        kpi_score = priority_map.get(priority, 0.6)

        logger.debug("KPI analysis", priority=priority, kpi_score=kpi_score)
    ...
    def _generate_reasoning(
        self, workflow_score: float, kpi_score: float, cost_score: float, recommendation: str
    ) -> str:
