...
def mock_engine():
    ...
    engine.scan_codebase = AsyncMock(return_value=[
        {'filepath': '/test.py', 'signal_type': 'modified', 'intensity': 0.7}
    ])
    engine.get_curiosity_scores = AsyncMock(return_value=[
        {'filepath': '/test.py', 'curiosity_score': 85.0}
    ])
    engine.get_exploration_summary = AsyncMock(return_value={
