...
class TestSynthesizeSingleSource:
    ...
    def test_synthesize_codebase_analysis(self, synthesizer):
        ...
        source_data = {
            ...
            'patterns': [
                {'name': 'repository', 'count': 5, 'confidence': 0.85}
            ],
            'dependencies': {
    ...
    def test_synthesize_pattern_discovery(self, synthesizer):
        ...
        source_data = {
            ...
            'suggestions': [
                {'pattern': 'factory', 'confidence': 0.7}
            ]
        }
...
class TestSynthesizeMultipleSources:
        ...
        sources = [
            {
                'source': 'codebase_explorer',
                'patterns': [
                    {'name': 'repository', 'locations': ['repo1.py', 'repo2.py']}
                ]
            },
            {
                'source': 'pattern_discovery',
    ...
    def test_calculate_aggregated_confidence(self, synthesizer):
        ...
        service_pattern = next((p for p in result.get('patterns', []) if p['name'] == 'service'), None)
...
class TestGenerateRecommendations:
    ...
    def test_recommend_pattern_application(self, synthesizer):
        ...
        data = {
            ...
            'missing_patterns': [
                {'name': 'factory', 'suggested_locations': ['responses.py']}
            ]
        }
...
class TestSynthesisConflictResolution:
    ...
    def test_resolve_conflicting_pattern_names(self, synthesizer):
        ...
        sources = [
            {
                'source': 'scout_a',
                'patterns': [{'name': 'Repository', 'confidence': 0.8}]
            },
            {
                'source': 'scout_b',
                'patterns': [{'name': 'repository', 'confidence': 0.7}]
            }
        ]
        ...
        repository_entries = [p for p in result.get('patterns', []) if p['name'].lower() == 'repository']
        assert len(repository_entries) == 1
        ...
        sources = [
            {
                ...
                'dependencies': {
                    'service.py': ['repo.py', 'utils.py']
                }
            },
            {
                'source': 'runtime_analysis',
                'dependencies': {
                    'service.py': ['repo.py', 'cache.py']
                }
            }
        ]
...
class TestGenerateActionableInsights:
    ...
    def test_estimate_effort_for_recommendations(self, synthesizer):
        ...
        simple_effort = next(r['effort'] for r in insights['recommendations'] if r['type'] == 'simple')
        complex_effort = next(r['effort'] for r in insights['recommendations'] if r['type'] == 'complex')
...
class TestSynthesisOutputFormats:
    ...
    def test_output_as_markdown_report(self, synthesizer):
        ...
        data = {
            ...
            'recommendations': [
                {'action': 'Extract factory', 'priority': 'high'}
            ]
        }
