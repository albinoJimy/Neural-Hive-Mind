...
class TestAggregateResults:
    ...
    def test_aggregate_results_combines_multiple_outputs(self, orchestrator):
        ...
        scout_results = {
            ...
            'code_searcher': {
                'solutions': [
                    {'approach': 'FastAPI', 'confidence': 0.9}
                ]
            }
        }
    ...
    def test_aggregate_results_deduplicates_similar_findings(self, orchestrator):
        ...
        scout_results = {
            'pattern_matcher': {
                'patterns': [
                    {'name': 'repository', 'occurrences': 1, 'locations': ['service/a']}
                ]
            },
            'code_searcher': {
                'patterns': [
        ...
        # Deve deduplicar
        assert len([p for p in result.get('patterns_discovered', []) if p['name'] == 'repository']) == 1

    def test_aggregate_results_calculates_confidence_scores(self, orchestrator):
        """Testa cálculo de confidence scores agregados."""
        scout_results = {
            'scout_a': {'confidence': 0.8},
            'scout_b': {'confidence': 0.6}
        }
