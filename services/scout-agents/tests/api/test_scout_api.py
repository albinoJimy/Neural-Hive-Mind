...
class TestScoutAPIInitialization:
    ...
    def test_api_initialization(self):
        ...
        api = ScoutAPI(
            scout_orchestrator=mock_orchestrator,
...
class TestStartExplorationEndpoint:
    ...
    def client(self):
        mock_orchestrator = AsyncMock()
        mock_orchestrator.coordinate_exploration = AsyncMock(return_value={
            'exploration_id': 'scout-exp-1',
            'status': 'running'
        })

        mock_ledger = AsyncMock()
        mock_ledger.save_exploration = AsyncMock(return_value={
            'exploration_id': 'scout-exp-1'
        })
    ...
    def test_start_exploration_returns_id(self, client):
        ...
        response = client.post("/explorations", json={
            'plan_id': 'plan-1',
...
class TestGetExplorationEndpoint:
    ...
    def client(self):
        mock_orchestrator = AsyncMock()
        mock_ledger = AsyncMock()
        mock_ledger.get_exploration = AsyncMock(return_value={
            'exploration_id': 'scout-exp-1',
            'status': 'completed',
            'results': {'patterns': []}
        })
...
class TestListExplorationsEndpoint:
    ...
    def client(self):
        mock_orchestrator = AsyncMock()
        mock_ledger = AsyncMock()
        mock_ledger.list_explorations = AsyncMock(return_value=[
...
class TestStatsEndpoint:
    ...
    def client(self):
        mock_orchestrator = AsyncMock()
        mock_ledger = AsyncMock()
        mock_ledger.get_exploration_stats = AsyncMock(return_value={
            'total': 42,
            'by_status': {'completed': 30, 'running': 10, 'failed': 2}
        })
