...
QueenAgentIntegration = pytest.importorskip(
    "src.integration.queen_agent_integration"
).QueenAgentIntegration
...
class TestQueenAgentIntegrationInitialization:
    ...
    def test_integration_initialization(self):
        ...
        integration = QueenAgentIntegration(
            channel=mock_channel, stub=mock_stub, agent_id="scout-agent-1"
        )
    ...
    def test_default_heartbeat_interval(self):
        ...
        integration = QueenAgentIntegration(channel=mock_channel, stub=mock_stub)
...
class TestAgentRegistration:
    ...
    def integration(self):
        mock_channel = AsyncMock()
        mock_stub = AsyncMock()
        mock_stub.RegisterAgent = AsyncMock(
            return_value=MagicMock(agent_id="registered-1", status="accepted")
        )
...
class TestHeartbeat:
    ...
    def integration(self):
        mock_channel = AsyncMock()
        mock_stub = AsyncMock()
        mock_stub.SendHeartbeat = AsyncMock(return_value=MagicMock(acknowledged=True))
...
class TestReportExplorationResults:
    ...
    def integration(self):
        mock_channel = AsyncMock()
        mock_stub = AsyncMock()
        mock_stub.ReportExploration = AsyncMock(
            return_value=MagicMock(received=True, exploration_id="scout-exp-1")
        )
...
class TestHandleQueenCommands:
    ...
    async def test_execute_registered_command(self, integration):
        ...
        result = await integration.handle_command(
            {"command": "explore", "params": {"target": "/code"}}
        )
...
class TestAgentStatusReporting:
    ...
    def integration(self):
        mock_channel = AsyncMock()
        mock_stub = AsyncMock()
        mock_stub.ReportStatus = AsyncMock(return_value=MagicMock(received=True))
