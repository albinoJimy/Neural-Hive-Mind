...
class SolutionSynthesizer:
    ...
    def synthesize(
        ...
        if 'patterns' in source_data or 'patterns_found' in source_data:
            ...
            result['patterns_count'] = len(patterns) if isinstance(patterns, list) else len(patterns.get('patterns', []))
    ...
    def _merge_patterns(
        ...
        for pattern_name, occurrences in all_patterns.items():
            ...
            merged.append({
                ...
                'confidence': round(sum(all_confidences) / len(all_confidences), 2) if all_confidences else 0.5,
                'locations': list(all_locations) if all_locations else occurrences[0].get('locations', [])
            })
    ...
    def generate_actionable_insights(
        ...
        if 'refactor' in issue.lower() or 'extract' in issue.lower():
            insights['steps'] = [
                {'action': 'Identify extraction target', 'file': data.get('affected_files', [''])[0]},
    ...
    def calculate_quality_metrics(
        ...
        sources_with_findings = sum(
            1 for s in sources
            if s.get('patterns') or s.get('findings') or s.get('files')
        )
