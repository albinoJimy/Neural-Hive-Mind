...
class DigitalEvent(BaseModel):
    ...
    @classmethod
    def serialize_timestamp(cls, dt: datetime) -> str:
    ...
    def to_raw_event(self) -> dict[str, Any]:
        ...
        event_type_str = (
            ...
            if isinstance(self.event_type, DigitalEventType)
            else str(self.event_type)
        )
        channel_str = (
            self.channel.value if isinstance(self.channel, DigitalChannel) else str(self.channel)
        )
    ...
    def is_valid(self) -> bool:
        ...
        try:
            DigitalEventType(self.event_type)
        except ValueError:
            return False

        # Check if channel is valid enum value (string or enum)
        try:
            DigitalChannel(self.channel)
        except ValueError:
            return False
