...
class PatternDiscovery:
    ...
    KNOWN_PATTERNS = {
        ...
        'repository': {
            ...
            'common_methods': ['find', 'save', 'delete', 'update', 'get', 'list', 'create', 'remove', 'query', 'insert'],
            'naming_suffix': ['Repository', 'Repo', 'DAO'],
            'category': 'creational'
        },
        'service': {
            'keywords': ['service', 'handler', 'manager'],
            'common_methods': ['create', 'update', 'delete', 'get', 'process', 'handle', 'execute', 'run'],
        ...
        'strategy': {
            'keywords': ['strategy'],
            'common_methods': ['execute', 'execute_algorithm', 'calculate', 'process', 'do_', 'compute'],
            'naming_suffix': ['Strategy', 'Algorithm', 'Policy'],
            'category': 'behavioral'
        },
        'observer': {
            'keywords': ['observer', 'listener', 'subscriber', 'notifier'],
            'common_methods': ['attach', 'detach', 'notify', 'update', 'on_', 'emit', 'subscribe', 'unsubscribe'],
    ...
    def get_patterns_by_category(self, category: str) -> List[str]:
        ...
        """
        return [
            name for name, config in self.patterns_db.items()
    ...
    def _pattern_specific_checks(
        ...
        # === Observer ===
            ...
            observer_methods = ['attach', 'detach', 'notify', 'subscribe', 'unsubscribe', 'register', 'unregister']
        ...
        # === Strategy ===
            ...
            has_execute = any('execute' in m.lower() or 'compute' in m.lower() or 'calculate' in m.lower() for m in methods)
            if has_execute:
                bonus += 0.2
        ...
        # === Composite ===
            ...
            has_children_attr = any('children' in a for a in self._get_all_attribute_names(class_node))
        ...
        # === Proxy ===
            ...
            attrs = self._get_all_attribute_names(class_node)
            has_wrapped = any('_wrapped' in a or '_subject' in a or '_real' in a or '_target' in a for a in attrs)
            if has_getattr:
                bonus += 0.3
            if has_wrapped:
                bonus += 0.2
        ...
        # === Adapter ===
            ...
            has_adapt_method = any('adapt' in m.lower() or 'convert' in m.lower() or 'transform' in m.lower() for m in methods)
            if has_adaptee:
                bonus += 0.2
            if has_adapt_method:
                bonus += 0.2
        ...
        # === Facade ===
            ...
            if len(methods) >= 5:
                bonus += 0.1
            attrs = self._get_all_attribute_names(class_node)
            has_components = any('_subsystem' in a or '_component' in a or '_service' in a for a in attrs)
            if has_components:
                bonus += 0.2
        ...
        # === Builder ===
            ...
            has_with = any(m.startswith('with_') or m.startswith('set_') or m.startswith('add_') for m in methods)
            has_result = any('_result' in a or '_built' in a or '_product' in a for a in self._get_all_attribute_names(class_node))
            if has_build:
                bonus += 0.2
            if has_with:
                bonus += 0.2
                ...
                if isinstance(node, ast.ClassDef) and node.name == class_name:
                                ...
                                if isinstance(body_item, ast.Assign):
                                    for target in body_item.targets:
                                        if isinstance(target, ast.Attribute) and isinstance(target.value, ast.Name) and target.value.id == 'self':
                                            ...
                                            if 'repo' in attr_name.lower() or 'service' in attr_name.lower() or 'repository' in attr_name.lower():
                    ...
                    for item in node.body:
                        if isinstance(item, ast.Assign):
                            for target in item.targets:
                                if isinstance(target, ast.Name) and ('repo' in target.id.lower() or 'service' in target.id.lower()):
