...
class MultiLanguagePatternDiscovery:
    ...
    LANGUAGE_PATTERNS = {
        PatternLanguage.PYTHON: {
            ...
            'method_keywords': {
                'repository': ['find', 'save', 'delete', 'update', 'get', 'list', 'create', 'remove', 'query', 'insert'],
        ...
        PatternLanguage.TYPESCRIPT: {
            'class_keywords': {
                ...
                'observer': ['Observer', 'Listener', 'Subscriber', 'Subject', 'observer', 'listener'],
    ...
    def _discover_yaml_patterns(self, code: str, filename: str) -> List[PatternMatch]:
        ...
        patterns = []
        config = self.LANGUAGE_PATTERNS[PatternLanguage.YAML]
    ...
    def _analyze_js_ts_class(
        ...
        for pattern_name, keywords in class_keywords.items():
            ...
            if pattern_name == 'singleton':
                ...
                if any('getinstance' in m.lower() or 'get_instance' in m.lower() for m in method_names):
                    confidence += 0.4
                # Nome comum para singleton
                if any(suffix in class_name_lower for suffix in ['connection', 'manager', 'instance', 'database', 'config']):
                    confidence += 0.15

            elif pattern_name == 'observer':
                if any('observer' in a.lower() or 'listener' in a.lower() or 'subscriber' in a.lower()
                       for a in attrs):
                    confidence += 0.2
                if any('attach' in m or 'detach' in m or 'notify' in m or 'emit' in m
