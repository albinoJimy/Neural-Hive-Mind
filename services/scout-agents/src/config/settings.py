...
class KafkaConfig(BaseModel):
    ...
    sasl_password: Optional[str] = Field(
        default=None, description="SASL password (OBRIGATÓRIO se SASL habilitado)"
    )
...
class PheromoneConfig(BaseModel):
    ...
    redis_key_prefix: str = Field(
        default="pheromone:exploration:", description="DEPRECATED - Redis key prefix"
    )
    redis_url: str = Field(
        default="redis://neural-hive-cache.redis-cluster.svc.cluster.local:6379",
...
class DetectionConfig(BaseModel):
    ...
    curiosity_threshold: float = Field(
        default=0.6, description="Minimum curiosity score to publish"
    )
    ...
    @field_validator(
        "curiosity_threshold", "confidence_threshold", "relevance_threshold", "risk_threshold"
    )
    @classmethod
...
class ObservabilityConfig(BaseModel):
    ...
    jaeger_endpoint: str = Field(
        default="http://jaeger-collector:14268/api/traces", description="Jaeger endpoint"
    )
