...
class TypeScriptParser:
    ...
    def _has_syntax_errors(self, code: str) -> bool:
        ...
        code_without_arrows = re.sub(r'\w+\s*(?::\s*[^=]+)?\s*=\s*(?:async\s+)?(?:\([^)]*\)|\w+)\s*=>', '', code)
    ...
    def _extract_functions(self, code: str) -> List[Dict]:
        ...
        for match in re.finditer(single_param_pattern, code):
            ...
            if param in ['string', 'number', 'boolean', 'void', 'any', 'never', 'unknown', 'null', 'undefined', 'object', 'Promise', 'Array', 'Map', 'Set', 'Function', 'Type']:
                continue
    ...
    def _extract_imports(self, code: str) -> List[Dict]:
        ...
        for match in re.finditer(r"import\s+(?:(\*)\s+as\s+(\w+)|\{([^}]+)\}|(\w+))\s+from\s+['\"]([^'\"]+)['\"]", code):
