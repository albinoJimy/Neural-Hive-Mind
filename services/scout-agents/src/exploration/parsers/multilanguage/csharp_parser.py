...
class CSharpParser:
    ...
    def parse(self, code: str, filename: str) -> Optional[Dict[str, Any]]:
        ...
        if self._ts_parser:
            try:
                ...
                logger.warning(
                    f"tree_sitter_parse_failed: {filename} - {str(e)}"
                )
        ...
        for child in node.children:
            if child.type == 'modifiers':
                for mod in child.children:
                    ...
                    elif mod.type in ['public', 'private', 'protected', 'internal', 'static', 'abstract', 'sealed', 'partial']:
        ...
        for child in node.children:
            if child.type == 'modifiers':
                for mod in child.children:
                    if mod.type == 'attribute':
                        info['attributes'].append(self._extract_attribute(mod, code))
                    elif mod.type in ['public', 'private', 'protected', 'internal', 'static', 'abstract', 'sealed']:
                        info['modifiers'].append(mod.type)
            ...
            if child.type == 'modifiers':
                for mod in child.children:
                    if mod.type == 'attribute':
                        ...
                        info['modifiers'].append(mod.type)
                    elif mod.type in ['public', 'private', 'protected', 'internal', 'sealed', 'extern', 'unsafe']:
                        info['modifiers'].append(mod.type)
    ...
    def _parse_with_regex(self, code: str, filename: str) -> Dict[str, Any]:
        ...
        for class_match in re.finditer(
            ...
            if class_end:
                ...
                # Contar propriedades
                properties_count = len(re.findall(
                    r'\{[^}]*\{[^}]*get[^}]*\}[^}]*\}',
                    class_body
                )) + len(re.findall(
    ...
    def _extract_methods_from_class_body(self, class_body: str, full_code: str, class_start: int, result: Dict, class_name: str):
        ...
        for match in re.finditer(
            ...
            where_clause = match.group(5) if len(match.groups()) >= 5 else None

            # Filtrar palavras-chave que não são nomes válidos de métodos
            if method_name in ['class', 'interface', 'enum', 'struct', 'record', 'get', 'set', 'init', 'add', 'remove']:
                continue
            ...
            is_async = 'async' in match.group(0) or 'Task' in return_type or 'ValueTask' in return_type
