...
class JavaParser:
            ...
            if child.type == 'enum_constant':
                ...
                if const_name:
                    info['constants'].append(
                        code[const_name.start_byte:const_name.end_byte]
                    )
    ...
    def _calculate_complexity_ts(self, tree) -> int:
        ...
        for node in tree.root_node.descendants_of_type({
            ...
            'catch_clause', 'conditional_expression', 'switch_expression'
        }):
            complexity += 1
    ...
    def _parse_with_regex(self, code: str, filename: str) -> Dict[str, Any]:
        ...
        for match in re.finditer(
            ...
            if interface_end:
                ...
                methods_count = len(re.findall(r'\w+\s*\([^)]*\)\s*(?:throws\s+[^{;]+)?\s*;', interface_body))
                self._extract_methods_from_class_body(
    ...
    def _extract_methods_from_class_body(self, class_body: str, full_code: str, class_start: int, result: Dict, class_name: str):
        ...
        for match in re.finditer(
            ...
            if method_name in ['class', 'interface', 'enum', 'implements', 'extends', 'new', 'return', 'if', 'else', 'for', 'while', 'switch', 'case', 'default', 'try', 'catch', 'finally', 'import', 'package']:
                continue
