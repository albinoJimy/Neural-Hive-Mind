...
class RustParser:
    ...
    def _parse_with_tree_sitter(self, code: str, filename: str) -> dict[str, Any]:
        ...
        for node in tree.root_node.children:
            if node.type == "struct_item":
                ...
                result["macros"].append({"lineno": code[: node.start_byte].count("\n") + 1})
    ...
    def _extract_impl(self, node, code: str) -> Optional[dict]:
        ...
        impl_info = {"lineno": code[: node.start_byte].count("\n") + 1, "type": "", "trait": ""}
    ...
    def _parse_with_regex(self, code: str, filename: str) -> dict[str, Any]:
        ...
        # Structs
        for struct_match in re.finditer(r"(?:pub\s+)?struct\s+(\w+)(?:\s*<[^>]*>)?\s*\{", code):
        ...
        # Enums
        for enum_match in re.finditer(r"(?:pub\s+)?enum\s+(\w+)(?:\s*<[^>]*>)?\s*\{", code):
            result["enums"].append(
                {
        ...
        # Traits
        for trait_match in re.finditer(r"(?:pub\s+)?trait\s+(\w+)(?:\s*<[^>]*>)?\s*\{", code):
        ...
        for func_match in re.finditer(
            r"(?:pub\s*)?(?:async\s*)?(?:unsafe\s*)?fn\s+(\w+)\s*\(", code
        ):
        ...
        for macro_match in re.finditer(r"(\w+)!", code):
            result["macros"].append({"lineno": code[: macro_match.start()].count("\n") + 1})

        # Attributes (#[])
        for attr_match in re.finditer(r"#\[\w+", code):
            result["macros"].append({"lineno": code[: attr_match.start()].count("\n") + 1})
