...
# Lifecycle metrics
scout_startup_total = Counter("scout_agent_startup_total", "Total number of Scout Agent startups")

scout_registered_total = Counter(
    "scout_agent_registered_total", "Total number of Service Registry registrations"
)

scout_heartbeat_total = Counter(
    "scout_agent_heartbeat_total", "Total number of heartbeats sent", ["status"]
)

scout_deregistered_total = Counter(
    "scout_agent_deregistered_total", "Total number of deregistrations"
)

# Detection metrics
events_consumed_total = Counter(
    "scout_agent_events_consumed_total", "Total events consumed", ["source"]
)

signals_detected_total = Counter(
    "scout_agent_signals_detected_total", "Total signals detected", ["signal_type", "domain"]
)
...
signals_discarded_total = Counter(
    "scout_agent_signals_discarded_total", "Total signals discarded", ["reason"]
)
...
kafka_publish_total = Counter(
    "scout_agent_kafka_publish_total", "Total Kafka publish attempts", ["topic", "status"]
)

kafka_publish_errors_total = Counter(
    "scout_agent_kafka_publish_errors_total", "Total Kafka publish errors", ["topic", "error_type"]
)
...
active_detections = Gauge(
    "scout_agent_active_detections", "Number of detections currently in progress"
)

queue_size = Gauge("scout_agent_queue_size", "Size of internal signal queue")

rate_limit_exceeded_total = Counter(
    "scout_agent_rate_limit_exceeded_total", "Total times rate limit was exceeded"
)
...
class ScoutMetrics:
    ...
    def record_signal_detected(signal_type: str, domain: str):
        ...
        signals_detected_total.labels(signal_type=signal_type, domain=domain).inc()
    ...
    def record_signal_published(signal_type: str, domain: str, channel: str):
        ...
        signals_published_total.labels(
            signal_type=signal_type, domain=domain, channel=channel
        ).inc()
    ...
    def record_pheromone_published(pheromone_type: str, domain: str):
        ...
        pheromones_published_total.labels(pheromone_type=pheromone_type, domain=domain).inc()
    ...
    def record_kafka_error(topic: str, error_type: str):
        ...
        kafka_publish_errors_total.labels(topic=topic, error_type=error_type).inc()

    @staticmethod
    def record_memory_layer_request(operation: str, success: bool):
        """Record Memory Layer API request"""
        status = "success" if success else "failure"
        memory_layer_requests_total.labels(operation=operation, status=status).inc()

    @staticmethod
    def record_service_registry_request(operation: str, success: bool):
        """Record Service Registry request"""
        status = "success" if success else "failure"
        service_registry_requests_total.labels(operation=operation, status=status).inc()
    ...
    def record_digital_event_consumed(event_type: str, channel: str):
        ...
        digital_events_consumed_total.labels(type=event_type, channel=channel).inc()
