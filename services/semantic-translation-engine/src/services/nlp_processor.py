...
class NLPProcessor:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self._nlp_pt = spacy.load(
                self._model_pt_name, disable=["ner"]  # NER será usado apenas quando necessário
            )
            self._nlp_en = spacy.load(self._model_en_name, disable=["ner"])
    ...
    async def _get_cached_extraction(self, cache_key: str) -> dict | None:
        """
    ...
    async def _cache_extraction(self, cache_key: str, result: dict, ttl: int | None = None):
        """
        ...
        try:
            await self.redis_client.cache_query_result(cache_key, result, ttl or self._cache_ttl)
        except Exception as e:
            logger.warning("Erro ao cachear NLP", key=cache_key, error=str(e))

    async def extract_keywords_async(self, text: str, max_keywords: int = 10) -> list[str]:
        """
    ...
    def extract_keywords(self, text: str, max_keywords: int = 10) -> list[str]:
        ...
        try:
            ...
            nlp_extraction_errors_total.labels(
                operation="keywords", error_type=type(e).__name__
            ).inc()
    ...
    def _extract_keywords_fallback(self, text: str, max_keywords: int) -> list[str]:
        ...
        keywords = [w for w in words if w not in stop_words and len(w) > 2 and w.isalnum()]
    ...
    def extract_objectives(self, text: str) -> list[str]:
        ...
        try:
            ...
            nlp_extraction_errors_total.labels(
                operation="objectives", error_type=type(e).__name__
            ).inc()
    ...
    def extract_entities_advanced(self, text: str) -> list[dict]:
        ...
        try:
            ...
            nlp_extraction_errors_total.labels(
                operation="entities", error_type=type(e).__name__
            ).inc()
