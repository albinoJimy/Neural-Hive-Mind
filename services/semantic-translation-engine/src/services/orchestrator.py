...
class SemanticTranslationOrchestrator:
    ...
    async def process_intent(self, intent_envelope: dict, trace_context: dict):
        ...
        envelope_correlation_id = intent_envelope.get("correlationId") or intent_envelope.get(
            "correlation_id"
        )
        envelope_format = (
            "camelCase"
            if intent_envelope.get("correlationId")
        ...
        try:
            ...
            try:
                neo4j_success = await self.neo4j.persist_intent_to_graph(
                    intent_envelope, cognitive_plan.plan_id, "success"
                )
                if neo4j_success:
                    logger.info(
            ...
            try:
                await self.neo4j.persist_intent_to_graph(intent_envelope, "N/A", "error")
            except Exception as neo4j_error:
                logger.warning(
    ...
    def _validate_intent_envelope(self, intent_envelope: dict):
        ...
        if confidence < 0.5:
            logger.warning(
                "Confiança muito baixa", intent_id=intent_envelope.get("id"), confidence=confidence
            )
    ...
    def _create_cognitive_plan(
        ...
        correlation_id = (
            trace_context.get("correlation_id")
            or intent_envelope.get("correlationId")
            or intent_envelope.get("correlation_id")
        )
        ...
        return CognitivePlan(
            ...
            metadata={
                ...
                "num_similar_intents": len(
                    ((intermediate_repr or {}).get("historical_context") or {}).get(
                        "similar_intents", []
                    )
                ),
