...
class RiskScorer:
    ...
    DOMAIN_WEIGHTS = {"business": 0.4, "security": 0.35, "operational": 0.25}

    # Floor de risco por severidade de operação destrutiva
    DESTRUCTIVE_FLOOR = {"low": 0.7, "medium": 0.7, "high": 0.8, "critical": 0.9}
    ...
    def score(
        self, intermediate_repr: dict, tasks: list[TaskNode]
    ...
    def score_multi_domain(
        ...
        overall_score = self._aggregate_scores(
            business_assessment.score, security_assessment.score, operational_assessment.score
        )
    ...
    def _apply_risk_floor(self, base_score: float, is_destructive: bool, severity: str) -> float:
        """
    ...
    def _aggregate_scores(
        self, business_score: float, security_score: float, operational_score: float
    ) -> float:
        """
