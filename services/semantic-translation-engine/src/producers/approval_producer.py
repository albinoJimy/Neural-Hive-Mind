...
class KafkaApprovalProducer:
    ...
    async def send_approval_request(self, cognitive_plan):
        ...
        try:
            ...
            if cognitive_plan.risk_matrix and "destructive_severity" in cognitive_plan.risk_matrix:
                headers.append(
                    (
                        ...
                        str(cognitive_plan.risk_matrix["destructive_severity"]).encode("utf-8"),
                    )
                )

            if cognitive_plan.correlation_id:
                headers.append(("correlation-id", cognitive_plan.correlation_id.encode("utf-8")))

            if cognitive_plan.trace_id:
                headers.append(("trace-id", cognitive_plan.trace_id.encode("utf-8")))

            if cognitive_plan.span_id:
                headers.append(("span-id", cognitive_plan.span_id.encode("utf-8")))
            ...
            logger.exception(
                "Erro ao publicar plan para aprovacao", plan_id=cognitive_plan.plan_id, error=str(e)
            )
    ...
    def _delivery_callback(self, err, msg):
        ...
        if err:
            logger.error("Falha na entrega do plan para aprovacao", error=err, topic=msg.topic())
        else:
            logger.debug(
