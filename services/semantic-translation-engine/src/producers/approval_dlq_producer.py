...
class ApprovalDLQProducer:
    ...
    async def send_dlq_entry(self, dlq_entry: ApprovalDLQEntry) -> None:
        ...
        try:
            ...
            if dlq_entry.correlation_id:
                headers.append(("correlation-id", dlq_entry.correlation_id.encode("utf-8")))

            if dlq_entry.trace_id:
                headers.append(("trace-id", dlq_entry.trace_id.encode("utf-8")))

            if dlq_entry.span_id:
                headers.append(("span-id", dlq_entry.span_id.encode("utf-8")))

            if dlq_entry.risk_band:
                headers.append(("risk-band", dlq_entry.risk_band.encode("utf-8")))
            ...
            logger.exception(
                "Erro ao publicar entrada na DLQ", plan_id=dlq_entry.plan_id, error=str(e)
            )
    ...
    def _delivery_callback(self, err, msg):
        ...
        if err:
            logger.error("Falha na entrega da mensagem DLQ", error=err, topic=msg.topic())
        else:
            logger.debug(
