...
class IntentConsumer:
    ...
    async def initialize(self):
        ...
        try:
            ...
            missing_topics = [
                topic for topic in self.settings.kafka_topics if topic not in available_topics
            ]
            ...
            if missing_topics:
                logger.error(
                    ...
                    available_topics=sorted(available_topics)[:20],  # Limitar output
                )
                raise RuntimeError(
                    f"Tópicos Kafka não encontrados: {missing_topics}. "
                    f"Verifique se os tópicos foram criados no cluster. "
                    f"Tópicos disponíveis: {sorted(available_topics)[:10]}"
                )
            ...
            logger.exception(
                "Falha ao validar tópicos Kafka", error=str(e), error_type=type(e).__name__
            )
        ...
        if self.settings.schema_registry_url and self.settings.schema_registry_url.strip():
            ...
            if os.path.exists(schema_path):
                ...
                logger.info(
                    "Schema Registry enabled for consumer", url=self.settings.schema_registry_url
                )
            else:
    ...
    async def _consume_async_loop(self, processor_callback: Callable):
        ...
        executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="kafka-poller"
        )
        ...
        while self.running:
            try:
                ...
                logger.exception("Error in consumer loop", error=str(e))
    ...
    def _deserialize_message(self, msg) -> dict:
        ...
        if self.avro_deserializer:
            try:
                ...
                logger.debug(
                    "Auto-detecção: Avro falhou, tentando JSON", avro_error=str(avro_error)
                )
