...
class Neo4jClient:
    ...
    def __init__(self, settings: Settings, nlp_processor: Optional["NLPProcessor"] = None):
    ...
    async def query_similar_intents(
        self, intent_text: str, domain: str, limit: int = 5
    ) -> list[dict]:
        """
    ...
    async def query_ontology(self, entity_type: str) -> dict:
        ...
        Returns:
            Ontology definition
        """
        logger.debug("Buscando ontologia no Neo4j", entity_type=entity_type)
        ...
        async with self.driver.session(database=self.settings.neo4j_database) as session:
            try:
                ...
                return {"canonical_type": entity_type, "properties": {}, "constraints": {}}
    ...
    async def query_causal_relationships(self, intent_id: str) -> list[dict]:
        ...
        """
        logger.debug("Buscando relacionamentos causais no Neo4j", intent_id=intent_id)
        ...
        async with self.driver.session(database=self.settings.neo4j_database) as session:
            try:
                result = await session.run(
                    query, intent_id=intent_id, timeout=self.settings.neo4j_query_timeout / 1000.0
                )
                records = await result.data()

                logger.debug(
                    "Causal relationships queried", intent_id=intent_id, count=len(records)
                )
    ...
    async def persist_intent_to_graph(
        self, intent_envelope: dict, cognitive_plan_id: str, outcome: str
    ) -> bool:
        """
    ...
    async def _persist_intent_entities(self, session, intent_id: str, entities: list[dict]):
        """
        ...
        for entity in entities:
            try:
                await session.run(
                    ...
                    entity_value=self._sanitize_text_for_cypher(str(entity.get("value", ""))),
                    timeout=self.settings.neo4j_query_timeout / 1000.0,
                )
            except Exception as e:
                logger.warning(
