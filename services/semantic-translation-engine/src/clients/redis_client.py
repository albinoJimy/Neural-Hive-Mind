...
class RedisClient:
    ...
    async def initialize(self):
        ...
        if self.settings.redis_cluster_enabled:
            ...
            try:
                ...
                logger.warning(
                    "Redis cluster initialization failed, falling back to standalone", error=str(e)
                )
    ...
    async def get_cached_query(self, query_key: str) -> dict | None:
        """
    ...
    async def cache_query_result(self, query_key: str, result: dict, ttl: int | None = None):
        """
        Cache query result
    ...
    async def get_enriched_context(self, intent_id: str) -> dict | None:
        """
    ...
    async def cache_enriched_context(self, intent_id: str, context: dict, ttl: int = 300):
        """
