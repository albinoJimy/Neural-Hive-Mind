...
class MongoDBClient:
    ...
    async def get_operational_context(self, intent_id: str) -> dict | None:
        """
    ...
    async def save_operational_context(self, intent_id: str, context: dict):
        ...
        await self.context_collection.update_one(
            {"intent_id": intent_id}, {"$set": document}, upsert=True
        )
    ...
    async def append_to_ledger(self, cognitive_plan) -> str:
        ...
        try:
            ...
            logger.info(
                "Plano registrado no ledger", plan_id=cognitive_plan.plan_id, hash=plan_hash
            )

            return plan_hash

        except DuplicateKeyError:
            logger.warning("Plano já existe no ledger", plan_id=cognitive_plan.plan_id)
            raise
    ...
    async def query_ledger(self, plan_id: str) -> dict | None:
        """
    ...
    async def update_plan_approval_status(
        ...
        saga_failure_reason: str | None = None,
    ) -> bool:
        """
        ...
        try:
            result = await self.ledger_collection.update_one(
                {"plan_id": plan_id}, {"$set": update_fields}
            )
            ...
            logger.exception(
                "Erro ao atualizar status de aprovação no ledger", plan_id=plan_id, error=str(e)
            )
            raise

    async def revert_plan_approval_status(
        self, plan_id: str, saga_state: str, compensation_reason: str
    ) -> bool:
        """
        ...
        try:
            ...
            logger.exception("Erro ao reverter status de aprovação", plan_id=plan_id, error=str(e))
            raise

    async def update_plan_saga_state(
        self, plan_id: str, saga_state: str, saga_failure_reason: str | None = None
    ) -> bool:
        """
        ...
        try:
            ...
            if result.modified_count > 0:
                logger.info(
                    "Saga state atualizado no ledger", plan_id=plan_id, saga_state=saga_state
                )
                return True
            logger.warning(
            ...
            logger.exception("Erro ao atualizar saga_state", plan_id=plan_id, error=str(e))
            raise
    ...
    async def update_plan_dlq_status(
        ...
        try:
            ...
            logger.exception("Erro ao atualizar DLQ status", plan_id=plan_id, error=str(e))
            raise
