...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line(
        "markers", "e2e: Testes end-to-end que requerem infraestrutura completa"
    )
    config.addinivalue_line("markers", "slow: Testes que levam mais de 30 segundos")
    config.addinivalue_line("markers", "approval_flow: Testes específicos do fluxo de aprovação")
    config.addinivalue_line("markers", "destructive: Testes envolvendo operações destrutivas")
    config.addinivalue_line("markers", "metrics: Testes que validam métricas Prometheus")
    config.addinivalue_line(
        "markers", "failure_scenarios: Testes de cenários de falha e resiliência"
    )
    config.addinivalue_line("markers", "integration: Testes de integração entre componentes")
