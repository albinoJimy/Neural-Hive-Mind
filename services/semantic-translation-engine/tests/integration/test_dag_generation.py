...
class TestPatternBasedDecomposition:
    ...
    def intermediate_repr_onboarding(self):
        ...
        return {
            ...
            "domain": "business",
            "objectives": ["create"],
            "entities": [{"type": "user", "value": "new_customer", "confidence": 0.9}],
            "constraints": {"priority": "high", "security_level": "internal"},
            "text": "Create new user onboarding registration flow",
        }

    def test_user_onboarding_pattern_generates_subtasks(
        self, mock_settings, pattern_matcher_with_onboarding, intermediate_repr_onboarding
    ):
        """Verifica que padrão 'user_onboarding' gera subtasks corretas"""
        dag_generator = DAGGenerator(
            pattern_matcher=pattern_matcher_with_onboarding, task_splitter=None
        )
            ...
            mock_load.return_value = {
                "patterns": {
                    "low_confidence_pattern": {
                        "name": "Low Confidence",
                        "match_criteria": {"objectives": {"any_of": ["query"]}},
                        "template": {"subtasks": []},
                    }
                },
                "matching_config": {
    ...
    def test_pattern_dependencies_mapped_correctly(
        self, pattern_matcher_with_onboarding, intermediate_repr_onboarding
    ):
        """Verifica que dependências do template são mapeadas para task_ids reais"""
        dag_generator = DAGGenerator(pattern_matcher=pattern_matcher_with_onboarding)
        ...
        has_validate_dep = any(validate_task.task_id in t.dependencies for t in create_tasks)
...
class TestTaskSplittingIntegration:
    ...
    def intermediate_repr_complex(self):
        ...
        return {
            ...
            "constraints": {"priority": "high", "security_level": "confidential"},
        }

    def test_complex_task_split_into_subtasks(self, task_splitter, intermediate_repr_complex):
        """Verifica que task complexa é dividida em validation → main → verification"""
        dag_generator = DAGGenerator(pattern_matcher=None, task_splitter=task_splitter)
        ...
        has_split_tasks = any(
            role in ["validation", "main_operation", "verification"] for role in task_roles
        )
    ...
    def test_recursive_splitting_respects_max_depth(self, mock_settings):
        ...
        max_depth = max((t.metadata.get("split_depth", 0) for t in tasks if t.metadata), default=0)
        assert max_depth <= mock_settings.task_splitting_max_depth

    def test_split_tasks_maintain_parent_task_id(self, task_splitter, intermediate_repr_complex):
        ...
        split_tasks = [t for t in tasks if t.metadata and t.metadata.get("split_method")]
...
class TestEntityBasedDependencies:
    ...
    def test_write_before_read_dependency(self, dag_generator):
        ...
        intermediate_repr = {
            ...
            "entities": [{"type": "user", "name": "customer", "confidence": 0.9}],
            "constraints": {},
        }
    ...
    def test_shared_entity_creates_dependency(self, dag_generator):
        ...
        intermediate_repr = {
            ...
            "entities": [{"type": "record", "name": "order", "confidence": 0.9}],
            "constraints": {},
        }
    ...
    def test_no_cycles_created_by_entity_analysis(self, dag_generator):
        ...
        intermediate_repr = {
            ...
            "entities": [{"type": "resource", "name": "data", "confidence": 0.9}],
            "constraints": {},
        }
...
class TestParallelismDetection:
    ...
    def test_independent_tasks_grouped_as_parallel(self):
        ...
        intermediate_repr = {"objectives": ["query", "validate"], "entities": [], "constraints": {}}

        tasks, _ = dag_generator.generate(intermediate_repr)

        # Verificar se há grupos paralelos atribuídos
        tasks_with_parallel_group = [
            t for t in tasks if t.metadata and t.metadata.get("parallel_group")
        ]
    ...
    def test_dependent_tasks_not_in_same_parallel_group(self, dag_generator):
        ...
        if create_task and update_task:
            create_group = (
                create_task.metadata.get("parallel_group") if create_task.metadata else None
            )
            update_group = (
                update_task.metadata.get("parallel_group") if update_task.metadata else None
            )
...
class TestEndToEndAdvancedDecomposition:
    ...
    def test_complex_intent_full_flow(self, mock_settings, pattern_matcher):
        ...
        dag_generator = DAGGenerator(pattern_matcher=pattern_matcher, task_splitter=task_splitter)
    ...
    def test_fallback_to_heuristic_when_no_pattern(self, mock_settings):
        ...
        intermediate_repr = {"objectives": ["query"], "entities": [], "constraints": {}}
    ...
    def test_dag_remains_acyclic_after_all_enhancements(self, mock_settings, pattern_matcher):
        ...
        test_cases = [
            ...
            {"objectives": ["transform"], "entities": [], "constraints": {"priority": "critical"}},
        ]
...
class TestEntityDependencyIntegration:
    ...
    def test_end_to_end_with_conflicts(self):
        ...
        if create_task and query_task:
            assert execution_order.index(create_task.task_id) < execution_order.index(
                query_task.task_id
            )
            ...
            mock_load.return_value = {
                "patterns": {
                    "crud_pattern": {
                        ...
                        "match_criteria": {"objectives": {"all_of": ["create", "query"]}},
                        "template": {
                            "subtasks": [
                                {
                                    "id": "validate",
    ...
    def test_fuzzy_matching_in_integration(self):
        ...
        dag_generator = DAGGenerator(
            entity_matching_fuzzy_enabled=True, entity_matching_fuzzy_threshold=0.8
        )
    ...
    def test_conflict_with_existing_heuristic_dependency(self):
        ...
        if create_task and update_task:
            ...
            assert create_task.task_id in update_task.dependencies or execution_order.index(
                create_task.task_id
            ) < execution_order.index(update_task.task_id)
