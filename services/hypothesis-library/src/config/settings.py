...
class Settings(BaseSettings):
    ...
    mongodb_versions_collection: str = Field(
        default="hypothesis_versions",
        description="MongoDB collection for hypothesis versions"
    )
    ...
    kafka_hypothesis_events_topic: str = Field(
        default="hypothesis.events",
        description="Kafka topic for hypothesis events"
    )
    kafka_consumer_group_id: str = Field(
        default="hypothesis-library",
    ...
    max_versions_per_hypothesis: int = Field(
        default=50,
        description="Maximum versions to keep per hypothesis"
    )
    auto_archive_days: int = Field(
        default=180,
        description="Days after which to auto-archive completed hypotheses"
    )
    require_approval_for_testing: bool = Field(
        default=True,
        description="Require approval before starting test"
    )
    enable_versioning: bool = Field(
        default=True,
        description="Enable hypothesis versioning"
    )
    ...
    experimentation_engine_timeout: int = Field(
        default=30,
        description="Timeout for Experimentation Engine requests (seconds)"
    )
