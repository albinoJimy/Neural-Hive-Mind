"""Service Registry gRPC client para Data Migration Service.

Este cliente registra o serviço data-migration no Service Registry e gerencia
...
class DataMigrationServiceRegistryClient:
    """Cliente gRPC para Service Registry para o serviço data-migration."""

    def __init__(
        self,
        service_name: str = "data-migration",
        agent_type: Optional[int] = None,  # DATA_MIGRATION = 11
    ):
        """
        Inicializa o cliente.

        Args:
            service_name: Nome do serviço (default: "data-migration")
            agent_type: Tipo do agente do proto AgentType (default: DATA_MIGRATION)
        """
        settings = get_settings()
        self.service_name = service_name
        self.agent_type = agent_type or 11  # DATA_MIGRATION
    ...
    async def register(
        ...
        try:
            ...
            # Capabilities padrão do data-migration
...
async def register_data_migration_service(
    service_name: str = "data-migration",
    capabilities: Optional[List[str]] = None,
    metadata: Optional[Dict[str, str]] = None,
) -> Optional[DataMigrationServiceRegistryClient]:
    """
    Registra o serviço data-migration no Service Registry.
    ...
    """
    client = DataMigrationServiceRegistryClient(service_name)
    if await client.initialize():
        agent_id = await client.register(capabilities, metadata)
        if agent_id:
            logger.info(
                "data_migration_service_registered",
                service=service_name,
                agent_id=agent_id,
            )
            return client
        else:
            logger.error("data_migration_service_registration_failed", service=service_name)
            await client.close()
            return None
    else:
        logger.error("data_migration_service_client_init_failed", service=service_name)
        return None
