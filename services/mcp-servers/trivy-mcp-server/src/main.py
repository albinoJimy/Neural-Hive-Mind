"""
Entry point do Trivy MCP Server.
...
class TrivyMCPServer(BaseMCPServer):
    """Servidor MCP para Trivy."""

    async def initialize(self) -> None:
        """Inicializa recursos do servidor."""
        setup_metrics()
        logger.info("trivy_mcp_server_resources_initialized")

    async def cleanup(self) -> None:
        """Libera recursos do servidor."""
        logger.info("trivy_mcp_server_cleanup_complete")
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    logger.info("trivy_mcp_server_stopped")
