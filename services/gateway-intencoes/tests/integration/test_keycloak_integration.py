...
class TestKeycloakIntegration:
    ...
    async def admin_token(self, keycloak_settings):
        ...
        token_url = (
            f"{keycloak_settings.keycloak_url}/auth/realms/master/protocol/openid-connect/token"
        )
    ...
    async def test_realm_setup(self, keycloak_settings, admin_token):
        ...
        async with httpx.AsyncClient() as client:
            # Create realm
            realm_response = await client.post(admin_api_url, headers=headers, json=realm_config)
            ...
            client_response = await client.post(clients_url, headers=headers, json=client_config)

            # Create user in realm
            users_url = f"{admin_api_url}/neural-hive-test/users"
            user_response = await client.post(users_url, headers=headers, json=user_config)
...
class TestKeycloakFailureScenarios:
    ...
    async def test_jwks_fallback_behavior(self):
        ...
        mock_jwks = {"keys": [{"kid": "test-key", "kty": "RSA", "n": "test", "e": "AQAB"}]}
