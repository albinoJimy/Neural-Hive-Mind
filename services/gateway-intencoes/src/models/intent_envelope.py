...
class Constraint(BaseModel):
    ...
    timeout_ms: Optional[int] = Field(None, gt=0, description="Timeout em millisegundos")
    required_capabilities: List[str] = Field(default_factory=list, description="Capacidades necessárias")
    security_level: SecurityLevel = Field(default=SecurityLevel.INTERNAL, description="Nível de segurança")

class QualityOfService(BaseModel):
    """Garantias de QoS"""
    delivery_mode: DeliveryMode = Field(default=DeliveryMode.EXACTLY_ONCE, description="Modo de entrega")
...
class IntentEnvelope(BaseModel):
    ...
    def to_avro_dict(self) -> Dict[str, Any]:
        ...
        avro_data = {
            ...
            "context": {
                ...
                "channel": convert_enum(self.context.channel) if self.context and self.context.channel else None,
                ...
                } if self.context and self.context.geolocation else None
            } if self.context else None,
            "constraints": {
                "priority": convert_enum(self.constraints.priority),
                "deadline": int(self.constraints.deadline.timestamp() * 1000) if self.constraints.deadline else None,
    ...
    def to_cache_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "intent": {
                "text": self.intent.text[:500] if len(self.intent.text) > 500 else self.intent.text,  # Truncar texto longo
...
class IntentRequest(BaseModel):
    ...
    correlation_id: Optional[str] = Field(None, description="ID de correlação")
    constraints: Optional[Constraint] = Field(None, description="Restrições de processamento")
