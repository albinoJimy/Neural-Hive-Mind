...
class TestApprovalGateThresholds:
    ...
    async def test_custom_thresholds(self, mock_git_client):
        ...
        gate = ApprovalGate(
            git_client=mock_git_client, auto_approval_threshold=0.95, min_quality_score=0.6
        )
...
class TestApprovalGateCommitAndPush:
    ...
    async def test_commit_and_push_with_artifact(
        ...
        result = await gate._commit_and_push(
            sample_pipeline_context_with_artifacts, auto_approved=False
        )
    ...
    async def test_commit_and_push_without_mongodb(
        ...
        gate = ApprovalGate(git_client=mock_git_client, mongodb_client=None)
    ...
    async def test_commit_creates_readme_with_metadata(
        ...
        await gate._commit_and_push(sample_pipeline_context_with_artifacts, auto_approved=True)
    ...
    async def test_commit_saves_commit_metadata(
        ...
        await gate._commit_and_push(sample_pipeline_context_with_artifacts, auto_approved=False)
