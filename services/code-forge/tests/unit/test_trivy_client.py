...
def trivy_client_with_trivy():
    ...
    with patch("shutil.which", return_value="/usr/bin/trivy"):
        client = TrivyClient(enabled=True, severity="CRITICAL,HIGH", timeout=300)
...
class TestTrivyClientInitialization:
    ...
    def test_init_with_custom_values(self):
        ...
        client = TrivyClient(enabled=False, severity="HIGH,MEDIUM", timeout=120)
...
class TestTrivyResultCreation:
    ...
    def test_create_result_with_vulnerabilities(self, trivy_client):
        ...
        result = trivy_client._create_result(
            status=ValidationStatus.FAILED, score=0.3, counts=counts, duration_ms=5000
        )
    ...
    async def test_scan_container_image_success(self, trivy_client_with_trivy):
        ...
        trivy_output = {
            "Results": [
                {
                    "Target": "nginx:latest",
                    "Vulnerabilities": [{"Severity": "HIGH", "VulnerabilityID": "CVE-2023-1234"}],
                }
            ]
        }
    ...
    async def test_scan_filesystem_success(self, trivy_client_with_trivy):
        ...
        trivy_output = {
            "Results": [
                {
                    "Target": "/tmp/app",
                    "Vulnerabilities": [{"Severity": "MEDIUM", "VulnerabilityID": "CVE-2023-0001"}],
                }
            ]
        }
    ...
    async def test_scan_iac_success(self, trivy_client_with_trivy):
        ...
        trivy_output = {
            "Results": [
                {
                    "Target": "main.tf",
                    "Misconfigurations": [{"Severity": "LOW", "Title": "Missing encryption"}],
                }
            ]
        }
