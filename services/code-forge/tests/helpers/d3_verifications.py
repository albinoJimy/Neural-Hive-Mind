...
def verify_all_stages_completed(pipeline_result: PipelineResult) -> bool:
    ...
    assert (
        len(pipeline_result.pipeline_stages) == 6
...
def verify_pipeline_timeout_not_exceeded(
    pipeline_result: PipelineResult, max_timeout_ms: int = 14400000  # 4 horas
) -> bool:
    """
    Verifica se o timeout não foi excedido.
    """
    assert (
        pipeline_result.total_duration_ms <= max_timeout_ms
    ), f"Timeout excedido: {pipeline_result.total_duration_ms}ms > {max_timeout_ms}ms"
    return True


def verify_pipeline_duration_within_sla(
    pipeline_result: PipelineResult, max_duration_ms: int = 30000  # 30 segundos para testes
) -> bool:
    """
    Verifica se a duração está dentro do SLA.
    """
    assert (
        pipeline_result.total_duration_ms < max_duration_ms
...
def verify_artifacts_generated(pipeline_result: PipelineResult) -> bool:
    ...
    assert (
        ArtifactCategory.CONTAINER in artifact_types
...
def verify_container_artifact(artifact: CodeForgeArtifact) -> bool:
    ...
    """
    assert (
        artifact.artifact_type == ArtifactCategory.CONTAINER
    ...
    assert artifact.content_hash.startswith(
        "sha256:"
...
def verify_sbom_generated(artifact: CodeForgeArtifact, format: str = "SPDX") -> bool:
    ...
    assert any(
        prefix in artifact.sbom_uri for prefix in ["s3://", "gs://", "http"]
    ), f"sbom_uri inválido: {artifact.sbom_uri}"

    # Verificar se contém .spdx.json ou .spdx
    assert any(
        ext in artifact.sbom_uri for ext in [".spdx.json", ".spdx", ".json"]
...
def verify_artifact_metadata(artifact: CodeForgeArtifact) -> bool:
    ...
    for field in required_fields:
        assert (
            hasattr(artifact, field) and getattr(artifact, field) is not None
...
def verify_security_scan_executed(context: PipelineContext) -> bool:
    ...
    assert (
        ValidationType.SECURITY_SCAN in validation_types or ValidationType.SAST in validation_types
...
def verify_kafka_message_published(
    mock_producer: MagicMock, expected_status: PipelineStatus = PipelineStatus.COMPLETED
) -> bool:
    """
    ...
    assert (
        mock_producer.publish_result.call_count >= 1
...
def verify_mongodb_artifact_persisted(mock_mongodb: MagicMock, ticket_id: str) -> bool:
    """
    Verifica se artefato foi persistido no MongoDB.
    """
    assert (
        mock_mongodb.save_artifact_content.called or mock_mongodb.save_pipeline_result.called
...
def verify_postgres_pipeline_persisted(mock_postgres: MagicMock, pipeline_id: str) -> bool:
    """
...
def verify_postgres_pipeline_record(record: Dict[str, Any]) -> bool:
    ...
    """
    required_fields = ["pipeline_id", "ticket_id", "status", "total_duration_ms", "created_at"]
...
def verify_build_metrics_emitted(mock_metrics: MagicMock) -> bool:
    ...
    for metric_name in metrics_to_verify:
        assert hasattr(mock_metrics, metric_name), f"Métrica {metric_name} não existe no mock"
...
def verify_pipeline_metrics_emitted(mock_metrics: MagicMock) -> bool:
    ...
    """
    metrics_to_verify = ["pipelines_total", "pipeline_duration_seconds", "stage_duration_seconds"]
...
def verify_api_metrics_emitted(
    mock_metrics: MagicMock, method: str = "trigger_pipeline", status: str = "success"
) -> bool:
    """
    Verifica se métricas de API foram emitidas.
    """
    assert hasattr(
        mock_metrics, "code_forge_api_calls_total"
...
def verify_no_error_logs(log_records: List[Dict[str, Any]]) -> bool:
    ...
    for record in log_records:
        ...
        assert level not in error_levels, f"Log de erro encontrado: {record.get('message', record)}"
