...
def s3_client():
    ...
    with patch.dict("sys.modules", {"boto3": MagicMock(), "botocore.config": MagicMock()}):
        client = S3ArtifactClient(
            bucket="test-bucket", region="us-east-1", endpoint=None, metrics=None
        )
        return client
...
def mock_boto3_client():
    ...
    mock_client.list_objects_v2 = MagicMock(
        return_value={
            "Contents": [
                {
                    "Key": "sboms/ticket1/artifact1/sbom.json",
                    "Size": 1024,
                    "LastModified": "2024-01-01",
                    "ETag": '"abc"',
                },
                {
                    "Key": "sboms/ticket1/artifact2/sbom.json",
                    "Size": 2048,
...
async def test_upload_sbom_success(s3_client, mock_boto3_client):
    ...
    try:
        result = await s3_client.upload_sbom(
            local_path=temp_path, artifact_id="artifact-123", ticket_id="ticket-456"
        )
...
async def test_download_sbom_invalid_uri(s3_client):
    ...
    result = await s3_client.download_sbom(
        sbom_uri="file:///tmp/sbom.json", local_path="/tmp/local.json"
    )
...
async def test_download_sbom_failure(s3_client, mock_boto3_client):
    ...
    result = await s3_client.download_sbom(
        sbom_uri="s3://test-bucket/sboms/test/sbom.json", local_path="/tmp/local.json"
    )
...
async def test_get_sbom_metadata_success(s3_client, mock_boto3_client):
    ...
    result = await s3_client.get_sbom_metadata(sbom_uri="s3://test-bucket/sboms/test/sbom.json")
...
async def test_delete_sbom_success(s3_client, mock_boto3_client):
    ...
    result = await s3_client.delete_sbom(sbom_uri="s3://test-bucket/sboms/test/sbom.json")
...
async def test_verify_sbom_integrity_success(s3_client, mock_boto3_client):
    ...
    try:
        result = await s3_client.verify_sbom_integrity(
            sbom_uri="s3://test-bucket/sboms/test/sbom.json", expected_checksum=expected_checksum
        )
...
def test_client_initialization_without_endpoint():
    ...
    with patch.dict("sys.modules", {"boto3": MagicMock(), "botocore.config": MagicMock()}):
        client = S3ArtifactClient(bucket="my-bucket", region="eu-west-1", endpoint=None)
...
def test_client_initialization_with_endpoint():
    ...
    with patch.dict("sys.modules", {"boto3": MagicMock(), "botocore.config": MagicMock()}):
        client = S3ArtifactClient(
            bucket="test-bucket", region="us-east-1", endpoint="http://localhost:4566"
        )
