...
def mock_llm_client():
    ...
    client.generate_code = AsyncMock(
        return_value={"code": 'print("Hello World")', "confidence_score": 0.85}
    )
    return client
...
def mock_analyst_client():
    ...
    client.find_similar_templates = AsyncMock(
        return_value=[
            {"text": "FastAPI microservice template", "similarity": 0.9},
            {"text": "Python REST API template", "similarity": 0.85},
        ]
    )
    client.get_architectural_patterns = AsyncMock(
        return_value=["microservices", "event-driven", "REST API"]
    )
    return client
...
async def test_build_rag_context_success(mock_mongodb_client, mock_analyst_client, sample_ticket):
    ...
    composer = CodeComposer(
        mongodb_client=mock_mongodb_client, llm_client=None, analyst_client=mock_analyst_client
    )
...
async def test_build_rag_context_no_analyst_client(mock_mongodb_client, sample_ticket):
    ...
    composer = CodeComposer(
        mongodb_client=mock_mongodb_client, llm_client=None, analyst_client=None
    )
...
def test_build_llm_prompt(mock_mongodb_client, sample_ticket):
    ...
    rag_context = {
        "similar_templates": [{"text": "FastAPI template", "similarity": 0.9}],
...
def test_generate_heuristic_library(mock_mongodb_client):
    ...
    parameters = {"artifact_type": "LIBRARY", "language": "python", "service_name": "my-lib"}
...
def test_generate_heuristic_script(mock_mongodb_client):
    ...
    parameters = {"artifact_type": "SCRIPT", "language": "python", "service_name": "my-script"}
