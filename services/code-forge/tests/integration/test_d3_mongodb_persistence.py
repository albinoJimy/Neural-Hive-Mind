...
class TestD3MongoDBConnection:
    ...
    async def test_d3_mongodb_connection_string_format(self):
        ...
        # Verificar formato
        assert mongodb_url.startswith("mongodb://") or mongodb_url.startswith(
            "mongodb+srv://"
...
class TestD3ArtifactPersistence:
    ...
    async def test_d3_save_artifact_content(
        ...
        await mock_mongodb_client.save_artifact_content(artifact.artifact_id, content)
...
class TestD3PipelinePersistence:
    ...
    async def test_d3_save_pipeline_result(self, mock_mongodb_client, d3_expected_pipeline_result):
        ...
        await mock_mongodb_client.save_pipeline_result(
            d3_expected_pipeline_result.pipeline_id, d3_expected_pipeline_result
        )
    ...
    async def test_d3_query_pipelines_by_ticket(self, mock_mongodb_client):
        ...
        # Mock query
        mock_mongodb_client.query_pipelines_by_ticket = AsyncMock(return_value=[])
...
class TestD3MongoDBQueries:
    ...
    async def test_d3_query_artifacts_by_type(self, mock_mongodb_client):
        ...
        - Índice utilizado
        """
        mock_mongodb_client.query_artifacts_by_type = AsyncMock(return_value=[])

        artifacts = await mock_mongodb_client.query_artifacts_by_type(ArtifactCategory.CONTAINER)
    ...
    async def test_d3_query_pipelines_by_date_range(self, mock_mongodb_client):
        ...
        mock_mongodb_client.query_pipelines_by_date_range = AsyncMock(return_value=[])

        pipelines = await mock_mongodb_client.query_pipelines_by_date_range(start_date, end_date)
...
class TestD3MongoDBTransactions:
    ...
    async def test_d3_transactional_artifact_save(
        ...
        mock_mongodb_client.save_artifacts_transactional = AsyncMock(return_value=True)
    ...
    async def test_d3_artifact_indexes_exist(self):
        ...
        try:
            ...
            for required in required_indexes:
                assert required in index_fields, f"Índice {required} não encontrado"
...
class TestD3MongoDBCleanup:
    ...
    async def test_d3_delete_old_artifacts(self, mock_mongodb_client):
        ...
        mock_mongodb_client.delete_artifacts_older_than = AsyncMock(return_value=10)

        deleted = await mock_mongodb_client.delete_artifacts_older_than(retention_days)
    ...
    async def test_d3_archive_old_pipelines(self, mock_mongodb_client):
        ...
        mock_mongodb_client.archive_pipelines_older_than = AsyncMock(return_value=50)

        archived = await mock_mongodb_client.archive_pipelines_older_than(archive_days)
...
class TestD3MongoDBSecurity:
    ...
    async def test_d3_sensitive_data_encryption(self, mock_mongodb_client):
        ...
        sensitive_data = {"api_key": "secret-key-123", "password": "secret-password"}

        # Mock de criptografia
        mock_mongodb_client.save_with_encryption = AsyncMock(return_value=True)

        await mock_mongodb_client.save_with_encryption("collection", sensitive_data)
