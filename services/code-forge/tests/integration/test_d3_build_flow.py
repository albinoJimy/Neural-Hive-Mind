...
class TestD3BuildFlowEndToEnd:
    ...
    async def test_d3_build_ticket_creation_and_processing(
        ...
        mock_d3_pipeline_engine.ticket_client.get_ticket = AsyncMock(return_value=d3_build_ticket)
        ...
        assert duration_ms < 30000, f"Tempo execução: {duration_ms}ms >= 30000ms (SLA excedido)"
    ...
    async def test_d3_pipeline_with_container_artifact(
        ...
        - signature presente
        """
        result = await mock_d3_pipeline_engine.execute_pipeline(d3_build_ticket_with_container)
        ...
        container_artifacts = [
            a for a in result.artifacts if a.artifact_type == ArtifactCategory.CONTAINER
        ]
        ...
        assert (
            "ghcr.io" in container.content_uri
            or "docker.io" in container.content_uri
            or "gcr.io" in container.content_uri
        ), f"content_uri inválida: {container.content_uri}"

        assert container.content_hash is not None, "content_hash vazio"
        assert container.content_hash.startswith(
            "sha256:"
    ...
    async def test_d3_pipeline_with_sbom_generation(
        ...
        """
        result = await mock_d3_pipeline_engine.execute_pipeline(d3_build_ticket_with_sbom)

        # Verificar artefatos com SBOM
        artifacts_with_sbom = [a for a in result.artifacts if a.sbom_uri is not None]
    ...
    async def test_d3_pipeline_timeout_handling(self, d3_build_ticket, mock_d3_pipeline_engine):
        ...
        mock_d3_pipeline_engine.template_selector.select = AsyncMock(side_effect=_slow_stage)
    ...
    async def test_d3_pipeline_retry_logic(
        ...
        mock_d3_pipeline_engine.template_selector.select = AsyncMock(side_effect=_failing_stage)
        ...
        assert (
            mock_ticket_client.create_compensation_ticket.called
...
class TestD3PipelineStages:
    ...
    async def test_d3_template_selection_stage(
        ...
        assert d3_pipeline_context.selected_template is not None, "Template não selecionado"
    ...
    async def test_d3_code_composition_stage(self, d3_pipeline_context, mock_d3_code_composer):
        ...
        assert d3_pipeline_context.metadata.get("code_composed") == "true", "Código não composto"
    ...
    async def test_d3_validation_stage(self, d3_pipeline_context, mock_d3_validator):
        ...
        assert len(d3_pipeline_context.validation_results) > 0, "Nenhuma validação executada"
    ...
    async def test_d3_packaging_stage(self, d3_pipeline_context, mock_d3_packager):
        ...
        assert len(d3_pipeline_context.generated_artifacts) > 0, "Nenhum artefato gerado"

        container = d3_pipeline_context.generated_artifacts[0]
        assert (
            container.artifact_type == ArtifactCategory.CONTAINER
    ...
    async def test_d3_approval_gate_stage(self, d3_pipeline_context, mock_d3_approval_gate):
        ...
        assert (
            d3_pipeline_context.metadata.get("approval_status") == "AUTO_APPROVED"
...
class TestD3ExternalServicesIntegration:
    ...
    async def test_d3_ticket_client_status_update(
        ...
        assert mock_ticket_client.update_status.called, "update_status não foi chamado"
    ...
    async def test_d3_postgres_persistence(
        ...
        assert mock_postgres_client.save_pipeline.called, "save_pipeline não foi chamado"
...
class TestD3FailureScenarios:
    ...
    async def test_d3_packaging_failure_creates_compensation(
        ...
        assert result.status == PipelineStatus.FAILED, f"Status: {result.status}"
...
class TestD3Performance:
    ...
    async def test_d3_pipeline_duration_within_sla(self, d3_build_ticket, mock_d3_pipeline_engine):
        ...
        assert duration_ms < 30000, f"Duração: {duration_ms}ms excede SLA de 30000ms"
    ...
    async def test_d3_concurrent_pipeline_limit(self, mock_d3_pipeline_engine):
        ...
        for i in range(5):
            ticket = ExecutionTicket(
                ...
                sla=SLA(deadline=datetime.now(), timeout_ms=30000, max_retries=3),
                qos=QoS(
        ...
        results = await asyncio.gather(
            *[mock_d3_pipeline_engine.execute_pipeline(ticket) for ticket in tickets]
        )
