...
async def migrate_sboms(dry_run: bool = True):
    ...
    try:
        ...
        cursor = collection.find({"sbom_uri": {"$regex": "^file://"}})
        ...
        async for artifact in cursor:
            ...
            if not os.path.exists(local_path):
                logger.warning(
                    "sbom_file_not_found", artifact_id=artifact_id, local_path=local_path
                )
                skipped += 1
                continue
            ...
            try:
                ...
                new_uri = await s3_client.upload_sbom(
                    local_path=local_path, artifact_id=artifact_id, ticket_id=ticket_id
                )
                ...
                logger.info(
                    "sbom_migrated", artifact_id=artifact_id, old_uri=sbom_uri, new_uri=new_uri
                )
                migrated += 1

            except Exception as e:
                logger.error("migration_failed", artifact_id=artifact_id, error=str(e))
                failed += 1
...
def main():
    parser = argparse.ArgumentParser(description="Migra SBOMs de file:// para S3")
