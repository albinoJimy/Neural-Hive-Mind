...
class Settings(BaseSettings):
    ...
    KAFKA_TICKETS_TOPIC: str = Field(
        default="execution.tickets", description="Tópico de Execution Tickets"
    )
    KAFKA_RESULTS_TOPIC: str = Field(
        default="code-forge.results", description="Tópico de resultados do Code Forge"
    )
    ...
    HEARTBEAT_INTERVAL_SECONDS: int = Field(
        default=30, description="Intervalo de heartbeat (segundos)"
    )
    ...
    TEMPLATES_GIT_BRANCH: str = Field(
        default="main", description="Branch do repositório de templates"
    )
    TEMPLATES_LOCAL_PATH: str = Field(
        default="/app/templates", description="Caminho local para templates"
    )
    ...
    SONARQUBE_SCANNER_TIMEOUT: int = Field(
        default=900, description="Timeout do scanner SonarQube (segundos)"
    )
    SONARQUBE_POLL_INTERVAL: int = Field(
        default=5, description="Intervalo de polling SonarQube (segundos)"
    )
    SONARQUBE_POLL_TIMEOUT: int = Field(
        default=300, description="Timeout de polling SonarQube (segundos)"
    )
    ...
    SIGSTORE_FULCIO_URL: str = Field(
        default="https://fulcio.sigstore.dev", description="URL Fulcio"
    )
    ...
    PIPELINE_TIMEOUT_SECONDS: int = Field(
        default=3600, description="Timeout de pipeline (segundos)"
    )
    AUTO_APPROVAL_THRESHOLD: float = Field(
        default=0.9, description="Threshold para aprovação automática"
    )
    ...
    OTEL_EXPORTER_ENDPOINT: str = Field(
        ...
        description="Endpoint OpenTelemetry",
    )
    ...
    OTEL_SERVICE_NAME: str = Field(
        default="code-forge", description="Nome do serviço OpenTelemetry"
    )
    ...
    MCP_TOOL_CATALOG_HOST: str = Field(
        default="mcp-tool-catalog", description="MCP Tool Catalog host"
    )
    ...
    def validate_llm_config(self) -> "Settings":
        ...
        if self.LLM_PROVIDER == "openai" and not self.LLM_API_KEY:
            raise ValueError("LLM_API_KEY obrigatório quando LLM_PROVIDER='openai'")

        if self.LLM_PROVIDER == "anthropic" and not self.LLM_ANTHROPIC_API_KEY:
            raise ValueError("LLM_ANTHROPIC_API_KEY obrigatório quando LLM_PROVIDER='anthropic'")

        if self.LLM_PROVIDER == "local" and not self.LLM_BASE_URL:
            raise ValueError("LLM_BASE_URL obrigatório quando LLM_PROVIDER='local'")
