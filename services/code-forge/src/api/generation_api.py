...
generation_requests_total = Counter(
    "neural_hive_code_forge_generation_requests_total", "Total generation requests", ["status"]
)
generation_duration_seconds = Histogram(
    "neural_hive_code_forge_generation_duration_seconds", "Generation request duration", ["method"]
)
...
async def _get_generation_state(
    redis_client: RedisClient, request_id: str
...
async def _update_generation_state(
    redis_client: RedisClient, request_id: str, updates: dict[str, Any]
):
    ...
    try:
        with tracer.start_as_current_span("generation.process") as span:
            ...
            await _update_generation_state(
                redis_client,
                request_id,
                {"status": "processing", "updated_at": datetime.now(UTC).isoformat()},
            )
            ...
            ticket = ExecutionTicket(
                ...
                sla=SLA(deadline=datetime.now(UTC), timeout_ms=300000, max_retries=1),
                qos=QoS(
            ...
            context = PipelineContext(
                pipeline_id=request_id, ticket=ticket, trace_id=request_id, span_id=request_id[:16]
            )
            ...
            if artifact:
                ...
                code_content = await code_composer.mongodb_client.get_artifact_content(
        ...
        logger.error("generation_task_failed", request_id=request_id, error=str(e), exc_info=True)
...
async def create_generation_request(
    payload: dict[str, Any], redis_client: Optional[RedisClient] = Depends(get_redis_client)
) -> GenerationResponse:
    """
...
async def get_generation_status(
    request_id: str, redis_client: RedisClient = Depends(get_redis_client)
...
async def list_generation_requests(
    ...
    return ListGenerationResponse(
        total=len(requests_list), requests=requests_list, status_counts=status_counts
    )
