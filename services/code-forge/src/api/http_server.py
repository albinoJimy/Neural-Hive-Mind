...
def create_app() -> FastAPI:
    ...
    async def ready(request: Request):
        ...
        try:
            if (
                hasattr(request.app.state, "sonarqube_client")
                and request.app.state.sonarqube_client
            ):
        ...
        try:
            if (
                hasattr(request.app.state, "s3_artifact_client")
                and request.app.state.s3_artifact_client
            ):
                s3_enabled = True
        ...
        try:
            if (
                hasattr(request.app.state, "service_registry")
                and request.app.state.service_registry
            ):
        ...
        optional_ready = all(
            dependencies.get(dep) in ("connected", "disabled") for dep in optional_deps
        )
