...
class Validator:
    ...
    async def validate(self, context: PipelineContext):
        ...
        validation_tools = [t for t in selected_tools if t.get("category") == "VALIDATION"]
        ...
        for result in results:
            ...
            if self.metrics:
                self.metrics.validations_run_total.labels(
                    validation_type=result.validation_type, tool=result.tool_name
                ).inc()

                self.metrics.validation_issues_found.labels(severity="critical").observe(
                    result.critical_issues
                )
                self.metrics.validation_issues_found.labels(severity="high").observe(
                    result.high_issues
                )
                self.metrics.validation_issues_found.labels(severity="medium").observe(
                    result.medium_issues
                )
                self.metrics.validation_issues_found.labels(severity="low").observe(
                    result.low_issues
                )
        ...
        if self.mcp_client and getattr(context, "mcp_selection_id", None):
            await self._send_mcp_feedback(
                context, validation_success_count, validation_failure_count
            )

    async def _send_mcp_feedback(
        self, context: PipelineContext, success_count: int, failure_count: int
    ):
        """
        ...
        try:
            validation_tools = [
                t
                for t in getattr(context, "selected_tools", [])
            ...
            for tool in validation_tools:
                ...
                if execution_time_ms == 0 and execution_times_by_tool:
                    ...
                    execution_time_ms = int(
                        sum(execution_times_by_tool.values()) / len(execution_times_by_tool)
                    )
                ...
                try:
                    result = await asyncio.wait_for(
                        self.mcp_client.send_tool_feedback(feedback), timeout=3.0
                    )
