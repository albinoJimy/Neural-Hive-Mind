...
class Packager:
        ...
        for artifact in context.generated_artifacts:
            ...
            if self.artifact_registry_client and sbom_uri and sbom_uri.startswith("s3://"):
                ...
                registry_ref = await self.artifact_registry_client.register_sbom(
                    sbom_uri=sbom_uri, artifact_id=artifact.artifact_id, metadata=metadata
                )
                if registry_ref:
            ...
            if self.postgres_client:
                try:
                    ...
                    logger.debug("artifact_metadata_saved", artifact_id=artifact.artifact_id)
        ...
        for attempt in range(max_retries + 1):
            ...
            if attempt < max_retries:
                ...
                new_sbom_uri = await self.sigstore_client.generate_sbom(
                    artifact_path=artifact_path, artifact_id=artifact_id, ticket_id=ticket_id
                )
                if new_sbom_uri and new_sbom_uri.startswith("s3://"):
                    sbom_uri = new_sbom_uri
                else:
                    logger.error("sbom_reupload_failed", artifact_id=artifact_id)
                    return False
            else:
                logger.error(
    ...
    async def _scan_artifact(self, artifact) -> Optional["ValidationResult"]:
        ...
        try:
            ...
            logger.warning("artifact_scan_failed", artifact_id=artifact.artifact_id, error=str(e))
            return None
