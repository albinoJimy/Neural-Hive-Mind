...
class RedisClient:
    ...
    async def _start_standalone(self, host: str, port: int, password: Optional[str]):
        ...
        try:
            self.client = Redis(
                host=host, port=port, password=password, ssl=self.ssl_enabled, decode_responses=True
            )
    ...
    async def _start_cluster(self, host: str, port: int, password: Optional[str]):
        ...
        try:
            ...
            logger.info(
                "redis_client_started", mode="cluster", host=host, port=port, ssl=self.ssl_enabled
            )
        ...
        try:
            ...
            if self.metrics:
                ...
                self.metrics.redis_operations_total.labels(
                    operation="cache_template", status="success"
                ).inc()
                self.metrics.redis_operation_duration_seconds.labels(
            ...
            if self.metrics:
                self.metrics.redis_operations_total.labels(
                    operation="cache_template", status="failure"
                ).inc()
    ...
    async def get_cached_template(self, template_id: str) -> Optional[Any]:
        ...
        try:
            ...
            if self.metrics:
                duration = time.perf_counter() - start_time
                self.metrics.redis_operations_total.labels(
                    operation="get_template", status="success"
                ).inc()
                self.metrics.redis_operation_duration_seconds.labels(
            ...
            if self.metrics:
                self.metrics.redis_operations_total.labels(
                    operation="get_template", status="failure"
                ).inc()
    ...
    async def set_pipeline_state(self, pipeline_id: str, state: dict[str, Any], ttl: int = 86400):
        ...
        try:
            ...
            if self.metrics:
                duration = time.perf_counter() - start_time
                self.metrics.redis_operations_total.labels(
                    operation="set_state", status="success"
                ).inc()
                self.metrics.redis_operation_duration_seconds.labels(operation="set_state").observe(
                    duration
                )
            ...
            if self.metrics:
                self.metrics.redis_operations_total.labels(
                    operation="set_state", status="failure"
                ).inc()
    ...
    async def update_pipeline_state(self, pipeline_id: str, updates: dict[str, Any]):
        ...
        try:
            ...
            logger.debug(
                "pipeline_state_updated", pipeline_id=pipeline_id, fields=list(updates.keys())
            )
    ...
    async def acquire_lock(
        self, resource_id: str, timeout: int = 30, owner: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if self.metrics:
                duration = time.perf_counter() - start_time
                self.metrics.redis_operations_total.labels(
                    operation="acquire_lock", status="success"
                ).inc()
                self.metrics.redis_operation_duration_seconds.labels(
            ...
            if self.metrics:
                self.metrics.redis_operations_total.labels(
                    operation="acquire_lock", status="failure"
                ).inc()
