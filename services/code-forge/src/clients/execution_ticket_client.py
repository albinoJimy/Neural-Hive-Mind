...
class ExecutionTicketClient:
    ...
    async def update_status(
        ...
        try:
            payload = {"status": status.value, "metadata": metadata or {}}
    ...
    async def create_compensation_ticket(self, ticket_id: str, reason: str) -> str:
        ...
        try:
            payload = {"original_ticket_id": ticket_id, "reason": reason}

            response = await self.client.post("/api/v1/tickets/compensation", json=payload)
            response.raise_for_status()
            ...
            logger.error("create_compensation_ticket_failed", ticket_id=ticket_id, error=str(e))
            raise
