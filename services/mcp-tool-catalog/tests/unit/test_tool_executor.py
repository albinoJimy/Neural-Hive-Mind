...
class TestToolExecutorRouting:
    ...
    async def test_routes_to_adapter_when_no_mcp_configured(
        ...
        with patch("src.config.get_settings", return_value=mock_settings):
            ...
            result = await executor.execute_tool(
                tool=cli_tool, execution_params={"verbose": True}, context={"working_dir": "/app"}
            )
    ...
    async def test_routes_to_mcp_when_configured(
        ...
        mock_mcp_response = MCPToolCallResponse(
            content=[MCPContentItem(type="text", text="MCP result")], isError=False
        )
        ...
        with patch("src.config.get_settings", return_value=mock_settings_with_mcp):
            ...
            result = await executor.execute_tool(
                tool=cli_tool, execution_params={"target": "nginx:latest"}, context={}
            )
...
class TestToolExecutorGracefulDegradation:
    ...
    async def test_fallback_to_adapter_on_mcp_failure(
        ...
        mock_adapter_result = ExecutionResult(
            success=True, output="adapter fallback output", execution_time_ms=150.0, exit_code=0
        )
        ...
        with patch("src.config.get_settings", return_value=mock_settings_with_mcp):
            ...
            mock_mcp_client.call_tool = AsyncMock(
                side_effect=MCPError("Connection failed", code=-1)
            )
            executor.mcp_clients["trivy-001"] = mock_mcp_client
            ...
            result = await executor.execute_tool(tool=cli_tool, execution_params={}, context={})
...
class TestToolExecutorMetrics:
    ...
    async def test_updates_tool_registry_feedback(
        ...
        with patch("src.config.get_settings", return_value=mock_settings):
            executor = ToolExecutor(tool_registry=mock_tool_registry, metrics=mock_metrics)
            executor.cli_adapter.execute = AsyncMock(return_value=mock_result)
            executor.cli_adapter.validate_tool_availability = AsyncMock(return_value=True)
    ...
    async def test_execute_tools_batch(self, benchmark_tools, mock_settings, mock_metrics):
        ...
        with patch("src.config.get_settings", return_value=mock_settings):
            ...
            results = await executor.execute_tools_batch(
                tools=benchmark_tools[:3], execution_params={}, context={}  # Usar 3 ferramentas
            )
    ...
    async def test_batch_handles_individual_failures(
        ...
        async def mock_execute(*args, **kwargs):
            ...
            return ExecutionResult(
                success=True, output="success", execution_time_ms=100.0, exit_code=0
            )
        ...
        with patch("src.config.get_settings", return_value=mock_settings):
            ...
            results = await executor.execute_tools_batch(
                tools=benchmark_tools[:3], execution_params={}, context={}
            )
