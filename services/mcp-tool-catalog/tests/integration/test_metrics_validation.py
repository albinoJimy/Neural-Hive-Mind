...
class TestCounters:
    ...
    def test_tool_selections_total_incremented(self, metrics):
        ...
        metrics.record_selection(
            method="genetic_algorithm", cached=False, duration=1.5, fitness=0.85
        )
    ...
    def test_cache_hits_total_incremented(self, metrics):
        ...
        # Cache hit
        metrics.record_selection(method="cache", cached=True, duration=0.01, fitness=0.9)
    ...
    def test_cache_misses_total_incremented(self, metrics):
        ...
        # Cache miss
        metrics.record_selection(
            method="genetic_algorithm", cached=False, duration=2.0, fitness=0.8
        )
    ...
    def test_mcp_fallback_total_incremented(self, metrics):
        ...
        metrics.record_mcp_fallback(tool_id="trivy-001", reason="circuit_breaker")
    ...
    def test_execution_route_total_incremented(self, metrics):
        ...
        # Execucao via MCP
        metrics.record_tool_execution(
            tool_id="tool-1", category="TEST", status="success", duration=1.0, execution_route="mcp"
        )
        ...
        mcp_count = metrics.execution_route_total._metrics[("mcp", "success")]._value.get()
        adapter_count = metrics.execution_route_total._metrics[("adapter", "success")]._value.get()
...
class TestHistograms:
    ...
    def test_tool_selection_duration_recorded(self, metrics):
        ...
        metrics.record_selection(
            method="genetic_algorithm", cached=False, duration=2.5, fitness=0.85
        )
    ...
    def test_tool_execution_duration_recorded(self, metrics):
        ...
        histogram_sum = metrics.tool_execution_duration_seconds._metrics[("pytest-001",)]._sum.get()
    ...
    def test_genetic_algorithm_duration_recorded(self, metrics):
        ...
        metrics.record_genetic_algorithm(
            converged=True, timeout=False, duration=4.2, generations=50
        )
    ...
    def test_fitness_score_recorded(self, metrics):
        ...
        metrics.record_selection(
            method="genetic_algorithm", cached=False, duration=1.0, fitness=0.92
        )
...
class TestGauges:
    ...
    def test_registered_tools_total_gauge(self, metrics):
        ...
        metrics.update_tool_registry(category="SECURITY", total=10, healthy=8)

        total = metrics.registered_tools_total._metrics[("SECURITY",)]._value.get()
        assert total == 10
    ...
    def test_healthy_tools_total_gauge(self, metrics):
        ...
        healthy = metrics.healthy_tools_total._metrics[("SECURITY",)]._value.get()
        assert healthy == 8

    def test_mcp_clients_connected_gauge(self, metrics):
        """Verifica gauge de clientes MCP conectados."""
        metrics.update_mcp_clients_status(connected=3, circuit_breakers_open=1)
    ...
    def test_mcp_circuit_breakers_open_gauge(self, metrics):
        ...
        metrics.update_mcp_clients_status(connected=3, circuit_breakers_open=2)
    ...
    def test_genetic_algorithm_generations_gauge(self, metrics):
        ...
        metrics.record_genetic_algorithm(
            converged=True, timeout=False, duration=3.0, generations=75
        )
...
class TestLabels:
    ...
    def test_metrics_include_mcp_server_label(self, metrics):
        ...
        assert (
            ...
            "http://trivy-mcp:3000",
            "success",
        ) in metrics.mcp_server_executions_total._metrics
...
class TestIntegratedScenarios:
    ...
    def test_complete_mcp_execution_flow_metrics(self, metrics):
        ...
        metrics.record_selection(
            method="genetic_algorithm", cached=False, duration=2.0, fitness=0.88
        )
        ...
        # Verificacoes
        assert (
            metrics.tool_selections_total._metrics[("genetic_algorithm", "False")]._value.get() >= 1
        )
        ...
        assert metrics.tool_feedback_total._metrics[("trivy-001", "True")]._value.get() >= 1
    ...
    def test_complete_fallback_flow_metrics(self, metrics):
        ...
        # Verificacoes
        assert (
            metrics.mcp_fallback_total._metrics[("trivy-001", "circuit_breaker")]._value.get() >= 1
        )

        assert (
            metrics.adapter_executions_total._metrics[
    ...
    def test_multiple_tool_categories_metrics(self, metrics):
        ...
        for category, total, healthy in categories:
            metrics.update_tool_registry(category=category, total=total, healthy=healthy)

        # Verificar cada categoria
        for category, total, healthy in categories:
            reg_total = metrics.registered_tools_total._metrics[(category,)]._value.get()
            reg_healthy = metrics.healthy_tools_total._metrics[(category,)]._value.get()
