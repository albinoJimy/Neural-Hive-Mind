...
class ArchitectureAnalysisTestHelper:
    ...
    def analyze_solid_principles(tasks: List[Dict], cognitive_plan: Dict) -> float:
        ...
        solid_keywords = [
            'single responsibility', 'srp', 'open closed', 'ocp', 'extensible',
            'liskov', 'lsp', 'substitution', 'interface segregation', 'isp',
            'dependency inversion', 'dip', 'dependency injection', 'abstraction'
        ]

        violation_keywords = [
            'god class', 'multiple responsibilities', 'tight coupling',
            'hardcoded dependencies', 'fat interface', 'concrete dependency'
        ]
    ...
    def analyze_modularity(tasks: List[Dict]) -> float:
        ...
        modularity_keywords = [
            'module', 'component', 'layer', 'package', 'encapsulation',
            'namespace', 'boundary', 'api', 'contract', 'interface'
        ]
...
class TestCouplingCohesionAnalysis:
    ...
    def test_coupling_high_with_many_dependencies(self):
        tasks = [
            {'description': 'Task 1', 'dependencies': ['task-2', 'task-3', 'task-4'], 'agent_id': 'service-a'},
...
class TestSeparationOfConcernsAnalysis:
    ...
    def test_separation_with_mixed_concerns(self):
        tasks = [
            {'description': 'Handle UI view and business logic and database'}
        ]
