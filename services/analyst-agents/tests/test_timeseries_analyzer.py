...
async def test_detect_anomalies_async_zscore(timeseries_analyzer):
    ...
    data = [
        (datetime.now(UTC) - timedelta(minutes=i), 50.0 + np.random.randn() * 2) for i in range(20)
    ]
    # Adicionar anomalia
    data[10] = (data[10][0], 95.0)
...
async def test_detect_anomalies_async_iqr(timeseries_analyzer):
    ...
    np.random.seed(42)
    data = [
        (datetime.now(UTC) - timedelta(minutes=i), 50.0 + np.random.randn() * 3) for i in range(20)
    ]
...
async def test_anomaly_summary_severity(timeseries_analyzer):
    ...
    data = [(datetime.now(UTC) - timedelta(minutes=i), 50.0) for i in range(30)]
