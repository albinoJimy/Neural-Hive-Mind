...
class AnalystServicer(analyst_agent_pb2_grpc.AnalystAgentServiceServicer):
        ...
        with tracer.start_as_current_span("get_insight") as span:
            try:
                ...
                if cached_insight:
                    ...
                    return analyst_agent_pb2.GetInsightResponse(insight=proto_insight, found=True)
            ...
            try:
                ...
                if request.use_graph_enrichment and request.related_entity_id and self.neo4j_client:
                    ...
                    if related_insight_ids:
                        ...
                        logger.info(
                            "neo4j_graph_enrichment_applied", related_count=len(related_insight_ids)
                        )
                ...
                logger.info(
                    "grpc_query_insights_success", page_count=page_count, total_count=total_count
                )

                return analyst_agent_pb2.QueryInsightsResponse(
                    insights=proto_insights, total_count=total_count
                )
        ...
        with tracer.start_as_current_span("execute_analysis") as span:
            try:
                ...
                if analysis_type == "anomaly_detection":
                        ...
                        if hasattr(self.query_engine, "get_metric_timeseries"):
                            ...
                            if data:
                                anomalies = self.analytics_engine.detect_anomalies(
                                    metric_name=metric_name, values=data, threshold=threshold
                                )
                                results = {
                        ...
                        if hasattr(self.query_engine, "get_metric_timeseries"):
                            ...
                            if data:
                                trend = self.analytics_engine.calculate_trend(
                                    metric_name=metric_name, values=data
                                )
                                results = {
                    ...
                    span.set_status(
                        Status(StatusCode.ERROR, f"unknown analysis_type: {analysis_type}")
                    )
                    return analyst_agent_pb2.ExecuteAnalysisResponse()
                ...
                logger.info(
                    "grpc_execute_analysis_success",
                    analysis_type=analysis_type,
                    analysis_id=analysis_id,
                )

                return analyst_agent_pb2.ExecuteAnalysisResponse(
                    analysis_id=analysis_id, results=results, confidence=confidence
                )
    ...
    async def HealthCheck(self, request, context):
        ...
        with tracer.start_as_current_span("health_check") as span:
            try:
                ...
                return analyst_agent_pb2.HealthCheckResponse(healthy=all_healthy, status=status)

            except Exception as e:
                logger.error("grpc_health_check_failed", error=str(e))
                span.record_exception(e)
                span.set_status(Status(StatusCode.ERROR, str(e)))
                return analyst_agent_pb2.HealthCheckResponse(healthy=False, status="UNHEALTHY")
            ...
            try:
                ...
                try:
                    ...
                    return analyst_agent_pb2.GenerateInsightResponse(
                        success=False, error_message=f"Invalid insight_type: {request.insight_type}"
                    )
                ...
                insight = await self.insight_generator.generate_insight(
                    data=data, insight_type=insight_type
                )
                ...
                insight_dict["insight_type"] = (
                    insight_dict["insight_type"].value
                    if hasattr(insight_dict["insight_type"], "value")
                    else str(insight_dict["insight_type"])
                )
                insight_dict["priority"] = (
                    insight_dict["priority"].value
                    if hasattr(insight_dict["priority"], "value")
                    else str(insight_dict["priority"])
                )
                ...
                return analyst_agent_pb2.GenerateInsightResponse(
                    insight=proto_insight, success=True
                )
    ...
    async def _persist_insight_to_neo4j(self, insight):
        ...
        RETURN i
        """
        await self.neo4j_client.query_patterns(
            create_insight_query,
            {
        ...
        for r in insight_dict.get("recommendations", []):
            if isinstance(r, dict):
                recommendations.append(
                    analyst_agent_pb2.Recommendation(
                        action=r.get("action", ""),
