...
async def list_insights(
    ...
    try:
        ...
        return {"insights": results, "count": len(results), "limit": limit, "offset": offset}
...
async def get_insights_by_entity(
    entity_type: str, entity_id: str, request: Request, limit: int = 100
):
...
async def get_insights_by_tags(request: Request, tags: str = Query(...), limit: int = 100):
    ...
    try:
        ...
        return {"insights": results, "count": len(results), "tags": tag_list}
    ...
    try:
        ...
        if query_request.query_type == QueryType.BY_ID and query_request.insight_id:
            ...
            results = await mongodb_client.get_insights_by_type(
                query_request.insight_type, limit=query_request.limit
            )
            ...
            results = await mongodb_client.get_insights_by_priority(
                query_request.priority, limit=query_request.limit
            )
            ...
            results = await mongodb_client.get_insights_by_entity(
                query_request.entity_type, query_request.entity_id, limit=query_request.limit
            )

        elif query_request.query_type == QueryType.BY_TAG and query_request.tags:
            results = await mongodb_client.get_insights_by_tags(
                query_request.tags, limit=query_request.limit
            )
        ...
        return InsightQueryResponse(
            insights=results, total_count=len(results), query_time_ms=query_time, cached=False
        )
