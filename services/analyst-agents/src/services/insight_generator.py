...
class InsightGenerator:
    ...
    def generate_recommendations(
        ...
        if insight_type == InsightType.ANOMALY:
            ...
            recommendations.append(
                Recommendation(
                    action="Otimizar recursos alocados", priority="MEDIUM", estimated_impact=0.7
                )
            )
    ...
    def _find_related_entities(self, data: Dict) -> List[RelatedEntity]:
        ...
        if "intent_id" in data:
            entities.append(
                RelatedEntity(
                    entity_type="intent", entity_id=data["intent_id"], relationship="source"
                )
            )
        ...
        if "plan_id" in data:
            entities.append(
                RelatedEntity(
                    entity_type="plan", entity_id=data["plan_id"], relationship="derived_from"
                )
            )
