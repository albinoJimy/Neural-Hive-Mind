...
class QueenAgentGrpcClient:
    ...
    async def initialize(self):
        ...
        if spiffe_x509_enabled:
            try:
                ...
                logger.warning("queen_agent_falling_back_to_insecure", endpoint=endpoint)
        ...
        self.channel = instrument_grpc_channel(
            self.channel, service_name="analyst-agents", target_service="queen-agent"
        )
    ...
    async def close(self):
        ...
        if self.spiffe_manager:
            try:
                ...
                logger.warning("queen_agent_spiffe_manager_close_error", error=str(e))
    ...
    async def send_strategic_insight(self, insight: AnalystInsight) -> bool:
        ...
        request = queen_agent_pb2.SubmitInsightRequest(
            ...
            recommendations=[
                queen_agent_pb2.Recommendation(
                    action=rec.action, priority=rec.priority, estimated_impact=rec.estimated_impact
                )
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                response = await self.stub.SubmitInsight(request, metadata=metadata, timeout=10.0)

                if response.accepted:
                    logger.info(
                        "strategic_insight_sent",
                        insight_id=insight.insight_id,
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            if status:
                return {"status": "SERVING", "details": status}
            return {"status": "NOT_SERVING", "error": "Falha ao obter status do sistema"}
        except Exception as e:
