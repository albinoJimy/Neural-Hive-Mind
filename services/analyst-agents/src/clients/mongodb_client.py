...
class MongoDBClient:
    ...
    async def initialize(self):
        ...
        try:
            self.client = AsyncIOMotorClient(
                self.uri, maxPoolSize=self.max_pool_size, minPoolSize=self.min_pool_size
            )
    ...
    async def _create_indexes(self):
        ...
        try:
            ...
            await self.collection.create_index(
                [("insight_type", 1), ("priority", 1), ("created_at", -1)]
            )
    ...
    async def update_insight(self, insight_id: str, updates: dict) -> bool:
        """Atualizar insight"""
        try:
            result = await self.collection.update_one({"insight_id": insight_id}, {"$set": updates})
    ...
    async def delete_expired_insights(self) -> int:
        ...
        try:
            ...
            result = await self.collection.delete_many({"valid_until": {"$ne": None, "$lt": now}})
    ...
    async def get_insight_statistics(self, time_filter: Optional[Dict] = None) -> Dict:
        ...
        try:
            ...
            pipeline = [
                ...
                {
                    "$facet": {
                        "by_type": [{"$group": {"_id": "$insight_type", "count": {"$sum": 1}}}],
            ...
            by_type = {
                item["_id"]: item["count"] for item in result.get("by_type", []) if item["_id"]
            }
            by_priority = {
                item["_id"]: item["count"] for item in result.get("by_priority", []) if item["_id"]
            }
