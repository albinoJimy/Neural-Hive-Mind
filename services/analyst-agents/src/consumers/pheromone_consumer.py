...
class PheromoneConsumer:
    ...
    async def process_pheromone_signal(self, message: dict):
        ...
        try:
            ...
            message.get("specialist_type", "")

            logger.debug(
                "pheromone_signal_received", type=pheromone_type, domain=domain, strength=strength
            )
            ...
            if stats["count"] >= 10:  # Mínimo de 10 sinais
                ...
                if failure_rate > 0.5:  # 50% threshold
                    await self._generate_high_failure_rate_insight(
                        domain, pheromone_type, failure_rate, stats
                    )
    ...
    async def _generate_high_failure_rate_insight(
        self, domain: str, pheromone_type: str, failure_rate: float, stats: dict
    ):
        ...
        try:
            ...
            insight_data = {
                "title": "Alta taxa de falha em trilha de feromônios",
            ...
            logger.info(
                "pheromone_failure_insight_generated", insight_id=insight.insight_id, domain=domain
            )
