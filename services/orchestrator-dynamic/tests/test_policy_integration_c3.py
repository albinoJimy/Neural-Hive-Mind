...
def policy_result_warning():
    return ValidationResult(
        valid=True,
        warnings=[
            PolicyWarning(policy_name="resource_limits", rule="soft_limit", message="near limit")
        ],
...
async def test_c3_validation_violation_blocks_allocation(
    ...
    validator = AsyncMock()
    validator.validate_resource_allocation.return_value = ValidationResult(
...
async def test_c3_validation_fail_open_on_opa_error(
    ...
    set_activity_dependencies(
        None, None, None, mock_scheduler, validator, mock_config_fail_open, None
    )
    ticket = await allocate_resources(dict(sample_ticket))
    _reset_dependencies()
...
async def test_c3_validation_metrics_recorded(sample_ticket, mock_scheduler, mock_config):
    ...
    validator = AsyncMock()
    validator.validate_resource_allocation.return_value = ValidationResult(
        valid=True, policy_decisions=decisions
    )
