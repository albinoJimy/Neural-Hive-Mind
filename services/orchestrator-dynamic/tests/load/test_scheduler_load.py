...
class TestSchedulerThroughput:
    ...
    async def test_throughput_sequential_100_tickets(
        ...
        for ticket in tickets:
            ...
            if (
                result.get("allocation_metadata", {}).get("allocation_method")
        ...
        ), f"Throughput mínimo 10 t/s, obtido: {metrics.throughput:.2f}"
        assert (
            metrics.latency_p95 < 200
        ), f"Latência P95 deve ser < 200ms, obtido: {metrics.latency_p95:.2f}ms"
    ...
    async def test_throughput_concurrent_50_tickets(
        ...
        assert (
            metrics.latency_p95 < 200
        ), f"Latência P95 < 200ms, obtido: {metrics.latency_p95:.2f}ms"
    ...
    async def test_throughput_concurrent_200_tickets(
        ...
        assert (
            metrics.latency_p95 < 300
        ), f"Latência P95 < 300ms, obtido: {metrics.latency_p95:.2f}ms"
...
class TestSchedulerRiskBandPrioritization:
    ...
    async def test_priority_ordering_under_load(
        ...
        # Calcular médias
        avg_scores = {risk: statistics.mean(scores) for risk, scores in priority_scores.items()}
        ...
        assert (
            avg_scores["high"] > avg_scores["normal"]
...
class TestSchedulerConcurrencySafety:
    ...
    async def test_concurrent_cache_updates(
        ...
        success_count = sum(
            1 for r in results if r.get("allocation_metadata", {}).get("agent_id") is not None
        )
