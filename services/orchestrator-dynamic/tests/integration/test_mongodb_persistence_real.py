...
pytestmark = [
    ...
    pytest.mark.skipif(not TESTCONTAINERS_AVAILABLE, reason="testcontainers-python não instalado"),
]
...
def sample_ticket() -> dict:
    ...
    return {
        ...
        "metadata": {"workflow_id": "test-workflow-1", "generated_by": "test-suite"},
        "schema_version": 1,
    }
...
async def test_ticket_id_index_is_unique(mongodb_client_real):
    ...
    ticket_id_index = next((idx for idx in indexes if idx["name"] == "ticket_id_1"), None)
