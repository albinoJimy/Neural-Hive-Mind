...
async def test_record_allocation_outcome_for_ticket_success(
    ...
    await result_consolidation.record_allocation_outcome_for_ticket(
        sample_ticket_completed, mock_scheduling_optimizer
    )
...
async def test_record_allocation_outcome_missing_data(mock_scheduling_optimizer):
    ...
    await result_consolidation.record_allocation_outcome_for_ticket(
        ticket_without_metadata, mock_scheduling_optimizer
    )
...
async def test_consolidate_results_calls_feedback_loop(
    ...
    tickets = [{"ticket": sample_ticket_completed}, {"ticket": pending_ticket}]
    ...
    sla_monitor_instance.check_workflow_sla = AsyncMock(
        return_value={
    ...
    sla_monitor_instance.check_budget_threshold = AsyncMock(
        return_value=(False, {"status": "HEALTHY", "error_budget_remaining": 0})
    )
...
def test_compute_and_record_ml_error_records_accuracy(mock_metrics):
    ...
    mock_metrics.record_ml_prediction_accuracy.assert_called_once_with(
        model_type="duration", predicted_ms=50000, actual_ms=45000
    )
