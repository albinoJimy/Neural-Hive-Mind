    ...
    with patch("src.activities.result_consolidation.get_settings", return_value=mock_settings):
        with patch("src.activities.result_consolidation.get_metrics") as mock_get_metrics:
            ...
            with patch("src.activities.result_consolidation.get_redis_client") as mock_get_redis:
                ...
                # Mock Kafka producer
                with patch(
                    "src.activities.result_consolidation.KafkaProducerClient"
                ) as MockKafkaProducer:
                    ...
                    with patch("src.activities.result_consolidation.SLAMonitor") as MockSLAMonitor:
                        ...
                        with patch(
                            "src.activities.result_consolidation.AlertManager"
                        ) as MockAlertManager:
                            mock_monitor = AsyncMock()
                            mock_monitor.initialize = AsyncMock()
                            mock_monitor.close = AsyncMock()
                            mock_monitor.check_workflow_sla = AsyncMock(
                                return_value={
                            ...
                            mock_monitor.check_budget_threshold = AsyncMock(
                            ...
                            result = await consolidate_results(
                                tickets_deadline_approaching, "workflow-1"
                            )
                            ...
                            mock_monitor.check_workflow_sla.assert_called_once_with(
                                "workflow-1", tickets_deadline_approaching
                            )
    ...
    with patch("src.activities.result_consolidation.get_settings", return_value=mock_settings):
        with patch("src.activities.result_consolidation.get_metrics") as mock_get_metrics:
            ...
            with patch("src.activities.result_consolidation.SLAMonitor") as MockSLAMonitor:
                ...
                mock_monitor.check_budget_threshold = AsyncMock(
                    return_value=(False, {"error_budget_remaining": 0.75, "status": "HEALTHY"})
                )
                MockSLAMonitor.return_value = mock_monitor
...
async def test_publish_telemetry_with_sla_metrics(tickets_healthy):
    ...
    workflow_result = {
        ...
        "plan_id": "plan-5",
        "status": "SUCCESS",
        "metrics": {"total_tickets": 1, "successful_tickets": 1},
        "tickets_summary": [{"ticket_id": "ticket-3", "status": "COMPLETED"}],
        "sla_status": {
            "deadline_approaching": False,
...
async def test_multiple_tickets_with_varying_durations(
    ...
    with patch("src.activities.ticket_generation.activity") as mock_activity:
        ...
        set_activity_dependencies(
            kafka_producer=None, mongodb_client=None, config=mock_config_integration
        )
...
async def test_timeout_with_config_defaults_fallback(consolidated_decision):
    ...
    with patch("src.activities.ticket_generation.activity") as mock_activity:
        ...
        set_activity_dependencies(kafka_producer=None, mongodb_client=None, config=None)
