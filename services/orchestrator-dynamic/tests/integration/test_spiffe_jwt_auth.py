...
async def test_jwt_svid_fetch_and_attach_metadata(mock_spiffe_manager, mock_settings, caplog):
    ...
    with patch("src.clients.service_registry_client.service_registry_pb2") as mock_pb2, patch(
        ...
        await client.discover_agents(
            capabilities=["code_generation"], filters={"namespace": "production"}, max_results=5
        )
        ...
        assert (
            "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9" in auth_header
        ), "Token JWT deve estar presente"

        # Verificar log de debug
        assert any(
            "jwt_svid_attached" in record.msg for record in caplog.records if hasattr(record, "msg")
        )
...
async def test_jwt_svid_cache_hit(mock_spiffe_manager, mock_settings):
    ...
    with patch("src.clients.service_registry_client.service_registry_pb2") as mock_pb2, patch(
        ...
        await client.discover_agents(capabilities=["testing"], filters={}, max_results=5)
...
async def test_fallback_no_auth_when_spiffe_disabled(mock_settings):
    ...
    with patch("src.clients.service_registry_client.service_registry_pb2") as mock_pb2, patch(
        ...
        assert (
            metadata is None or len(metadata) == 0
...
async def test_spiffe_unavailable_fallback(mock_spiffe_manager, mock_settings, caplog):
    ...
    mock_spiffe_manager.fetch_jwt_svid.side_effect = SPIFFEConnectionError(
    ...
    with patch("src.clients.service_registry_client.service_registry_pb2") as mock_pb2, patch(
        ...
        assert any(
            "jwt_svid_fetch_failed" in record.msg
