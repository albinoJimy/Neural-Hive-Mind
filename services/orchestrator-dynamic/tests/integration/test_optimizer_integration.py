...
def mock_resource_allocator():
    ...
    allocator.select_best_worker = Mock(
        return_value={"agent_id": "worker-1", "agent_type": "worker-agent", "score": 0.85}
    )
    return allocator
...
class TestOptimizerGrpcClient:
    ...
    async def test_get_load_forecast_timeout(self, mock_config):
        ...
        mock_stub = AsyncMock()
        mock_stub.GetLoadForecast = AsyncMock(side_effect=grpc.RpcError())
        client.stub = mock_stub
    ...
    async def test_get_scheduling_recommendation_grpc_error(self, mock_config):
        ...
        mock_stub = AsyncMock()
        mock_stub.GetSchedulingRecommendation = AsyncMock(side_effect=grpc.RpcError())
        client.stub = mock_stub
...
class TestIntelligentSchedulerOptimizerIntegration:
    ...
    async def test_schedule_ticket_with_ml_predictions(
        ...
        scheduler = IntelligentScheduler(
            mock_config, mock_metrics, mock_priority_calculator, mock_resource_allocator
        )
    ...
    async def test_schedule_ticket_invalid_ml_predictions_rejection(
        ...
        invalid_ticket = {
            ...
            "qos": {},
            "sla": {},
            "predictions": {"duration_ms": -1000, "anomaly": {"is_anomaly": True}},  # Inválido
        }
    ...
    async def test_schedule_ticket_metadata_enrichment(
        ...
        ticket_with_anomaly = {
            **sample_ticket,
            "predictions": {"duration_ms": 180000, "anomaly": {"is_anomaly": True, "score": 0.95}},
        }
    ...
    async def test_full_flow_with_forecast_and_recommendation(
        ...
        mock_forecast_response.forecast = [
            Mock(
                timestamp="2024-01-01T12:00:00Z",
                ticket_count=150 + i,
                resource_demand=Mock(cpu_cores=15, memory_mb=15000),
            )
            for i in range(60)
        ]
