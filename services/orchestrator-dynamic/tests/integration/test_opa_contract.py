...
class TestResourceLimitsContract:
    ...
    def test_output_schema_validation(self):
        ...
        success_output = {"result": {"allow": True, "violations": [], "warnings": []}}

        is_valid, missing = validate_output_schema(
            success_output, RESOURCE_LIMITS_INPUT_SCHEMA["output_fields"]
        )
        ...
        is_valid, missing = validate_output_schema(
            failure_output, RESOURCE_LIMITS_INPUT_SCHEMA["output_fields"]
        )
        assert is_valid, f"Campos ausentes: {missing}"
    ...
    def test_edge_case_empty_capabilities(self):
        ...
        input_data = {
            "input": {
                ...
                "parameters": {"allowed_capabilities": [], "max_concurrent_tickets": 100},
...
class TestSLAEnforcementContract:
    ...
    def test_output_schema_with_warnings(self):
        ...
        is_valid, missing = validate_output_schema(
            output_with_warnings, SLA_ENFORCEMENT_INPUT_SCHEMA["output_fields"]
        )
        assert is_valid, f"Campos ausentes: {missing}"
    ...
    def test_qos_modes_valid_values(self):
        ...
        for mode in valid_delivery_modes:
            input_data = {
                "input": {"resource": {"qos": {"delivery_mode": mode, "consistency": "STRONG"}}}
            }
...
class TestFeatureFlagsContract:
    ...
    def test_input_schema_minimal(self):
        ...
        minimal_input = {
            "input": {
                "resource": {"ticket_id": "ff-001", "risk_band": "critical"},
                "flags": {
    ...
    def test_output_schema_all_flags(self):
        ...
        is_valid, missing = validate_output_schema(
            expected_output, FEATURE_FLAGS_INPUT_SCHEMA["output_fields"]
        )
...
class TestSecurityConstraintsContract:
    ...
    def test_output_schema_with_security_context(self):
        ...
        is_valid, missing = validate_output_schema(
            output_with_context, SECURITY_CONSTRAINTS_INPUT_SCHEMA["output_fields"]
        )
        assert is_valid, f"Campos ausentes: {missing}"
