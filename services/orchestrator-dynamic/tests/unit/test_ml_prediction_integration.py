...
def mock_resource_allocator():
    ...
    allocator.select_best_worker = AsyncMock(
        return_value={
            "agent_id": "agent-1",
            "agent_type": "worker-agent",
            "score": 0.8,
...
def mock_scheduling_predictor():
    ...
    predictor.predict_resources = AsyncMock(
        return_value={"cpu_cores": 1.5, "memory_mb": 256, "confidence": 0.7}
    )
    return predictor
...
async def test_priority_boost_by_duration_ratio(
    ...
    mock_scheduling_predictor.predict_duration = AsyncMock(
        return_value={
    ...
    mock_scheduling_predictor.predict_resources = AsyncMock(
        return_value={"cpu_cores": 1.0, "memory_mb": 256, "confidence": 0.5}
    )
