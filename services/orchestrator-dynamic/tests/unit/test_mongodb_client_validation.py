...
async def test_save_ticket_retries_on_transient_error(mock_config_fail_closed):
    ...
    client.execution_tickets.insert_one = AsyncMock(side_effect=[PyMongoError("Transient"), None])
...
async def test_save_ticket_no_retry_on_duplicate_key(mock_config_fail_closed):
    ...
    client.execution_tickets.insert_one = AsyncMock(side_effect=DuplicateKeyError("Duplicate key"))
...
async def test_validate_indexes_records_metrics():
    ...
    mock_collection.list_indexes.return_value.to_list = AsyncMock(
        return_value=[
            {"name": "_id_"},
            {"name": "ticket_id_1"},
            {"name": "plan_id_1"},
