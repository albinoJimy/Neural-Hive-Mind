...
class TestAuthorizationAuditLogging:
    ...
    async def test_authorization_audit_logged_on_deny(
        ...
        with patch("src.policies.opa_client.get_metrics", return_value=mock_metrics):
            ...
            violations = [{"severity": "critical", "msg": "Tenant nao autorizado"}]
    ...
    async def test_authorization_audit_logged_on_violations_without_allow_field(
        ...
        with patch("src.policies.opa_client.get_metrics", return_value=mock_metrics):
            ...
            input_data = {"input": {"resource": {"tenant_id": "tenant-123"}, "context": {}}}

            violations = [{"severity": "warning", "msg": "Limite de recursos excedido"}]
    ...
    async def test_authorization_audit_fail_open(
        ...
        with patch("src.policies.opa_client.get_metrics", return_value=mock_metrics):
            ...
            with patch.object(opa_client, "_evaluate_policy_internal") as mock_eval:
                mock_eval.return_value = {"result": {"allow": True}, "policy_path": policy_path}
