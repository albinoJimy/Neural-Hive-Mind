...
async def test_send_proactive_alert_deadline_approaching(
    ...
    result = await manager.send_proactive_alert(
        "DEADLINE_APPROACHING", deadline_approaching_context
    )
...
async def test_publish_sla_violation_with_defaults(mock_config, mock_kafka_producer, mock_metrics):
    ...
    violation = {"workflow_id": "orch-111", "ticket_id": "ticket-222", "violation_type": "TIMEOUT"}
