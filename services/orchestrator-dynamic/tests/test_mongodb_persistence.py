...
async def test_create_indexes(client):
    ...
    assert any(
        call.args[0] == "workflow_id" and call.kwargs.get("unique") for call in workflow_index_calls
    )
...
async def test_save_execution_ticket_records_error_metrics(client):
    ...
    ticket = {"ticket_id": "ticket-error-1", "plan_id": "plan-1", "status": "PENDING"}
    ...
    with patch("src.clients.mongodb_client.get_metrics") as mock_metrics:
        ...
        mock_metrics_instance.record_mongodb_persistence_error.assert_called_once_with(
            "execution_tickets", "insert", "RetryError"
        )
...
async def test_save_execution_ticket_propagates_critical_errors(client):
    ...
    client.execution_tickets.insert_one.side_effect = ServerSelectionTimeoutError(
        "MongoDB unreachable"
    )

    ticket = {"ticket_id": "ticket-critical-1", "plan_id": "plan-1", "status": "PENDING"}
