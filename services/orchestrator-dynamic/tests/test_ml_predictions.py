...
class TestDurationPredictor:
    ...
    async def test_predict_duration_without_model(
        self, mock_config, mock_mongodb, mock_model_registry, mock_metrics, sample_ticket
    ):
    ...
    async def test_predict_duration_with_model(
        self, mock_config, mock_mongodb, mock_model_registry, mock_metrics, sample_ticket
    ):
...
class TestAnomalyDetector:
    ...
    async def test_detect_anomaly_normal_ticket(
        self, mock_config, mock_mongodb, mock_model_registry, mock_metrics, sample_ticket
    ):
    ...
    async def test_detect_anomaly_anomalous_ticket(
        self, mock_config, mock_mongodb, mock_model_registry, mock_metrics
    ):
...
class TestMLPredictor:
    ...
    async def test_predict_and_enrich(
        self, mock_config, mock_mongodb, mock_model_registry, mock_metrics, sample_ticket
    ):
        ...
        # Mock dos predictors
        predictor.duration_predictor.predict_duration = AsyncMock(
            return_value={"duration_ms": 75000.0, "confidence": 0.85}
        )
        predictor.anomaly_detector.detect_anomaly = AsyncMock(
            return_value={
                "is_anomaly": False,
                "anomaly_score": 0.2,
                "anomaly_type": None,
    ...
    def test_estimate_resources(self, mock_config, mock_mongodb, mock_model_registry, mock_metrics):
        ...
        resources = predictor._estimate_resources(
            duration_ms=120000.0, task_type="BUILD", risk_band="high"
        )
...
class TestTrainingPipeline:
    ...
    async def test_query_training_data(
        self, mock_config, mock_mongodb, mock_model_registry, mock_metrics
    ):
        ...
        mock_mongodb.db["execution_tickets"].find = Mock(
            return_value=Mock(to_list=AsyncMock(return_value=mock_tickets))
        )
    ...
    async def test_run_training_cycle_insufficient_data(
        self, mock_config, mock_mongodb, mock_model_registry, mock_metrics
    ):
