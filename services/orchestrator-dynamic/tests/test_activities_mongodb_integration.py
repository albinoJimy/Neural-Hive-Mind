...
async def test_audit_validation_calls_mongodb(monkeypatch):
    ...
    plan_validation.set_activity_dependencies(
        policy_validator=None, config=None, mongodb_client=mock_client
    )
    ...
    mock_client.save_validation_audit.assert_awaited_once_with(
        "plan-100", {"valid": True}, "wf-100"
    )
...
async def test_audit_validation_without_mongodb_client(monkeypatch):
    ...
    plan_validation.set_activity_dependencies(
        policy_validator=None, config=None, mongodb_client=None
    )
...
async def test_consolidate_results_saves_workflow_result(minimal_config):
    ...
    result_consolidation.set_activity_dependencies(
        scheduling_optimizer=None, config=minimal_config, mongodb_client=mock_client
    )
...
def mock_kafka_producer():
    ...
    producer = AsyncMock()
    producer.publish_ticket = AsyncMock(
        return_value={
...
def valid_ticket():
    ...
    return {
        ...
        "intent_id": "intent-1",
        "status": "PENDING",
        "allocation_metadata": {"agent_id": "worker-1", "allocated_at": 1700000000},
        "metadata": {},
    }
