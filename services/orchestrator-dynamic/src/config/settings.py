...
class OrchestratorSettings(BaseSettings):
    ...
    temporal_port: int = Field(
        default=7233, description="Porta do Temporal Server (ignorada se host incluir porta)"
    )
    temporal_namespace: str = Field(default="default", description="Namespace Temporal")
    temporal_task_queue: str = Field(
        default="orchestration-tasks", description="Fila de tarefas Temporal"
    )
    temporal_workflow_id_prefix: str = Field(
        default="orch-", description="Prefixo para workflow IDs"
    )
    ...
    kafka_consumer_group_id: str = Field(
        default="orchestrator-dynamic", description="Group ID do consumer"
    )
    ...
    kafka_tickets_topic: str = Field(
        default="execution.tickets", description="Tópico de tickets de execução"
    )
    kafka_saga_events_topic: str = Field(
        default="saga.events", description="Tópico de eventos de Saga"
    )
    ...
    schemas_base_path: str = Field(
        default="/app/schemas", description="Diretório base para schemas Avro"
    )

    # Kafka Consumer (execution.results) - GAP-02
    execution_result_consumer_enabled: bool = Field(
        default=True, description="Habilitar consumer de execution.results"
    )
    ...
    execution_result_workers: int = Field(
        default=1, description="Número de workers para consumer de execution.results"
    )
    ...
    self_healing_tls_verify: bool = Field(
        default=True, description="Verificar certificado TLS do Self-Healing Engine"
    )
    self_healing_ca_bundle: str | None = Field(
        default=None, description="Caminho para CA bundle do Self-Healing Engine"
    )
    self_healing_enabled: bool = Field(
        default=True, description="Flag para habilitar acionamento de autocura"
    )
    self_healing_timeout_seconds: int = Field(
        default=30, description="Timeout para chamadas HTTP ao Self-Healing Engine"
    )
    ...
    mongodb_database: str = Field(
        default="neural_hive_orchestration", description="Database MongoDB"
    )
    mongodb_collection_tickets: str = Field(
        default="execution_tickets", description="Collection de tickets"
    )
    mongodb_collection_workflows: str = Field(
        default="workflows", description="Collection de workflows"
    )
    ...
    redis_password: str | None = Field(
        default=None, description="Senha Redis. ⚠️ Obrigatório em produção (validação automática)."
    )
    ...
    def validate_redis_password_in_production(cls, v: str | None, info) -> str | None:
        ...
        if environment in ["production", "prod"] and (v is None or v == ""):
            raise ValueError(
    ...
    service_registry_timeout_seconds: int = Field(
        default=3, description="Timeout para chamadas gRPC"
    )
    ...
    service_registry_cache_ttl_seconds: int = Field(
        default=10, description="TTL do cache de descobertas"
    )

    # Scheduler
    enable_intelligent_scheduler: bool = Field(
        default=True, description="Habilitar scheduler inteligente"
    )
    ...
    scheduler_max_parallel_tickets: int = Field(
        default=100, description="Máximo de tickets paralelos"
    )
    ...
    scheduler_enable_affinity: bool = Field(
        default=True, description="Habilitar affinity/anti-affinity para alocação de tickets"
    )
    scheduler_affinity_plan_weight: float = Field(
        default=0.6, description="Peso de plan affinity no score (data locality)"
    )
    scheduler_affinity_anti_weight: float = Field(
        default=0.3, description="Peso de anti-affinity para tickets críticos (fault tolerance)"
    )
    scheduler_affinity_intent_weight: float = Field(
        default=0.1, description="Peso de intent affinity no score (workflow co-location)"
    )
    scheduler_affinity_plan_threshold: int = Field(
        default=3, description="Número de tickets para affinity máxima (3+ = score 1.0)"
    )
    scheduler_affinity_cache_ttl_seconds: int = Field(
        default=14400, description="TTL do cache de alocações em Redis (4 horas, alinhado com SLA)"
    )
    scheduler_affinity_anti_affinity_risk_bands: list[str] = Field(
        default=["critical", "high"], description="Risk bands que ativam anti-affinity"
    )
    scheduler_affinity_anti_affinity_priorities: list[str] = Field(
        default=["CRITICAL", "HIGH"], description="Priorities que ativam anti-affinity"
    )
    ...
    scheduler_preemption_min_preemptor_priority: str = Field(
        default="HIGH", description="Prioridade mínima para preemptar (HIGH ou CRITICAL)"
    )
    scheduler_preemption_max_preemptable_priority: str = Field(
        default="LOW", description="Prioridade máxima que pode ser preemptada (LOW ou MEDIUM)"
    )
    ...
    scheduler_preemption_max_concurrent: int = Field(
        default=5, ge=1, le=20, description="Limite de preempções simultâneas"
    )
    ...
    scheduler_preemption_retry_preempted_tasks: bool = Field(
        default=True, description="Reagendar tasks preemptadas automaticamente"
    )
    ...
    ml_predictions_enabled: bool = Field(
        default=True, description="Habilita predições ML centralizadas"
    )
    mlflow_tracking_uri: str = Field(
        default="https://mlflow.mlflow.svc.cluster.local:5000", description="URI do servidor MLflow"
    )
    mlflow_tls_verify: bool = Field(default=True, description="Verificar certificado TLS do MLflow")
    mlflow_ca_bundle: str | None = Field(
        default=None, description="Caminho para CA bundle do MLflow"
    )
    ml_scheduling_model_type: str = Field(
        default="xgboost", description="Tipo de modelo para SchedulingPredictor"
    )
    ml_anomaly_model_type: str = Field(
        default="isolation_forest", description="Tipo de modelo para AnomalyDetector"
    )
    ml_load_forecast_horizons: list[int] = Field(
        default=[60, 360, 1440], description="Horizontes de forecast em minutos"
    )
    ml_forecast_cache_ttl_seconds: int = Field(default=300, description="TTL do cache de forecasts")
    ml_drift_detection_enabled: bool = Field(default=True, description="Habilita detecção de drift")

    # SLA Tracking
    sla_default_timeout_ms: int = Field(default=3600000, description="Timeout padrão (1 hora)")
    sla_check_interval_seconds: int = Field(
        default=30, description="Intervalo de verificação de SLA"
    )
    sla_alert_threshold_percent: float = Field(
        default=0.8, description="Threshold para alertas (80%)"
    )
    ...
    sla_management_enabled: bool = Field(
        default=True, description="Habilitar integração com SLA Management System"
    )
    ...
    sla_management_port: int = Field(default=8000, description="Porta do SLA Management System")
    ...
    vault_kubernetes_role: str = Field(
        default="orchestrator-dynamic", description="Role Kubernetes para autenticação Vault"
    )
    ...
    vault_mount_database: str = Field(
        default="database", description="Mount point do database secrets"
    )
    ...
    spiffe_jwt_audience: str = Field(
        default="vault.neural-hive.local", description="Audience para JWT-SVID"
    )
    ...
    grpc_server_port: int = Field(
        default=50053, description="Porta do servidor gRPC para comandos estratégicos"
    )

    sla_management_cache_ttl_seconds: int = Field(
        default=10, description="TTL do cache de budgets em Redis"
    )
    sla_budget_critical_threshold: float = Field(
        default=0.2, description="Threshold para alertas de budget crítico (20%)"
    )
    sla_deadline_warning_threshold: float = Field(
        default=0.8, description="Threshold para alertas de deadline (80%)"
    )
    ...
    optimizer_grpc_timeout: int = Field(
        default=10, description="Timeout para chamadas gRPC ao Optimizer (segundos)"
    )
    optimizer_forecast_horizon_minutes: int = Field(
        default=60, description="Horizonte de previsão de carga em minutos"
    )
    ...
    ml_local_load_cache_ttl_seconds: int = Field(
        default=30, description="TTL do cache Redis para predições locais de carga"
    )
    ml_local_load_window_minutes: int = Field(
        default=60, description="Janela de dados históricos para predição local (em minutos)"
    )
    ml_queue_prediction_weight: float = Field(
        default=0.2, description="Peso da predição de queue time no score composto de worker (0-1)"
    )
    ml_load_prediction_weight: float = Field(
        default=0.1, description="Peso da predição de carga no score composto de worker (0-1)"
    )
    ml_optimization_timeout_seconds: int = Field(
        default=2, description="Timeout para chamadas de otimização ML (local + remote)"
    )
    ...
    sla_alerts_topic: str = Field(
        default="sla.alerts", description="Tópico Kafka para publicação de alertas proativos SLA"
    )
    sla_alert_deduplication_ttl_seconds: int = Field(
        default=300, description="TTL para deduplicação de alertas (5 minutos)"
    )
    ...
    retry_backoff_coefficient: float = Field(
        default=2.0, description="Coeficiente de backoff exponencial"
    )
    retry_max_interval_ms: int = Field(default=60000, description="Intervalo máximo de retry")
    CIRCUIT_BREAKER_ENABLED: bool = Field(
        default=True, description="Habilitar circuit breaker global MongoDB"
    )
    CIRCUIT_BREAKER_FAIL_MAX: int = Field(
        default=5, description="Falhas consecutivas para abrir circuito"
    )
    CIRCUIT_BREAKER_TIMEOUT: int = Field(
        default=60, description="Tempo em segundos com circuito aberto"
    )
    CIRCUIT_BREAKER_RECOVERY_TIMEOUT: int = Field(
        default=30, description="Tempo em segundos para half-open"
    )

    # Kafka Circuit Breaker
    KAFKA_CIRCUIT_BREAKER_ENABLED: bool = Field(
        default=True, description="Habilitar circuit breaker para Kafka Producer"
    )
    KAFKA_CIRCUIT_BREAKER_FAIL_MAX: int = Field(
        default=5, description="Falhas consecutivas para abrir circuito Kafka"
    )
    KAFKA_CIRCUIT_BREAKER_TIMEOUT: int = Field(
        default=60, description="Tempo em segundos com circuito Kafka aberto"
    )
    KAFKA_CIRCUIT_BREAKER_RECOVERY_TIMEOUT: int = Field(
        default=30, description="Tempo de recuperação do circuito Kafka"
    )

    # Temporal Circuit Breaker
    TEMPORAL_CIRCUIT_BREAKER_ENABLED: bool = Field(
        default=True, description="Habilitar circuit breaker para Temporal Client"
    )
    TEMPORAL_CIRCUIT_BREAKER_FAIL_MAX: int = Field(
        default=5, description="Falhas consecutivas para abrir circuito Temporal"
    )
    TEMPORAL_CIRCUIT_BREAKER_TIMEOUT: int = Field(
        default=60, description="Tempo em segundos com circuito Temporal aberto"
    )
    TEMPORAL_CIRCUIT_BREAKER_RECOVERY_TIMEOUT: int = Field(
        default=30, description="Tempo de recuperação do circuito Temporal"
    )

    # Redis Circuit Breaker
    REDIS_CIRCUIT_BREAKER_ENABLED: bool = Field(
        default=True, description="Habilitar circuit breaker para Redis Client"
    )
    REDIS_CIRCUIT_BREAKER_FAIL_MAX: int = Field(
        default=5, description="Falhas consecutivas para abrir circuito Redis"
    )
    REDIS_CIRCUIT_BREAKER_TIMEOUT: int = Field(
        default=60, description="Tempo em segundos com circuito Redis aberto"
    )
    REDIS_CIRCUIT_BREAKER_RECOVERY_TIMEOUT: int = Field(
        default=60, description="Tempo de recuperação do circuito Redis (maior para cache)"
    )
    ...
    MONGODB_FAIL_OPEN_VALIDATION_AUDIT: bool = Field(
        default=True, description="Permitir fail-open para auditoria de validação"
    )
    MONGODB_FAIL_OPEN_WORKFLOW_RESULTS: bool = Field(
        default=True, description="Permitir fail-open para resultados de workflow"
    )

    # Compensacao Automatica (Saga Pattern)
    compensation_enabled: bool = Field(
        default=True, description="Habilitar compensacao automatica quando workflows falham"
    )
    compensation_max_retries: int = Field(
        default=3, description="Maximo de tentativas para compensacao"
    )
    compensation_timeout_seconds: int = Field(
        default=60, description="Timeout para cada operacao de compensacao"
    )
    compensation_fail_open: bool = Field(
        default=False, description="Se True, falha na compensacao nao bloqueia workflow"
    )
    ...
    opa_host: str = Field(
        default="opa.neural-hive.svc.cluster.local", description="Host do OPA server"
    )
    ...
    opa_circuit_breaker_enabled: bool = Field(
        default=True, description="Habilitar circuit breaker para OPA (previne cascading failures)"
    )
    opa_circuit_breaker_failure_threshold: int = Field(
        default=5, description="Número de falhas consecutivas antes de abrir o circuito"
    )
    opa_circuit_breaker_reset_timeout: int = Field(
        default=60, description="Segundos antes de tentar fechar o circuito (half-open state)"
    )
    ...
    opa_security_enabled: bool = Field(
        default=True, description="Habilitar validação de security constraints"
    )
    ...
    opa_tenant_rate_limits: dict = Field(
        default_factory=dict, description="Rate limits por tenant (req/min)"
    )
    opa_global_rate_limit: int = Field(default=1000, description="Rate limit global (req/min)")
    ...
    opa_intelligent_scheduler_enabled: bool = Field(
        default=True, description="Habilitar intelligent scheduler"
    )
    opa_burst_capacity_enabled: bool = Field(default=True, description="Habilitar burst capacity")
    opa_burst_threshold: float = Field(default=0.8, description="Threshold para burst capacity")
    opa_predictive_allocation_enabled: bool = Field(
        default=False, description="Habilitar alocação preditiva"
    )
    opa_auto_scaling_enabled: bool = Field(default=False, description="Habilitar auto scaling")
    ...
    mlflow_experiment_name: str = Field(
        default="orchestrator-predictive-models", description="Nome do experimento MLflow"
    )
    ...
    ml_training_interval_hours: int = Field(
        default=24, description="Intervalo de retreinamento periódico em horas"
    )
    ml_min_training_samples: int = Field(
        default=100, description="Mínimo de amostras necessárias para treinamento"
    )
    ...
    ml_model_cache_ttl_seconds: int = Field(
        default=3600, description="TTL do cache de modelos (1 hora)"
    )
    ml_feature_cache_ttl_seconds: int = Field(
        default=3600, description="TTL do cache de estatísticas de features (1 hora)"
    )

    # Drift Detection
    ml_drift_check_window_days: int = Field(
        default=7, description="Janela de dados para drift detection em dias"
    )
    ml_drift_psi_threshold: float = Field(
        default=0.25, description="Threshold PSI para drift significativo"
    )
    ml_drift_mae_ratio_threshold: float = Field(
        default=1.5, description="Threshold de degradação de MAE (ratio atual/treino)"
    )
    ml_drift_ks_pvalue_threshold: float = Field(
        default=0.05, description="P-value threshold para K-S test (target drift)"
    )
    ...
    ml_training_job_timeout_seconds: int = Field(
        default=3600, description="Timeout para jobs de treinamento (1 hora)"
    )
    ml_training_job_memory_limit: str = Field(
        default="4Gi", description="Limite de memória para jobs de treinamento"
    )
    ml_training_job_cpu_limit: str = Field(
        default="2000m", description="Limite de CPU para jobs de treinamento"
    )
    ml_backfill_errors: bool = Field(
        default=False, description="Habilitar backfill de erros históricos durante treinamento"
    )
    ml_backfill_history_retention_days: int = Field(
        default=90, description="Retenção de histórico de backfill em dias"
    )
    ...
    pushgateway_tls_verify: bool = Field(
        default=True, description="Verificar certificado TLS do Prometheus Pushgateway"
    )
    pushgateway_ca_bundle: str | None = Field(
        default=None, description="Caminho para CA bundle do Prometheus Pushgateway"
    )

    # ClickHouse Integration
    clickhouse_host: str = Field(
        default="clickhouse.clickhouse.svc.cluster.local", description="Host do servidor ClickHouse"
    )
    clickhouse_port: int = Field(default=9000, description="Porta do ClickHouse (native protocol)")
    clickhouse_database: str = Field(
        default="neural_hive_analytics", description="Database ClickHouse"
    )
    ...
    ml_retraining_triggers_enabled: bool = Field(
        default=True, description="Habilitar triggers automáticos de re-treinamento"
    )
    ml_drift_trigger_threshold: float = Field(
        default=0.25, description="Threshold PSI para trigger de re-treinamento por drift"
    )
    ml_performance_trigger_threshold: float = Field(
        default=1.5, description="Threshold de degradação de MAE (ratio atual/treino) para trigger"
    )
    ml_data_volume_trigger_threshold: int = Field(
        default=10000, description="Volume de novos dados para trigger de re-treinamento"
    )
    ...
    ml_validation_mae_threshold: float = Field(
        default=0.15, description="Threshold MAE (15%) para validação de DurationPredictor"
    )
    ml_validation_precision_threshold: float = Field(
        default=0.75, description="Threshold precision para validação de AnomalyDetector"
    )
    ml_auto_rollback_enabled: bool = Field(
        default=True, description="Habilitar rollback automático quando modelo novo performa pior"
    )

    # Continuous Validation Configuration
    ml_continuous_validation_enabled: bool = Field(
        default=True, description="Habilitar validação contínua de modelos em produção"
    )
    ml_validation_check_interval_seconds: int = Field(
        default=300, description="Intervalo entre checks de validação contínua (5 minutos)"
    )
    ...
    ml_validation_mongodb_collection: str = Field(
        default="model_predictions", description="Collection MongoDB para predições e actuals"
    )
    ml_validation_windows: list[str] = Field(
        default=["1h", "24h", "7d"], description="Janelas temporais para métricas agregadas"
    )
    ml_validation_alert_cooldown_minutes: int = Field(
        default=30, description="Cooldown entre alertas do mesmo tipo (evitar spam)"
    )
    ml_validation_latency_enabled: bool = Field(
        default=True, description="Habilitar coleta de métricas de latência (p50, p95, p99)"
    )
    ml_validation_r2_enabled: bool = Field(
        default=True, description="Habilitar cálculo de R² (coeficiente de determinação)"
    )

    # Canary Deployment
    ml_canary_enabled: bool = Field(
        default=False, description="Habilitar canary deployment para modelos ML"
    )
    ml_canary_traffic_percentage: float = Field(
        default=0.1, description="Percentual de tráfego para canary (10%)"
    )
    ml_canary_duration_minutes: int = Field(
        default=60, description="Duração do período de canary em minutos"
    )

    # Shadow Mode Configuration
    ml_shadow_mode_enabled: bool = Field(
        default=False, description="Habilitar shadow mode para validação de modelos antes de canary"
    )
    ...
    ml_shadow_mode_min_predictions: int = Field(
        default=1000, description="Mínimo de predições shadow antes de considerar válido"
    )
    ml_shadow_mode_agreement_threshold: float = Field(
        default=0.90, description="Threshold de agreement rate para passar shadow mode (90%)"
    )
    ml_shadow_mode_sample_rate: float = Field(
        default=1.0, description="Taxa de amostragem para shadow predictions (1.0 = 100%)"
    )
    ml_shadow_mode_persist_comparisons: bool = Field(
        default=True, description="Persistir comparações no MongoDB para análise"
    )
    ml_shadow_mode_circuit_breaker_enabled: bool = Field(
        default=True, description="Habilitar circuit breaker para falhas do modelo shadow"
    )
    ml_shadow_model_version: str | None = Field(
        default=None,
    ...
    ml_audit_enabled: bool = Field(
        default=True, description="Habilitar audit logging para ciclo de vida de modelos ML"
    )
    ...
    ml_audit_retention_days: int = Field(
        default=365, description="Retenção de audit logs em dias (TTL index)"
    )

    # Gradual Rollout Configuration
    ml_gradual_rollout_enabled: bool = Field(
        default=True, description="Habilitar gradual rollout com checkpoints para modelos ML"
    )
    ...
    ml_checkpoint_duration_minutes: int = Field(
        default=30, description="Duração de cada checkpoint de rollout em minutos"
    )
    ml_checkpoint_mae_threshold_pct: float = Field(
        default=20.0, description="Threshold de aumento de MAE (%) para rollback em checkpoint"
    )
    ...
    fluxo_g_approval_url: str = Field(
        default="http://approval-service:8004",
        description="URL do serviço Approval Service (migrado de approval-gateway:8017)",
    )
