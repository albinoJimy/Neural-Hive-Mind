...
class ValidationResult:
    ...
    )  # indexado por policy_path; entradas especiais como 'feature_flags' podem ser adicionadas
...
class PolicyValidator:
        ...
        try:
            ...
            for violation in validation_result.violations:
                self.metrics.record_opa_rejection(
                    violation.policy_name, violation.rule, violation.severity
                )

            # Registrar warnings
            for warning in validation_result.warnings:
                self.metrics.record_opa_warning(warning.policy_name, warning.rule)
            ...
            logger.error("Erro ao validar plano cognitivo", error=str(e), exc_info=True)
                ...
                return ValidationResult(
                    ...
                    violations=[
                        PolicyViolation(
                            ...
                            message=f"Erro na avaliação de políticas: {e!s}",
                            severity="critical",
                        )
                    ],
    ...
    async def validate_execution_ticket(self, ticket: dict) -> ValidationResult:
        ...
        try:
            ...
            context = {
                ...
                "request_count_last_minute": await self._get_request_count(
                    ticket.get("tenant_id", "default")
                ),
            }
            ...
            for violation in validation_result.violations:
                ...
                if violation.policy_name == "security_constraints":
                    self.metrics.record_security_violation(
                        context["tenant_id"], violation.rule, violation.severity
                    )
                    if violation.rule == "cross_tenant_access":
                        ...
                        denied_capability = ""
                        if (
                            isinstance(ticket.get("required_capabilities"), list)
                            and ticket["required_capabilities"]
                        ):
    ...
    async def validate_resource_allocation(
        self, ticket: dict, agent_info: dict
    ) -> ValidationResult:
        """
    ...
    async def _get_request_count(self, tenant_id: str) -> int:
        ...
        try:
            ...
            if not hasattr(self.config, "redis_cluster_nodes") or not getattr(
                self.config, "redis_cluster_nodes", None
            ):
                return 0
    ...
    def _build_opa_input(self, data: dict, context: dict) -> dict:
        ...
        """
        return {"input": {"resource": data, "context": context}}
