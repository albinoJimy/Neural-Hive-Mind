...
class AuthorizationAuditQuery(BaseModel):
    ...
    decision: str | None = None  # 'allow' ou 'deny'
...
class AppState:
    ...
    def __init__(self):
        self.temporal_client: TemporalClientWrapper | None = None
        self.kafka_consumer: DecisionConsumer | None = None
        ...
        self.spiffe_manager: Any | None = None
    ...
    try:
        ...
        if config.vault_enabled:
            if VAULT_AVAILABLE and OrchestratorVaultClient:
                try:
                    ...
                    logger.exception(
                        "Erro ao inicializar Vault integration", error=str(vault_error)
                    )
                    if not getattr(config, "vault_fail_open", True):
                        raise
                    logger.warning(
                        "Vault fail-open habilitado, continuando com credenciais estáticas"
                    )
                    vault_client = None
                    app_state.vault_client = None
            elif config.vault_enabled and not VAULT_AVAILABLE:
        ...
        if otel_enabled:
            try:
                otel_endpoint = os.getenv(
                    "OTEL_EXPORTER_ENDPOINT", getattr(config, "otel_exporter_endpoint", None)
                )
                init_observability(
                ...
                async def _mongodb_credential_update_callback(new_creds: dict[str, Any]):
                    ...
                    try:
                        new_uri = _build_mongodb_uri_with_credentials(
                            config.mongodb_uri, new_creds["username"], new_creds["password"]
                        )
                        ...
                        logger.exception(
                            "Falha ao recriar MongoDB client com novas credenciais", error=str(e)
                        )
                        raise
        ...
        if app_state.mongodb_client and getattr(config, "ml_audit_enabled", True):
            try:
                ...
                app_state.audit_logger = ModelAuditLogger(
                    mongodb_client=app_state.mongodb_client.client, config=config, metrics=metrics
                )
                logger.info(
                ...
                logger.warning("Falha ao inicializar Model Audit Logger", error=str(audit_error))
        ...
        if getattr(config, "ml_predictions_enabled", False):
            ...
            try:
                ...
                scheduling_model_trained = app_state.scheduling_predictor.model is not None
                ...
                logger.info(
                    "SchedulingPredictor inicializado", model_trained=scheduling_model_trained
                )
                ...
                load_config = {
                    ...
                    "forecast_horizons": getattr(
                        config, "ml_load_forecast_horizons", [60, 360, 1440]
                    ),
                ...
                load_model_trained = app_state.load_predictor.model is not None
                ml_init_metrics.record_ml_model_init_training_status(
                    model_name="load-predictor",
                ...
                app_state.anomaly_detector = AnomalyDetector(
                    config=anomaly_config, model_registry=app_state.model_registry, metrics=metrics
                )
                ...
                logger.warning(
                    "Falha ao inicializar modelos preditivos, continuando sem ML", error=str(e)
                )
                # Registrar falha para todos os preditores (status de componente)
                for predictor_name in [
                    "scheduling_predictor",
                    "load_predictor",
                    "anomaly_detector",
                ]:
                    ml_init_metrics.record_component_initialization_status(
                ...
                for model_name in ["scheduling-predictor", "load-predictor", "anomaly-detector"]:
                    ml_init_metrics.record_ml_model_init_training_status(
                        model_name=model_name, status="failed", component="ml", layer="intelligence"
                    )
        ...
        try:
            app_state.kafka_producer = KafkaProducerClient(
                config, sasl_username_override=kafka_username, sasl_password_override=kafka_password
            )
            ...
            logger.exception("Falha ao inicializar Kafka Producer", error=str(kafka_init_error))
            raise

        # Inicializar Execution Ticket Service client com SPIFFE (fail-open)
        try:
            app_state.execution_ticket_client = ExecutionTicketClient(
                config, spiffe_manager=spiffe_manager
            )
            await app_state.execution_ticket_client.initialize()
            logger.info(
                "Execution Ticket client inicializado", spiffe_enabled=config.spiffe_enabled
            )
        ...
        app_state.self_healing_client = SelfHealingClient(
            base_url=config.self_healing_engine_url, timeout=config.self_healing_timeout_seconds
        )
        ...
        if config.temporal_enabled:
            try:
                ...
                temporal_client_raw = await create_temporal_client(
                    config, postgres_user=postgres_user, postgres_password=postgres_password
                )
                if temporal_client_raw:
                    ...
                    app_state.temporal_client = TemporalClientWrapper(
                        ...
                        recovery_timeout=getattr(
                            config, "TEMPORAL_CIRCUIT_BREAKER_RECOVERY_TIMEOUT", 30
                        ),
                    )
                ...
                logger.warning(
                    "Falha ao criar cliente Temporal, continuando em modo degradado", error=str(e)
                )
                app_state.temporal_client = None
        else:
        ...
        if app_state.temporal_client:
            ...
            try:
                ...
                logger.info(
                    ...
                    policy_validator_injected=app_state.temporal_worker.policy_validator
                    is not None,
                    opa_host=getattr(config, "opa_host", None),
                    opa_port=getattr(config, "opa_port", None),
                )
                ...
                logger.exception(
                    "Falha ao inicializar Temporal Worker", error=str(temporal_init_error)
                )
        ...
        if getattr(config, "enable_grpc_server", True):
            try:
                from src.grpc_server import OrchestratorStrategicServicer, start_grpc_server
                ...
                if config.opa_enabled:
                    app_state.opa_client = OPAClient(
                        config, mongodb_client=app_state.mongodb_client
                    )
...
async def get_cached_workflow_state(workflow_id: str) -> dict[str, Any] | None:
    """
...
async def cache_workflow_state(
    workflow_id: str, state: dict[str, Any], ttl: int = WORKFLOW_CACHE_TTL_SECONDS
) -> bool:
    """
...
async def get_cached_tickets_by_workflow(workflow_id: str) -> list | None:
    """
...
async def cache_tickets_by_workflow(
    workflow_id: str, tickets: list, ttl: int = WORKFLOW_CACHE_TTL_SECONDS
) -> bool:
    """
...
async def get_tickets_from_mongodb_fallback(workflow_id: str) -> list | None:
    """
    ...
    try:
        ...
        if "orchestrator-" in workflow_id:
            plan_id = workflow_id.rsplit("orchestrator-", maxsplit=1)[-1]
        ...
        logger.info("mongodb_fallback_attempt", workflow_id=workflow_id, extracted_plan_id=plan_id)
        ...
        if tickets:
            metrics.record_mongodb_fallback(
                source="temporal_validation_error", status="success", tickets_count=len(tickets)
            )
            return tickets
        ...
        metrics.record_mongodb_fallback(source="temporal_validation_error", status="not_found")
        return None

    except Exception as e:
        logger.error("mongodb_fallback_error", workflow_id=workflow_id, error=str(e), exc_info=True)
        metrics.record_mongodb_fallback(source="temporal_validation_error", status="error")
        return None


async def get_cached_flow_c_status() -> dict[str, Any] | None:
    """
...
def _is_predictor_loaded(model_name: str) -> bool:
    ...
    if "scheduling" in model_name and app_state.scheduling_predictor:
        return (
            hasattr(app_state.scheduling_predictor, "model")
            and app_state.scheduling_predictor.model is not None
        )
    if "load" in model_name and app_state.load_predictor:
        return (
            hasattr(app_state.load_predictor, "model")
            and app_state.load_predictor.model is not None
        )
    if "anomaly" in model_name and app_state.anomaly_detector:
        return (
            hasattr(app_state.anomaly_detector, "model")
            and app_state.anomaly_detector.model is not None
        )
    return False


def _get_predictor_info(model_name: str) -> dict | None:
    """
    ...
    if "scheduling" in model_name and app_state.scheduling_predictor:
        return {
            ...
            "loaded": hasattr(app_state.scheduling_predictor, "model")
            and app_state.scheduling_predictor.model is not None,
        }
    if "load" in model_name and app_state.load_predictor:
        return {
            "version": getattr(app_state.load_predictor, "model_version", "unknown"),
            "loaded": hasattr(app_state.load_predictor, "model")
            and app_state.load_predictor.model is not None,
        }
    if "anomaly" in model_name and app_state.anomaly_detector:
        return {
            "version": getattr(app_state.anomaly_detector, "model_version", "unknown"),
            "loaded": hasattr(app_state.anomaly_detector, "model")
            and app_state.anomaly_detector.model is not None,
        }
    return None
...
async def opa_health_check():
    ...
    try:
        ...
        return JSONResponse(
            status_code=200 if status in ["healthy", "degraded"] else 503, content=response
        )

    except Exception as e:
        logger.exception("opa_health_check_error", error=str(e))
        return JSONResponse(
            status_code=503,
            content={
                "status": "error",
...
async def kafka_producer_health_check():
    ...
    # Verificar config
    checks["config"] = {"valid": False, "service_name": None}
    ...
    checks["circuit_breaker"] = {"initialized": False, "state": "unknown"}
    ...
    checks["kafka_connection"] = {"initialized": False}
...
async def ml_health_check():
    ...
    try:
        ...
        health_info = {"status": "healthy", "predictors": {}}
        ...
        if app_state.scheduling_predictor:
            try:
                ...
                if (
                    not hasattr(app_state.scheduling_predictor, "model")
                    or app_state.scheduling_predictor.model is None
                ):
                    await app_state.scheduling_predictor.initialize()
                ...
                health_info["predictors"]["scheduling_predictor"] = {
                    ...
                    "model_version": getattr(
                        app_state.scheduling_predictor, "model_version", "unknown"
                    ),
                    "model_type": app_state.scheduling_predictor.config.get(
                        "model_type", "unknown"
                    ),
                }
            except Exception as e:
        ...
        if app_state.load_predictor:
            try:
                if (
                    not hasattr(app_state.load_predictor, "model")
                    or app_state.load_predictor.model is None
                ):
                    await app_state.load_predictor.initialize()
                ...
                health_info["predictors"]["load_predictor"] = {"loaded": False, "error": str(e)}

        # Check AnomalyDetector
        if app_state.anomaly_detector:
            try:
                if (
                    not hasattr(app_state.anomaly_detector, "model")
                    or app_state.anomaly_detector.model is None
                ):
                    await app_state.anomaly_detector.initialize()
                ...
                health_info["predictors"]["anomaly_detector"] = {
                    ...
                    "model_version": getattr(
                        app_state.anomaly_detector, "model_version", "unknown"
                    ),
                    "model_type": app_state.anomaly_detector.config.get("model_type", "unknown"),
                }
            except Exception as e:
                health_info["predictors"]["anomaly_detector"] = {"loaded": False, "error": str(e)}

        # Determinar status geral
        all_loaded = all(p.get("loaded", False) for p in health_info["predictors"].values())
        ...
        logger.exception("ml_health_check_error", error=str(e))
...
async def get_redis_stats():
    ...
    try:
        ...
        logger.exception("redis_stats_error", error=str(e))
...
async def query_authorization_audit(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao consultar auditoria: {e!s}")
        ...
        try:
            ...
            if cached_data:
                try:
                    ...
                    logger.info("ticket_cache_hit", ticket_id=ticket_id)
            ...
            if not app_state.mongodb_client:
                logger.error(
                    "ticket_query_rejected", reason="mongodb_unavailable", ticket_id=ticket_id
                )
            ...
            try:
                ...
                logger.warning(
                    "ticket_cache_write_error", ticket_id=ticket_id, error=str(cache_err)
                )
            ...
            logger.exception(
                "ticket_query_error", ticket_id=ticket_id, error=str(e), error_type=type(e).__name__
            )
            span.record_exception(e)
            raise HTTPException(status_code=500, detail=f"Error retrieving ticket: {e!s}")


@app.get("/api/v1/tickets/by-plan/{plan_id}")
async def get_tickets_by_plan(
    plan_id: str, status: str | None = None, limit: int = 100, offset: int = 0
):
    """
    ...
    with tracer.start_as_current_span(
        ...
        try:
            ...
            if not app_state.mongodb_client:
                logger.error(
                    "tickets_list_query_rejected", reason="mongodb_unavailable", plan_id=plan_id
                )
                raise HTTPException(status_code=503, detail="MongoDB not available")
            ...
            try:
                ...
                logger.warning(
                    "tickets_list_cache_write_error", plan_id=plan_id, error=str(cache_err)
                )
            ...
            span.record_exception(e)
            raise HTTPException(status_code=500, detail=f"Error retrieving tickets: {e!s}")
...
async def get_flow_c_status():
    ...
    try:
        ...
        pipeline = [
            {
                "$facet": {
                    "total": [{"$count": "count"}],
                    "success": [{"$match": {"status": "completed"}}, {"$count": "count"}],
                    "latency": [
        ...
        logger.exception("flow_c_status_error", error=str(e))
        raise HTTPException(status_code=500, detail=f"Erro ao obter status: {e!s}")
...
class WorkflowStartRequest(BaseModel):
    ...
    sla_deadline_seconds: int = Field(
        default=14400, description="Deadline SLA em segundos (default: 4h)"
    )
...
class WorkflowQueryRequest(BaseModel):
    ...
    query_name: str = Field(
        ..., description="Nome da query a executar (ex: get_tickets, get_status)"
    )
...
async def trigger_manual_training(window_days: int | None = None, backfill_errors: bool = False):
    """
    ...
    try:
        ...
        async def run_training():
            try:
                ...
                logger.exception(f"Training job {job_id} failed", error=str(e))
        ...
        return JSONResponse(
            status_code=202,
            content={"job_id": job_id, "status": "queued", "message": "Treinamento iniciado"},
        )

    except HTTPException:
        raise
    except Exception as e:
        logger.exception("trigger_training_error", error=str(e))
        raise HTTPException(status_code=500, detail=f"Erro ao iniciar treinamento: {e!s}")
...
async def list_ml_models(model_type: str | None = None):
    ...
    try:
        ...
        logger.exception("list_models_error", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_model_details(model_name: str):
    ...
    try:
        ...
        logger.exception("get_model_details_error", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def promote_model(model_name: str, request: ModelPromotionRequest):
    ...
    try:
        ...
        logger.exception("promote_model_error", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def check_model_drift():
    ...
    try:
        ...
        logger.exception("drift_check_error", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_prediction_statistics():
    ...
    try:
        ...
        result = (
            await app_state.mongodb_client.db["execution_tickets"].aggregate(pipeline).to_list(1)
        )
        ...
        logger.exception("prediction_stats_error", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
    ...
    with tracer.start_as_current_span(
        ...
        if cached_data:
            try:
                ...
                logger.info("workflow_status_cache_hit", workflow_id=workflow_id)
                span.set_attribute("cache.hit", True)
                cached_status["cached"] = True
                return JSONResponse(status_code=200, content=cached_status)
            except json.JSONDecodeError:
        ...
        logger.info("workflow_status_query", workflow_id=workflow_id)
        ...
        try:
            ...
            workflow_info = (
                ...
                if hasattr(description, "workflow_execution_info")
                else description
            )
            ...
            if hasattr(workflow_info, "close_time") and workflow_info.close_time:
                try:
                    ...
                    logger.warning(
                        "workflow_result_unavailable", workflow_id=workflow_id, error=str(e)
                    )

            logger.info(
                "workflow_status_retrieved", workflow_id=workflow_id, status=status_data["status"]
            )
            ...
            try:
                ...
                logger.warning(
                    "workflow_status_cache_write_error",
                    workflow_id=workflow_id,
                    error=str(cache_err),
                )
            ...
            if "not found" in error_str.lower() or "does not exist" in error_str.lower():
                logger.warning("workflow_not_found", workflow_id=workflow_id, error=error_str)
                raise HTTPException(status_code=404, detail=f"Workflow {workflow_id} not found")

            logger.exception(
                "workflow_status_query_error", workflow_id=workflow_id, error=error_str
            )
            raise HTTPException(
                status_code=500, detail=f"Error retrieving workflow status: {error_str}"
            )
    ...
    try:
        ...
        return JSONResponse(
            ...
            content=WorkflowStartResponse(
                workflow_id=workflow_id, status="started", correlation_id=request.correlation_id
            ).model_dump(),
        )
...
async def query_workflow(workflow_id: str, request: WorkflowQueryRequest):
    ...
    with tracer.start_as_current_span(
        ...
        logger.info(
            "workflow_query_attempt", workflow_id=workflow_id, query_name=request.query_name
        )
        ...
        try:
            ...
            if request.query_name == "get_tickets":
                ...
                try:
                    ...
                    metrics.record_workflow_query(
                        query_name="get_tickets", status="validation_error"
                    )
            ...
            logger.info(
                "workflow_query_success", workflow_id=workflow_id, query_name=request.query_name
            )
            span.set_attribute("query.success", True)
            span.set_attribute("query.fallback", False)
            ...
            return JSONResponse(
                status_code=200,
                content=WorkflowQueryResponse(
                    workflow_id=workflow_id, query_name=request.query_name, result=query_result
                ).model_dump(),
            )
            ...
            raise HTTPException(status_code=500, detail=f"Failed to query workflow: {error_str}")
