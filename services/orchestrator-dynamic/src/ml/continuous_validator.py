...
class ValidationWindow:
    ...
    def get_valid_pairs(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        for pred, actual, ts in zip(self.predictions, self.actuals, self.timestamps, strict=False):
            if ts >= cutoff:
...
class LatencyWindow:
    ...
    def get_valid_latencies(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        for latency, error, ts in zip(self.latencies, self.errors, self.timestamps, strict=False):
            if ts >= cutoff:
...
class ContinuousValidator:
    ...
    def __init__(
        ...
        self.mongodb_collection = getattr(
            config, "ml_validation_mongodb_collection", "model_predictions"
        )
        ...
        self._training_baseline: dict[str, Any] | None = None
        self._baseline_cache_ttl_hours = 24
        self._baseline_last_fetched: datetime | None = None
    ...
    async def record_actual(
        self, ticket_id: str, actual_duration_ms: float, had_error: bool = False
    ) -> dict[str, Any] | None:
        """
        ...
        if self.metrics:
            self.metrics.record_prediction_error(
                model_name="duration-predictor", error_ms=error, error_pct=error_pct
            )

        return result

    async def start_continuous_validation(self, check_interval_seconds: int = 300) -> None:
        """
        ...
        self._stop_validation = False
        self._validation_task = asyncio.create_task(self._validation_loop(check_interval_seconds))
    ...
    async def _fetch_predictions_from_mongodb(
        self, model_name: str, window_hours: int
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.warning("mongodb_fetch_failed", model_name=model_name, error=str(e))
            return []

    async def populate_windows_from_mongodb(self, model_name: str = "duration-predictor") -> None:
        """
        ...
        for window_name, window in self.windows.items():
            predictions = await self._fetch_predictions_from_mongodb(
                model_name=model_name, window_hours=window.window_hours
            )
            ...
            for pred_doc in predictions:
                try:
                    ...
                    self.logger.warning("populate_window_error", window=window_name, error=str(e))

            self.logger.info(
                "window_populated", window=window_name, samples=len(window.predictions)
            )
    ...
    async def _validation_loop(self, interval_seconds: int) -> None:
        ...
        while not self._stop_validation:
            try:
                ...
                self.logger.exception("validation_loop_error", error=str(e))
    ...
    async def _fetch_training_baseline(
        ...
        try:
            ...
            baseline_doc = await self.mongodb_client.db["ml_training_history"].find_one(
                {"model_name": model_name, "status": "completed", "metrics": {"$exists": True}},
                sort=[("trained_at", -1)],
            )
            ...
            self.logger.warning("training_baseline_not_found", model_name=model_name)
            return None

        except Exception as e:
            self.logger.warning(
                "training_baseline_fetch_failed", model_name=model_name, error=str(e)
            )
            return None
    ...
    async def _generate_alert(
        self, alert_type: str, severity: str, message: str, metrics: dict[str, Any], window: str
    ) -> None:
    ...
    async def _store_alert(self, alert: ValidationAlert) -> None:
        ...
        try:
            await self.mongodb_client.db["ml_validation_alerts"].insert_one(alert.to_dict())
        except Exception as e:
    ...
    async def _reload_windows_from_mongodb(self, model_name: str = "duration-predictor") -> None:
        """
        ...
        self.logger.debug("windows_cleared_for_reload", model_name=model_name)
    ...
    async def get_metrics_history(
        self, hours: int = 24, window: str = "24h"
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.exception("get_metrics_history_failed", error=str(e))
            return []
