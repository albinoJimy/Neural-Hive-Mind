...
def push_metrics_to_gateway(
    metrics: dict[str, Any], pushgateway_url: str, job_name: str = "ml_training"
) -> None:
    """
...
def main() -> int:
    ...
    try:
        ...
        duration_predictor = DurationPredictor(config=config, mongodb_client=mongodb_client)

        anomaly_detector = AnomalyDetector(config=config, mongodb_client=mongodb_client)
        ...
        if hasattr(config, "prometheus_pushgateway_url"):
            push_metrics_to_gateway(result, config.prometheus_pushgateway_url)
        ...
        metrics.record_training_job(status=job_status, trigger="scheduled")
        ...
        if result.get("status") == "skipped":
            logger.warning(
                "training_skipped", reason=result.get("reason"), message=result.get("message")
            )
            # Retornar sucesso mesmo quando skipado (dados insuficientes não é erro)
            return 0

        if result.get("status") != "completed":
            logger.error("training_failed", status=result.get("status"), error=result.get("error"))
        ...
        if duration_mae_pct > 50:
            logger.warning(
                "duration_model_poor_quality", mae_percentage=duration_mae_pct, threshold=50
            )

        if anomaly_precision < 0.5:
            logger.warning("anomaly_model_poor_quality", precision=anomaly_precision, threshold=0.5)
        ...
        logger.error(
            "ml_training_job_failed", error=str(e), error_type=type(e).__name__, exc_info=True
        )
        ...
        if mongodb_client:
            try:
                ...
                logger.exception("failed_to_close_mongodb", error=str(e))
