...
class MLPredictor:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.exception("ml_predictor_init_failed", error=str(e))
            raise
    ...
    async def ensure_models_trained(self) -> dict[str, bool]:
        ...
        try:
            ...
            status = {"duration": duration_ready, "anomaly": anomaly_ready}
            ...
            self.logger.exception("ensure_models_trained_failed", error=str(e))
    ...
    async def predict_and_enrich(self, ticket: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            resource_estimate = self._estimate_resources(
                duration_ms=predicted_duration, task_type=task_type, risk_band=risk_band
            )
            ...
            self.logger.exception(
                "predict_and_enrich_failed", error=str(e), ticket_id=ticket.get("ticket_id")
            )
            # Retorna ticket sem modificação em caso de erro
            return ticket

    def _estimate_resources(
        self, duration_ms: float, task_type: str, risk_band: str
    ) -> dict[str, int]:
        """
        ...
        try:
            ...
            baseline = self.RESOURCE_BASELINE.get(
                task_type.upper(), {"cpu_m": 300, "memory_mb": 512}
            )

            # Risk multiplier
            risk_multiplier = self.RISK_MULTIPLIERS.get(risk_band.lower(), 1.0)
            ...
            return {"cpu_m": cpu_m, "memory_mb": memory_mb}

        except Exception as e:
            self.logger.warning("resource_estimation_failed", error=str(e))
            return {"cpu_m": 300, "memory_mb": 512}
    ...
    async def close(self):
        ...
        try:
            ...
            self.logger.exception("ml_predictor_close_failed", error=str(e))
