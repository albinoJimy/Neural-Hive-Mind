...
def push_drift_metrics_to_gateway(
    drift_report: dict[str, Any], pushgateway_url: str, job_name: str = "ml_drift_detection"
) -> None:
    """
    ...
    try:
        ...
        if drift_report.get("target_drift"):
            ...
            target_drift_metric = Gauge(
                "orchestration_ml_drift_target_pvalue", "P-value do K-S test", registry=registry
            )
...
def main() -> int:
    ...
    try:
        ...
        drift_detector = DriftDetector(
            config=config, mongodb_client=mongodb_client, metrics=metrics
        )
        ...
        if drift_report.get("feature_drift"):
            metrics.update_drift_status(
                model_name="duration-predictor", drift_type="feature", status=overall_status
            )

        if drift_report.get("prediction_drift"):
            metrics.update_drift_status(
                model_name="duration-predictor", drift_type="prediction", status=overall_status
            )

        if drift_report.get("target_drift"):
            metrics.update_drift_status(
                model_name="duration-predictor", drift_type="target", status=overall_status
            )
        ...
        if hasattr(config, "prometheus_pushgateway_url"):
            push_drift_metrics_to_gateway(drift_report, config.prometheus_pushgateway_url)
        ...
        logger.error(
            "ml_drift_job_failed", error=str(e), error_type=type(e).__name__, exc_info=True
        )
        ...
        if hasattr(config, "prometheus_pushgateway_url"):
            try:
                push_drift_metrics_to_gateway(
                    {"overall_status": "error", "error": str(e)}, config.prometheus_pushgateway_url
                )
