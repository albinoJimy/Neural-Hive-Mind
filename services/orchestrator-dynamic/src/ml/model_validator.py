...
class ModelValidator:
    ...
    def validate_duration_predictor(
        ...
        task_types: np.ndarray | None = None,
        ...
        if confidence is not None:
            checks["confidence"] = self._validate_confidence_calibration(y_true, y_pred, confidence)
        ...
        passed = (
            checks.get("performance", {}).get("passed", False)
            and checks.get("business", {}).get("passed", False)
    ...
    def validate_anomaly_detector(
        self, y_true: np.ndarray, y_pred: np.ndarray, anomaly_scores: np.ndarray | None = None
    ) -> ModelValidationResult:
        """
        ...
        if anomaly_scores is not None:
            checks["score_distribution"] = self._validate_score_distribution(y_true, anomaly_scores)
        ...
        passed = checks.get("classification", {}).get("passed", False) and checks.get(
            "fpr", {}
        ).get("passed", False)
        ...
        self.logger.info("anomaly_detector_validated", passed=passed, overall_score=overall_score)
    ...
    def _validate_performance_metrics(
        ...
        result = {
            ...
            "thresholds": {"mae_pct": self.mae_threshold, "r2": self.r2_threshold},
        }
    ...
    def _validate_business_rules(self, y_pred: np.ndarray) -> dict[str, Any]:
    ...
    def _validate_confidence_calibration(
        self, y_true: np.ndarray, y_pred: np.ndarray, confidence: np.ndarray
    ) -> dict[str, Any]:
        ...
        calibrated = (
            np.isnan(high_conf_mae) or np.isnan(low_conf_mae) or high_conf_mae < low_conf_mae
        )
        ...
        result = {
            ...
            "metrics": {
                ...
                "error_confidence_correlation": float(correlation)
    ...
    def _validate_anomaly_rate(self, y_pred: np.ndarray) -> dict[str, Any]:
