...
from .anomaly_training_utils import prepare_anomaly_training_data, validate_anomaly_training_data
...
class AnomalyDetector:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.exception("anomaly_detector_init_failed", error=str(e))
    ...
    async def _load_model(self) -> IsolationForest | None:
        """
        ...
        try:
            ...
            self.logger.exception("anomaly_model_load_failed", error=str(e))
    ...
    async def _refresh_historical_stats(self):
        ...
        try:
            ...
            self.historical_stats = await compute_historical_stats(
                mongodb_client=self.mongodb_client, window_days=self.config.ml_training_window_days
            )
            ...
            try:
                ...
                if shadow_runner:
                    ...
                    self.metrics.record_ml_prediction(
                        model_type="anomaly", status="success", duration=duration_seconds
                    )
                ...
                self.logger.exception(
                    "detect_anomaly_failed", error=str(e), ticket_id=ticket.get("ticket_id")
                )
    ...
    def _explain_anomaly(
        self, features: dict[str, float], ticket: dict[str, Any]
    ) -> tuple[str, str]:
        """
        ...
        try:
            ...
            # Anomalia genérica
            return ("unknown", "Padrão de features atípico detectado pelo modelo")
    ...
    async def train_model(
        self, training_window_days: int | None = None, hyperparameter_tuning: bool = False
    ) -> dict[str, float]:
        """
        ...
        with self.tracer.start_as_current_span(
            ...
            try:
                ...
                if not is_valid and len(X_train) < 50:
                    self.logger.warning("training_data_validation_failed", issues=issues)
                ...
                if hyperparameter_tuning and len(X_train) >= 200:
                    model, best_params = self._tune_hyperparameters(X_train.values, y_train)
                    self.logger.info("hyperparameter_tuning_completed", best_params=best_params)
                else:
                ...
                min_precision = getattr(self.config, "ml_validation_precision_threshold", 0.75)
                if precision >= min_precision:
                    self.model = model
                    self.logger.info(
                        "model_updated_in_memory", precision=precision, threshold=min_precision
                    )
                    promoted = True
                ...
                self.metrics.record_ml_training(
                    model_type="anomaly", duration=training_duration, metrics=metrics
                )
    ...
    def _tune_hyperparameters(self, X_train: np.ndarray, y_train: np.ndarray) -> tuple:
        """
    ...
    async def _train_model_legacy(self, window_days: int, span) -> dict[str, float]:
        """
