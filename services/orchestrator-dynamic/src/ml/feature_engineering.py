...
def encode_risk_band(risk_band: str) -> float:
    ...
    """
    risk_weights = {"critical": 1.0, "high": 0.7, "medium": 0.5, "low": 0.3}
...
def encode_qos(qos: dict[str, Any]) -> dict[str, float]:
    ...
    """
    delivery_map = {"at_least_once": 0.5, "at_most_once": 0.6, "exactly_once": 1.0}

    consistency_map = {"eventual": 0.85, "strong": 1.0, "causal": 0.95}

    durability_map = {"ephemeral": 0.9, "transient": 0.9, "persistent": 1.0}
...
def extract_ticket_features(
    ticket: dict[str, Any], historical_stats: dict | None = None
) -> dict[str, float]:
    """
...
async def compute_historical_stats(
    mongodb_client, window_days: int = 30, clickhouse_client=None
    ...
    if clickhouse_client is not None:
        try:
            ...
            logger.warning("clickhouse_stats_failed_fallback_mongodb", error=str(e))
    ...
    try:
        ...
        pipeline = [
            ...
            {
                "$group": {
                    "_id": {"task_type": "$task_type", "risk_band": "$risk_band"},
...
async def compute_historical_stats_from_clickhouse(
    ...
    try:
        ...
        logger.info(
            "historical_stats_from_clickhouse", window_days=window_days, task_types=len(stats)
        )

        return stats

    except Exception as e:
        logger.exception("clickhouse_historical_stats_failed", error=str(e))
        return {}
