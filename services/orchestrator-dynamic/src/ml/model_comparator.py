...
class ModelComparator:
    ...
    def __init__(
        ...
        self.logger = logger or structlog.get_logger(__name__).bind(component="model_comparator")
        ...
        self.confidence_threshold = getattr(config, "ml_comparison_confidence_threshold", 0.7)
    ...
    async def compare_models(
        ...
        try:
            ...
            result.visualizations = self._generate_visualizations(
                y_test, current_preds, candidate_preds, model_type, current_model, candidate_model
            )
            ...
            if self.metrics:
                try:
                    self.metrics.ml_model_comparison_total.labels(
                        model_name=model_name, recommendation=result.recommendation
                    ).inc()
                    self.metrics.ml_model_comparison_duration_seconds.labels(
                        model_name=model_name
                    ).observe(duration)
                    self.metrics.ml_model_comparison_confidence.labels(model_name=model_name).set(
            ...
            self.logger.exception("model_comparison_failed", model_name=model_name, error=str(e))
            result.recommendation = "manual_review"
            result.recommendation_reason = f"Erro na comparação: {e!s}"
            result.confidence_score = 0.0
            return result

    async def _predict(
        self, model, X: np.ndarray, collect_latencies: bool = False
        ...
        try:
            ...
            self.logger.exception("prediction_failed", error=str(e))
            return None, None
    ...
    def _determine_model_type(self, model_name: str, y: np.ndarray) -> str:
        ...
        if len(unique_values) <= 10 and all(
            isinstance(v, (int, np.integer, bool)) for v in unique_values
        ):
            return "classification"
        return "regression"
    ...
    def _compare_metrics(
        ...
        if model_type == "regression":
            ...
            diff_pct = (
                ((candidate_rmse - current_rmse) / current_rmse * 100) if current_rmse > 0 else 0
            )
            ...
            diff_pct = (
                ((candidate_r2 - current_r2) / abs(current_r2) * 100) if current_r2 != 0 else 0
            )
            ...
            try:
                current_prec = precision_score(
                    y_true, current_preds, average=average, zero_division=0
                )
                candidate_prec = precision_score(
                    y_true, candidate_preds, average=average, zero_division=0
                )
                diff_pct = (
                    ((candidate_prec - current_prec) / current_prec * 100)
                    if current_prec > 0
                    else 0
                )
            ...
            try:
                ...
                candidate_rec = recall_score(
                    y_true, candidate_preds, average=average, zero_division=0
                )
                diff_pct = (
                    ((candidate_rec - current_rec) / current_rec * 100) if current_rec > 0 else 0
                )
    ...
    def _compare_latency_metrics(
        self, current_latencies: list[float], candidate_latencies: list[float]
    ) -> dict[str, dict[str, float]]:
        """
    ...
    def _mcnemar_test(
        self, y_true: np.ndarray, current_preds: np.ndarray, candidate_preds: np.ndarray
    ) -> dict[str, Any]:
        """
    ...
    def _fairness_analysis(
        ...
        metric_name = "accuracy" if model_type == "classification" else "mae"
        ...
        for group_field in group_fields:
            ...
            for group in unique_groups:
                ...
                if model_type == "classification":
                    ...
                    improvement = (
                        ((candidate_metric - current_metric) / current_metric * 100)
                        if current_metric > 0
                        else 0
                    )
                else:
                    ...
                    improvement = (
                        ((current_metric - candidate_metric) / current_metric * 100)
                        if current_metric > 0
                        else 0
                    )
            ...
            if group_metrics:
                ...
                metrics_values = [m[f"candidate_{metric_name}"] for m in group_metrics.values()]
    ...
    def _generate_feature_importance_diff(
        self, current_model: Any, candidate_model: Any, feature_names: list[str]
    ) -> str | None:
        """Gera gráfico de diferença de feature importance."""
        if not (
            hasattr(current_model, "feature_importances_")
            and hasattr(candidate_model, "feature_importances_")
        ):
            return None
    ...
    def _determine_recommendation(
        self, result: ComparisonResult, confidence_threshold: float | None = None
    ) -> None:
        """
        ...
        for metric, values in result.metrics_comparison.items():
            ...
            elif abs(values.get("diff_pct", 0)) >= self.degradation_threshold * 100:
                degraded_metrics += 1
        ...
        # Determinar recomendação
        improvement_ratio = improved_metrics / total_metrics if total_metrics > 0 else 0.5

        if result.confidence_score >= effective_threshold and improvement_ratio >= 0.6:
            result.recommendation = "promote"
            result.recommendation_reason = (
                "Candidato apresenta melhorias significativas. " + "; ".join(reasons[:3])
            )
        elif result.confidence_score < 0.3 or degraded_metrics >= total_metrics * 0.5:
            result.recommendation = "reject"
            result.recommendation_reason = (
                "Candidato apresenta degradação significativa. " + "; ".join(reasons[:3])
            )
        else:
            result.recommendation = "manual_review"
            result.recommendation_reason = (
                "Resultados inconclusivos, revisão manual recomendada. " + "; ".join(reasons[:3])
            )
...
<body>
    ...
    async def save_report_to_mlflow(
        self, html_report: str, model_name: str, run_id: str | None = None
    ) -> bool:
        """
        ...
        try:
            ...
            try:
                if run_id:
                    ...
                    with mlflow.start_run(run_id=run_id):
                        ...
                        mlflow.set_tag("comparison.timestamp", datetime.now(UTC).isoformat())
                else:
                    # Criar novo run
            ...
            self.logger.exception(
                "save_report_to_mlflow_failed", model_name=model_name, error=str(e)
            )
            return False
