...
_self_healing_client: SelfHealingClient | None = None
...
def _get_self_healing_client(config) -> SelfHealingClient | None:
...
def _extract_affected_tickets(tickets_context: list[dict[str, Any]] | None) -> list[str]:
...
def _extract_worker_map(tickets_context: list[dict[str, Any]] | None) -> dict[str, str]:
...
def _extract_worker_from_inconsistencies(inconsistencies: list[str]) -> str | None:
...
def _extract_sla_details(
    tickets_context: list[dict[str, Any]] | None, workflow_result: dict[str, Any] | None
) -> dict[str, Any]:
    ...
    if tickets_context:
        for t in tickets_context:
            ...
            actual_duration_ms = ticket.get("actual_duration_ms") or ticket.get(
    ...
    return {"violation_type": violation_type, "delay_ms": int(delay_ms)}
...
def _recommend_playbook(incident_type: str, inconsistencies: list[str] | None = None) -> str | None:
...
def compute_and_record_ml_error(ticket: dict[str, Any], metrics) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.warning("ml_accuracy_record_failed", ticket_id=ticket_id, error=str(metric_err))

    except Exception as e:
        # Fail-open: log erro mas não bloqueia
        logger.warning(
            "ml_error_tracking_failed", ticket_id=ticket.get("ticket_id", "unknown"), error=str(e)
        )


async def record_allocation_outcome_for_ticket(
    ticket: dict[str, Any], scheduling_optimizer: SchedulingOptimizer
) -> None:
    """
    ...
    try:
        ...
        worker = {
            ...
            "ml_enriched": allocation_metadata.get("ml_scheduling_enriched")
            or allocation_metadata.get("ml_enriched", False),
        }

        await scheduling_optimizer.record_allocation_outcome(
            ticket=ticket, worker=worker, actual_duration_ms=actual_duration_ms
        )
...
async def consolidate_results(tickets: list[dict[str, Any]], workflow_id: str) -> dict[str, Any]:
    ...
    """
    logger.info(f"Consolidando resultados do workflow {workflow_id}")

    try:
        global _config
        # Calcular métricas
        total_tickets = len(tickets)
        successful_tickets = sum(
            1 for t in tickets if t.get("ticket", {}).get("status") == "COMPLETED"
        )
        failed_tickets = sum(1 for t in tickets if t.get("ticket", {}).get("status") == "FAILED")
        compensated_tickets = sum(
            1 for t in tickets if t.get("ticket", {}).get("status") == "COMPENSATED"
        )

        # Calcular duração total (estimada, pois tickets ainda não foram executados)
        total_duration_ms = sum(
            t.get("ticket", {}).get("estimated_duration_ms", 0) for t in tickets
        )

        # Calcular total de retries
        total_retries = sum(t.get("ticket", {}).get("retry_count", 0) for t in tickets)
        ...
        for ticket_data in tickets:
            ...
            if status not in [
                ...
                integrity_errors.append(
                    f'Status inválido para ticket {ticket.get("ticket_id")}: {status}'
                )
        ...
        try:
            ...
            logger.warning(f"metrics_unavailable workflow_id={workflow_id} error={metrics_error}")
        ...
        try:
            ...
            for ticket_data in tickets:
                ...
                if (
                    ...
                    and ticket.get("actual_duration_ms") is not None
                ):
                    if metrics:
                    ...
                    if _scheduling_optimizer and _config and _config.ml_allocation_outcomes_enabled:
                        try:
                            await record_allocation_outcome_for_ticket(
                                ticket, _scheduling_optimizer
                            )
                            logger.info(
            ...
            if config.sla_management_enabled:
                ...
                if metrics is None:
                    logger.warning(f"sla_monitoring_skipped_no_metrics workflow_id={workflow_id}")
                else:
                    ...
                    sla_status["deadline_approaching"] = sla_check_result.get(
                        "deadline_approaching", False
                    )
                            ...
                            if deadline_check:
                                ...
                                try:
                                    await alert_manager.send_deadline_alert(
                                        workflow_id, ticket_id, deadline_data
                                    )
                                except Exception as e:
                                    logger.warning(
                    ...
                    if is_critical and budget_data:
                        ...
                        try:
                            await alert_manager.send_budget_alert(
                                workflow_id, service_name, budget_data
                            )
                        except Exception as e:
                            logger.warning(
                        ...
                        if ticket.get("status") in ["FAILED", "COMPENSATED"]:
                            ...
                            if timeout_ms > 0 and actual_duration_ms > timeout_ms:
                                ...
                                if completed_at:
                                    ...
                                    actual_completion = (
                                        ticket.get("created_at", 0) + actual_duration_ms
                                    )
            ...
            logger.exception(f"sla_monitoring_failed workflow_id={workflow_id} error={e}")
        ...
        try:
            if _mongodb_client:
                ...
                logger.info(f"workflow_result_persisted workflow_id={workflow_id}")
            else:
                logger.warning(f"mongodb_client_not_initialized workflow_id={workflow_id}")
        except CircuitBreakerError:
            logger.warning(f"workflow_result_persist_circuit_open workflow_id={workflow_id}")
        ...
        logger.error(f"Erro ao consolidar resultados: {e}", exc_info=True)
        raise
...
async def trigger_self_healing(
    ...
    tickets_context: list[dict[str, Any]] | None = None,
    ...
    try:
        ...
        logger.exception(f"self_healing.config_load_failed error={cfg_error}")
        raise
    ...
    if config.self_healing_enabled and recommended_playbook:
        ...
        if client:
            namespace = (
                getattr(config, "kubernetes_namespace", "default")
                if hasattr(config, "kubernetes_namespace")
                else "default"
            )
            ...
            if incident_type == "TICKET_TIMEOUT":
                ...
                worker_id = (
                    worker_map.get(ticket_id)
                    or _first_non_empty(list(worker_map.values()))
                    or _extract_worker_from_inconsistencies(inconsistencies)
                    or "unknown-worker"
                )
                parameters.update(
                    {
                ...
                worker_id = (
                    _first_non_empty(list(worker_map.values()))
                    or _extract_worker_from_inconsistencies(inconsistencies)
                    or "unknown-worker"
                )
                parameters.update(
    ...
    try:
        # Criar Telemetry Frame com métricas de SLA
        {
            "correlation": {
                ...
                "ticket_ids": [t["ticket_id"] for t in workflow_result.get("tickets_summary", [])],
            },
            "metrics": {
                **workflow_result.get("metrics", {}),
                "sla_metrics": {
                    "deadline_approaching": workflow_result.get("sla_status", {}).get(
                        "deadline_approaching", False
                    ),
                    "budget_critical": workflow_result.get("sla_status", {}).get(
                        "budget_critical", False
                    ),
                    "violations_count": workflow_result.get("sla_status", {}).get(
                        "violations_count", 0
                    ),
        ...
        try:
            ...
            if sla_status.get("violations_count", 0) > 0:
                ...
                for _ in range(sla_status["violations_count"]):
                    ...
                    )  # Placeholder para risk_band e task_type

            if sla_status.get("deadline_approaching"):
                metrics.record_deadline_approaching()

        except Exception as e:
            logger.warning(f"metrics_export_failed error={e}")
        ...
        logger.info("Telemetria publicada com sucesso")

    except Exception as e:
        logger.error(f"Erro ao publicar telemetria: {e}", exc_info=True)
        raise
...
async def buffer_telemetry(telemetry_frame: dict[str, Any]) -> None:
    ...
    """
    logger.warning("Buffering telemetria devido a falha de envio")
    ...
    try:
        ...
        if _mongodb_client:
            try:
                ...
                logger.exception(f"telemetry_buffer_persist_failed error={mongo_error}")
        else:
            logger.warning("mongodb_client_not_initialized")

        logger.info("Telemetria buffered com sucesso")

    except Exception as e:
        logger.error(f"Erro ao buffer telemetria: {e}", exc_info=True)
        raise
