...
class SagaRetryConfig(BaseModel):
    ...
    max_attempts: int = Field(
        ...
        ge=1,
        le=10,
        description='Numero maximo de tentativas (1-10)'
    )
    initial_delay_ms: int = Field(
        default=1000,
        ge=100,
        le=60000,
        description='Atraso inicial em milissegundos'
    )
    max_delay_ms: int = Field(
        default=30000,
        ge=1000,
        le=300000,
        description='Atraso maximo em milissegundos'
    )
    ...
    jitter: bool = Field(
        default=True,
        description='Aplicar jitter para evitar thundering herd'
    )
    ...
    non_retryable_errors: Set[str] = Field(
        default_factory=lambda: NON_RETRYABLE_ERRORS.copy(),
    ...
    def max_delay_must_be_gte_initial(cls, v: int, info) -> int:
        ...
        if 'initial_delay_ms' in info.data and v < info.data['initial_delay_ms']:
            raise ValueError(
                'max_delay_ms deve ser maior ou igual a initial_delay_ms'
            )
        return v
