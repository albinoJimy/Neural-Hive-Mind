...
class MongoDBClient:
    ...
    EXPECTED_INDEXES = {
        ...
        "cognitive_ledger": ["plan_id_1", "intent_id_1", "created_at_1", "status_1_created_at_-1"],
        "workflows": ["workflow_id_1", "plan_id_1", "status_1"],
        "validation_audit": ["plan_id_1", "workflow_id_1", "timestamp_1", "plan_id_1_timestamp_-1"],
        "workflow_results": [
            "workflow_id_1",
            "plan_id_1",
            "status_1",
    ...
    def __init__(self, config, uri_override: str | None = None):
        """
    ...
    async def ensure_collection_exists(self, collection_name: str) -> bool:
        ...
        try:
            ...
            logger.exception("collection_creation_failed", collection=collection_name, error=str(e))
            raise
        ...
        for collection_name, expected_indexes in self.EXPECTED_INDEXES.items():
            try:
                ...
                if missing:
                    ...
                    metrics.record_mongodb_index_validation(
                        collection_name, "missing", len(missing)
                    )
                else:
                    logger.info(
                        "indexes_validated", collection=collection_name, count=len(expected_indexes)
                    )
                    metrics.record_mongodb_index_validation(
                        collection_name, "validated", len(expected_indexes)
                    )

            except Exception as e:
                logger.exception(
                    "index_validation_failed", collection=collection_name, error=str(e)
                )
    ...
    async def get_cognitive_plan(self, plan_id: str) -> dict[str, Any] | None:
        """
    ...
    async def save_execution_ticket(self, ticket: dict[str, Any]):
        ...
        async def _persist_with_retry():
            await self._execute_with_breaker(
                self.execution_ticket_breaker, self.execution_tickets.insert_one, document
            )
        ...
        try:
            ...
            try:
                ...
                duration = (datetime.now() - start_time).total_seconds()
                metrics.record_mongodb_persistence_duration("execution_tickets", "update", duration)

                logger.info("execution_ticket_updated", ticket_id=ticket_id, plan_id=plan_id)
            except CircuitBreakerError:
                metrics.record_mongodb_persistence_error(
                    "execution_tickets", "update", "CircuitBreakerError"
                )
                logger.exception(
                    "execution_ticket_update_circuit_open", ticket_id=ticket_id, plan_id=plan_id
                )
                raise
            except Exception as e:
                metrics.record_mongodb_persistence_error(
                    "execution_tickets", "update", type(e).__name__
                )
                logger.exception(
            ...
            metrics.record_mongodb_persistence_error(
                "execution_tickets", "insert", "CircuitBreakerError"
            )
            logger.exception("execution_ticket_circuit_open", ticket_id=ticket_id, plan_id=plan_id)
            raise
            ...
            if fail_open:
                ...
                raise RuntimeError(f"Falha crítica na persistência do ticket {ticket_id}: {e!s}")
    ...
    async def update_ticket_status(self, ticket_id: str, status: str, **kwargs):
        ...
        await self.execution_tickets.update_one({"ticket_id": ticket_id}, {"$set": update_fields})
    ...
    async def update_ticket_compensation(
        ...
        try:
            ...
            logger.warning("ticket_compensation_update_failed", ticket_id=ticket_id, error=str(e))
    ...
    async def save_validation_audit(
        self, plan_id: str, validation_result: dict[str, Any], workflow_id: str
    ) -> None:
        """
        ...
        async def _persist():
            await self._execute_with_breaker(
                self.validation_audit_breaker, self.validation_audit.insert_one, document
            )

        try:
            await _persist()
            logger.info("validation_audit_saved", plan_id=plan_id, workflow_id=workflow_id)
        except Exception as e:
            logger.exception(
            ...
            logger.warning(
                "validation_audit_circuit_open", plan_id=plan_id, workflow_id=workflow_id
            )
            raise
    ...
    async def save_workflow_result(self, workflow_result: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.exception("workflow_result_save_failed", workflow_id=workflow_id, error=str(e))
        except CircuitBreakerError:
            logger.warning("workflow_result_circuit_open", workflow_id=workflow_id)
            raise
    ...
    async def save_incident(self, incident_event: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.exception(
                "incident_save_failed", workflow_id=incident_event.get("workflow_id"), error=str(e)
            )
