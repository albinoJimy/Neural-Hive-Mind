...
grpc_jwt_auth_failures_total = Counter(
    "grpc_jwt_auth_failures_total", "Falhas de autenticacao JWT-SVID", ["service", "reason"]
)
...
class ServiceRegistryClient:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.info("grpc_channel_ready", target=target, secure=use_mtls)

        except Exception as e:
            self.logger.exception("grpc_initialization_error", error=str(e), target=target)
            raise

    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=0.5, max=2))
    async def discover_agents(
        self, capabilities: list[str], filters: dict, max_results: int = 5
    ) -> list[dict]:
        """
        ...
        try:
            ...
            request = service_registry_pb2.DiscoverRequest(
                capabilities=capabilities, filters=proto_filters, max_results=max_results
            )
            ...
            if self.spiffe_manager and self.config.spiffe_enabled:
                try:
                    ...
                    audience = (
                        self.config.spiffe_jwt_audience or "service-registry.neural-hive.local"
                    )
                    ...
                    grpc_jwt_auth_attempts_total.labels(
                        service="service_registry", status="success"
                    ).inc()

                    self.logger.debug(
                        "jwt_svid_attached", audience=audience, spiffe_id=jwt_svid.spiffe_id
                    )
                    ...
                    grpc_jwt_auth_attempts_total.labels(
                        service="service_registry", status="error"
                    ).inc()
                    grpc_jwt_auth_failures_total.labels(
                        service="service_registry", reason="fetch_failed"
                    ).inc()
                    ...
                    if isinstance(e, SPIFFEFetchError):
                        ...
                        grpc_jwt_auth_attempts_total.labels(
                            service="service_registry", status="fallback"
                        ).inc()
                    else:
            ...
            self.logger.info("agents_discovered", count=len(agents), capabilities=capabilities)
            ...
            if e.code() == grpc.StatusCode.UNAUTHENTICATED:
                grpc_jwt_auth_failures_total.labels(
                    service="service_registry", reason="unauthenticated"
                ).inc()
                self.logger.exception(
                ...
                grpc_jwt_auth_failures_total.labels(
                    service="service_registry", reason="permission_denied"
                ).inc()
                self.logger.exception(
                ...
                self.logger.exception(
                    "grpc_discovery_error", error=str(e), code=e.code(), details=e.details()
                )
            raise

        except Exception as e:
            self.logger.exception("discovery_error", error=str(e), error_type=type(e).__name__)
            raise
