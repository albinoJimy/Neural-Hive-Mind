...
class OptimizerGrpcClient:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.channel = instrument_grpc_channel(
                self.channel, service_name="orchestrator-dynamic", target_service="optimizer-agents"
            )
            ...
            self.logger.exception(
                "optimizer_grpc_initialization_error", error=str(e), target=target
            )
            # Não falhar - permitir que orchestrator funcione sem optimizer
            self.stub = None
    ...
    async def get_load_forecast(
        self, horizon_minutes: int, include_confidence_intervals: bool = False
    ) -> dict | None:
        """
        ...
        self.logger.info("requesting_load_forecast", horizon_minutes=horizon_minutes)
        ...
        try:
            ...
            self.logger.info("load_forecast_received", points=len(forecast["forecast"]))
            ...
            self.logger.exception("load_forecast_rpc_error", code=e.code(), details=e.details())
            return None
        except Exception as e:
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=0.5, max=2))
    async def get_scheduling_recommendation(self, current_state: dict) -> dict | None:
        """
        ...
        try:
            ...
            jwt_metadata = await self._get_grpc_metadata()
            metadata = inject_context_to_metadata(jwt_metadata)
            response = await self.stub.GetSchedulingRecommendation(
            ...
            self.logger.exception(
                "scheduling_recommendation_rpc_error", code=e.code(), details=e.details()
            )
            return None
        except Exception as e:
            self.logger.exception("scheduling_recommendation_error", error=str(e))
            return None
    ...
    async def close(self):
        ...
        if self.channel:
            try:
                ...
                self.logger.exception("optimizer_channel_close_error", error=str(e))
        ...
        if self.spiffe_manager:
            try:
                ...
                self.logger.exception("spiffe_manager_close_error", error=str(e))
