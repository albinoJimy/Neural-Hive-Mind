...
class AffinityTracker:
    ...
    def __init__(self, redis_client, config: OrchestratorSettings, metrics: OrchestratorMetrics):
        """
        ...
        self.cache_ttl_seconds = getattr(config, "scheduler_affinity_cache_ttl_seconds", 14400)
    ...
    async def get_plan_allocations(self, plan_id: str | None) -> dict[str, int]:
        """
        ...
        try:
            ...
            self.logger.warning("plan_allocations_fetch_failed", plan_id=plan_id, error=str(e))
            self.metrics.record_affinity_cache_operation("get_plan", "failure")
            return {}

    async def get_intent_allocations(self, intent_id: str | None) -> dict[str, int]:
        """
        ...
        try:
            ...
            self.logger.warning(
                "intent_allocations_fetch_failed", intent_id=intent_id, error=str(e)
            )
    ...
    async def get_critical_tickets_on_worker(self, worker_id: str) -> set[str]:
        ...
        try:
            ...
            self.logger.warning("critical_tickets_fetch_failed", worker_id=worker_id, error=str(e))
    ...
    async def record_allocation(self, ticket: dict, worker_id: str) -> bool:
        ...
        try:
            ...
            is_critical = risk_band.lower() in [
                rb.lower() for rb in self.anti_affinity_risk_bands
            ] or priority.upper() in [p.upper() for p in self.anti_affinity_priorities]
            ...
            self.logger.warning(
                "allocation_record_failed", ticket_id=ticket_id, worker_id=worker_id, error=str(e)
            )
            self.metrics.record_affinity_cache_operation("record", "failure")
            return False

    async def cleanup_completed_ticket(
        self, ticket_id: str, plan_id: str | None, intent_id: str | None, worker_id: str
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.warning(
                "ticket_cleanup_failed", ticket_id=ticket_id, worker_id=worker_id, error=str(e)
            )
