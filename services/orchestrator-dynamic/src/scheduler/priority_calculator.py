...
class PriorityCalculator:
    ...
    RISK_WEIGHTS = {"critical": 1.0, "high": 0.7, "normal": 0.5, "low": 0.3}

    # Mapeamento de delivery_mode para peso base
    QOS_DELIVERY_WEIGHTS = {"EXACTLY_ONCE": 1.0, "AT_LEAST_ONCE": 0.7, "AT_MOST_ONCE": 0.5}

    # Ajuste de consistência
    QOS_CONSISTENCY_MULTIPLIER = {"STRONG": 1.0, "EVENTUAL": 0.85}

    # Ajuste de durabilidade
    QOS_DURABILITY_MULTIPLIER = {"PERSISTENT": 1.0, "TRANSIENT": 0.9, "EPHEMERAL": 0.8}
    ...
    def _calculate_risk_weight(self, risk_band: str) -> float:
        ...
        """
        return self.RISK_WEIGHTS.get(risk_band.lower(), 0.5)
    ...
    def _calculate_qos_weight(self, qos: dict) -> float:
        ...
        consistency_multiplier = self.QOS_CONSISTENCY_MULTIPLIER.get(consistency, 0.85)

        # Ajustar por durabilidade
        durability_multiplier = self.QOS_DURABILITY_MULTIPLIER.get(durability, 1.0)
        ...
        if isinstance(value, str):
            try:
                ...
                try:
                    ...
                    self.logger.warning(
                        "invalid_timestamp_format", value=value, using_current_time=True
                    )
    ...
    def apply_adaptive_adjustment(self, base_priority: float, adaptive_adjustment: float) -> float:
        """
