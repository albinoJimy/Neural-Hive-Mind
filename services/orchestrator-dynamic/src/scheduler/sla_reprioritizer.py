...
class SLARePrioritizer:
    ...
    def __init__(self, reprioritizer: RePrioritizer, queue_manager: QueueManager):
        """
    ...
    async def on_sla_warning(self, event: dict[str, Any]) -> dict[str, Any]:
        """
        ...
        if not self.reprioritizer.should_reprioritize_on_sla_warning(
            sla_urgency, deadline_remaining_pct
        ):
            self.logger.debug(
                "sla_warning_no_reprioritization", ticket_id=ticket_id, reason="thresholds_not_met"
            )
            return {"ticket_id": ticket_id, "action": "none", "reason": "thresholds_not_met"}

        # Determinar nova prioridade
        new_priority = self._calculate_sla_based_priority(sla_urgency, deadline_remaining_pct)
    ...
    async def on_risk_band_changed(self, event: dict[str, Any]) -> dict[str, Any]:
        """
    ...
    async def on_deadline_approaching(self, event: dict[str, Any]) -> dict[str, Any]:
        """
        ...
        if not new_priority:
            return {"ticket_id": ticket_id, "action": "none", "reason": "deadline_not_critical"}
    ...
    async def on_sla_breach(self, event: dict[str, Any]) -> dict[str, Any]:
        """
        ...
        self.logger.warning("sla_breach_occurred", ticket_id=ticket_id, breach_type=breach_type)
    ...
    def _calculate_sla_based_priority(
        ...
        if (
            sla_urgency > self.SLA_URGENCY_CRITICAL_THRESHOLD
            or deadline_remaining_pct < self.DEADLINE_REMAINING_CRITICAL_THRESHOLD
        ):
            return "CRITICAL"
    ...
    def _map_risk_band_to_priority(self, risk_band: str) -> str:
        ...
        Returns:
            Nível de prioridade
        """
        mapping = {"critical": "CRITICAL", "high": "HIGH", "normal": "NORMAL", "low": "LOW"}
