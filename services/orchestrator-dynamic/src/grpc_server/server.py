...
async def start_grpc_server(
    ...
    if enable_mtls and spiffe_manager and SECURITY_LIB_AVAILABLE:
        try:
            ...
            logger.info("grpc_server_mtls_configured", port=port, spiffe_id=x509_svid.spiffe_id)

        except Exception as e:
            logger.warning("grpc_server_mtls_failed_fallback_insecure", error=str(e), port=port)
