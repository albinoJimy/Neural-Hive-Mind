...
class OrchestratorStrategicServicer(orchestrator_strategic_pb2_grpc.OrchestratorStrategicServicer):
    ...
    async def _validate_with_opa(self, policy_path: str, input_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.exception("opa_evaluation_failed", policy=policy_path, error=str(e))
            return bool(self.config and getattr(self.config, "opa_fail_open", False))

    async def _audit_operation(
        self, operation: str, request_data: dict[str, Any], success: bool, message: str
    ):
        """
    ...
    async def AdjustPriorities(
        self, request: AdjustPrioritiesRequest, context: grpc.aio.ServicerContext
    ) -> AdjustPrioritiesResponse:
        """
        ...
        try:
            ...
            if not 1 <= request.new_priority <= 10:
                await context.abort(
                    grpc.StatusCode.INVALID_ARGUMENT, "new_priority deve estar entre 1 e 10"
                )
            ...
            if not await self._validate_with_opa(
                "neuralhive/orchestrator/strategic_adjustments", opa_input
            ):
            ...
            if self.temporal_client:
                try:
                    workflow_handle = self.temporal_client.get_workflow_handle(request.workflow_id)
            ...
            if self.kafka_producer:
                try:
                    ...
                    await self.kafka_producer.send_and_wait(
                        topic=getattr(
                            self.config, "KAFKA_TOPICS_STRATEGIC", "strategic.adjustments"
                        ),
            ...
            self.logger.exception("adjust_priorities_failed", error=str(e))
            self.metrics.grpc_requests_total.labels(method=method_name, status="error").inc()
            await context.abort(grpc.StatusCode.INTERNAL, str(e))

        finally:
            duration = time.time() - start_time
            self.metrics.grpc_request_duration_seconds.labels(method=method_name).observe(duration)

    async def RebalanceResources(
        self, request: RebalanceResourcesRequest, context: grpc.aio.ServicerContext
    ) -> RebalanceResourcesResponse:
        """
        ...
        try:
            ...
            if not await self._validate_with_opa(
                "neuralhive/orchestrator/resource_limits", opa_input
            ):
                self.metrics.grpc_requests_total.labels(
                    method=method_name, status="permission_denied"
                ).inc()
                ...
                if target:
                    ...
                    if self.scheduler:
                        ...
                        scheduler_result = await self.scheduler.reallocate_resources(
                            workflow_id=workflow_id, target_allocation=target_dict
                        )
                        self.logger.info(
                    ...
                    results.append(
                        WorkflowRebalanceResult(
                            ...
                            message=scheduler_result.get(
                                "message", "Recursos rebalanceados com sucesso"
                            ),
            ...
            self.logger.exception("rebalance_resources_failed", error=str(e))
            self.metrics.grpc_requests_total.labels(method=method_name, status="error").inc()
            await context.abort(grpc.StatusCode.INTERNAL, str(e))
    ...
    async def PauseWorkflow(
        self, request: PauseWorkflowRequest, context: grpc.aio.ServicerContext
    ) -> PauseWorkflowResponse:
        """
        ...
        try:
            ...
            if self.temporal_client:
                try:
                    ...
                    self.logger.info("temporal_pause_signal_sent", workflow_id=request.workflow_id)
                except Exception as temporal_error:
                    self.logger.warning(
            ...
            if self.mongodb_client:
                ...
                await self.mongodb_client.db.workflows.update_one(
                    {"workflow_id": request.workflow_id}, {"$set": update_data}, upsert=True
                )
            ...
            self.logger.exception("pause_workflow_failed", error=str(e))
            self.metrics.grpc_requests_total.labels(method=method_name, status="error").inc()
            await context.abort(grpc.StatusCode.INTERNAL, str(e))
    ...
    async def ResumeWorkflow(
        self, request: ResumeWorkflowRequest, context: grpc.aio.ServicerContext
    ) -> ResumeWorkflowResponse:
        """
        ...
        try:
            ...
            if self.temporal_client:
                try:
                    workflow_handle = self.temporal_client.get_workflow_handle(request.workflow_id)
                    await workflow_handle.signal(
                        "resume", {"reason": request.reason, "adjustment_id": request.adjustment_id}
                    )
                    self.logger.info("temporal_resume_signal_sent", workflow_id=request.workflow_id)
                except Exception as temporal_error:
                    self.logger.warning(
            ...
            if self.mongodb_client:
                await self.mongodb_client.db.workflows.update_one(
                    {"workflow_id": request.workflow_id},
                    {
                        ...
                        "$unset": {"paused_at": "", "pause_reason": "", "scheduled_resume_at": ""},
                    },
                )
            ...
            self.logger.exception("resume_workflow_failed", error=str(e))
            self.metrics.grpc_requests_total.labels(method=method_name, status="error").inc()
            await context.abort(grpc.StatusCode.INTERNAL, str(e))
    ...
    async def TriggerReplanning(
        self, request: TriggerReplanningRequest, context: grpc.aio.ServicerContext
    ) -> TriggerReplanningResponse:
        """
        ...
        try:
            ...
            if self.kafka_producer:
                ...
                await self.kafka_producer.produce(
                    topic="plans.replanning_requested", key=request.plan_id, value=replanning_event
                )
                self.logger.info(
            ...
            self.logger.exception("trigger_replanning_failed", error=str(e))
            self.metrics.grpc_requests_total.labels(method=method_name, status="error").inc()
            await context.abort(grpc.StatusCode.INTERNAL, str(e))
    ...
    async def GetWorkflowStatus(
        self, request: GetWorkflowStatusRequest, context: grpc.aio.ServicerContext
    ) -> GetWorkflowStatusResponse:
        """
        ...
        try:
            ...
            if self.mongodb_client:
                workflow_data = (
                    await self.mongodb_client.db.workflows.find_one(
                        {"workflow_id": request.workflow_id}
                    )
                    or {}
                )
            ...
            current_state = state_map.get(
                workflow_data.get("state", "PENDING"), WorkflowState.WORKFLOW_STATE_UNSPECIFIED
            )
            ...
            if request.include_history and self.mongodb_client:
                events_cursor = (
                    self.mongodb_client.db.workflow_events.find(
                        {"workflow_id": request.workflow_id}
                    )
                    .sort("timestamp", -1)
                    .limit(50)
            ...
            self.logger.exception("get_workflow_status_failed", error=str(e))
            self.metrics.grpc_requests_total.labels(method=method_name, status="error").inc()
            await context.abort(grpc.StatusCode.INTERNAL, str(e))
