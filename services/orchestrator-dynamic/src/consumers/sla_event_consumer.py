...
class SLAEventConsumer:
    ...
    def __init__(
        self,
        config,
        sla_reprioritizer: SLARePrioritizer,
        queue_manager,
        metrics=None
    ):
        """
    ...
    async def initialize(self):
        ...
        consumer_config = {
            ...
            'group_id': getattr(
                self.config,
                'sla_event_consumer_group',
                'orchestrator-sla-events'
            ),
    ...
    async def _process_event(self, message):
        ...
        try:
            ...
            if not self.sla_reprioritizer.should_reprioritize_on_sla_event(
                event_type,
                event_data
            ):
                logger.debug(
                    'sla_event_no_reprioritization',
                    event_type=event_type,
                    ticket_id=ticket_id
                )
                await self.consumer.commit()
    ...
    def _deserialize(self, message) -> Dict[str, Any]:
        ...
        if isinstance(raw_value, bytes):
            try:
                ...
                logger.error(
                    'sla_event_deserialization_failed',
                    error=str(e)
                )
