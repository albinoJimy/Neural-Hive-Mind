...
class InsightsConsumer:
    ...
    async def initialize(self):
        ...
        if self.security_protocol and self.security_protocol != 'PLAINTEXT':
            ...
            logger.info(
                'InsightsConsumer configurado com SASL',
    ...
    async def _process_message(self, message):
        ...
        if isinstance(raw_value, bytes):
            try:
                ...
                logger.error('falha_deserializar_insight', error=str(e))
                return
        else:
            insight = raw_value
        ...
        if priority not in ['HIGH', 'CRITICAL']:
            logger.debug('insight_filtrado_baixa_prioridade', insight_id=insight_id, priority=priority)
            return
        ...
        # Atualizar métricas
        if self.metrics:
            self.metrics.insights_consumed_total.labels(
                type=insight_type,
                priority=priority
            ).inc()
