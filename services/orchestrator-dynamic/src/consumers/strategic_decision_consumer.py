...
class StrategicDecisionConsumer:
    ...
    def __init__(
        ...
        self.running = False
        self.sasl_username = sasl_username_override if sasl_username_override is not None else getattr(config, 'kafka_sasl_username', None)
        self.sasl_password = sasl_password_override if sasl_password_override is not None else getattr(config, 'kafka_sasl_password', None)
    ...
    async def initialize(self):
        ...
        if self.security_protocol and self.security_protocol != 'PLAINTEXT':
            ...
            logger.info(
                'StrategicDecisionConsumer configurado com SASL',
                mechanism=self.sasl_mechanism
            )

        self.consumer = instrument_kafka_consumer(
            AIOKafkaConsumer(topic, **consumer_config)
        )
    ...
    async def _process_message(self, message):
        ...
        if isinstance(raw_value, bytes):
            try:
                ...
                logger.error('falha_deserializar_decisao', error=str(e))
                return
        else:
            decision = raw_value
        ...
        # Atualizar métricas
        if self.metrics:
            self.metrics.strategic_decisions_consumed_total.labels(
                type=decision_type
            ).inc()
    ...
    async def _apply_cancellation(self, plan_id: str, decision: Dict[str, Any], current_status: str) -> None:
        ...
        if self.mongodb_client:
            await self.mongodb_client.update_cognitive_plan(
                ...
                updates={
                    ...
                    'cancellation_reason': decision.get('parameters', {}).get('reason', 'Strategic decision')
                }
            )
    ...
    async def _apply_workflow_adjustment(self, plan_id: str, decision: Dict[str, Any]) -> None:
        ...
        if self.mongodb_client:
            ...
            await self.mongodb_client.update_cognitive_plan(
                plan_id=plan_id,
                updates={'workflow_adjustments': existing_adjustments}
            )
    ...
    async def _apply_resource_reallocation(self, plan_id: str, decision: Dict[str, Any]) -> None:
        ...
        if self.mongodb_client:
            await self.mongodb_client.update_cognitive_plan(
                plan_id=plan_id,
                updates={'resource_allocation': resources}
            )
    ...
    async def _store_decision(self, decision: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(
                'decisao_estrategica_armazenada',
