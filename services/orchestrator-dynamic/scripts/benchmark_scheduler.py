...
async def run_benchmarks(
    iterations: int, tickets_per_iteration: int, concurrency: int, discovery_latency_ms: float
) -> tuple:
...
def print_report(results: List[BenchmarkResult], summary: BenchmarkSummary):
    ...
    print(
        f'  {"Iter":<6} {"Tickets":<10} {"Throughput":<15} {"P95 (ms)":<12} {"P99 (ms)":<12} {"Cache Hit":<12}'
    )
    print("  " + "-" * 67)

    for r in results:
        print(
            f"  {r.iteration:<6} {r.tickets_count:<10} {r.throughput:<15.2f} "
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark do IntelligentScheduler")
    parser.add_argument(
        "--iterations", "-i", type=int, default=5, help="Número de iterações (default: 5)"
    )
    parser.add_argument(
        "--tickets", "-t", type=int, default=500, help="Tickets por iteração (default: 500)"
    )
    parser.add_argument(
        "--concurrency", "-c", type=int, default=50, help="Concorrência (default: 50)"
    )
    parser.add_argument(
        "--discovery-latency",
        "-d",
    ...
    parser.add_argument("--output", "-o", type=str, default=None, help="Arquivo de saída JSON")
