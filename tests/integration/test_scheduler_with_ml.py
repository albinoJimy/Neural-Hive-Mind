...
async def test_scheduler_enriches_ticket_with_predictions():
    ...
    scheduling_predictor.predict_duration = AsyncMock(
        return_value={"predicted_duration_ms": 5000, "confidence": 0.85, "model_type": "xgboost"}
    )
    scheduling_predictor.predict_resources = AsyncMock(
        return_value={"cpu_cores": 2.0, "memory_mb": 1024, "confidence": 0.8}
    )

    anomaly_detector = AsyncMock()
    anomaly_detector.detect_anomaly = AsyncMock(
        return_value={
            "is_anomaly": False,
            "anomaly_score": 0.1,
            "anomaly_type": None,
    ...
    resource_allocator.discover_workers = AsyncMock(
        return_value=[{"agent_id": "worker-1", "agent_type": "worker-agent", "score": 0.8}]
    )
    resource_allocator.select_best_worker = AsyncMock(
        return_value={"agent_id": "worker-1", "agent_type": "worker-agent", "score": 0.8}
    )
