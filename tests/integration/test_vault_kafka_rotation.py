...
async def test_kafka_credentials_renewal_at_threshold(mock_config, mock_vault_client):
    ...
    vault_integration._kafka_credentials_expiry = datetime.now(timezone.utc) + timedelta(seconds=19)
...
async def test_kafka_credentials_no_renewal_before_threshold(mock_config, mock_vault_client):
    ...
    vault_integration._kafka_credentials_expiry = datetime.now(timezone.utc) + timedelta(seconds=50)
...
async def test_kafka_credentials_renewal_on_expiry(mock_config, mock_vault_client):
    ...
    vault_integration._kafka_credentials_expiry = datetime.now(timezone.utc) - timedelta(seconds=10)
