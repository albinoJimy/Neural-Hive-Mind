...
try:
    from ml.model_audit_logger import ModelAuditLogger, AuditEventContext, ModelLifecycleEvent
except ImportError:
...
async def audit_logger(mongodb_ml_client, clean_ml_collections):
    ...
    logger = ModelAuditLogger(mongodb_client=mongodb_ml_client, config=config, metrics=mock_metrics)
...
async def audit_logger_disabled(mongodb_ml_client):
    ...
    logger = ModelAuditLogger(mongodb_client=mongodb_ml_client, config=config, metrics=None)
...
class TestAuditLoggerBasicOperations:
    ...
    async def test_log_training_started(self, audit_logger, mongodb_ml_client, event_loop):
        """
        ...
        # Verifica no MongoDB
        event = await mongodb_ml_client.db["model_audit_log"].find_one({"audit_id": audit_id})
        ...
        audit_logger.metrics.increment_model_audit_event.assert_called_with(
            model_name=model_name, event_type="training_started"
        )

    async def test_log_training_completed(self, audit_logger, mongodb_ml_client):
        """
        ...
        model_name = "duration_predictor"
        model_version = "v2.0"
        training_metrics = {"mae_percentage": 8.5, "precision": 0.85, "training_samples": 10000}
        context = AuditEventContext(duration_seconds=3600.0, metadata={"mlflow_run_id": "run_456"})
    ...
    async def test_log_promotion_initiated(self, audit_logger, mongodb_ml_client):
        """
        ...
        context = AuditEventContext(user_id="ml_engineer", triggered_by="manual")
        promotion_config = {
    ...
    async def test_log_shadow_mode_started(self, audit_logger, mongodb_ml_client):
        """
    ...
    async def test_log_shadow_mode_completed(self, audit_logger, mongodb_ml_client):
        """
        ...
        model_name = "duration_predictor"
        shadow_version = "v2.0"
        shadow_results = {"prediction_count": 150, "agreement_rate": 0.947, "avg_latency_ms": 5.2}
        context = AuditEventContext(duration_seconds=3660.0)
    ...
    async def test_log_rollback_executed(self, audit_logger, mongodb_ml_client):
        """
    ...
    async def test_log_model_promoted(self, audit_logger, mongodb_ml_client):
        """
    ...
    async def test_audit_logger_disabled_returns_empty(self, audit_logger_disabled):
        """
...
class TestAuditLoggerQueries:
    ...
    async def test_get_model_history(self, audit_logger, mongodb_ml_client):
        """
        ...
        await audit_logger.log_training_started(
            model_name=model_name, model_version="v2.0", context=AuditEventContext()
        )
        ...
        # Act
        events = await audit_logger.get_model_history(model_name=model_name, limit=10)
    ...
    async def test_get_model_history_with_event_type_filter(self, audit_logger, mongodb_ml_client):
        """
        ...
        await audit_logger.log_training_completed(
            model_name=model_name, model_version="v2.0", metrics={}, context=AuditEventContext()
        )
        ...
        events = await audit_logger.get_model_history(
            model_name=model_name, event_types=["training_started", "training_completed"], limit=10
        )
    ...
    async def test_get_events_by_type(self, audit_logger, mongodb_ml_client):
        """
        ...
        await audit_logger.log_training_started(
            model_name="model_a", model_version="v3.0", context=AuditEventContext()
        )

        # Act
        rollback_events = await audit_logger.get_events_by_type(
            event_type="rollback_executed", limit=10
        )
    ...
    async def test_get_events_by_type_with_date_range(self, audit_logger, mongodb_ml_client):
        """
        ...
        await audit_logger.log_training_started(
            model_name="model_a", model_version="v1.0", context=AuditEventContext()
        )
    ...
    async def test_get_audit_summary(self, audit_logger, mongodb_ml_client):
        """
        ...
        # Act
        summary = await audit_logger.get_audit_summary(model_name=model_name, days=30)
    ...
    async def test_get_event_by_id(self, audit_logger, mongodb_ml_client):
        """
    ...
    async def test_get_event_by_id_not_found(self, audit_logger):
        """
    ...
    async def test_get_model_timeline(self, audit_logger, mongodb_ml_client):
        """
        ...
        # Eventos para v1.0
        await audit_logger.log_training_started(
            model_name=model_name, model_version="v1.0", context=AuditEventContext()
        )
        ...
        # Act
        timeline = await audit_logger.get_model_timeline(model_name=model_name, days=90)
...
class TestAuditLoggerIntegration:
    ...
    async def test_audit_logger_handles_mongodb_errors_gracefully(self, mongodb_ml_client):
        """
        ...
        logger = ModelAuditLogger(mongodb_client=broken_client, config=config, metrics=None)
    ...
    async def test_multiple_concurrent_audit_events(self, audit_logger, mongodb_ml_client):
        """
        ...
        for i in range(num_events):
            tasks.append(
                audit_logger.log_training_started(
                    ...
                    context=AuditEventContext(metadata={"batch_index": i}),
                )
            )
    ...
    async def test_audit_event_context_serialization(self, audit_logger, mongodb_ml_client):
        """
        ...
        context = AuditEventContext(
            ...
            metadata={"nested": {"key": "value"}, "array": [1, 2, 3], "float": 3.14},
        )

        # Act
        audit_id = await audit_logger.log_training_started(
            model_name="test_model", model_version="v1.0", context=context
        )
