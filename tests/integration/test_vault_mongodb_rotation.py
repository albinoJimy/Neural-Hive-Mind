"""
Testes de integração para rotação de credenciais MongoDB via Vault
"""
...
def mock_config():
    ...
    config.mongodb_uri = "mongodb://static_user:static_pass@localhost:27017/admin"
...
@pytest.mark.asyncio
async def test_mongodb_credentials_renewal_at_threshold(mock_config, mock_vault_client):
    """
    Testa renovação automática de credenciais MongoDB quando threshold atingido
    """
    ...
    initial_creds = await vault_integration.get_mongodb_credentials()
    ...
    assert vault_integration._mongodb_credentials["username"] == "mongo_user2"
    assert mock_vault_client.get_database_credentials.call_count == 2


@pytest.mark.asyncio
async def test_mongodb_credentials_no_renewal_before_threshold(mock_config, mock_vault_client):
    """
    Testa que credenciais MongoDB NÃO são renovadas antes do threshold
    """
    OrchestratorVaultClient = get_vault_integration_class()
...
@pytest.mark.asyncio
async def test_mongodb_credentials_renewal_on_expiry(mock_config, mock_vault_client):
    """
    Testa renovação imediata de credenciais MongoDB quando expiradas
    """
    OrchestratorVaultClient = get_vault_integration_class()
...
@pytest.mark.asyncio
async def test_mongodb_credentials_fallback_when_vault_disabled(mock_config):
    """
    ...
    # Buscar credenciais
    creds = await vault_integration.get_mongodb_credentials()

    # Verificar que retornou credenciais extraídas da URI
    assert creds["username"] == "static_user"
    assert creds["ttl"] == 0


@pytest.mark.asyncio
async def test_mongodb_credentials_fail_open_on_error(mock_config, mock_vault_client):
    """
...
@pytest.mark.asyncio
async def test_mongodb_credentials_fail_closed_on_error(mock_config, mock_vault_client):
    """
...
@pytest.mark.asyncio
async def test_mongodb_credentials_parse_uri_fallback(mock_config):
    """
    ...
    vault_integration = OrchestratorVaultClient(mock_config)
    vault_integration.vault_client = None
    vault_integration.config = mock_config

    # Buscar credenciais
    creds = await vault_integration.get_mongodb_credentials()

    # Verificar que username foi extraído corretamente
