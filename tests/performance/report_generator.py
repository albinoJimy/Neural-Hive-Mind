...
class PerformanceReportGenerator:
    ...
    def generate_executive_summary(self) -> str:
        ...
        lines.append(
            f"- {throughput_emoji} **Throughput:** {metrics.tickets_per_second:.2f} tickets/s (target: >= 10)"
        )

        if metrics.latency_p95_seconds:
            lines.append(
                f"- {latency_emoji} **Latencia P95:** {metrics.latency_p95_seconds/3600:.2f}h (target: < 4h)"
            )

        lines.append(
            f"- {success_emoji} **Taxa de Sucesso:** {metrics.success_rate*100:.2f}% (target: >= 99%)"
        )
        lines.append(
            f"- **Autoscaling:** {self.results.min_replicas_observed} -> {self.results.max_replicas_observed} replicas"
        )
        lines.append("")
    ...
    def generate_resource_analysis(self) -> str:
        ...
        # CPU
        peak_cpu = (
            f"{self.results.peak_cpu_percent:.1f}%" if self.results.peak_cpu_percent else "N/A"
        )
        ...
        # Memoria
        peak_mem = (
            f"{self.results.peak_memory_percent:.1f}%"
            if self.results.peak_memory_percent
            else "N/A"
        )
        avg_mem = (
            f"{self.results.avg_memory_percent:.1f}%" if self.results.avg_memory_percent else "N/A"
        )
