...
KAFKA_BOOTSTRAP="${KAFKA_BOOTSTRAP:-kafka-bootstrap.kafka.svc.cluster.local:9092}"
TEMPORAL_NAMESPACE="${TEMPORAL_NAMESPACE:-temporal}"
TEMPORAL_FRONTEND="${TEMPORAL_FRONTEND:-temporal-frontend.temporal.svc.cluster.local:7233}"
...
test_prerequisites() {
    ...
    # Verificar Kafka
    if ! kubectl get statefulset kafka-kafka -n "$KAFKA_NAMESPACE" &> /dev/null; then
        log_error "Kafka StatefulSet não encontrado no namespace $KAFKA_NAMESPACE"
        all_ok=false
    fi
