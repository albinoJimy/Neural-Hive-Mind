...
class TestTemporalWorkflowSignals:
    ...
    async def test_signal_with_response(self):
        ...
        response = await mock_handle.signal(
            "approval_request",
            {"ticket_id": str(uuid4()), "action": "approve"}
        )
...
class TestTemporalWorkflowTimeouts:
    ...
    async def test_execution_timeout(self):
        ...
        mock_workflow.execute = AsyncMock(
            side_effect=TimeoutError(f"Workflow exceeded {timeout}")
        )
...
class TestTemporalChildWorkflows:
    ...
    async def test_start_child_workflow(self):
        ...
        mock_parent.start_child_workflow = MagicMock(
            return_value=MagicMock(id=mock_child_id)
        )

        child_handle = mock_parent.start_child_workflow(
            "child-workflow",
            args={"parent_id": "parent-123"}
        )
...
class TestTemporalSearchAttributes:
    ...
    async def test_query_by_search_attributes(self):
        ...
        mock_client.list_workflows = MagicMock(return_value=[
            MagicMock(id="workflow-1"),
            MagicMock(id="workflow-2")
        ])

        workflows = mock_client.list_workflows(
            query='ticket_id = "123" AND priority = "high"'
        )
