...
class TestCompensationBuildFailure:
    ...
    async def test_compensation_build_failure_deletes_artifacts(
        self, mock_kafka_producer, mock_mongodb_client, mock_metrics
    ):
        """
        ...
        with patch("src.activities.compensation.activity"):
            compensation_ticket_id = await compensate_ticket(build_ticket, reason="task_failed")
...
class TestCompensationDeployFailure:
    ...
    async def test_compensation_deploy_failure_triggers_rollback(
        self, mock_kafka_producer, mock_mongodb_client, mock_argocd_client, mock_metrics
    ):
        """
        ...
        with patch("src.activities.compensation.activity"):
            compensation_ticket_id = await compensate_ticket(
                deploy_ticket, reason="workflow_inconsistent"
            )
...
class TestCompensationPartialWorkflow:
    ...
    async def test_compensation_partial_workflow_reverse_order(
        ...
        with patch("src.activities.compensation.activity"):
            compensation_order = await build_compensation_order(failed_tickets, all_tickets)
...
class TestCompensationMetrics:
    ...
    async def test_compensation_metrics_recorded(
        ...
        ticket = {
            ...
            "task_type": "EXECUTE",
            "status": "FAILED",
            "parameters": {"execution_id": "exec-123"},
        }
...
class TestCompensationIdempotency:
    ...
    async def test_compensation_idempotent_multiple_executions(
        ...
        ticket = {
            ...
            "task_type": "BUILD",
            "status": "FAILED",
            "parameters": {"artifact_ids": ["artifact-1"]},
        }
...
class TestCompensationWorkflowIntegration:
    ...
    async def test_workflow_triggers_compensation_on_failure(self):
        ...
        failed_tickets = [
            t for t in published_tickets if t.get("ticket", {}).get("status") == "FAILED"
        ]
...
class TestCompensationErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_compensation_continues_on_kafka_failure(self, mock_mongodb_client, mock_metrics):
        """
    ...
    @pytest.mark.asyncio
    async def test_compensation_continues_without_mongodb(self, mock_kafka_producer, mock_metrics):
        """
