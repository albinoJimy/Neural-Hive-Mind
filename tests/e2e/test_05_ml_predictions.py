...
async def test_ml_duration_prediction_enabled(
    ...
    baseline = await query_prometheus(
        prom_url, 'sum(orchestration_ml_predictions_total{status="success"})'
    )
    before_val = (
...
async def test_ml_anomaly_detection(
    gateway_client, test_mongodb_collections, sample_intent_request
):
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_ml_prediction_error_tracking(
    gateway_client, test_mongodb_collections, sample_intent_request
):
    response = await gateway_client.post("/intentions", json=sample_intent_request)
    assert response.status_code == 200
...
async def test_ml_fallback_on_mlflow_failure(
    ...
    try:
        ...
        for _ in range(6):
            ...
            after_val = (
                float(metrics["data"]["result"][0]["value"][1])
