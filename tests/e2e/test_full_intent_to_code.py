...
def test_context():
    ...
    return TestContext(intent_id=intent_id, correlation_id=correlation_id)
...
async def step_1_submit_intent(test_config: Dict, ctx: TestContext):
    ...
    payload = {
        ...
        "domain": "security",
        "context": {"framework": "fastapi", "database": "mongodb", "authentication": "jwt"},
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        response = await client.post(
            ...
            json=payload,
            headers={"X-Correlation-ID": ctx.correlation_id, "X-Request-ID": str(uuid.uuid4())},
        )
        ...
        logger.info(
            "step_1_intent_submitted", intent_id=ctx.intent_id, status_code=response.status_code
        )
    ...
    while (asyncio.get_event_loop().time() - start_time) < max_wait:
        ...
        try:
            ...
            if value.get("intent_id") == ctx.intent_id or value.get("intentionId") == ctx.intent_id:
                ...
                assert (
                    found_correlation_id == ctx.correlation_id
...
async def step_5_validate_trace_propagation(test_config: Dict, ctx: TestContext):
    ...
    if ctx.trace_id:
        ...
        logger.warning(
            "step_5_traceparent_not_found", note="traceparent header not found, checking fallback"
        )
...
async def step_6_wait_for_completion(test_config: Dict, ctx: TestContext):
    ...
    logger.info(
        "step_6_completion_assumed", note="Full E2E test validation pending API availability"
    )
...
async def test_concurrent_intentions(test_config):
    ...
    tasks = [submit_intent(intent_ids[i], correlation_ids[i]) for i in range(concurrent_requests)]
    ...
    for result in results:
        if isinstance(result, Exception):
            ...
            if status_code in (200, 201, 202):
                ...
                logger.warning(
                    "concurrent_request_rejected", intent_id=intent_id, status_code=status_code
                )
