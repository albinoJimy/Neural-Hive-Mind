...
async def test_build_deploy_failure_rollback(saga_config):
    ...
    ctx = CompensationTestContext(intent_id=str(uuid.uuid4()), correlation_id=str(uuid.uuid4()))
...
async def test_approval_revert_on_failure(saga_config):
    ...
    logger.info(
        "saga_test_starting", test_name="test_approval_revert_on_failure", intent_id=ctx.intent_id
    )
...
async def test_cascading_compensation(saga_config):
    ...
    logger.info(
        "saga_test_starting", test_name="test_cascading_compensation", intent_id=ctx.intent_id
    )
...
async def _approve_plan(config: Dict, ctx: CompensationTestContext):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        response = await client.get(
            f"{config['approval_service_url']}/api/v1/approvals/pending", params={"limit": 10}
        )
...
async def _wait_for_tickets(
    config: Dict, ctx: CompensationTestContext, expected_types: List[str] = None
):
...
async def _wait_for_compensation(
    config: Dict, ctx: CompensationTestContext, expected_compensation: str
):
    """Aguardar execução da compensação"""
    logger.info(
        "waiting_for_compensation", expected_compensation=expected_compensation, plan_id=ctx.plan_id
    )
...
async def _validate_all_compensations_executed(config: Dict, ctx: CompensationTestContext):
    ...
    logger.info("validating_all_compensations", executed=ctx.compensations_executed)
