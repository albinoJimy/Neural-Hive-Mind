        ...
        for attempt in range(30):
            ...
            if status_response.status_code == 200:
                ...
                if "tickets" in status_data and len(status_data["tickets"]) > 0:
                    ...
                    assert (
                        anomaly["score"] > 0.5
                    ), f"Score de anomalia muito baixo: {anomaly['score']}"
...
async def test_load_forecast_real(orchestrator_url):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        response = await client.get(
            f"{orchestrator_url}/api/v1/ml/load-forecast", params={"horizon_minutes": 60}
        )
...
async def test_prediction_latency_acceptable(orchestrator_url, sample_ticket):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        p95_latency = (
            latencies_sorted[p95_index] if p95_index < len(latencies) else latencies_sorted[-1]
        )
...
async def test_ml_fallback_when_model_unavailable(orchestrator_url, sample_ticket):
    ...
    fallback_predictions = {
        ...
        "anomaly": {"is_anomaly": False, "score": 0.0, "type": None, "confidence": 0.0},
