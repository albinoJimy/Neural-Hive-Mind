...
command -v bc >/dev/null 2>&1 || { echo -e "${RED}bc não encontrado${NC}"; exit 1; }
...
# 2.9 Executar teste funcional Python do Fluxo C
echo -e "\n${BLUE}2.9 Executando teste funcional Python do Fluxo C...${NC}"

# Verificar se Python e dependências estão disponíveis
if command -v python3 &> /dev/null; then
  # Verificar se o arquivo de teste existe
  PYTHON_TEST_FILE="${SCRIPT_DIR}/e2e/test_phase2_flow_c_complete.py"

  if [ -f "$PYTHON_TEST_FILE" ]; then
    echo "   Executando teste Python com IDs capturados..."
    echo "   Intent ID: ${TEST_INTENT_ID:-'auto-generated'}"
    echo "   Plan ID: ${TEST_PLAN_ID:-'auto-generated'}"
    echo "   Decision ID: ${TEST_DECISION_ID:-'auto-generated'}"

    # Configurar variáveis de ambiente para o teste Python
    export TEMPORAL_ENDPOINT="${TEMPORAL_ENDPOINT:-temporal-frontend.neural-hive-temporal.svc.cluster.local:7233}"
    export MONGODB_URI="${MONGODB_URI:-mongodb://mongodb-cluster.mongodb-cluster.svc.cluster.local:27017}"
    export POSTGRESQL_TICKETS_URI="${POSTGRESQL_TICKETS_URI:-postgresql://postgres:password@postgresql-tickets.neural-hive-orchestration.svc.cluster.local:5432/execution_tickets}"
    export KAFKA_BOOTSTRAP="${KAFKA_BOOTSTRAP:-neural-hive-kafka-bootstrap.neural-hive-kafka.svc.cluster.local:9092}"

    # Executar o teste Python
    PYTHON_TEST_ARGS=""
    [ -n "$TEST_INTENT_ID" ] && PYTHON_TEST_ARGS="$PYTHON_TEST_ARGS --intent-id=${TEST_INTENT_ID}"
    [ -n "$TEST_PLAN_ID" ] && PYTHON_TEST_ARGS="$PYTHON_TEST_ARGS --plan-id=${TEST_PLAN_ID}"
    [ -n "$TEST_DECISION_ID" ] && PYTHON_TEST_ARGS="$PYTHON_TEST_ARGS --decision-id=${TEST_DECISION_ID}"

    python3 "$PYTHON_TEST_FILE" $PYTHON_TEST_ARGS 2>&1 | tee /tmp/python_test_output.log
    PYTHON_TEST_EXIT=${PIPESTATUS[0]}

    if [ ${PYTHON_TEST_EXIT} -eq 0 ]; then
      echo -e "${GREEN}✓${NC} Teste funcional Python do Fluxo C passou"
      check_status 0 "Teste funcional Python Fluxo C"
    else
      echo -e "${RED}✗${NC} Teste funcional Python do Fluxo C falhou (exit code: ${PYTHON_TEST_EXIT})"
      check_status 1 "Teste funcional Python Fluxo C"

      # Mostrar últimas linhas do log para debugging
      echo "   Últimas linhas do log:"
      tail -20 /tmp/python_test_output.log | sed 's/^/   /'
    fi
  else
    echo -e "${YELLOW}⚠${NC} Arquivo de teste Python não encontrado: $PYTHON_TEST_FILE"
    echo "   Pulando teste funcional Python"
  fi
else
  echo -e "${YELLOW}⚠${NC} Python3 não disponível, pulando teste funcional Python"
fi
...
# 8.1 Validar latência p95 (Prometheus)
echo -e "\n${BLUE}8.1 Validando latência p95 (Prometheus)...${NC}"
...
STEP_TIMEOUT_EXPR='histogram_quantile(0.99,rate(neural_hive_flow_c_steps_duration_seconds_bucket[5m]))'

LATENCY_VALUE=$(curl -s "${PROM_URL}/api/v1/query?query=${LATENCY_EXPR}" 2>/dev/null | jq -r '.data.result[0].value[1]' || echo "")
if [ -n "$LATENCY_VALUE" ] && [ "$LATENCY_VALUE" != "null" ]; then
  LATENCY_SECONDS=$(printf "%.0f" "$(echo "$LATENCY_VALUE" | bc -l)")
  check_status $((LATENCY_SECONDS < 14400 ? 0 : 1)) "Latência p95 <4h (${LATENCY_SECONDS}s)"
else
  echo -e "${YELLOW}⚠${NC} Latência p95 não disponível"
fi

# 8.2 Taxa de sucesso >99%
echo -e "\n${BLUE}8.2 Validando taxa de sucesso (10m)...${NC}"
SUCCESS_VALUE=$(curl -s "${PROM_URL}/api/v1/query?query=${SUCCESS_EXPR}" 2>/dev/null | jq -r '.data.result[0].value[1]' || echo "")
if [ -n "$SUCCESS_VALUE" ] && [ "$SUCCESS_VALUE" != "null" ]; then
  SUCCESS_PERCENT=$(echo "$SUCCESS_VALUE * 100" | bc -l 2>/dev/null | xargs printf "%.2f")
  check_status $(( $(echo "$SUCCESS_PERCENT >= 99" | bc -l) ? 0 : 1 )) "Taxa de sucesso >99% (${SUCCESS_PERCENT}%)"
else
  echo -e "${YELLOW}⚠${NC} Taxa de sucesso não disponível"
fi

# 8.3 Violações de SLA
echo -e "\n${BLUE}8.3 Verificando violações de SLA (30m)...${NC}"
SLA_VALUE=$(curl -s "${PROM_URL}/api/v1/query?query=${SLA_EXPR}" 2>/dev/null | jq -r '.data.result[0].value[1]' || echo "")
if [ -n "$SLA_VALUE" ] && [ "$SLA_VALUE" != "null" ]; then
  SLA_RATE=$(printf "%.4f" "$SLA_VALUE")
  check_status $(( $(echo "$SLA_RATE == 0" | bc -l) ? 0 : 1 )) "SLA violations (30m) == 0 (atual: ${SLA_RATE}/s)"
else
  echo -e "${YELLOW}⚠${NC} Métrica de violações de SLA não disponível"
fi

# 8.4 Timeout por step (p99 <1h)
echo -e "\n${BLUE}8.4 Validando timeout de steps (p99)...${NC}"
STEP_VALUE=$(curl -s "${PROM_URL}/api/v1/query?query=${STEP_TIMEOUT_EXPR}" 2>/dev/null | jq -r '.data.result[0].value[1]' || echo "")
if [ -n "$STEP_VALUE" ] && [ "$STEP_VALUE" != "null" ]; then
  STEP_SECONDS=$(printf "%.0f" "$(echo "$STEP_VALUE" | bc -l)")
  check_status $((STEP_SECONDS < 3600 ? 0 : 1)) "Step p99 <1h (${STEP_SECONDS}s)"
else
  echo -e "${YELLOW}⚠${NC} Métrica de duração por step não disponível"
fi

kill $PF_PID 2>/dev/null || true

# 8.5 Verificar auto-approval rate >80%
echo -e "\n${BLUE}8.5 Verificando auto-approval rate >80%...${NC}"
...
# 8.6 Verificar test coverage >90%
echo -e "\n${BLUE}8.6 Verificando test coverage >90%...${NC}"
...
# ========================================
# FASE 9: Validacao Funcional de Servicos
# ========================================
echo -e "\n${YELLOW}FASE 9: Validacao Funcional de Servicos...${NC}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
VALIDATION_SCRIPTS_DIR="${PROJECT_ROOT}/scripts/validation"

# 9.1 Executar validacao completa de servicos
echo -e "\n${BLUE}9.1 Executando validacao completa de servicos Fase 2...${NC}"

if [ -f "${VALIDATION_SCRIPTS_DIR}/validate-phase2-services.sh" ]; then
  if bash "${VALIDATION_SCRIPTS_DIR}/validate-phase2-services.sh" --quick; then
    check_status 0 "Validacao de servicos Fase 2 passou"
  else
    check_status 1 "Validacao de servicos Fase 2 falhou"
  fi
else
  echo -e "${YELLOW}⚠${NC} Script validate-phase2-services.sh nao encontrado"
fi

# 9.2 Executar validacao de conectividade de bancos de dados
echo -e "\n${BLUE}9.2 Executando validacao de conectividade de bancos de dados...${NC}"

if [ -f "${VALIDATION_SCRIPTS_DIR}/test_database_connectivity.py" ] && command -v python3 &> /dev/null; then
  if python3 "${VALIDATION_SCRIPTS_DIR}/test_database_connectivity.py" --quiet 2>/dev/null; then
    check_status 0 "Conectividade de bancos de dados validada"
  else
    echo -e "${YELLOW}⚠${NC} Algumas conexoes de banco de dados falharam (pode ser falta de bibliotecas)"
  fi
else
  echo -e "${YELLOW}⚠${NC} Script test_database_connectivity.py nao encontrado ou Python nao disponivel"
fi

# 9.3 Executar validacao de comunicacao gRPC
echo -e "\n${BLUE}9.3 Executando validacao de comunicacao gRPC...${NC}"

if [ -f "${VALIDATION_SCRIPTS_DIR}/test_grpc_communication.py" ] && command -v python3 &> /dev/null; then
  if python3 "${VALIDATION_SCRIPTS_DIR}/test_grpc_communication.py" --quiet 2>/dev/null; then
    check_status 0 "Comunicacao gRPC validada"
  else
    echo -e "${YELLOW}⚠${NC} Algumas comunicacoes gRPC falharam"
  fi
else
  echo -e "${YELLOW}⚠${NC} Script test_grpc_communication.py nao encontrado ou Python nao disponivel"
fi

# 9.4 Executar validacao de Kafka consumers
echo -e "\n${BLUE}9.4 Executando validacao de Kafka consumers...${NC}"

if [ -f "${VALIDATION_SCRIPTS_DIR}/test_kafka_consumers.sh" ]; then
  if bash "${VALIDATION_SCRIPTS_DIR}/test_kafka_consumers.sh" --quiet 2>/dev/null; then
    check_status 0 "Kafka consumers validados"
  else
    echo -e "${YELLOW}⚠${NC} Alguns Kafka consumers nao estao ativos"
  fi
else
  echo -e "${YELLOW}⚠${NC} Script test_kafka_consumers.sh nao encontrado"
fi

# 9.5 Verificar endpoints HTTP de todos os servicos
echo -e "\n${BLUE}9.5 Verificando endpoints HTTP de todos os 13 servicos...${NC}"

SERVICES_HTTP_OK=0
SERVICES_HTTP_TOTAL=0
...
for service_ns in "${PHASE2_SERVICES[@]}"; do
  ...
  if [ -n "$POD" ]; then
    HEALTH_CODE=$(kubectl exec -n ${namespace} ${POD} -- curl -s -o /dev/null -w "%{http_code}" "http://localhost:8000/health" 2>/dev/null || echo "000")

    if [ "$HEALTH_CODE" = "200" ]; then
      echo -e "${GREEN}✓${NC} ${service}: /health OK"
      ((SERVICES_HTTP_OK++))
    else
      echo -e "${RED}✗${NC} ${service}: /health falhou (HTTP ${HEALTH_CODE})"
    fi
  else
    echo -e "${YELLOW}⚠${NC} ${service}: Pod nao encontrado"
  fi
done

check_status $((SERVICES_HTTP_OK >= 10 ? 0 : 1)) "Endpoints HTTP funcionais (${SERVICES_HTTP_OK}/${SERVICES_HTTP_TOTAL})"

# 9.6 Verificar servicos gRPC
echo -e "\n${BLUE}9.6 Verificando servicos gRPC...${NC}"

GRPC_SERVICES_LIST=("queen-agent:neural-hive-queen" "service-registry:neural-hive-service-registry" "optimizer-agents:neural-hive-optimizers")
GRPC_OK=0

for service_ns in "${GRPC_SERVICES_LIST[@]}"; do
  IFS=':' read -r service namespace <<< "$service_ns"
  ...
  if [ -n "$POD" ]; then
    ...
    if [ -n "$PORT_CHECK" ]; then
      echo -e "${GREEN}✓${NC} ${service}: gRPC porta 50051 aberta"
      ((GRPC_OK++))
    else
      echo -e "${RED}✗${NC} ${service}: gRPC porta 50051 nao aberta"
    fi
  fi
done

check_status $((GRPC_OK >= 2 ? 0 : 1)) "Servicos gRPC funcionais (${GRPC_OK}/3)"
...
echo -e "${GREEN}✓${NC} Validacao Funcional (HTTP, gRPC, DB, Kafka)"

echo -e "\n${YELLOW}Phase 2 SLO Results:${NC}"
echo "  • Latência p95 (5m): ${LATENCY_SECONDS:-N/A}s (target: <14400s = 4h)"
echo "  • Taxa de sucesso (10m): ${SUCCESS_PERCENT:-N/A}% (target: >99%)"
echo "  • Violações de SLA (30m): ${SLA_RATE:-N/A}/s (target: 0)"
echo "  • Step p99: ${STEP_SECONDS:-N/A}s (target: <3600s)"
