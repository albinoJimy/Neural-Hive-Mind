...
async def test_opa_resource_limits_enforcement(
    ...
    before_val = (
        float(baseline["data"]["result"][0]["value"][1]) if baseline["data"]["result"] else 0.0
    )

    response = await orchestrator_client.post(
        "/api/v1/flow-c/execute", json=sample_opa_policy_violation
    )
    ...
    for _ in range(6):
        ...
        current = (
            float(metrics["data"]["result"][0]["value"][1]) if metrics["data"]["result"] else 0.0
        )
        ...
        pytest.fail(
            "Métrica orchestration_opa_policy_rejections_total não incrementou após violação de recursos"
        )


@pytest.mark.e2e
@pytest.mark.asyncio
async def test_opa_sla_enforcement(
    orchestrator_client, sample_execution_ticket, port_forward_manager
):
    prom_url = port_forward_manager["prometheus"]
    baseline = await query_prometheus(prom_url, "sum(orchestration_opa_policy_rejections_total)")
    before_val = (
...
async def test_opa_feature_flags(orchestrator_client, port_forward_manager):
    ...
    flag_state = (
        data.get("intelligent_scheduler_enabled", False) if isinstance(data, dict) else False
    )
...
async def test_opa_burst_capacity(orchestrator_client, sample_execution_ticket):
    ...
    accepted = sum(
        1 for r in results if str(r.get("status")).lower() in {"accepted", "ok", "created", "200"}
    )
    rejected = sum(
        1 for r in results if str(r.get("status")).lower() in {"rejected", "denied", "429", "403"}
    )
    if results:
...
async def test_opa_circuit_breaker(orchestrator_client, k8s_client, port_forward_manager):
    ...
    current_scale = k8s_client.read_namespaced_deployment_scale(
    ...
    try:
        ...
        await asyncio.sleep(5)
        response = await orchestrator_client.post(
            "/api/v1/flow-c/execute", json={"ticket_id": "opa-cb-test"}
        )
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_opa_security_tenant_isolation(
    orchestrator_client, sample_execution_ticket, port_forward_manager
):
    ...
    for _ in range(6):
        metrics = await query_prometheus(
            prom_url, "orchestration_security_tenant_isolation_violations_total"
        )
        results = metrics.get("data", {}).get("result", [])
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_opa_security_missing_authentication(
    orchestrator_client, sample_execution_ticket, port_forward_manager
):
    ...
    metrics = await query_prometheus(
        prom_url, "orchestration_security_authentication_failures_total"
    )
    results = metrics.get("data", {}).get("result", [])
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_opa_security_insufficient_permissions(
    orchestrator_client, sample_execution_ticket, port_forward_manager
):
    ...
    metrics = await query_prometheus(
        prom_url, 'orchestration_security_authorization_denials_total{user_id="viewer@example.com"}'
    )
    results = metrics.get("data", {}).get("result", [])
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_opa_security_pii_handling(
    orchestrator_client, sample_execution_ticket, port_forward_manager
):
    ...
    metrics = await query_prometheus(
        prom_url, "orchestration_security_data_governance_violations_total"
    )
    results = metrics.get("data", {}).get("result", [])
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_opa_security_rate_limiting(
    orchestrator_client, sample_execution_ticket, port_forward_manager
):
...
async def test_opa_security_compliance_dashboard(port_forward_manager):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(
            f"{grafana_url}/api/search?tag=security", auth=aiohttp.BasicAuth("admin", "admin")
        ) as response:
            ...
            security_dashboard = next(
                (d for d in dashboards if "security" in d.get("title", "").lower()), None
            )
