...
class QueuedCase(BaseModel):
    ...
    information_value: float = Field(..., ge=0.0, le=1.0, description="Valor informacional")
    ...
    status: QueueStatus = Field(default=QueueStatus.PENDING, description="Status na fila")
...
class PriorityFeedbackQueue:
    ...
    def __init__(
        self, collection, strategy=None, claim_expiry_hours: int = DEFAULT_CLAIM_EXPIRY_HOURS
    ):
        """
        Inicializa a fila.
        ...
        logger.info("PriorityFeedbackQueue initialized", claim_expiry_hours=claim_expiry_hours)
    ...
    def enqueue_plan_for_review(
        ...
        if self.strategy and dataset_stats:
            information_value = self.strategy.calculate_from_prediction(prediction, dataset_stats)
        else:
    ...
    def mark_feedback_submitted(self, queue_id: str, feedback_id: str) -> Optional[dict[str, Any]]:
        ...
        logger.info("Case marked as completed", queue_id=queue_id, feedback_id=feedback_id)

        return {"queue_id": queue_id, "status": QueueStatus.COMPLETED, "feedback_id": feedback_id}
    ...
    def cleanup_completed(self, older_than_hours: int = 24) -> int:
        ...
        if count > 0:
            logger.info(
                "Completed cases cleaned up", count=count, older_than_hours=older_than_hours
            )
