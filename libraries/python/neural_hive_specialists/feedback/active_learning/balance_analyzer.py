...
class DatasetBalanceAnalyzer:
    ...
    def __init__(self, collection, target_balance: float = None):
        ...
        logger.info("DatasetBalanceAnalyzer initialized", target_balance=self.target_balance)
    ...
    def get_priority_recommendations(self, limit: int = 10) -> List[PriorityRecommendation]:
        ...
        recommendations = [PriorityRecommendation(**r) for r in metrics.priority_recommendations]
    ...
    def _calculate_semantic_features_stats(self, total: int) -> tuple[int, float]:
        ...
        try:
            ...
            pipeline = [
                {"$match": {"reasoning_factors": {"$type": "array", "$not": {"$size": 0}}}},
