...
class BalanceMetrics(BaseModel):
    ...
    domain_distribution: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict,
        description="Distribuição por domínio NLP"
    )
    semantic_features_count: int = Field(
        default=0,
        description="Contagem de feedbacks com features semânticas"
    )
    semantic_features_percentage: float = Field(
        default=0.0,
        description="Porcentagem de feedbacks com features semânticas"
    )
    priority_recommendations: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Lista de recomendações de prioridade"
    )
    last_updated: str = Field(
        default='',
        description="Timestamp da última atualização (ISO format)"
    )
...
class DatasetBalanceAnalyzer:
    ...
    def __init__(self, collection, target_balance: float = None):
        ...
        logger.info(
            'DatasetBalanceAnalyzer initialized',
    ...
    def get_priority_recommendations(
        ...
        recommendations = [
            PriorityRecommendation(**r)
            for r in metrics.priority_recommendations
        ]
    ...
    def _calculate_confidence_distribution(
        ...
        distribution = {key: {'count': 0, 'percentage': 0.0}
                       for key in self.CONFIDENCE_RANGES}
    ...
    def _calculate_domain_distribution(
        ...
        try:
            ...
            pipeline = [
                {'$match': {'nlp_features.primary_domain': {'$type': 'string', '$ne': None}}},
    ...
    def _calculate_semantic_features_stats(
        self,
        total: int
    ) -> tuple[int, float]:
            ...
            for doc in results:
                ...
                if factors:
                    ...
                    for factor in factors:
                        ...
                        if any(name.startswith(prefix) for prefix in self.SEMANTIC_PREFIXES):
                            semantic_count += 1
                            break
