...
class OpinionDocumentV2(BaseModel):
    ...
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="Timestamp de criação no ledger"
    )
...
class SchemaVersionManager:
    ...
    def migrate_to_v2(v1_document: Dict[str, Any]) -> OpinionDocumentV2:
        ...
        """
        logger.info(
            "Migrating opinion document from v1 to v2",
            opinion_id=v1_document.get("opinion_id"),
        )
        ...
        opinion = Opinion(
            ...
            reasoning_factors=[
                ...
                for factor in v1_document["opinion"].get("reasoning_factors", [])
            ],
