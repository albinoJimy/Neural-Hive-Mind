...
class DisasterRecoveryManager:
        ...
        try:
                        ...
                        if result["success"]:
                            ...
                            if hasattr(self.specialist, "metrics"):
                                self.specialist.metrics.observe_backup_component_duration(
                                    component_name, result.get("duration_seconds", 0)
                                )
                        else:
                            logger.warning(
            ...
            try:
                upload_success = self.storage_client.upload_backup(
                    backup_archive_path, backup_filename
                )
            ...
            try:
                checksum_upload = self.storage_client.upload_backup(
                    checksum_file, f"{backup_filename}.sha256"
                )
    ...
    def _backup_ledger(
        ...
        try:
            logger.info("Iniciando backup de ledger MongoDB", tenant_id=tenant_id)
    ...
    def _backup_cache(self, backup_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.specialist, "opinion_cache"):
                ...
                cache_data = {"keys": cache_keys, "key_count": len(cache_keys)}
    ...
    def _backup_feature_store(
        ...
        try:
            logger.info("Iniciando backup de feature store", tenant_id=tenant_id)
            ...
            return {
                "success": True,
                "duration_seconds": duration,
                "metadata": {"feature_count": len(features), "tenant_id": tenant_id},
            }
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if result["success"]:
                            ...
                            component_tarball = os.path.join(
                                tempfile.gettempdir(), f"{component_name}.tar.gz"
                            )
                            ...
                            if self._blob_exists(blob_key):
                                ...
                                if not self.storage_client.upload_backup(
                                    component_tarball, blob_key
                                ):
                                    raise Exception(
                                        f"Falha no upload do componente {component_name}"
                                    )
    ...
    def garbage_collect_blobs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Blobs encontrados em components/", total_blobs=len(all_blobs))
            ...
            for blob_obj in all_blobs:
                ...
                if blob_sha256 not in referenced_sha256s:
                    ...
                    if self.storage_client.delete_backup(blob_key):
                        ...
                        logger.warning("Falha ao deletar blob", blob_key=blob_key)
        ...
        try:
            ...
            for component_name, sha256_digest in component_refs.items():
                ...
                try:
                    ...
                    # Download do blob
                    blob_local = os.path.join(
                        tempfile.gettempdir(),
                        f"{component_name}-{sha256_digest[:8]}.tar.gz",
                    )
                    ...
                    if not skip_validation:
                        ...
                        logger.info("SHA-256 validado", component=component_name)
    ...
    def restore_specialist_state(
        ...
        try:
            ...
            if is_snapshot:
                ...
                return self._restore_from_snapshot(
                    backup_id, target_dir, skip_validation, skip_smoke_tests
                )
            ...
            if not skip_validation and checksum_local_path:
                ...
                if actual_checksum != expected_checksum:
                    raise Exception(
                        f"Checksum inválido! Esperado: {expected_checksum}, Atual: {actual_checksum}"
                    )
    ...
    def test_recovery(self, backup_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        logger.info("Iniciando teste de recovery", backup_id=backup_id or "latest")
        ...
        try:
            ...
            if backup_id:
                ...
                # Usar mais recente
                selected_backup = backup_files[
                    0
                ]  # Já ordenado por timestamp (mais recente primeiro)
    ...
    def _restore_model(self, component_dir: str, target_dir: Optional[str]) -> None:
        ...
        try:
            ...
            experiment_name = model_metadata.get(
                "mlflow_experiment_name", self.config.mlflow_experiment_name
            )
            ...
            logger.error(
                "Erro ao restaurar modelo MLflow",
                error=str(e),
                error_type=type(e).__name__,
            )
            raise
    ...
    def _restore_ledger(self, component_dir: str, target_dir: Optional[str]) -> None:
        ...
        if os.path.exists(json_file):
            ...
            try:
                ...
                logger.error(
                    "Erro ao restaurar ledger via driver",
                    error=str(e),
                    error_type=type(e).__name__,
                )
                raise
    ...
    def _restore_feature_store(
        ...
        try:
            ...
            logger.info("Restaurando feature store", feature_count=len(features_data))
