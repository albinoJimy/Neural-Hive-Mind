...
class SpecialistConfig(BaseSettings):
    ...
    mlflow_experiment_name: str = Field(
        ..., env="MLFLOW_EXPERIMENT_NAME", description="Nome do experimento MLflow"
    )
    mlflow_model_name: str = Field(
        ..., env="MLFLOW_MODEL_NAME", description="Nome do modelo registrado"
    )
    mlflow_model_stage: str = Field(
        default="Production", env="MLFLOW_MODEL_STAGE", description="Stage do modelo"
    )
    ...
    mongodb_ledger_collection: str = Field(
        default="cognitive_ledger", env="MONGODB_LEDGER_COLLECTION"
    )
    mongodb_feature_store_collection: str = Field(
        default="plan_features", env="MONGODB_FEATURE_STORE_COLLECTION"
    )
    ...
    opinion_cache_enabled: bool = Field(
        default=True,
        env="OPINION_CACHE_ENABLED",
        description="Feature flag para habilitar cache de opinions",
    )
    opinion_cache_ttl_seconds: int = Field(
        default=3600,
        env="OPINION_CACHE_TTL_SECONDS",
        description="TTL específico para cache de opinions (1 hora)",
    )
    opinion_cache_key_prefix: str = Field(
        default="opinion:",
        env="OPINION_CACHE_KEY_PREFIX",
        description="Prefixo para chaves Redis de opinions",
    )
    opinion_cache_max_size_mb: int = Field(
        default=100,
        env="OPINION_CACHE_MAX_SIZE_MB",
    ...
    enable_tracing: bool = Field(
        default=True,
        env="ENABLE_TRACING",
        description="Feature flag para habilitar/desabilitar tracing",
    )
    otel_insecure: bool = Field(
        default=True, env="OTEL_INSECURE", description="Usar conexão insegura (sem TLS)"
    )
    trace_sampling_rate: float = Field(
        default=1.0,
        env="TRACE_SAMPLING_RATE",
        description="Taxa de amostragem de traces (0.0 a 1.0)",
    )
    trace_batch_size: int = Field(
        default=512,
        env="TRACE_BATCH_SIZE",
        description="Tamanho do batch para exportação",
    )
    trace_export_timeout_ms: int = Field(
        default=30000,
        env="TRACE_EXPORT_TIMEOUT_MS",
        description="Timeout de exportação em ms",
    )

    # Span Configuration
    enable_detailed_spans: bool = Field(
        default=True,
        env="ENABLE_DETAILED_SPANS",
        description="Habilitar spans detalhados por etapa",
    )
    span_include_plan_content: bool = Field(
        default=False,
        env="SPAN_INCLUDE_PLAN_CONTENT",
        description="Incluir conteúdo do plano em spans (pode ser grande)",
    )
    span_include_model_predictions: bool = Field(
        default=True,
        env="SPAN_INCLUDE_MODEL_PREDICTIONS",
        description="Incluir predições do modelo em spans",
    )

    # Batch Evaluation Configuration
    batch_max_concurrency: int = Field(
        default=10,
        env="BATCH_MAX_CONCURRENCY",
        description="Concorrência máxima para batch evaluation",
    )
    batch_timeout_seconds: int = Field(
        default=30, env="BATCH_TIMEOUT_SECONDS", description="Timeout total para batch"
    )
    batch_enable_partial_results: bool = Field(
        default=True,
        env="BATCH_ENABLE_PARTIAL_RESULTS",
        description="Retornar resultados parciais se algumas avaliações falharem",
    )
    ...
    warmup_enabled: bool = Field(
        default=True,
        env="WARMUP_ENABLED",
        description="Feature flag para habilitar warmup automático",
    )
    warmup_on_startup: bool = Field(
        default=True,
        env="WARMUP_ON_STARTUP",
        description="Executar warmup automaticamente no startup",
    )
    warmup_dummy_plan_path: Optional[str] = Field(
        default=None,
        env="WARMUP_DUMMY_PLAN_PATH",
        description="Caminho para plano dummy customizado",
    )
    ...
    jwt_token_expiration_seconds: int = Field(
        default=3600, description="Tempo de expiração do token JWT"
    )
    ...
    circuit_breaker_failure_threshold: int = Field(
        default=5, description="Número de falhas antes de abrir o circuito"
    )
    circuit_breaker_recovery_timeout: int = Field(
        default=60, description="Segundos antes de tentar recuperação"
    )
    circuit_breaker_expected_exception: type = Field(
        default=Exception, description="Tipo de exceção a rastrear"
    )
    enable_circuit_breaker: bool = Field(
        default=True,
        description="Feature flag para habilitar/desabilitar circuit breakers",
    )
    ...
    feedback_rating_min: float = Field(
        default=0.0, env="FEEDBACK_RATING_MIN", description="Rating mínimo permitido"
    )
    feedback_rating_max: float = Field(
        default=1.0, env="FEEDBACK_RATING_MAX", description="Rating máximo permitido"
    )
    ...
    training_validation_split: float = Field(
        default=0.2, env="TRAINING_VALIDATION_SPLIT", description="Split de validação"
    )
    training_test_split: float = Field(
        default=0.1, env="TRAINING_TEST_SPLIT", description="Split de teste"
    )
    ...
    envoy_gateway_port: int = Field(
        default=50051, env="ENVOY_GATEWAY_PORT", description="Porta do gateway Envoy"
    )
    ...
    backup_s3_region: str = Field(
        default="us-west-2", env="BACKUP_S3_REGION", description="Região AWS S3"
    )
    ...
    def validate_shap_background_path(cls, v):
        ...
        if v and not os.path.exists(v):
            logger.warning("SHAP background dataset not found", path=v)
        return v
    ...
    def validate_feedback_rating_range(cls, v):
        ...
        if not 0.0 <= v <= 1.0:
            raise ValueError(
                "feedback_rating_min e feedback_rating_max devem estar entre 0.0 e 1.0"
            )
        return v
    ...
    def validate_training_splits(cls, v):
        ...
        if not 0.0 < v < 1.0:
            raise ValueError(
                "training_validation_split e training_test_split devem estar entre 0.0 e 1.0"
            )
        return v
    ...
    def validate_anomaly_threshold(cls, v):
        ...
        if v >= 0:
            raise ValueError(
                "anomaly_alert_threshold deve ser negativo (scores de anomalia são negativos)"
            )
        return v
    ...
    def validate_backup_prefix(cls, v, info):
        ...
        if (
            v == "specialists/backups"
            and backup_s3_prefix
            and backup_s3_prefix != "specialists/backups"
        ):
            logger.warning(
