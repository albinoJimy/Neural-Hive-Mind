...
class DescriptionQualityValidator:
    ...
    SECURITY_KEYWORDS: Dict[str, List[str]] = {
        'confidential': [
            'encrypt', 'auth', 'audit', 'permission', 'sanitize',
            'credential', 'token', 'secure', 'private', 'protected'
        ],
        'restricted': [
            'encrypt', 'auth', 'audit', 'permission', 'sanitize',
            'classified', 'secret', 'clearance', 'compartment', 'need-to-know'
        ],
        'internal': [
            'validate', 'verify', 'check', 'access'
        ],
        'public': []
    }
    ...
    QOS_KEYWORDS: Dict[str, List[str]] = {
        'exactly_once': ['idempotent', 'transaction', 'rollback', 'exactly-once', 'dedup', 'deduplication'],
    ...
    def _compile_patterns(self):
        ...
        for domain, keywords in self.DOMAIN_KEYWORDS.items():
            self._domain_patterns[domain] = [
                re.compile(rf'\b{kw}\w*\b', re.IGNORECASE)
                for kw in keywords
            ]
    ...
    def validate_description(
        ...
        suggestions = self._generate_suggestions(
            description, domain, security_level, qos,
            length_score, diversity_score, domain_score
        )
        ...
        logger.debug(
            ...
            result['score'], result['metrics']['word_count'], domain, security_level, len(issues)
        )
    ...
    def validate_plan_descriptions(
        ...
        is_valid = avg_score >= min_quality_score and len(low_quality_tasks) == 0
    ...
    def _generate_suggestions(
        ...
        if qos and qos in self.QOS_KEYWORDS:
            ...
            suggestions.append(
                f"Considere hints de QoS: {', '.join(qos_kws)}"
            )
    ...
    def suggest_improvements(
        ...
        logger.info(
            ...
            description, improved, domain, security_level
        )
