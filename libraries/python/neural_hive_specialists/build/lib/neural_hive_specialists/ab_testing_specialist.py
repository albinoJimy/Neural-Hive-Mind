...
class ABTestingSpecialist(BaseSpecialist):
        ...
        try:
            with tracer.start_as_current_span("ab_test.load_model_a") as span:
                ...
                self.model_a = self.mlflow_client.load_model(
                    model_a_name,
                    self.config.ab_test_model_a_stage
                )
        ...
        try:
            with tracer.start_as_current_span("ab_test.load_model_b") as span:
                ...
                self.model_b = self.mlflow_client.load_model(
                    model_b_name,
                    self.config.ab_test_model_b_stage
                )
            ...
            try:
                ...
                if not result:
                    ...
                    if variant == 'model_b' and self.model_a:
                        ...
                        return self._predict_with_fallback_model(cognitive_plan, 'model_a', self.model_a)
                    elif variant == 'model_a' and self.model_b:
                        logger.warning("Fazendo fallback para modelo B")
                        return self._predict_with_fallback_model(cognitive_plan, 'model_b', self.model_b)
                    return None
                ...
                model_metadata = self.model_a_metadata if variant == 'model_a' else self.model_b_metadata
    ...
    def _predict_with_fallback_model(
        ...
        try:
            ...
            if result:
                result['metadata'] = result.get('metadata', {})
                result['metadata'].update({
                    'ab_test_variant': f'{variant}_fallback',
                    'ab_test_fallback': True
                })
    ...
    def get_ab_test_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            min_samples = self.config.ab_test_minimum_sample_size if hasattr(self.config, 'ab_test_minimum_sample_size') else 30
            ...
            if sample_a >= min_samples and sample_b >= min_samples:
                try:
                    ...
                    if is_significant:
                        winner = 'model_b' if stats['model_b']['avg_confidence'] > stats['model_a']['avg_confidence'] else 'model_a'
                    ...
                    logger.warning("scipy não disponível, não é possível calcular significância estatística")
                    stats['recommendation'] = 'scipy_not_available'
                except Exception as e:
                    logger.error(
                        "Erro ao calcular significância estatística",
                        error=str(e)
                    )
            else:
                stats['recommendation'] = f'insufficient_data (need {min_samples} per variant, have {sample_a}/{sample_b})'
            ...
            logger.error(
                "Erro ao coletar estatísticas de A/B test",
                error=str(e),
                exc_info=True
            )
            return {
                ...
                'statistical_significance': {'p_value': None, 'is_significant': False, 'winner': None},
    ...
    def _collect_variant_stats(self, variant: str) -> Dict[str, Any]:
        ...
        try:
            ...
            sample_size = 0
            try:
                sample_size = int(self.metrics.ab_test_variant_usage_total.labels(
            ...
            for recommendation in ['approve', 'reject', 'review_required', 'conditional']:
                try:
                    count = int(self.metrics.ab_test_variant_recommendation_distribution.labels(
                        self.specialist_type,
                        variant,
                        recommendation
                    )._value.get())
