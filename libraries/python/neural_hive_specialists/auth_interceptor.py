...
class AuthInterceptor(grpc.ServerInterceptor):
    ...
    def intercept_service(
        ...
        if not auth_header:
            logger.warning("Missing authorization header", method=full_method)
            if self.metrics:
        ...
        try:
            ...
            logger.warning("Expired JWT token", method=full_method)
            if self.metrics:
                self.metrics.increment_auth_request(full_method, "failed")
            ...
            logger.warning("Invalid JWT token", method=full_method, error=str(e))
            if self.metrics:
                self.metrics.increment_auth_request(full_method, "failed")
