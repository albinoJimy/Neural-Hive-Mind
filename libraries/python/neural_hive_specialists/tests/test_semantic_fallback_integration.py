...
class TestSemanticFallbackIntegration:
    ...
    def test_config_default_semantic_fallback_enabled(self):
        ...
        with patch.dict('os.environ', {
            ...
            assert config.use_semantic_fallback is True, \
    ...
    def test_config_semantic_fallback_from_env_var(self):
        ...
        with patch.dict('os.environ', {
            ...
            config = SpecialistConfig()
            assert config.use_semantic_fallback is False, \
    ...
    def test_confidence_calibration_for_semantic_fallback(self):
        ...
        assert calibrated_confidence == pytest.approx(expected_calibrated, rel=1e-3), \
...
class TestSemanticPipelineQuality:
    ...
    def test_overall_confidence_aggregation(self):
        ...
        weights = {'security': 0.3, 'architecture': 0.25, 'performance': 0.2, 'quality': 0.25}

        weighted_sum = sum(
            semantic_scores[dim] * weights[dim]
            for dim in semantic_scores
        )
