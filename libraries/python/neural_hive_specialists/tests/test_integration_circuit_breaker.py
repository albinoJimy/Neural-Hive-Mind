...
def test_ledger_circuit_breaker_recovery_and_buffer_flush(mongodb_uri):
    ...
    with patch.object(ledger, '_save_opinion_impl', side_effect=ConnectionFailure('Simulated failure')):
        for i in range(4):
            opinion = {
...
def test_mlflow_circuit_breaker_expired_cache_fallback(mocker):
    ...
    with patch('mlflow.MlflowClient') as mock_mlflow:
        mock_mlflow.return_value.get_latest_versions.side_effect = ConnectionError('MLflow unreachable')
