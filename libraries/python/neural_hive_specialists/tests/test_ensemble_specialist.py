...
def mock_mlflow_client():
    ...
    with patch(
        ...
        client_instance.get_model_metadata = Mock(
            return_value={"version": "1", "run_id": "test-run-id"}
        )
...
class TestEnsembleIntegration:
    ...
    def test_full_ensemble_prediction_flow(self, ensemble_specialist):
        ...
        ensemble_specialist._execute_parallel_predictions = Mock(
            return_value={
                "model-rf": {
                    "confidence_score": 0.8,
                    "risk_score": 0.2,
...
class TestEnsembleCalibrationMetadata:
    ...
    def test_combine_predictions_calibration_metadata_all_calibrated(
        ...
        ensemble_specialist.metrics.observe_ensemble_calibration_coverage.assert_called_once_with(
            1.0
        )
    ...
    def test_combine_predictions_calibration_metadata_majority_calibrated(
        self, ensemble_specialist
    ):
