...
class TestPriorityFeedbackQueue:
    ...
    def test_claim_case_calculates_expiry(self, queue):
        ...
        assert abs(delta.total_seconds() - expected_delta.total_seconds()) < 10  # 10s tolerance
    ...
    def test_claim_case_returns_none_if_not_found(self, queue):
        ...
        result = queue.claim_case(queue_id="queue-nonexistent", assigned_to="user@example.com")
    ...
    def test_mark_feedback_submitted(self, queue):
        ...
        result = queue.mark_feedback_submitted(queue_id="queue-123", feedback_id="feedback-456")
    ...
    def test_get_queue_size_by_status(self, queue):
        ...
        assert size == 10
        queue.collection.count_documents.assert_called_once_with({"status": QueueStatus.PENDING})
