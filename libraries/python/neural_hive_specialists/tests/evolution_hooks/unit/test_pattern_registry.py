...
class TestPatternRegistry:
    ...
    async def test_store_evaluation(
    ...
    async def test_get_statistics(
        ...
        for i in range(5):
            await registry.store_evaluation(
                f"plan-{i}", sample_fingerprint, sample_evaluation
            )

        # Adicionar feedback a alguns
        await registry.add_feedback(
            "plan-0",
            FeedbackData(outcome=FeedbackOutcome.APPROVE, source=FeedbackSource.HUMAN),
        )
        await registry.add_feedback(
            "plan-1",
            FeedbackData(outcome=FeedbackOutcome.REJECT, source=FeedbackSource.HUMAN),
        )
