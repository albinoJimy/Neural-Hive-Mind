...
class TestComplianceFlowIntegration:
    ...
    def test_full_compliance_flow_without_pii(self, integration_config, mock_mongodb, mock_metrics):
        ...
        assert encrypted_doc['_compliance']['encrypted_fields'] == integration_config.fields_to_encrypt
    ...
    def test_compliance_with_ledger_client(self, integration_config, mock_mongodb, mock_metrics):
        ...
        """
        # Setup
        with patch('neural_hive_specialists.ledger_client.MongoClient', return_value=mock_mongodb):
            # Criar LedgerClient
    ...
    def test_audit_logging_integration(self, integration_config, mock_mongodb, mock_metrics):
        ...
        with patch('neural_hive_specialists.compliance.audit_logger.MongoClient', return_value=mock_mongodb):
            ...
            audit_entries = list(audit_collection.find({'event_type': 'field_encrypted'}))
    ...
    @pytest.mark.skipif(
        ...
        reason="Teste de retenção pode ser lento"
    )
...
@pytest.mark.skipif(
    os.getenv('CI') == 'true',
    reason="Requer modelos spaCy instalados"
)
