...
def mock_feedback_collector():
    ...
    collector.get_feedback_statistics = Mock(
        return_value={
            "count": 10,
            "avg_rating": 0.85,
    ...
    collector.get_opinion_metadata = Mock(
        return_value={"plan_id": "plan-test", "specialist_type": "technical"}
    )
    return collector
...
class TestFeedbackAPI:
    ...
    def test_submit_feedback_opinion_not_found(
        self, test_client, valid_feedback_request, mock_feedback_collector
    ):
        """Teste quando opinião não existe."""
        mock_feedback_collector.get_opinion_metadata.side_effect = ValueError(
            "Opinião opinion-test não encontrada no ledger"
        )
    ...
    def test_get_feedback_stats_success(self, test_client, mock_feedback_collector):
        ...
        response = test_client.get(
            "/api/v1/feedback/stats?specialist_type=technical&window_days=30"
        )
...
class TestFeedbackAPIAuthentication:
    ...
    def auth_app(self, auth_config, mock_feedback_collector, mock_audit_logger):
        ...
        router = create_feedback_router(
            mock_feedback_collector, auth_config, audit_logger=mock_audit_logger
        )
        app.include_router(router, prefix="/api/v1")
    ...
    def test_submit_feedback_with_valid_token(
        ...
        response = auth_client.post(
            "/api/v1/feedback", json=valid_feedback_request, headers=headers
        )
