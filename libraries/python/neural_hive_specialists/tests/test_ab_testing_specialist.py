...
def mock_mlflow_client():
    ...
    with patch('neural_hive_specialists.ab_testing_specialist.MLflowClient') as mock_client:
...
class TestABTestingPrediction:
    ...
    def test_fallback_on_prediction_failure(self, ab_testing_specialist):
        ...
        with patch.object(ABTestingSpecialist.__bases__[0], '_predict_with_model', side_effect=mock_predict_with_failure):
            # Forçar seleção de modelo B
            ab_testing_specialist._select_model_for_request = Mock(return_value=('model_b', ab_testing_specialist.model_b))
...
class TestABTestStatistics:
    ...
    def test_statistics_collection(self, ab_testing_specialist):
        ...
        ab_testing_specialist.metrics.ab_test_variant_usage_total.labels = Mock(side_effect=mock_label_value)
        ...
        ab_testing_specialist.metrics.ab_test_variant_confidence_score.labels = Mock(side_effect=mock_histogram)
        ab_testing_specialist.metrics.ab_test_variant_processing_time_seconds.labels = Mock(side_effect=mock_histogram)
        ...
        ab_testing_specialist.metrics.ab_test_variant_consensus_agreement.labels = Mock(side_effect=mock_agreement)
        ...
        ab_testing_specialist.metrics.ab_test_variant_recommendation_distribution.labels = Mock(side_effect=mock_recommendation)
    ...
    def test_insufficient_samples_recommendation(self, ab_testing_specialist):
        ...
        ab_testing_specialist.metrics.ab_test_variant_confidence_score.labels = Mock(
            return_value=MagicMock(_sum=MagicMock(get=Mock(return_value=8.0)), _count=MagicMock(get=Mock(return_value=10)))
        )
        ab_testing_specialist.metrics.ab_test_variant_processing_time_seconds.labels = Mock(
            return_value=MagicMock(_sum=MagicMock(get=Mock(return_value=5.0)), _count=MagicMock(get=Mock(return_value=10)))
        )
...
class TestABTestingErrorHandling:
    ...
    def test_missing_plan_id_generates_temporary_id(self, ab_testing_specialist):
        ...
        with patch.object(ABTestingSpecialist.__bases__[0], '_predict_with_model') as mock_predict:
            ...
            # Plano sem plan_id
            cognitive_plan = {
                'description': 'Test plan without ID'
            }
