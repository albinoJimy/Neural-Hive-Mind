...
def monitor_running_triggers(
    ...
    try:
        ...
        running_triggers = list(triggers_collection.find({"status": "running"}))
        ...
        for trigger in running_triggers:
            ...
            try:
                ...
                if run_status == "FINISHED":
                    ...
                    triggers_collection.update_one(
                        {"trigger_id": trigger_id}, {"$set": update_data}
                    )
        ...
        logger.info("Monitoring completed", **stats)

        mongo_client.close()
        return stats

    except Exception as e:
        logger.error("Fatal error in monitoring", error=str(e))
        stats["errors"] += 1
        return stats
...
def main():
    ...
    parser.add_argument(
        "--mongodb-database", default="neural_hive", help="Nome do database MongoDB"
    )
    parser.add_argument(
        "--mlflow-tracking-uri",
