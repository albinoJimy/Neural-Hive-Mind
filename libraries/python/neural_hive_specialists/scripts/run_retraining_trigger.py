...
def load_config() -> Dict[str, Any]:
    ...
    try:
        config = {
            ...
            "mongodb_opinions_collection": os.getenv(
                "MONGODB_OPINIONS_COLLECTION", "cognitive_ledger"
            ),
            "feedback_mongodb_collection": os.getenv(
            ...
            "retraining_mlflow_project_uri": os.getenv(
                "RETRAINING_MLFLOW_PROJECT_URI", "./ml_pipelines/training"
            ),
            "retraining_min_feedback_quality": float(
        ...
        print(
            "  ⚙️  ENABLE_RETRAINING_TRIGGER - Habilitar trigger (default: true)",
            file=sys.stderr,
        )
        print(
            "  ⚙️  RETRAINING_FEEDBACK_THRESHOLD - Threshold de feedbacks (default: 100)",
            file=sys.stderr,
        )
        print(
            "  ⚙️  MLFLOW_TRACKING_URI - URI do MLflow (default: http://mlflow:5000)",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        if dry_run:
            ...
            if should_trigger:
                ...
                print(
                    f"   Feedback count: {feedback_count} >= {config['retraining_feedback_threshold']}"
                )
            else:
            ...
            trigger_id = retraining_trigger.check_and_trigger(
                specialist_type=specialist_type, force=force
            )
            ...
            if trigger_id:
                ...
                logger.info("No trigger needed", specialist_type=specialist_type)
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Ignorar cooldown e disparar forçadamente"
    )
