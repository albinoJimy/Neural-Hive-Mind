...
def load_config() -> Dict[str, Any]:
    ...
    try:
        config = {
            ...
            'consensus_collection_name': os.getenv('CONSENSUS_COLLECTION_NAME', 'consensus_decisions'),
            ...
            'enable_business_value_tracking': os.getenv('ENABLE_BUSINESS_VALUE_TRACKING', 'false').lower() == 'true',
            ...
            'enable_metrics_history': os.getenv('ENABLE_METRICS_HISTORY', 'false').lower() == 'true',
        ...
        print("  ⚙️  CONSENSUS_MONGODB_URI - URI do MongoDB do consensus (opcional, usa MONGODB_URI se não fornecido)", file=sys.stderr)
        print("  ⚙️  BUSINESS_METRICS_WINDOW_HOURS - Janela de análise em horas (default: 24)", file=sys.stderr)
        sys.exit(1)
...
def build_anomaly_features(metrics_summary: Dict[str, Any]) -> Dict[str, float]:
    ...
    features = {
        ...
        'avg_confidence_score': metrics_summary.get('accuracy_score', 0.0),  # accuracy_score é proxy de confidence
...
def push_metrics_to_gateway(config: Dict[str, Any], metrics_registry: Dict):
    ...
    try:
        ...
        push_to_gateway(
            pushgateway_url,
            job='business_metrics_collector',
            registry=REGISTRY
        )
...
def main():
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Simula execução sem atualizar métricas'
    )
    parser.add_argument(
        '--specialist-type',
        type=str,
    ...
    parser.add_argument(
        '--pushgateway-url',
        type=str,
        help='URL do Prometheus Pushgateway (opcional)'
    )
    ...
    anomaly_detector = initialize_anomaly_detector(config) if args.enable_anomaly_detection else None
        ...
        if anomaly_detector and not args.dry_run and result.get('status') == 'success':
            ...
            for specialist_type, metrics in metrics_registry.items():
                ...
                if not anomaly_detector.is_trained:
                    ...
                    if not model_loaded:
                        print(f"⚠️  Modelo não encontrado para {specialist_type}, pulando detecção de anomalias")
                        continue
