...
class ExplainabilityGenerator:
    ...
    def generate(
        ...
        try:
            ...
            feature_importances = self._extract_feature_importances(
                evaluation_result, model, method, cognitive_plan
            )
            ...
            if feature_importances and method in ["shap", "lime"]:
                try:
                    ...
                    summary = self.narrative_generator.generate_summary(
                        feature_importances, explanation_type=method
                    )
                    ...
                    if self._metrics:
                        ...
                        self._metrics.observe_narrative_generation_time(
                            self.config.narrative_language, narrative_time
                        )
            ...
            if (
                self.config.enable_explainability
                and self.config.enable_legacy_explainability_persistence
            ):
                try:
                    ...
                    if self._metrics:
                        self._metrics.increment_fallback_invocation(
                            "explainability", "skip_persistence"
                        )
            ...
            if self._metrics:
                ...
                if len(feature_importances) > 0:
                    self._metrics.observe_explainability_feature_count(
                        method, len(feature_importances)
                    )
    ...
    def _on_circuit_breaker_state_change(self, old_state: str, new_state: str):
        ...
        if self._metrics:
            ...
            self._metrics.increment_circuit_breaker_transition(
                "explainability", old_state, new_state
            )
        ...
        for factor in reasoning_factors:
            ...
            if normalized_factor in feature_index:
                ...
                for norm_feature_name, feature in feature_index.items():
                    ...
                    if (
                        normalized_factor in norm_feature_name
                        or norm_feature_name in normalized_factor
                    ):
                        links[feature["feature_name"]] = {
                            "factor_name": factor_name,
                            "factor_score": factor.get("score", 0.0),
    ...
    def _get_background_dataset_hash(self) -> Optional[str]:
        ...
        if self.shap_explainer.background_data is not None:
            try:
                ...
                stats["_meta"] = {"shape": list(df.shape), "columns": sorted_columns}
    ...
    def retrieve_explanation_impl(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to retrieve explanation", token=token, error=str(e))
            return None
