...
class NLPFeatureExtractor:
    ...
    DOMAIN_KEYWORDS = {
        'security': [
            ...
            'login', 'token', 'jwt', 'oauth', 'ssl', 'tls', 'encryption',
            'vulnerability', 'xss', 'csrf', 'injection', 'firewall', 'access',
            'permission', 'role', 'user', 'identity', 'credential', 'hash'
        ],
        'performance': [
            'performance', 'optimization', 'slow', 'fast', 'cache', 'redis',
            'latency', 'throughput', 'scale', 'load', 'database', 'query',
            'index', 'memcached', 'response', 'time', 'speed', 'benchmark',
            'profiling', 'async', 'concurrent', 'parallel'
        ],
        'architecture': [
            'architecture', 'design', 'pattern', 'microservice', 'monolith',
            'structure', 'component', 'module', 'layer', 'service', 'api',
            'endpoint', 'interface', 'coupling', 'cohesion', 'dependency',
            'diagram', 'schema', 'model', 'entity', 'repository'
        ],
        'database': [
            'database', 'db', 'sql', 'nosql', 'mongodb', 'postgres', 'mysql',
            'query', 'table', 'collection', 'index', 'migration', 'seed',
            'orm', 'relation', 'column', 'field', 'record', 'document'
        ],
        'testing': [
            'test', 'testing', 'unit', 'integration', 'e2e', 'mock', 'stub',
            'spec', 'coverage', 'tdd', 'bdd', 'assertion', 'fixture', 'pytest',
            'jest', 'cypress', 'selenium'
        ],
        'devops': [
            'deploy', 'deployment', 'ci', 'cd', 'pipeline', 'docker', 'kubernetes',
            'k8s', 'container', 'helm', 'terraform', 'ansible', 'jenkins',
            'github', 'gitlab', 'actions', 'workflow', 'release', 'build'
        ]
    }
    ...
    def _extract_domain_features(self, text: str) -> Dict[str, Any]:
        ...
        if domain_scores:
            ...
            domain_scores['primary_domain'] = primary_domain[0] if primary_domain[1] > 0 else 'unknown'
        else:
    ...
    def _extract_sentiment_features(self, text: str) -> Dict[str, Any]:
        ...
        positive_words = [
            'good', 'great', 'excellent', 'better', 'best', 'love', 'perfect',
            'works', 'working', 'success', 'successful', 'fix', 'solved', 'resolved',
            'improve', 'improvement', 'optimization', 'fast', 'quick', 'easy', 'simple'
        ]

        negative_words = [
            'bad', 'worst', 'hate', 'broken', 'fails', 'failing', 'error',
            'issue', 'problem', 'bug', 'crash', 'slow', 'difficult', 'complex',
            'complicated', 'confusing', 'urgent', 'critical', 'blocking', 'blocked'
        ]

        urgency_words = ['urgent', 'asap', 'immediately', 'critical', 'blocking', 'priority']
        ...
        return {
            ...
            'sentiment_neutral': round(1 - (positive_count + negative_count) / total_words, 4),
