...
class KafkaSettings(BaseSettings):
    ...
    kafka_security_protocol: str = Field(
        default="PLAINTEXT", description="Security protocol: PLAINTEXT, SASL_SSL, SASL_PLAINTEXT"
    )
    ...
    kafka_enable_auto_commit: bool = Field(default=False, description="Enable auto-commit offsets")
    kafka_enable_idempotence: bool = Field(default=True, description="Enable idempotent producer")
...
class MongoDBSettings(BaseSettings):
    ...
    mongodb_database: str = Field(default="neural_hive", description="Nome do database MongoDB")
...
class RedisSettings(BaseSettings):
    ...
    redis_ssl_enabled: bool = Field(default=False, description="Habilitar SSL/TLS para Redis")
...
class OpenTelemetrySettings(BaseSettings):
    ...
    otel_service_name: Optional[str] = Field(
        default=None, description="Nome do serviço para OTEL (sobrescreve service_name)"
    )
...
class GRPCSettings(BaseSettings):
    ...
    grpc_timeout_ms: int = Field(
        default=5000, description="Timeout padrão para chamadas gRPC (milissegundos)", gt=0
    )
    grpc_max_retries: int = Field(default=3, description="Número máximo de retries gRPC", ge=0)
    grpc_enable_retry: bool = Field(default=True, description="Habilitar retry policy gRPC")
    grpc_max_message_length: int = Field(
        default=4 * 1024 * 1024, description="Tamanho máximo de mensagem gRPC (bytes)", gt=0  # 4MB
    )
...
class SPIFFESettings(BaseSettings):
    ...
    spiffe_enable_x509: bool = Field(
        default=False, description="Habilitar mTLS via SPIFFE X.509-SVID"
    )
    ...
    spiffe_jwt_audience: str = Field(
        default="neural-hive.local", description="Audience para validação de JWT-SVID"
    )
...
class VaultSettings(BaseSettings):
    ...
    vault_auth_method: str = Field(
        default="kubernetes", description="Método de autenticação: kubernetes, token, github"
    )
    vault_kubernetes_role: str = Field(
        default="default", description="Role Kubernetes para autenticação Vault"
    )
    vault_mount_kv: str = Field(default="secret", description="Mount point do KV secrets engine")
    vault_timeout_seconds: int = Field(
        default=5, description="Timeout para requisições Vault (segundos)", gt=0
    )
    vault_max_retries: int = Field(default=3, description="Número máximo de retries Vault", ge=0)
    vault_fail_open: bool = Field(
        default=False, description="Fail-open em erros do Vault (fallback para env vars)"
    )
...
class ObservabilitySettings(BaseSettings):
    ...
    jaeger_sampling_rate: float = Field(
        default=1.0, description="Taxa de sampling Jaeger/OTLP (0.0 a 1.0)", ge=0.0, le=1.0
    )
    ...
    enable_logging: bool = Field(default=True, description="Habilitar logging estruturado")
...
class BaseInfrastructureSettings(BaseSettings):
    ...
    environment: str = Field(
        default="development", description="Ambiente de execução: development, staging, production"
    )
    debug: bool = Field(default=False, description="Modo debug (verbose logging, reload)")
    log_level: str = Field(
        default="INFO", description="Nível de log: DEBUG, INFO, WARNING, ERROR, CRITICAL"
    )
    service_name: str = Field(default="nhm-service", description="Nome do serviço")
    service_version: str = Field(default="1.0.0", description="Versão do serviço (semver)")

    # ===== Configurações de Rede =====
    grpc_port: int = Field(default=50051, description="Porta do servidor gRPC", gt=0)
    http_port: int = Field(default=8000, description="Porta do servidor HTTP/REST", gt=0)
    ...
    def validate_log_level(cls, v: str) -> str:
        ...
        if v_upper not in valid_levels:
            raise ValueError(
                f"log_level inválido: {v}. " f'Níveis válidos: {", ".join(sorted(valid_levels))}'
            )
        return v_upper
    ...
    def validate_environment(cls, v: str) -> str:
        ...
        if v_lower not in valid_envs:
            raise ValueError(
                f"environment inválido: {v}. " f'Válidos: {", ".join(sorted(valid_envs))}'
            )
        return v_lower
    ...
    def validate_https_in_production(self) -> "BaseInfrastructureSettings":
        ...
        if self.vault_enabled:
            ...
            if vault_address.startswith("http://") and not is_internal_cluster_endpoint(
                vault_address
            ):
...
def get_settings(
    settings_class: type = BaseInfrastructureSettings, force_reload: bool = False
