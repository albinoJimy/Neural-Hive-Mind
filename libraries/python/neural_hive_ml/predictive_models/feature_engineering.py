...
def extract_ticket_features(
    ...
    # Derived features
    features["risk_to_capabilities_ratio"] = features["risk_weight"] / max(
        features["capabilities_count"], 1
    )
    features["estimated_to_sla_ratio"] = features["estimated_duration_ms"] / max(
        features["sla_timeout_ms"], 1
    )
...
async def compute_historical_stats(
    ...
    try:
        ...
        pipeline_task = [
            {"$match": {"status": "COMPLETED", "completed_at": {"$gte": cutoff_date}}},
            {
                "$group": {
                    "_id": "$type",
        ...
        for band in ["LOW", "MEDIUM", "HIGH", "CRITICAL"]:
            pipeline_risk = [
                ...
                {"$group": {"_id": None, "avg_duration": {"$avg": "$actual_duration_ms"}}},
            ]

            async for doc in mongodb_client.execution_tickets.aggregate(pipeline_risk):
                task_stats[f"risk_{band}"] = {"avg_duration": doc.get("avg_duration", 0)}
...
def _qos_to_score(priority: str) -> float:
    ...
    priority_map = {"LOW": 0.25, "MEDIUM": 0.50, "HIGH": 0.75, "CRITICAL": 1.0}
...
def _consistency_to_score(consistency: str) -> float:
    ...
    consistency_map = {"AT_MOST_ONCE": 0.33, "AT_LEAST_ONCE": 0.67, "EXACTLY_ONCE": 1.0}
...
def _durability_to_score(durability: str) -> float:
    ...
    durability_map = {"TRANSIENT": 0.5, "PERSISTENT": 1.0}
