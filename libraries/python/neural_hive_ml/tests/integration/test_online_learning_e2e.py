...
class TestOnlineLearningE2E:
    ...
    async def test_full_retraining_cycle(self, mock_db, mock_kafka_producer, mock_mlflow_client):
        """
    ...
    async def test_drift_detection_flow(self, mock_db, mock_kafka_producer):
        ...
        drift_detector = DriftDetector(
            mongo_client=mock_db, kafka_producer=mock_kafka_producer, confidence_threshold=0.10
        )
    ...
    async def test_canary_deployment_flow(self, mock_db, mock_kafka_producer):
        """Testa fluxo de canary deployment."""
        # Setup model repo com métodos async
        model_repo = Mock()
        model_repo.get_model_version = AsyncMock(return_value={"version": "v9", "stage": "staging"})
        ...
        # 4. Promote
        final_result = await canary_deployer.promote_or_rollback(canary_id, should_promote=True)
...
class TestOnlineLearningIntegration:
    ...
    async def test_retraining_to_drift_pipeline(
        self, mock_db, mock_kafka_producer, mock_mlflow_client
    ):
        ...
        drift_detector = DriftDetector(mongo_client=mock_db, kafka_producer=mock_kafka_producer)
    ...
    async def test_drift_to_retraining_trigger(
        ...
        cursor_baseline.to_list = AsyncMock(
            return_value=[{"_id": None, "approve_rate": 0.65, "avg_confidence": 0.72, "count": 500}]
        )

        cursor_current = Mock()
        cursor_current.to_list = AsyncMock(
            return_value=[{"_id": None, "approve_rate": 0.50, "avg_confidence": 0.55, "count": 100}]
        )
...
class TestOnlineLearningScenarios:
    ...
    async def test_sudden_model_breakage_scenario(self, mock_db, mock_kafka_producer):
        ...
        cursor_mock = Mock()
        cursor_mock.to_list = AsyncMock(
            return_value=[{"_id": None, "approve_rate": 0.30, "avg_confidence": 0.45, "count": 100}]
        )
        mock_db.plan_approvals.aggregate = Mock(return_value=cursor_mock)
...
"""
    ...
    async def test_drift_detection_with_seasonal_pattern(self, mock_db, mock_kafka_producer):
        ...
        cursor_current = Mock()
        cursor_current.to_list = AsyncMock(
            return_value=[{"_id": None, "approve_rate": 0.60, "avg_confidence": 0.68, "count": 200}]
        )
        ...
        drift_detector = DriftDetector(
            mongo_client=mock_db, kafka_producer=mock_kafka_producer, confidence_threshold=0.08
        )
    ...
    async def test_canary_deployment_with_rollback(self, mock_db, mock_kafka_producer):
    ...
    async def test_model_performance_tracking(self, mock_db, mock_kafka_producer):
    ...
    async def test_feature_drift_detection(self, mock_db, mock_kafka_producer):
        """Testa detecção de drift em distribuição de features."""
        # Agregação para detectar drift em features específicas
        cursor_mock = Mock()
        cursor_mock.to_list = AsyncMock(
            return_value=[{"_id": None, "approve_rate": 0.60, "avg_confidence": 0.65, "count": 100}]
        )
        mock_db.plan_approvals.aggregate = Mock(return_value=cursor_mock)
    ...
    async def test_multi_model_drift_comparison(self, mock_db, mock_kafka_producer):
    ...
    async def test_canary_traffic_percentage_validation(self, mock_db, mock_kafka_producer):
    ...
    async def test_drift_alert_aggregation(self, mock_db, mock_kafka_producer):
        """Testa agregação de alertas de drift."""
        # Simula múltiplos alertas
        cursor_mock = Mock()
        cursor_mock.to_list = AsyncMock(
            return_value=[{"_id": None, "approve_rate": 0.45, "avg_confidence": 0.50, "count": 100}]
        )
        mock_db.plan_approvals.aggregate = Mock(return_value=cursor_mock)
