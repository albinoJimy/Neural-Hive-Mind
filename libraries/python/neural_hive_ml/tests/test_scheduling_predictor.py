...
def sample_ticket():
    ...
    return {
        ...
        "qos": {"priority": 0.7, "consistency": "AT_LEAST_ONCE", "durability": "DURABLE"},
...
async def test_calculate_confidence(mock_config, sample_ticket):
    ...
    features_dict = {"risk_weight": 50, "has_historical_stats": True, "task_type_frequency": 100}
...
async def test_predict_resources_based_on_duration(mock_config, sample_ticket):
    ...
    predictor.predict_duration = AsyncMock(
        return_value={"predicted_duration_ms": 10000, "confidence": 0.8}  # 10 segundos
    )
...
async def test_batch_prediction(mock_config, sample_ticket):
    ...
    tickets = [
        {**sample_ticket, "ticket_id": f"ticket-{i}", "risk_weight": i * 10} for i in range(1, 6)
    ]
...
async def test_feature_importance_extraction(mock_config, mock_registry, training_data):
    ...
    with patch("mlflow.set_tracking_uri"), patch("mlflow.set_experiment"), patch(
        ...
        importance = predictor._calculate_feature_importance(
            predictor.model, predictor._get_feature_names()
        )
...
async def test_confidence_calculation_with_high_risk(mock_config):
    ...
    features_dict = {"avg_duration_by_task": 0, "retry_count": 0}  # Sem estatísticas
...
async def test_confidence_calculation_with_historical_stats(mock_config):
    ...
    features_dict = {"avg_duration_by_task": 8000, "retry_count": 2}
