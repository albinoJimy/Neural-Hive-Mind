...
class RegexPIIDetector(IPIIDetector):
    ...
    PATTERNS: Dict[PIIType, re.Pattern] = {
        ...
        PIIType.PHONE: re.compile(
            r"\b(?:\+?55\s?)?(?:\(?\d{2,3}\)?[\s-]?)?\d{4,5}[\s-]?\d{4}\b", re.IGNORECASE
        ),
        PIIType.CPF: re.compile(r"\b\d{3}[\.\s]?\d{3}[\.\s]?\d{3}[-\s]?\d{2}\b", re.IGNORECASE),
        PIIType.CREDIT_CARD: re.compile(r"\b(?:\d{4}[\s-]?){3}\d{4}\b", re.IGNORECASE),
        PIIType.SSN: re.compile(r"\b\d{3}[-\s]?\d{2}[-\s]?\d{4}\b", re.IGNORECASE),
        PIIType.IP_ADDRESS: re.compile(r"\b(?:\d{1,3}\.){3}\d{1,3}\b", re.IGNORECASE),
        PIIType.URL: re.compile(r"\bhttps?://(?:www\.)?[^\s/$.?#].[^\s]*\b", re.IGNORECASE),
        # Novos tipos - PII Brasileiro adicional
        PIIType.PASSPORT: re.compile(
            r"\b[A-Z]{2}\d{7}\b", re.IGNORECASE  # Formato: AA####### (Brasil)
        ),
        PIIType.DRIVERS_LICENSE: re.compile(
            r"\b\d{11}\b", re.IGNORECASE  # CNH: 11 dígitos (similar CPF mas sem formatação)
        ),
